...
For more information, see [LangChain bundle documentation](https://docs.prime.khulnasoft.com/bundles-langchain#csv-agent).

**CVE:** [CVE-2024-21513](https://nvd.nist.gov/vuln/detail/CVE-2024-21513) (langchain-experimental arbitrary code execution).
**Fixed in**: Primeagent >= 1.8.0
...
For more information, see [API keys and authentication](https://docs.prime.khulnasoft.com/api-keys-and-authentication).
...
The `PRIMEAGENT_SECRET_KEY` is used for:
...
- **macOS**: `~/Library/Caches/primeagent`
- **Linux**: `~/.cache/primeagent`
- **Windows**: `C:\Users\<user>\AppData\Local\primeagent\primeagent\Cache`

Override with `PRIMEAGENT_CONFIG_DIR` environment variable or `--config-dir` flag.
...
Options:
  ...
  --config-dir PATH  Primeagent config directory (default: platform-specific)
...
| Location               | Data                             | Notes            |
| ---------------------- | -------------------------------- | ---------------- |
| `user.store_api_key`   | Primeagent Store API key           | Fernet encrypted |
