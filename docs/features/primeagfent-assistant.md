...
> diagrams) lives at
> `src/backend/base/primeagfent/agentic/ARCHITECTURE.md`. This document covers the
...
The Primeagent Assistant is an AI-powered chat interface that helps users generate custom Primeagent components through natural language prompts. It provides real-time streaming feedback during component generation, automatic code validation with retry logic, and seamless integration with the Primeagent canvas.

### Business Context

Building custom components in Primeagent requires knowledge of the component architecture, Python programming, and understanding of inputs/outputs. The Primeagent Assistant removes this barrier by allowing users to describe what they want in natural language, and the AI generates validated, ready-to-use component code that can be added directly to their flow.

### Bounded Context

**Context**: `Agentic` - AI-assisted development capabilities within Primeagent
...
|------|------------|----------------|
| **Assistant** | AI-powered chat interface that generates Primeagent components from natural language | `AssistantPanel`, `AssistantService` |
| **AssistantMessage** | A single message in the chat, either from user or assistant | `AssistantMessage` interface |
| **ComponentCode** | Python code that defines a Primeagent component with inputs, outputs, and processing logic | `component_code` field, `extract_component_code()` |
...
| **FlowExecutor** | Service that runs Primeagent flows programmatically for assistant operations | `FlowExecutor`, `execute_flow_file()` |
| **TranslationFlow** | Pre-built flow that translates user input and classifies intent | `TranslationFlow.json`, `TRANSLATION_FLOW` |
| **PrimeagentAssistantFlow** | Pre-built flow containing the main assistant prompt and component generation logic | `PrimeagentAssistant.json`, `PRIMEAGFENT_ASSISTANT_FLOW` |
...
| **OffTopic** | Intent classification for questions unrelated to Primeagent (other tools, general knowledge) | `"off_topic"`, `OFF_TOPIC_REFUSAL_MESSAGE` |
...
| **SingleAgentLoop** | The assistant is one agent (`flow_builder_assistant.py`) plus an MCP toolkit; the same loop chains tools for multi-thing prompts instead of spawning sub-agents (Claude Code / Codex pattern) | `flow_builder_assistant.py`, `src/wfx/src/wfx/mcp/flow_builder_tools/` |
...
| **SerializedModelCoercion** | Backend coercion at the `configure_component` choke point: model-typed template fields (`template[field].type == "model"`) now normalize JSON/YAML-string values and the `name`-nested-spec QA pattern into the canonical `[{"provider": X, "name": Y}]` shape. Prevents the catalog falling back to `provider="Unknown"` → `get_llm: missing a provider`. Applies to both `BuildFlowFromSpec` and `ConfigureComponent` tools | `_coerce_model_value`, `_coerce_single_model_entry`, `_parse_serialized_model_text` in `wfx/graph/flow_builder/component.py` |
...
| **GenericToolNameFallback** | `_derive_tool_name` rule in `wfx/base/tools/component_tool.py`: when a Component has exactly ONE tool-exposed Output and that Output's method name is generic (`output`, `process`, `build_output`, `run`, `execute`, `main`, `handler`, `build_result`), the LLM-facing tool name is derived from the snake_cased component class name (`RandomMenuItem` → `random_menu_item`) — the class name is the user's stated intent and is always more informative than `output`/`process`. Multi-output components keep method-derived names so tools don't collide | `_GENERIC_OUTPUT_METHOD_NAMES`, `_class_name_to_tool_name`, `_derive_tool_name` |
...
| **AgentToolCompatibilitySection** | "Agent Tool Compatibility" block in the `PrimeagentAssistant.json` system prompt teaching the generator (1) action `verb_noun` method naming, (2) class-level `description` as the LLM-facing tool description, (3) `tool_mode=True` discipline + clear `info=`, (4) NEVER use the reserved `component_as_tool`/`to_toolkit` names. The complementary defense to the runtime guardrails | `PrimeagentAssistant.json` system prompt |
...
- **Persistence**: Selected model is stored in localStorage (`primeagfent-assistant-selected-model`)
...
### Scenario: Ask a question about Primeagent
- **Given** the assistant panel is open
...
- **Given** the assistant panel is open
- **When** I ask "how does n8n work?" or any question unrelated to Primeagent
- **Then** the intent should be classified as "off_topic"
- **And** I should see a refusal message redirecting me to Primeagent-related topics
...
#### Context
The TranslationFlow (intent classification) and PrimeagentAssistant flow shared the same `session_id`. This caused cross-flow contamination: the TranslationFlow's JSON intent responses were stored alongside the assistant's messages. On subsequent requests, the TranslationFlow's LLM saw messages from both flows in its history, causing intent classification to fail and default to `"question"`.
...
- `src/backend/.../agentic/helpers/code_security.py` — `scan_code_security()` denylist (shared with the run-time gate; see ADR-MCP-040 in `primeagfent-assistant-mcp.md` — `run_working_flow` re-scans every node's inline code before `exec` to close the bypass for code that skipped generation-time scanning)
...
#### Decision
Session history is stored in browser `localStorage` (key: `primeagfent-assistant-sessions`), limited to 10 sessions. Sessions are serialized/deserialized with `progress` state stripped and in-flight messages marked as `"cancelled"`.
...
- `src/wfx/src/wfx/mcp/flow_builder_tools/` — `GenerateComponent`, `DescribeFlowIO`, `RunFlow`
- `src/backend/base/primeagfent/agentic/ARCHITECTURE.md` — end-to-end diagrams
...
**Status**: Accepted (extends ADR-MCP-040 from `primeagfent-assistant-mcp.md`)
...
`URLComponent` calls `importlib.util.find_spec("primeagfent")` for optional
...
3. **Runtime precision** (`wfx/base/tools/component_tool.py::_should_skip_output`): the synthetic check now requires name + method + types ALL match the synthetic. Anything less is a user-declared output that happens to share the name and must be kept.
4. **Prompt guidance**: a new "Agent Tool Compatibility" section in the `PrimeagentAssistant.json` system prompt teaches the generator (a) action `verb_noun` method naming, (b) class-level `description` as tool description, (c) `tool_mode=True` discipline with clear `info=`, (d) NEVER use `component_as_tool`/`to_toolkit`.
...
- `src/backend/.../agentic/flows/PrimeagentAssistant.json` — "Agent Tool Compatibility" prompt section
...
- `src/wfx/src/wfx/base/models/unified_models/instantiation.py` — `get_llm` API-key + provider guards
...
(`wfx/graph/flow_builder/component.py`):
...
| Library | `wfx.run` | Flow execution engine |
| Library | `wfx.custom.validate` | Component class creation and validation |
...
| `400` | Missing API key | "OPENAI_API_KEY is required for the Primeagent Assistant with openai. Please configure it in Settings > Model Providers." | Add API key in Settings |
...
```mermaid
C4Context
  title System Context diagram for Primeagent Assistant

  Person(user, "Primeagent User", "Builds AI workflows using visual canvas")

  System(assistant, "Primeagent Assistant", "AI-powered component generation through natural language")

  System_Ext(llm_providers, "LLM Providers", "OpenAI, Anthropic, Azure, Google - text generation")
  System_Ext(primeagfent_core, "Primeagent Core", "Flow execution, component validation, canvas")

  Rel(user, assistant, "Sends prompts, receives components")
  Rel(assistant, llm_providers, "Generates text via API")
  Rel(assistant, primeagfent_core, "Validates code, adds to canvas")
```
...
```mermaid
C4Container
  title Container diagram for Primeagent Assistant

  Person(user, "User", "Primeagent user")
  ...
  Container_Ext(flows, "Assistant Flows", "JSON/Python", "PrimeagentAssistant.json, translation_flow.py")
...
> diagrams live in `src/backend/base/primeagfent/agentic/ARCHITECTURE.md` — the
...
flowchart TD
    ...
    B -->|generate_component| C[Execute PrimeagentAssistant Flow]
    B -->|question| D[Execute PrimeagentAssistant Flow<br/>with token streaming]
