# Feature: Primeagent Assistant — MCP Flow Builder Integration
...
> Companion documents: [`primeagfent-assistant.md`](./primeagfent-assistant.md) — read first for base Assistant concepts (session model, SSE pipeline, provider configuration, off-topic guardrails); [`../../src/backend/base/primeagfent/agentic/ARCHITECTURE.md`](../../src/backend/base/primeagfent/agentic/ARCHITECTURE.md) — end-to-end Mermaid diagrams of the single-agent loop, MCP toolkit, and continuation flow.
>
...
>
> **2026-05-27 revision** records the cost + reliability hardening pass that lands across the MCP surface (full breakdown lives in `primeagfent-assistant.md`'s 2026-05-27 revision and ADRs 023–030). MCP-specific consequences:
...
> (6) **`configure_component` model-spec coercion** — `_coerce_model_value` in `wfx/graph/flow_builder/component.py` now normalizes JSON / YAML strings and the QA-observed nested-spec-in-`name` pattern into the canonical `[{"provider": X, "name": Y}]` shape at the single tool-write boundary. Both `BuildFlowFromSpec` and `ConfigureComponent` are covered. Prevents the catalog falling back to `provider="Unknown"` → `get_llm: missing a provider`.
> (7) **Generic tool-name fallback + reserved-name guardrails** — `_derive_tool_name` snake-cases the Component class when its single Output uses a generic method; `validate_component_code` refuses `Output(name="component_as_tool")` / `method="to_toolkit"` at the generator's first turn; `_should_skip_output` now requires name + method + types ALL match the synthetic sentinel so a user-declared `component_as_tool` is no longer dropped (production failure 2026-05-27). A new "Agent Tool Compatibility" section in the `PrimeagentAssistant.json` system prompt teaches the generator the action-`verb_noun` discipline and the reserved-name ban.
...
> See ADR-023 through ADR-030 in `primeagfent-assistant.md` for full per-decision context; the MCP-specific impact is reflected in the glossary and behavior sections below.
...
**Context**: `Agentic` — AI-assisted flow construction inside Primeagent.
...
- **Per-request tool cache** (`wfx.mcp.tool_cache`) — ContextVar-scoped LRU keyed by `(tool_name, args)`, reset at request start alongside `_working_flow_var`.
...
- **Power-user preferences** persisted client-side via three independent localStorage keys: `primeagfent-assistant-skip-all`, `primeagfent-assistant-input-history`, `primeagfent-assistant-selected-model` (last one pre-existing).
...
| `MCP (WFX)` | Partnership | The `wfx.mcp` package owns the FastMCP server and tool definitions. The Assistant's `flow_builder_assistant` flow imports and toolkits them. |
...
Terms below extend the glossary in `primeagfent-assistant.md`. Where a term overlaps, the MCP-specific meaning is noted.

| Term | Definition | Code Reference |
|------|------------|----------------|
| **MCP Flow Builder Tools** | A bundle of `wfx.custom.Component` subclasses exposed to the Agent as a toolkit. Each tool mutates the per-request working flow and emits an action event. Split by responsibility: `_state.py` (per-request working flow + emit + drain + node-shape utilities), `read_tools.py` (Search/Describe/GetFieldValue/DescribeFlowIO — pure reads), `edit_tools.py` (ProposeFieldEdit — validated user-reviewable edits), `mutate_tools.py` (Add/Remove/Connect/Configure — push events), `run_tools.py` (ProposePlan/BuildFlowFromSpec/RunFlow/GenerateComponent — orchestration). The package `__init__.py` is re-exports only, so existing `from wfx.mcp.flow_builder_tools import X` imports are unchanged. | `src/wfx/src/wfx/mcp/flow_builder_tools/` |
...
| **MCP Server (wfx)** | The FastMCP-based server in `wfx/mcp/server.py` exposing REST-backed tools (create_flow, run_flow, build_flow, batch). Talks to the Primeagent HTTP API. | `wfx/mcp/server.py`, `PrimeagentClient` |
| **MCP Server (agentic)** | A second FastMCP server in `primeagfent/agentic/mcp/server.py` exposing template/component search and flow visualization tools directly against the database. | `primeagfent/agentic/mcp/server.py` |
| **MCPToolPayload** | Telemetry event logged for every MCP tool invocation (tool name, success, duration, error type). | `_tracked` decorator in `wfx/mcp/server.py` |
| **batch action** | An MCP tool that executes multiple actions sequentially, with `$N.field` reference resolution for chaining outputs to inputs. | `batch()` in `wfx/mcp/server.py` |
...
| **ProposePlan tool** | The MCP tool the flow-builder agent calls FIRST in BUILD mode to negotiate a markdown plan with the user before any destructive action. Emits a `propose_plan` action and instructs the agent to stop until the user approves. | `ProposePlan` class in `wfx.mcp.flow_builder_tools` |
...
| **Input command history** | Shell/REPL-style recall of the last 10 user inputs. Persisted in `localStorage` under `primeagfent-assistant-input-history` (newest-first array). `pushHistory` ignores empty/whitespace and dedups against the most-recent entry. | `hooks/input-history-storage.ts` |
...
| **UserComponentRegistry** | Per-user, file-backed overlay of the static base component registry. Validated Component classes generated by the assistant are persisted into ``<sandbox>/.components/<ClassName>.py`` and surfaced to ``build_flow`` / ``search_components`` / ``describe_component`` / ``add_component`` via a registry overlay that the MCP tools query in place of the bare base registry. | `primeagfent.agentic.services.user_components`, `user_components_overlay` |
| **`.components/` reserved segment** | Second entry in ``RESERVED_SEGMENTS`` (alongside ``.lfsig``). The agent's 5 FS tools (`read_file`, `write_file`, `edit_file`, `glob_search`, `grep_search`) refuse any path that contains this segment (case-insensitive via ``casefold()``). Only the privileged ``register_user_component`` helper may write here; the overlay loader may read. | `wfx/components/files_and_knowledge/filesystem.py:RESERVED_SEGMENTS` |
...
| **`SerializedModelCoercion`** | Backend coercion at the `configure_component` choke point in `wfx/graph/flow_builder/component.py`: model-typed template fields (`template[field].type == "model"`) normalize JSON / YAML-string values and the `name`-nested-spec QA pattern into the canonical `[{"provider": X, "name": Y}]` shape. Applied to both `BuildFlowFromSpec` (via `_apply_node_config_to_template` flow) and `ConfigureComponent`. Mutates `params` in place so post-configure helpers (e.g. `_mirror_model_value_into_options`) read the canonical value. Bare model-name strings (`"gpt-4o"`) are left untouched so the catalog path still runs. | `_parse_serialized_model_text`, `_coerce_single_model_entry`, `_coerce_model_value`, `configure_component` in `wfx/graph/flow_builder/component.py` |
...
| **`GenericToolNameFallback`** | `_derive_tool_name` rule in `wfx/base/tools/component_tool.py`: when a Component has exactly ONE tool-exposed Output AND its method name is in `_GENERIC_OUTPUT_METHOD_NAMES` (`output`, `process`, `build_output`, `run`, `execute`, `main`, `handler`, `build_result`), the LLM-facing tool name is derived from the snake_cased component class name (acronym-preserving: `HTTPClient` → `http_client`, `S3Bucket` → `s3_bucket`). Multi-output components keep method-derived names so tools don't collapse. | `_GENERIC_OUTPUT_METHOD_NAMES`, `_class_name_to_tool_name`, `_derive_tool_name` |
| **`ReservedOutputName`** | The two synthetic-tool sentinels the wiring layer creates when a Component is flipped to Tool Mode: `Output.name = "component_as_tool"` + `Output.method = "to_toolkit"`. Generation-time `validate_component_code` rejects code that declares either with a hint to pick a value-descriptive name; runtime `_should_skip_output` was tightened to require name + method + types ALL match the synthetic so a user-declared `component_as_tool` is no longer dropped. | `_RESERVED_OUTPUT_NAME`, `_RESERVED_OUTPUT_METHOD` in `helpers/validation.py`; `_should_skip_output` in `wfx/base/tools/component_tool.py` |
| **`AgentToolCompatibilitySection`** | "Agent Tool Compatibility" block in the `PrimeagentAssistant.json` system prompt teaching the generator (1) action `verb_noun` method naming, (2) class-level `description` as LLM-facing tool description, (3) `tool_mode=True` discipline + clear `info=`, (4) NEVER use the reserved `component_as_tool`/`to_toolkit` names. The complementary defense to the runtime guardrails. | `PrimeagentAssistant.json` system prompt |
...
| **`ApiKeyDiagnosticPreservation`** | `get_llm` captures `original_api_key_input` BEFORE the Global-Variable resolution step so the missing-API-key error can name the user's unresolved variable back to them (in addition to the canonical key). Empty / `"Unknown"` provider is replaced by an actionable "reselect a model from the dropdown" message instead of the nonsense `"Unknown API key … UNKNOWN_API_KEY"`. | `get_llm` in `wfx/base/models/unified_models/instantiation.py` |
...
- **Root**: `wfx.mcp.tool_cache._cache_var: ContextVar[OrderedDict[str, Any] | None]`.
...
- **Root**: `localStorage["primeagfent-assistant-input-history"]` — JSON array, newest-first, max 10 entries.
...
- Given a user with an active Primeagent session.
...
- **When** I ask "how does Primeagent integrate with Slack?" (Q&A) or "how does n8n work?" (off-topic).
...
- **And** `skipAll` flips and `localStorage["primeagfent-assistant-skip-all"]` reflects the new value.
...
- **Then** `AssistantHeader` displays a muted "Skip-all" pill with the Zap icon next to "Primeagent Assistant" (testid `assistant-skip-all-badge`).
...
- **Given** `localStorage["primeagfent-assistant-input-history"]` holds `["latest", "older", "oldest"]`.
...
- **Then** `localStorage["primeagfent-assistant-input-history"]` contains only one `"hello"` (dedup against latest).
...
- **Given** `.components/` contains a valid `SumComponent.py` and a corrupted `BrokenComponent.py` (not parseable Python — e.g. a partial write from a non-Primeagent source).
- **When** `load_registry_with_user_overlay(user_id="user-alice")` runs.
...
**Key Files:**
- `src/wfx/src/wfx/mcp/flow_builder_tools/` — ContextVar declarations + helpers.
- `src/backend/base/primeagfent/agentic/services/assistant_service.py:247` — `reset_working_flow()` at start of request.
...
**Key Files:**
- `src/wfx/src/wfx/mcp/flow_builder_tools/` — emit and drain.
...
**Key Files:**
- `src/wfx/src/wfx/mcp/flow_builder_tools/` — `_find_orphan_nodes()` and the orphan check in `build_flow()`.

---

### ADR-MCP-007: Two MCP Servers — `wfx/mcp` (canvas-aware) and `agentic/mcp` (DB-aware)
...
- **`src/wfx/src/wfx/mcp/server.py`** — REST-backed tools (`create_flow`, `add_component`, `connect_components`, `run_flow`, `build_flow`, `batch`, …). Uses `PrimeagentClient` HTTP client. Used by external MCP clients (Claude Desktop, Cursor, etc.).
- **`src/backend/base/primeagfent/agentic/mcp/server.py`** — DB-backed tools (template search, component search, flow visualization). Uses `session_scope()` directly. Powers internal Assistant capabilities.

The Assistant's `FlowBuilderAssistant` flow uses the **canvas-scoped** tool *classes* directly (imports from `wfx.mcp.flow_builder_tools`) — it does not go through the external MCP transport. This keeps in-process performance (no HTTP round-trip) while reusing the same tool definitions exposed externally.
...
**Key Files:**
- `src/wfx/src/wfx/mcp/server.py` — external MCP server.
- `src/backend/base/primeagfent/agentic/mcp/server.py` — agentic MCP server.
- `src/backend/base/primeagfent/agentic/flows/flow_builder_assistant.py:10-20` — direct imports of tool classes.
...
External MCP clients calling Primeagent tools needed observability — without it, debugging integration issues (Claude Desktop failures, Cursor timeouts) was guesswork.

#### Decision

The `_tracked` decorator in `wfx/mcp/server.py` wraps every public MCP tool. On each call it captures the tool name, success/failure, duration in ms, and error type, then pushes an `MCPToolPayload` event to the `TelemetryService` (started by `_telemetry_lifespan` when the FastMCP server boots).
...
**Key Files:**
- `src/wfx/src/wfx/mcp/server.py` — `_tracked` decorator and `MCPToolPayload`.
...
**Key Files:**
- `src/backend/base/primeagfent/agentic/flows/flow_builder_assistant.py` — `build_toolkit()` appends wrapped FS tools.
- `src/backend/base/primeagfent/agentic/services/assistant_service.py` — `is_document_request` flag drives the step label only; same `flow_filename = FLOW_BUILDER_ASSISTANT_FLOW`.
...
**Key Files:**
- `src/backend/base/primeagfent/agentic/services/file_events.py` — `wrap_file_tool_with_event` captures `kwargs["content"]` for `write_file`.
- `src/backend/base/primeagfent/agentic/helpers/sse.py` — `format_file_written_event` ships `content` when present.
...
The proven `wfx.mcp.flow_builder_tools._flow_events_var` does the opposite: `reset_working_flow()` allocates a deque **in the parent context** before spawning child tasks. Children inherit the same deque object by reference; emits in children are immediately visible in the parent.
...
**Key Files:**
- `src/backend/base/primeagfent/agentic/services/file_events.py` — `reset_file_events()` allocates `deque()` in the parent.
...
- `hooks/skip-all-storage.ts` — pure primitives `readSkipAll()` / `writeSkipAll(bool)` with try/catch around `localStorage`. Storage key: `primeagfent-assistant-skip-all`. Corrupt or non-`"true"` values fail closed (treated as off).
...
A new module `wfx.mcp.tool_cache` exposes:
...
**Key Files:**
- `src/wfx/src/wfx/mcp/tool_cache.py`.
- `src/wfx/src/wfx/mcp/flow_builder_tools/` — `search_components` / `describe_component` wrappers.
...
A new module `primeagfent.agentic.services.conversation_buffer` exposes:
...
**Key Files:**
- `src/backend/base/primeagfent/agentic/services/conversation_buffer.py`.
- `src/backend/base/primeagfent/agentic/services/assistant_service.py` — `inject_conversation_history`, `record_conversation_turn`, `clear_session_history`; `final_response_text` tracking; `finally`-block recording.
...
- `hooks/input-history-storage.ts`: pure primitives with localStorage under `primeagfent-assistant-input-history` (newest-first JSON array). `pushHistory` trims, ignores empty, dedups against the most-recent entry, caps at 10. All operations defensive (try/catch).
...
**Key Files:**
- `src/wfx/src/wfx/components/files_and_knowledge/filesystem.py:RESERVED_SEGMENTS`, `_validate_path`.
- `src/backend/base/primeagfent/agentic/services/user_components.py`.
...
The MCP tools (`SearchComponentTypes`, `DescribeComponentType`, `AddComponent`, `BuildFlowFromSpec`) consume a `registry: dict[type_name → template_dict]`. The bundled registry is loaded by `wfx.graph.flow_builder.builder.load_local_registry()`. To make user-Components addressable, the registry needs to be **user-aware** at call time.
...
The MCP tools call a thin helper `_load_registry_user_aware()` instead of bare `load_local_registry()`. The helper delegates to `primeagfent.agentic.services.user_components_overlay.load_registry_for_current_user()` via a **lazy import inside a `try/except ImportError`** so the `wfx` package keeps standalone runnability — when primeagfent isn't installed alongside (e.g., the FastMCP server running independently), the call falls back to the bare base registry.
...
- `wfx` keeps the option of running standalone (e.g., as an external MCP server) — the import is optional.
- Tool call sites stay tiny: one function rename (`load_local_registry` → `_load_registry_user_aware`).

**Trade-offs:**
- The lazy import inside wfx code is unusual. Documented inline; the alternative (registry-provider injection at module init) is heavier and not yet needed.
...
**Key Files:**
- `src/backend/base/primeagfent/agentic/services/user_components_overlay.py` — overlay function + `load_registry_for_current_user()`.
- `src/backend/base/primeagfent/agentic/services/user_components_context.py` — `ContextVar` + setters.
- `src/wfx/src/wfx/mcp/flow_builder_tools/` — `_load_registry_user_aware()` helper used by Search / Describe / AddComponent.
- `src/wfx/src/wfx/graph/flow_builder/builder.py` — `build_flow_from_spec(spec, registry=None)` accepts injection.
- `src/backend/base/primeagfent/agentic/services/assistant_service.py` — `set_current_user_id` at request start + reset in `finally`.
...
**Key Files:**
- `src/backend/base/primeagfent/agentic/services/user_components.py:register_user_component_if_valid`.
- `src/backend/base/primeagfent/agentic/services/assistant_service.py` — both validated paths call the hook.
...
**Key Files:**
- `src/backend/base/primeagfent/agentic/api/sessions_router.py:reset_session` — the endpoint.
- `src/backend/base/primeagfent/agentic/services/user_components.py:clear_user_components`.
- `src/backend/base/primeagfent/api/router.py` — mounts `agentic_sessions_router`.
...
**Key Files:**
- `src/backend/base/primeagfent/agentic/services/user_components.py:MAX_CLASS_NAME_LENGTH`, `_validate_class_name`.
...
**Key Files:**
- `src/backend/base/primeagfent/agentic/flows/flow_builder_assistant.py` — the one agent.
- `src/backend/base/primeagfent/agentic/ARCHITECTURE.md` — end-to-end Mermaid diagrams.
...
**Key Files:**
- `src/wfx/src/wfx/mcp/flow_builder_tools/` — `GenerateComponent`.
- `src/backend/base/primeagfent/agentic/services/user_components.py` — registration reuse.
...
**Key Files:**
- `src/wfx/src/wfx/mcp/flow_builder_tools/` — `DescribeFlowIO`.
...
**Key Files:**
- `src/wfx/src/wfx/mcp/flow_builder_tools/` — `RunFlow`.
- `src/backend/base/primeagfent/agentic/services/flow_run.py:65,133` — `extract_graph_token_usage`, `run_working_flow`.
...
**Key Files:**
- `src/backend/base/primeagfent/agentic/services/helpers/intent_classification.py` — exact-match bypass.
- `src/backend/base/primeagfent/agentic/services/assistant_service.py` — `continuation_expected` gating.
...
**Key Files:**
- `src/backend/base/primeagfent/agentic/services/flow_preparation.py:25` — `available_model_providers`.
- `src/backend/base/primeagfent/agentic/services/agent_run_context.py` — `AgentRunModel`, `set_agent_run_model`.
...
**Key Files:**
- `src/wfx/src/wfx/mcp/flow_builder_tools/` — `ProposePlan` (now optional in prompt usage).
- `src/backend/base/primeagfent/agentic/flows/flow_builder_assistant.py` — prompt guidance.
...
**Key Files:**
- `src/backend/base/primeagfent/agentic/services/user_components_overlay.py` — `build_custom_component_template(Component(_code=code))`.
...
**Key Files:**
- `src/wfx/src/wfx/mcp/flow_builder_tools/` — `build_flow` in-place mutation.
- `src/backend/base/primeagfent/agentic/services/flow_run.py` — run engine reads the shared working flow.
...
- Pure helper is exhaustively unit-testable across every event ordering; the fix landed RED→GREEN with zero regression (988 agentic + 112 wfx green).
...
**Key Files:**
- `src/wfx/src/wfx/mcp/flow_builder_tools/` — `_emit("flow_ran", flow_id=…)` on successful run.
- `src/backend/base/primeagfent/agentic/services/assistant_service.py:284` — `_reconcile_flow_updates`; wired at both drain sites (`:628`, `:733`).
...
**Status**: Accepted (complements the generation-time scan of ADR-010 in `primeagfent-assistant.md`)
...
**Key Files:**
- `src/backend/base/primeagfent/agentic/helpers/code_security.py:181` — `scan_code_security`; denylist tables (`:115` `_SecurityChecker`).
- `src/backend/base/primeagfent/agentic/services/flow_run.py:134,160` — `_scan_flow_component_code`, gate in `run_working_flow`.
...
**Status**: Accepted (refines ADR-MCP-040 — see also ADR-025 in `primeagfent-assistant.md`)

#### Context

The run-time gate from ADR-MCP-040 correctly refused any node whose inline `code` triggered a denylist violation. But trusted built-ins legitimately use forbidden patterns: `URLComponent` calls `importlib.util.find_spec("primeagfent")` for optional dependency detection and `os.environ.get("HTTPS_PROXY")` for proxy config. After ADR-MCP-040 every run of a flow containing a built-in `URLComponent` was a false-positive "refused to run". Users saw the run blocked even though they had only ever asked the assistant to use stock registry components.
...
**Status**: Accepted (see also ADR-029 in `primeagfent-assistant.md`)
...
Coerce model values at the **single** `configure_component` choke point in `wfx/graph/flow_builder/component.py` so every tool path (today: `BuildFlowFromSpec`, `ConfigureComponent`; tomorrow: any future flow-builder tool that mutates a template) is covered:
...
**Key Files:**
- `src/wfx/src/wfx/graph/flow_builder/component.py` — `_parse_serialized_model_text`, `_coerce_single_model_entry`, `_coerce_model_value`, `configure_component`
- `src/wfx/tests/unit/test_build_flow_from_spec.py` — coverage for the QA patterns

---

### ADR-MCP-043: Per-Turn Usage / Duration Rollup over the Whole MCP Toolchain

**Status**: Accepted (see also ADR-023 in `primeagfent-assistant.md`)
...
**Status**: Accepted (see also ADR-026 in `primeagfent-assistant.md`)
...
3. **Toolkit runtime** (`wfx/base/tools/component_tool.py`): 
...
**Key Files:**
- `src/backend/.../agentic/flows/PrimeagentAssistant.json` — "Agent Tool Compatibility" section
- `src/backend/.../agentic/helpers/validation.py` — `_RESERVED_OUTPUT_NAME`, `_RESERVED_OUTPUT_METHOD`
- `src/wfx/src/wfx/base/tools/component_tool.py` — `_GENERIC_OUTPUT_METHOD_NAMES`, `_class_name_to_tool_name`, `_derive_tool_name`, tightened `_should_skip_output`
...
**Status**: Accepted (see also ADR-MCP-006 / ADR-MCP-014 for the protocol-string contract; ADR-023 in `primeagfent-assistant.md` for the cost rollup it complements)
...
| Library | `wfx.custom.Component` | Base class for all flow-builder tools. |
| Library | `wfx.io` (`MessageTextInput`, `Output`) | Declares tool inputs and outputs visible to the agent. |
| Library | `httpx.AsyncClient` | `PrimeagentClient` uses it for REST calls from the external MCP server. |
...
#### MCP Server (external, `wfx/mcp/server.py`) — selected tools
...
#### MCP Server (agentic, `primeagfent/agentic/mcp/server.py`)
...
#### In-process Flow Builder Tools (`wfx/mcp/flow_builder_tools/`)
...
#### Per-Request Tool Cache (`wfx.mcp.tool_cache`)
...
#### Conversation Buffer (`primeagfent.agentic.services.conversation_buffer`)
...
#### User-Components Registry (`primeagfent.agentic.services.user_components` + `user_components_overlay` + `user_components_context`)
...
| `RESERVED_SEGMENTS = (".lfsig", ".components")` | Tuple in `wfx/components/files_and_knowledge/filesystem.py`. The FS tool's `_validate_path` refuses any path containing any reserved segment (case-insensitive). |
...
- [ ] Verify session isolation: open two Primeagent tabs, build a flow in each — proposals do not cross over.
...
- [ ] Type `/skip-all` and press Enter: no backend call; inline assistant confirmation appears; header gains the "Skip-all" badge; `localStorage["primeagfent-assistant-skip-all"] === "true"`.
...
- [ ] Multi-user isolation: open Primeagent with two users in two browsers; user A registers `Foo`, user B's `search_components` does NOT see it.
...
C4Context
  ...
  Person(user, "Primeagent User", "Builds and edits flows visually and through chat")

  System(assistant, "Primeagent Assistant (MCP)", "Build/edit flows from natural language with live canvas feedback")
  ...
  System_Ext(canvas, "Primeagent Canvas", "ReactFlow surface owning user's flow state")
...
C4Container
  ...
  Container_Boundary(backend, "Backend") {
    ...
    Container(fb_tools, "Flow Builder Tools", "Python (wfx.custom.Component)", "AddComponent / Connect / Configure / BuildFlowFromSpec / ProposePlan / ...")
  ...
  Container_Boundary(mcp_servers, "MCP Servers") {
    Container(wfx_mcp, "wfx MCP Server", "FastMCP", "REST-backed external tools")
    Container(agentic_mcp, "Agentic MCP Server", "FastMCP", "DB-backed template/component tools")
  }

  System_Ext(llm, "LLM Provider")
  ContainerDb(db, "Primeagent DB", "Postgres/SQLite", "Flows, templates, components")
...
| External MCP shell launchers | bash | zsh/bash | PowerShell | The optional shell launchers under `src/wfx/src/wfx/mcp/shell/` ship platform-specific scripts; the in-process flow builder does not use them. |
...
- Windows users connecting external MCP clients should use the PowerShell shell launcher in `src/wfx/src/wfx/mcp/shell/` (this predates the integration but is the entry point for `wfx mcp` invocations from Cursor / Claude Desktop on Windows).
...
No installation step beyond running Primeagent. The MCP integration is built into the application.

**To expose Primeagent as an MCP server to an external client (Claude Desktop / Cursor):**

```bash
# Any platform — launch as a managed process
uv run wfx mcp serve
```
