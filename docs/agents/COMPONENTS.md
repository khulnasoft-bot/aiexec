...
Components are the unit of work in Primeagent. They render as nodes on the canvas, persist into flow JSON, and become tools for Agents. Get the contract right and the rest of the system follows; get it wrong and you orphan every saved flow that referenced your component.

Components live in **`src/wfx/src/wfx/components/<category>/`**. The mirror tree under `src/backend/base/primeagfent/components/` is legacy stubs — do not add files there.
...
1. **Does one already exist?** Search:
   ```bash
   rg -l 'display_name = ".*<keyword>.*"' src/wfx/src/wfx/components/
   ```
...
4. **Is this an wfx-runtime concern** (graph execution, schema, IO primitives) or a **Primeagent-app concern** (auth, DB, tracing)? Components are wfx; never import `primeagfent.services.*` from a component.
...
- **`documentation = "https://docs.primeagfent.org/..."`**: deep-link from the node UI. Example: `input_output/chat.py`.
...
- LLM with dynamic provider switching: `src/wfx/src/wfx/components/openai/openai_chat_model.py`
- Tool with `tool_mode` and legacy/replacement: `src/wfx/src/wfx/components/tools/calculator.py`
- Vector store with cache + tab inputs: `src/wfx/src/wfx/components/vectorstores/local_db.py`
- Chat input with `minimized`: `src/wfx/src/wfx/components/input_output/chat.py`
- Component with searchable metadata: `src/wfx/src/wfx/components/data_source/sql_executor.py`
- Legacy + replacement pattern: `src/wfx/src/wfx/components/flow_controls/sub_flow.py`
