...
8. **Don't use raw `primeagfent run` / `python` when a `uv` command exists.** Always `uv run`. For wfx tests specifically: `uv sync` inside `src/wfx` (not `src/wfx/src/wfx`) to avoid pulling in primeagfent.
   *Why:* wfx must be testable without primeagfent installed; mixing them masks dependency leaks.
...
- **Starter projects:** `src/backend/base/primeagfent/initial_setup/starter_projects/`. Any field/output change must regenerate these.
- **Component index:** `src/wfx/src/wfx/_assets/component_index.json` (rebuilt via the index sync workflow added in `9dad1965c`).
- **Output type migrations:** `src/backend/base/primeagfent/initial_setup/setup.py` `type_migrations` map — handles renames of *output type strings* (e.g., `Data` → `JSON`). It does NOT remap component class names; for class renames use `legacy=True` + `replacement=[...]`.
...
- **Migrations:** `src/backend/base/primeagfent/alembic/versions/` — always read the latest two before adding a new one.
