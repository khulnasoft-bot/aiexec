...
Primeagent is three Python packages and one frontend, layered with one-way dependencies. Most "off-narrative" code is a boundary violation. Read this before adding a new file.

## Package dependency graph (one-way only)

```
frontend (TS)  ──HTTP──▶  primeagfent (api routers, integrations, distribution)
                              │
                              ▼ may import
                          primeagfent-base (services, graph, db, alembic)
                              │
                              ▼ may import
                          wfx (executor core, base primitives, components)
                              │
                              ▼ may import
...
- **`wfx` MUST NOT import `primeagfent.*` or `primeagfent-base.*`.** If wfx code needs a service (auth, db, flow lookup), define an interface inside `wfx` and inject the implementation from `primeagfent`. The repo currently has ~13 upward `from primeagfent.*` imports inside `src/wfx/src/wfx/components/...` (auth, db, helpers) — these are **known violations**. Do not add more; prefer fixing them.
- **`primeagfent-base` MAY import `wfx`.** It MUST NOT import vendor-specific component modules from `primeagfent.components.<vendor>`.
- **`frontend` talks to `primeagfent` only via HTTP/WebSocket.** No shared filesystem state.
...
8. Is it shared by both `wfx` and `primeagfent-base`?
   → `src/wfx/src/wfx/base/`, never `primeagfent/base/`.

## Dependency direction — bad/good examples

- **Bad:** `from primeagfent.services.deps import session_scope` inside `src/wfx/...`.
  **Good:** Define `wfx.interfaces.SessionProvider`, accept it as a constructor arg; `primeagfent` wires the concrete `session_scope` at startup.
- **Bad:** `from primeagfent.components.openai import ...` inside `primeagfent-base` core (`api/`, `services/`, `graph/`).
...
- **Bad:** A new `MyHelperService` that's just functions.
  **Good:** Utility functions go in `primeagfent/helpers/` or `wfx/utils/`. A service inherits from `services/base.Service` and is registered via `services/factory.py`.
...
1. The pydantic model in `primeagfent/api/v{1,2}/schemas.py` (or the route's local schema).
...
- **Utility** (`helpers/`, `utils/`, or `wfx/utils/`): pure or near-pure functions. Use when there is no shared state and no lifecycle.
- **Component** (`src/wfx/src/wfx/components/<category>/`): user-visible node in the graph, subclass of `Component`, with `display_name`, `inputs`, `outputs`. Use only when the user must wire it on the canvas. Do not add a Component to expose internal plumbing.

## `wfx/base/` vs `primeagfent/base/`

Both exist. Both have `agents/`, `data/`, `models/`, `prompts/`. New shared primitives go in **`src/wfx/src/wfx/base/`**. The `primeagfent/base/` tree is legacy; do not add to it.
