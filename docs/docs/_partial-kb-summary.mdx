A Primeagent knowledge base is a vector database that stores embeddings for use in your flows.
...
* **Local storage by default**: Primeagent knowledge bases use Chroma local storage by default.
In contrast, only some vector store components support local databases.
* **Built-in embedding models**: Primeagent knowledge bases include built-in support for several embedding models.
...
* **Basic similarity search**: When querying Primeagent knowledge bases, only standard similarity search is supported.
...
* **Structured data**: Primeagent knowledge bases only support structured data.
