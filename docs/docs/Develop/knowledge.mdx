...
Primeagent provides several components to help you store and retrieve vector data in your flows, including embedding models, vector stores, and knowledge bases.
...
Knowledge bases are stored local to your Primeagent instance.
The default storage location depends on your operating system and installation method:

- **Primeagent Desktop**:
    - **macOS**: `/Users/<username>/.primeagfent/knowledge_bases`
    - **Windows**: `C:\Users\<name>\AppData\Roaming\com.PrimeagentDesktop\knowledge_bases`
- **Primeagent OSS**:
    - **macOS/Windows/Linux/WSL with `uv pip install`**: `<path_to_venv>/lib/python3.12/site-packages/primeagfent/knowledge_bases` (Python version can vary. Knowledge bases aren't shared between virtual environments.)
    - **macOS/Windows/Linux/WSL with `git clone`**: `<path_to_clone>/src/backend/base/primeagfent/knowledge_bases`

If you set the `PRIMEAGFENT_CONFIG_DIR` environment variable, the `knowledge_bases` subdirectory is created relative to that path.

To change the default `knowledge_bases` directory path, set the `PRIMEAGFENT_KNOWLEDGE_BASES_DIR` environment variable:

```bash
export PRIMEAGFENT_KNOWLEDGE_BASES_DIR="/path/to/parent/directory"
```
...
2. To connect Chroma Cloud to Primeagent, click **Settings**, and then click **DB Providers**.
...
5. Click **Save and Use Chroma Cloud**.
    ...
    Optionally, click **Test Connection** to verify that Primeagent can reach your Chroma Cloud instance before saving.

    The Chroma Cloud database is now connected to Primeagent as a knowledge base provider.
    To create a knowledge base using this provider, see [Create a knowledge base](#create-a-knowledge-base).

#### OpenSearch

To use OpenSearch as a database provider, you need a running OpenSearch cluster that is accessible to your Primeagent instance.
...
3. To connect the OpenSearch database to Primeagent as a knowledge base, click **Settings**, and then click **DB Providers**.
4. Select **OpenSearch**.
5. Enter the following values for the local OpenSearch container:
    ...
    - **Default Index name**: Enter `primeagfent_knowledge`. The OpenSearch index to write and read from. This index is created in the later ingestion step, so it isn't immediately available.
...
6. Click **Save and Use OpenSearch**.
    ...
    Optionally, click **Test Connection** to verify that Primeagent can reach your OpenSearch cluster before saving.

    The OpenSearch database is now connected to Primeagent as a knowledge base provider.
