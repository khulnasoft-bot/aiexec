...
3. In **File Name**, enter a file name and an optional path.
    ...
    * **Default location**: If you only provide a file name, then the file is stored in the Primeagent data directory. For example,`~/Library/Caches/primeagfent/data` on macOS.
...
5. To test the **Write File** component, click <Icon name="Play" aria-hidden="true" /> **Run component**, and then click <Icon name="TextSearch" aria-hidden="true" /> **Inspect output** to get the filepath where the file was saved.
    ...
    ```text
    DataFrame saved successfully as 'my_file.csv' at /Users/user.name/Library/Caches/primeagfent/data/my_file.csv
    ```
