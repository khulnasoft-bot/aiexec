...
for file in "${JS_FILES[@]}"; do
  ...
  if ! node --check "$file" >/tmp/primeagfent-js-check.out 2>/tmp/primeagfent-js-check.err; then
  ...
  if [[ "$MODE" == "execute" ]]; then
    ...
    if ! node "$file" >/tmp/primeagfent-js-example.out 2>/tmp/primeagfent-js-example.err; then
      echo "FAIL  $rel (execution)"
      print_failure_logs "/tmp/primeagfent-js-example.out" "/tmp/primeagfent-js-example.err"
      ((FAIL+=1))
      continue
    fi
  fi
