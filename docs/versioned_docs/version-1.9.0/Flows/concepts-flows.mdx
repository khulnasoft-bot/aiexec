...
Saved flow versions are stored in the database configured for your Primeagent deployment.

When restoring a saved version, if **Save current draft before restoring** is enabled, then Primeagent creates a backup of your current work before the selected version replaces it.
...
When previewing a saved version, Primeagent temporarily pauses auto-save so the preview doesn't overwrite your current draft.
