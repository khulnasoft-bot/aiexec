...
for file in "${SH_FILES[@]}"; do
  ...
  if [[ "$MODE" == "execute" ]]; then
    ...
    if ! bash "$file" >/tmp/primeagfent-curl-example.out 2>/tmp/primeagfent-curl-example.err; then
      echo "FAIL  $rel (execution)"
      print_failure_logs "/tmp/primeagfent-curl-example.out" "/tmp/primeagfent-curl-example.err"
      ((FAIL+=1))
      continue
    fi
  fi
