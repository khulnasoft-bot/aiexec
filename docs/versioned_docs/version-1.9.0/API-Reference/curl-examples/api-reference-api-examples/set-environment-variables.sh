...
export PRIMEAGFENT_API_KEY="${PRIMEAGFENT_API_KEY:-sk-local-placeholder}"
export PRIMEAGFENT_SERVER_URL="${PRIMEAGFENT_SERVER_URL:-http://localhost:7860}"
...
curl --request POST \
  --url "$PRIMEAGFENT_SERVER_URL/api/v1/run/$FLOW_ID?stream=false" \
  --header "Content-Type: application/json" \
  --header "x-api-key: $PRIMEAGFENT_API_KEY" \
  --data '{
