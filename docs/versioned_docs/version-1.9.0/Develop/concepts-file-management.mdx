...
To modify this value, change the `PRIMEAGFENT_MAX_FILE_SIZE_UPLOAD` [environment variable](/environment-variables).
...
* When you trigger a flow with the `/api/v1/run/$FLOW_ID` endpoint, you can use the `files` parameter to attach image data as a base64-encoded string:
   ...
   ```bash
   curl -X POST "http://$PRIMEAGFENT_SERVER_ADDRESS/api/v1/run/$FLOW_ID" \
...
* **Local storage**: Primeagent's default storage backend. Files are stored locally in your [Primeagent configuration directory](/memory). Set `PRIMEAGFENT_STORAGE_TYPE=local` or leave it unset to use local storage.
...
PRIMEAGFENT_STORAGE_TYPE=s3
PRIMEAGFENT_OBJECT_STORAGE_BUCKET_NAME=S3_BUCKET_NAME
PRIMEAGFENT_OBJECT_STORAGE_PREFIX=S3_BUCKET_DIRECTORY
...
|----------|--------|---------|-------------|
| `PRIMEAGFENT_STORAGE_TYPE` | String | `local` | Set the file storage backend. Supported values: `local` (files stored in the Primeagent configuration directory) or `s3` (files stored in AWS S3). For S3 storage, you must also configure AWS credentials and bucket settings. |
| `PRIMEAGFENT_OBJECT_STORAGE_BUCKET_NAME` | String | Not set | The name of the S3 bucket to use for file storage. Required when `PRIMEAGFENT_STORAGE_TYPE=s3`. |
| `PRIMEAGFENT_OBJECT_STORAGE_PREFIX` | String | Not set | Optional prefix/folder path within the S3 bucket where files will be stored. If not set, files are stored at the bucket root. |
| `PRIMEAGFENT_OBJECT_STORAGE_TAGS` | JSON object | Not set | Optional S3 object tags applied to stored files when `PRIMEAGFENT_STORAGE_TYPE=s3`. Ignored for local storage. Provided as a JSON map of string keys to string values, such as `{"env": "prod", "owner": "data-team"}`. |
