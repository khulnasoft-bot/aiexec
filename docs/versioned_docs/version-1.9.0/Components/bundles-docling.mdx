...
The Docling dependency is required to use the Docling components in Primeagent.
    ...
    * **Primeagent Desktop**: Set `PRIMEAGFENT_DOCLING=True` in your `.env` file to enable Docling dependency installation. For more information, see [Set environment variables for Primeagent Desktop](/environment-variables#set-environment-variables-for-primeagfent-desktop).

    * **Primeagent OSS**:
        * If you installed `primeagfent` (`uv pip install primeagfent`), Docling is included automatically through bundled extras.
        * If you installed `primeagfent-base` directly, install Docling with an extra, for example `uv pip install "primeagfent-base[docling]"`.
    ...
    * **Earlier versions**: Primeagent versions earlier than 1.6 don't include the Docling dependency by default.
