...
If you're creating custom components in a different location using the `PRIMEAGFENT_COMPONENTS_PATH` [environment variable](/environment-variables), components must be similarly organized in a specific directory structure to be displayed in the visual editor.
...
To increase the depth limit, modify `MAX_DEPTH` in `src/wfx/src/wfx/custom/directory_reader/directory_reader.py`. Subdirectories will still appear as separate categories, not nested hierarchies.
...
For an additional example of lazy loading, see the [FAISS component](https://github.com/khulnasoft/primeagfent/blob/main/src/wfx/src/wfx/components/FAISS/__init__.py).
...
1. To rebuild the backend and frontend, run `make install_frontend && make build_frontend && make install_backend && uv run primeagfent run --port 7860`.
...
When running Primeagent in Docker, mount your custom components directory and set the `PRIMEAGFENT_COMPONENTS_PATH` environment variable in the `docker run` command to point to the custom components directory.
...
docker run -d \
  ...
  -e PRIMEAGFENT_COMPONENTS_PATH=/app/custom_components \
...
```
/app/custom_components/          # PRIMEAGFENT_COMPONENTS_PATH
    └── data/
        ├── __init__.py
        └── dataframe_processor.py
```
...
For a list of all available parameters, see the [BaseInputMixin definition](https://github.com/khulnasoft/primeagfent/blob/main/src/wfx/src/wfx/inputs/input_mixin.py) in the Primeagent codebase.

For a list of all available input types, see the [input type definitions](https://github.com/khulnasoft/primeagfent/blob/main/src/wfx/src/wfx/inputs/inputs.py) in the Primeagent codebase.
...
The `build_df` method processes data and returns a `Table`. The `-> DataFrame` type annotation helps Primeagent validate connections and provides color-coding in the visual editor. You can also set `self.status` to show progress messages in the UI.

For a complete list of all available parameters, see the [Output class definition](https://github.com/khulnasoft/primeagfent/blob/main/src/wfx/src/wfx/template/field/base.py) in the Primeagent codebase. Common parameters include:
