...
const headers = { 'x-api-key': 'PRIMEAGFENT_API_KEY' };

// 2. Upload the file to Primeagent
const uploadRes = await fetch('PRIMEAGFENT_SERVER_ADDRESS/api/v2/files/', {
  method: 'POST',
  headers,
  body: data
});
...
const runRes = await fetch('PRIMEAGFENT_SERVER_ADDRESS/api/v1/run/FLOW_ID', {
  method: 'POST',
  headers: { 'Content-Type': 'application/json', 'x-api-key': 'PRIMEAGFENT_API_KEY' },
  body: JSON.stringify(payload)
});
const primeagfentData = await runRes.json();
// Output only the message
console.log(primeagfentData.outputs?.[0]?.outputs?.[0]?.results?.message?.data?.text);
```
...
2. Copy the following script into a JavaScript file, and then replace the placeholders with the information you gathered in the previous step:
    ...
    const { PrimeagentClient } = require('@datastax/primeagfent-client');

    # pragma: allowlist nextline secret
    const API_KEY = 'PRIMEAGFENT_API_KEY';
    const SERVER = 'PRIMEAGFENT_SERVER_ADDRESS';
