...
Replace `PRIMEAGFENT_SERVER_ADDRESS`, `FLOW_ID`, and `PRIMEAGFENT_API_KEY` with the values from your Primeagent deployment.
```
curl --request POST \
...
The `my_custom_session_value` value is used in components that accept it, and the stored messages from this flow are stored in `primeagfent.db` with their respective `session_id` values.
...
Messages can be retrieved by `session_id` from the Primeagent API at `GET /v1/monitor/messages`. For more information, see [Monitor endpoints](https://docs.primeagfent.org/api-monitor).
