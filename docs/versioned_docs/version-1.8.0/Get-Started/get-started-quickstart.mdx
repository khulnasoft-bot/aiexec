...
- Create a [Primeagent API key](/api-keys-and-authentication)
    ...
    5. To use your Primeagent API key in a request, set a `PRIMEAGFENT_API_KEY` environment variable in your terminal, and then include an `x-api-key` header or query parameter with your request.
    For example:

        ```bash
        # Set variable
        export PRIMEAGFENT_API_KEY="sk..."

        # Send request
        curl --request POST \
        --url "http://PRIMEAGFENT_SERVER_ADDRESS/api/v1/run/FLOW_ID" \
...
3. In the **Agent** component, select your configured model from the **Language Model** dropdown.
    ...
    * Edit Primeagent's global <Icon name="BrainCog" aria-hidden="true" /> **Models** configuration. These providers and models are part of Primeagent's core functionality. Use the **Ollama** provider to connect to any model hosted on a local or remote Ollama instance.
...
1. When editing a flow, click **Share**, and then click **API access**.
    ...
    The code snippets automatically include the `PRIMEAGFENT_SERVER_ADDRESS` and `FLOW_ID` values for the flow, and a script to include your `PRIMEAGFENT_API_KEY` if you've set it as an environment variable in your terminal session.
    ...
    url = "http://PRIMEAGFENT_SERVER_ADDRESS/api/v1/run/FLOW_ID"  # The complete API endpoint URL for this flow
    ...
    fetch('http://PRIMEAGFENT_SERVER_ADDRESS/api/v1/run/FLOW_ID', options)
...
This script runs a question-and-answer chat in your terminal and stores the agent's previous answer so you can compare them.
    ...
    const url = 'http://PRIMEAGFENT_SERVER_ADDRESS/api/v1/run/FLOW_ID';
