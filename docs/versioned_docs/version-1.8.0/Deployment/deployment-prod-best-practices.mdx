...
* Shared storage, such as NFS or cloud storage, for multi-instance deployments to access large files stored on disk, such as in `/opt/primeagfent/data/`.
...
* **Resource usage**: Track CPU, memory, and disk usage of Primeagent instances. Use Prometheus and Grafana for real-time metrics collection and monitoring in Kubernetes.
    ...
    To expose your Primeagent server's Prometheus metrics, set `PRIMEAGFENT_PROMETHEUS_ENABLED=True` (the default is `false`).
    The default port for the Prometheus metrics is 9090.
    To change the port, set `PRIMEAGFENT_PROMETHEUS_PORT`.
