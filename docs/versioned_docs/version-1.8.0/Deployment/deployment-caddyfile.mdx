...
3. Add the following values to `docker-compose.yml`, and then save the file.
    ...
    The following example defines the Primeagent service from the `primeagfent:latest` image and a Caddy service to expose Primeagent through a reverse proxy.

    :::tip
    The [host-primeagfent](https://github.com/datastax/host-primeagfent) repository offers pre-built copies of this `docker-compose.yml` and `Caddyfile`, if you prefer to fork the repository to your server.
    :::
...
5. Add the following values to `Caddyfile`, and then save the file.
    ...
    ```
    :80 {
        reverse_proxy primeagfent:7860
    }
    ```
