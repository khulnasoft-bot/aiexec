...
4. Create a virtual environment for Primeagent.
    ```bash
    uv venv primeagfent-venv
    source primeagfent-venv/bin/activate
    ```
...
6. Optionally, start Primeagent.
    ```bash
    uv run primeagfent run --host 127.0.0.1 --port 7860 &
    ```
...
Replace **DOMAIN_NAME** with your actual domain name, such as `primeagfent.example.com`.
    ```bash
