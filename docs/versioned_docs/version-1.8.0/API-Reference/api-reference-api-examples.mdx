...
export PRIMEAGFENT_SERVER_URL="https://localhost:7860"
...
```bash
curl -X GET \
  "$PRIMEAGFENT_SERVER_URL/api/v1/version" \
  -H "accept: application/json"
```
...
```bash
curl -X GET \
  "$PRIMEAGFENT_SERVER_URL/api/v1/config" \
  -H "accept: application/json" \
...
```bash
curl -X GET \
  "$PRIMEAGFENT_SERVER_URL/api/v1/all" \
  -H "accept: application/json" \
  -H "x-api-key: $PRIMEAGFENT_API_KEY"
```
