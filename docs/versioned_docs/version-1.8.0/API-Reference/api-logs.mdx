...
To enable log retrieval, set the following [environment variables](/environment-variables) in your Primeagent `.env` file, and then start Primeagent with `uv run primeagfent run --env-file .env`:

```text
PRIMEAGFENT_ENABLE_LOG_RETRIEVAL=True
PRIMEAGFENT_LOG_RETRIEVER_BUFFER_SIZE=10000 // Must be greater than 0
PRIMEAGFENT_LOG_LEVEL=DEBUG // Can be DEBUG, ERROR, INFO, WARNING, or CRITICAL
```
...
```text
{
  ...
  "1736354775699": "2025-01-08T11:46:15.699661-0500 DEBUG File 046-rocket.svg retrieved successfully from flow /Users/USER/Library/Caches/primeagfent/profile_pictures/Space.\n"
}
```
