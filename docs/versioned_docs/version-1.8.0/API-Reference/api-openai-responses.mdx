...
In the following examples, replace the values for `PRIMEAGFENT_SERVER_URL`, `PRIMEAGFENT_API_KEY`, and `FLOW_ID` with values from your deployment.
<Tabs groupId="client">
...
client = OpenAI(
  base_url="PRIMEAGFENT_SERVER_URL/api/v1/",
  default_headers={"x-api-key": "PRIMEAGFENT_API_KEY"},
...
| `X-PRIMEAGFENT-GLOBAL-VAR-*` | No | Global variables for the flow | `"X-PRIMEAGFENT-GLOBAL-VAR-API_KEY: sk-..."` For more, see [Pass global variables to your flows in headers](#global-var). |
...
```bash
curl -X POST \
  "http://$PRIMEAGFENT_SERVER_URL/api/v1/responses" \
  -H "x-api-key: $PRIMEAGFENT_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "$FLOW_ID",
...
The `/responses` endpoint accepts global variables as custom HTTP headers with the format `X-PRIMEAGFENT-GLOBAL-VAR-{VARIABLE_NAME}`.
...
Variables passed with `X-PRIMEAGFENT-GLOBAL-VAR-{VARIABLE_NAME}` are always available to your flow, regardless of whether they exist in the database.
