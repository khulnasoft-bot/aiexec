---
title: Deploy Primeagent with multiple workers
...
By default, Primeagent runs with a single worker process and stores build job state in memory.
...
A multi-worker deployment runs more than one worker process on the same host. Concurrency can be increased by increasing the number of `PRIMEAGFENT_WORKERS`, but each process keeps its own in-memory build queue unless you add a shared store.
...
To configure a multi-worker Primeagent process, follow the steps to enable the Redis job queue.

After the Redis job queue is configured, you can optionally follow [recommended Gunicorn settings](#recommended-gunicorn-settings) to reduce memory use and keep workers healthy. This tuning applies to Linux production hosts only. On Windows and macOS, `primeagfent run` uses a single Uvicorn process.

## Prerequisites

- **Redis 6 or later**, reachable from all Primeagent worker processes.
- All workers configured with the same `PRIMEAGFENT_JOB_QUEUE_TYPE`. Mixed-mode deployments (some workers using `asyncio`, others using `redis`) are not supported.
...
```text
PRIMEAGFENT_WORKERS=3  # any value > 1
PRIMEAGFENT_JOB_QUEUE_TYPE=redis
PRIMEAGFENT_REDIS_QUEUE_URL=redis://your-redis-host:6379/1
```

Redis authentication and TLS are only supported through `PRIMEAGFENT_REDIS_QUEUE_URL`.
The individual host/port settings `PRIMEAGFENT_REDIS_QUEUE_HOST` and `PRIMEAGFENT_REDIS_QUEUE_PORT` create a plain, unauthenticated connection.
If you use a managed Redis service with auth or TLS, you must use `PRIMEAGFENT_REDIS_QUEUE_URL`.

## Example: multi-worker Docker Compose

This example runs three Primeagent workers sharing a Redis job queue and a PostgreSQL database.
...
1. Paste the following example into a Docker Compose file named `docker-compose.yml`:
   ...
   services:
     primeagfent:
       image: khulnasoft/primeagfent:1.10.0
       pull_policy: always
       ports:
         - "7860:7860"
       depends_on:
       ...
       environment:
         ...
         - PRIMEAGFENT_WORKERS=3  # any value > 1
         - PRIMEAGFENT_GUNICORN_PRELOAD=true
         - PRIMEAGFENT_JOB_QUEUE_TYPE=redis
         - PRIMEAGFENT_REDIS_QUEUE_URL=redis://redis:6379/1
         - PRIMEAGFENT_SUPERUSER=admin
         - PRIMEAGFENT_SUPERUSER_PASSWORD=changeme
...
3. Watch the logs until Primeagent is ready:

   ```bash
   docker compose logs -f primeagfent
   ```
...
7. Open the Primeagent UI and build a flow by sending a message to the flow in the **Playground**.
...
|---|---|---|
| `PRIMEAGFENT_JOB_QUEUE_TYPE` | `asyncio` | Job queue backend. Set to `redis` to enable the cross-worker queue. |
| `PRIMEAGFENT_REDIS_QUEUE_URL` | Not set | Full Redis connection URL. Takes priority over `HOST`/`PORT`/`DB` when set. Use this for any Redis instance that requires authentication or TLS. |
| `PRIMEAGFENT_REDIS_QUEUE_HOST` | `PRIMEAGFENT_REDIS_HOST` | Redis host for the job queue. Falls back to the general Redis host setting. Does not support auth or TLS (use `PRIMEAGFENT_REDIS_QUEUE_URL` instead for secured instances). |
| `PRIMEAGFENT_REDIS_QUEUE_PORT` | `PRIMEAGFENT_REDIS_PORT` | Redis port for the job queue. Falls back to the general Redis port setting. |
| `PRIMEAGFENT_REDIS_QUEUE_DB` | `1` | Redis database index for the job queue. Must differ from the cache database index (default `0`) to avoid key collisions. |
| `PRIMEAGFENT_REDIS_QUEUE_TTL` | `3600` | TTL in seconds for job stream and ownership keys in Redis. |
| `PRIMEAGFENT_REDIS_QUEUE_STARTUP_GRACE_S` | `30.0` | Seconds a consumer waits for the producer's first write before treating a missing stream as end-of-stream. Increase this if your workers have slow cold-starts. Setting to `0` removes the grace period so a not-yet-created stream is treated as EOF immediately. |
| `PRIMEAGFENT_REDIS_QUEUE_CANCEL_CHANNEL_ENABLED` | `True` | When `true`, each worker runs a Redis pub/sub dispatcher so that `POST /build/{id}/cancel` cancels a build on any worker, not just the one that received the request. Closing a browser tab also signals cancel cross-worker. |
| `PRIMEAGFENT_REDIS_QUEUE_CANCEL_MARKER_TTL` | `60` | TTL in seconds for the cancel-marker key. The marker closes a race where a cancel signal is published before the target worker's dispatcher has subscribed. A non-positive value is rejected at startup. |
| `PRIMEAGFENT_REDIS_QUEUE_POLLING_STALE_THRESHOLD_S` | `90.0` | Seconds without client activity before the watchdog cancels an abandoned polling build. Set to `0` to disable the watchdog entirely. |
| `PRIMEAGFENT_REDIS_QUEUE_POLLING_WATCHDOG_INTERVAL_S` | `15.0` | How often in seconds the watchdog scans for stale jobs. Lower values reclaim resources faster at the cost of more Redis reads. |
| `PRIMEAGFENT_GUNICORN_PRELOAD` | `False` | **Experimental.** Loads the app in the Gunicorn master process before workers fork, reducing per-worker startup overhead. Pairs well with `PRIMEAGFENT_WORKERS`. Non-Windows only. |
...
```bash
curl -H "Authorization: Bearer $PRIMEAGFENT_SUPERUSER_TOKEN" \
...
| `cancel_stats.polling_watchdog_kills` | Number of abandoned polling builds reclaimed by the watchdog. Non-zero is normal under load. A very high count may indicate frequent client disconnects (consider increasing `PRIMEAGFENT_REDIS_QUEUE_POLLING_STALE_THRESHOLD_S`). |
...
On Windows and macOS, `primeagfent run` uses one Uvicorn process, so `PRIMEAGFENT_GUNICORN_PRELOAD` and `GUNICORN_CMD_ARGS` have no effect.
:::

These recommendations are starting points from Primeagent engineering benchmarks on Linux multi-worker deployments.
After you apply these values, monitor performance, and then make adjustments using `htop` or `btop` while you run flows.

Add the starter values to your server's `.env` file after the [Redis job queue](#enable-the-redis-job-queue) settings, then restart Primeagent. Copy the block that best matches your server:

<Tabs>
<TabItem value="small" label="Small / dev (4–8 GB RAM)" default>

Start with fewer workers so Primeagent and local databases do not hit OOM.

```text
PRIMEAGFENT_WORKERS=5
PRIMEAGFENT_WORKER_TIMEOUT=120
PRIMEAGFENT_GUNICORN_PRELOAD=true
...
A balanced default for a dedicated Primeagent host.

```text
PRIMEAGFENT_WORKERS=15
...
```text
PRIMEAGFENT_WORKERS=30
PRIMEAGFENT_WORKER_TIMEOUT=600
PRIMEAGFENT_GUNICORN_PRELOAD=true
...
- **`PRIMEAGFENT_WORKERS`** — How many Gunicorn worker processes run for concurrency.
- **`PRIMEAGFENT_WORKER_TIMEOUT`** — How long a worker may handle a single request before Gunicorn kills it. Raise it if you expect long agent runs.
- **`PRIMEAGFENT_GUNICORN_PRELOAD`** — Loads the app once in the Gunicorn master before workers fork so Linux can share memory across workers through Copy-on-Write. Recommended to leave enabled in multi-worker deployments for memory savings. Safe to leave off; behavior matches older releases when `false`.
...
For more information, see the [Scaling Primeagent blog post](https://www.primeagfent.org/blog/scaling-primeagfent).