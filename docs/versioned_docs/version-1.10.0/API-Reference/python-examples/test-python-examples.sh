...
for file in "${PY_FILES[@]}"; do
  ...
  if [[ "$MODE" == "execute" ]]; then
    # api-openai-responses/* call Primeagent's HTTP API (e.g. /api/v1/responses) with x-api-key;
    ...
    if [[ -z "${PRIMEAGFENT_API_KEY:-}" || ( -z "${PRIMEAGFENT_URL:-}" && -z "${PRIMEAGFENT_SERVER_URL:-}" ) ]]; then
      echo "SKIP  $rel (set PRIMEAGFENT_API_KEY and PRIMEAGFENT_URL or PRIMEAGFENT_SERVER_URL to execute)"
      ((SKIP+=1))
      continue
    fi
    ...
    if ! uv run python - "$file" "$PY_TIMEOUT_SECONDS" >/tmp/primeagfent-python-example.out 2>/tmp/primeagfent-python-example.err <<'PY'
import runpy
import signal
import sys
...
PY
    then
      ...
      print_failure_logs "/tmp/primeagfent-python-example.out" "/tmp/primeagfent-python-example.err"
      ((FAIL+=1))
      continue
    fi
  fi
