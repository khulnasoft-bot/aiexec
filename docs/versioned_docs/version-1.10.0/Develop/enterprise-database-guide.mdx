...
2. Obtain the connection string in the format `postgresql://user:password@host:port/dbname`, such as`postgresql://primeagfent:securepassword@postgres:5432/primeagfent`.
...
3. Configure Primeagent with the `.env` or `docker-compose.yml` files.
    ...
    1. Create a `.env` file in the `primeagfent` directory:
    ...
    2. Add the connection string to the `.env` file:
        ...
        ```text
        PRIMEAGFENT_DATABASE_URL="postgresql://primeagfent:securepassword@postgres:5432/primeagfent"
        ```
...
5. Optional: Run migrations.
    ...
    1. Run `primeagfent migration` to preview the changes.

    2. Review the changes to ensure that it's safe to proceed with the migration.

    3. Run `primeagfent migration --fix` to run the migration and permanently apply the changes.

        This is a destructive operation that can delete data.
        For more information, see [`primeagfent migration`](/configuration-cli#primeagfent-migration).
...
You can query the database in two ways:
    ...
    * Query the database container:
        ...
        ```
        docker exec -it <postgres-container> psql -U primeagfent -d primeagfent
        ```

    * Use SQL:

        ```
        SELECT * FROM pg_stat_activity WHERE datname = 'primeagfent';
        ```
...
If you have a multi-instance deployment, make sure all of your Primeagent instances connect to the same HA PostgreSQL database.
    ...
    * Use a virtual IP or DNS name that resolves to the current primary database, such as `postgresql://primeagfent:securepassword@db-proxy:5432/primeagfent?sslmode=require`.
    * Use the provided endpoint for a managed service, such as `primeagfent.cluster-xyz.us-east-1.rds.amazonaws.com`.
...
The following example configuration is for a production deployment that has three `primeagfent-runtime` replicas, uses the Kubernetes load balancer service to distribute traffic to healthy pods, and uses the HA PostgreSQL database connection string.

```yaml
apiVersion: apps/v1
kind: Deployment
metadata:
  name: primeagfent-runtime
spec:
  replicas: 3
  selector:
    matchLabels:
      app: primeagfent-runtime
  template:
    metadata:
      labels:
        app: primeagfent-runtime
    spec:
      ...
      - name: primeagfent
        ...
        - containerPort: 7860
        env:
        - name: PRIMEAGFENT_DATABASE_URL
          value: "postgresql://primeagfent:securepassword@db-proxy:5432/primeagfent?sslmode=require"
---
apiVersion: v1
kind: Service
metadata:
...
Primeagent, through [SQLAlchemy](https://docs.sqlalchemy.org/en/20/), supports reconnection attempts if `PRIMEAGFENT_DATABASE_CONNECTION_RETRY=True`, ensures recovery after failover, and reduces disruption once the database is back online.
