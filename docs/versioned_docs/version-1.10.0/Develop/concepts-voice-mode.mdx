...
:::warning
Voice mode is deprecated as of Primeagent 1.10.
...
Speech-to-text transcribes speech into the Playground's chat input field, but does not provide voice interaction with the Primeagent UI or text-to-speech responses.
...
To use voice mode, [Install the Primeagent OSS Python package](/get-started-installation#install-and-run-the-primeagfent-oss-python-package).
:::
...
Primeagent exposes an OpenAI Realtime API-compatible websocket endpoint for your flows.
...
The Primeagent API's websocket endpoint requires an [OpenAI API key](https://platform.openai.com/docs/overview) for authentication, and supports an optional [ElevenLabs](https://elevenlabs.io) integration with an ElevenLabs API key.
