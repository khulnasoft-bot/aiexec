...
This page is the reference for the `extension.json` manifest the Primeagent Extension loader consumes.

The canonical JSON Schema lives at [`https://schemas.primeagfent.org/extension/v1.json`](https://schemas.primeagfent.org/extension/v1.json) and is generated from the Pydantic model at [`wfx.extension.manifest.ExtensionManifest`](https://github.com/khulnasoft/primeagfent/blob/main/src/wfx/src/wfx/extension/manifest.py).

To export the live schema, run:

```bash
wfx extension schema --output extension.schema.json
```
...
| --- | --- | --- | --- |
| `id` | string | yes | Globally-unique extension ID. Lowercase, hyphenated, starts with a letter, 2-64 chars. Convention: `wfx-<name>`. |
| `version` | string | yes | SemVer 2.0.0 version string for this extension release. |
| `name` | string | yes | Human-readable display name shown in the Primeagent palette. 1-200 chars. |
| `description` | string \| null | no | Optional one-paragraph summary, max 2000 chars. |
| `wfx` | object | yes | [Compatibility declaration](#wfx-compatibility-declaration) against the BUNDLE_API contract. |
...
The schema strips these names from the published `properties` map but reserves them via [`x-deferred-fields`](https://schemas.primeagfent.org/extension/v1.json), so a manifest that sets one gets a specific error code instead of the generic "additional property" message.
...
If you'd rather not ship a separate `extension.json` next to `pyproject.toml`, declare the same fields under `[tool.primeagfent.extension]`:

```toml
[tool.primeagfent.extension]
id = "wfx-my-extension"
version = "0.1.0"
name = "My extension"

[tool.primeagfent.extension.wfx]
compat = ["1"]

[[tool.primeagfent.extension.bundles]]
name = "my_bundle"
...
The loader and validator both emit typed errors keyed by the manifest field that triggered them. The full code list is at [`wfx.extension.errors.ERROR_CODES`](https://github.com/khulnasoft/primeagfent/blob/main/src/wfx/src/wfx/extension/errors.py); the codes most relevant when authoring a manifest are:

| Code | Cause |
| --- | --- |
| `manifest-invalid` | Schema validation failed; the message names the field. |
| `manifest-not-found` | No `extension.json` and no `[tool.primeagfent.extension]` section at the extension root. |
| `version-constraint-unsatisfied` | `wfx.compat` does not include this Primeagent's `BUNDLE_API_VERSION`. |
...
Run `wfx extension validate <path>` to see every error as a structured object with `code`, `message`, `location`, `hint`, and `ref_url`.
