...
You can configure Primeagent to collect tracing data about your flow executions and automatically send the data to Openlayer for analysis, monitoring, and evaluation.
...
:::tip
If you need a flow to test the Openlayer integration, see the [Primeagent quickstart](/get-started-quickstart).
:::
...
3. Set your Openlayer credentials as environment variables in the same environment where you run Primeagent.
...
## Start Primeagent and view traces in Openlayer

1. Start Primeagent in the same environment where you set the Openlayer environment variables:

    ```bash
    uv run primeagfent run
    ```

2. Run a flow in Primeagent.

    Primeagent automatically collects and sends tracing data about the flow execution to Openlayer, including:
...
```bash
export OPENLAYER_PRIMEAGFENT_MAPPING='{"Flow Name 1":"pipeline-id-1","Flow Name 2":"pipeline-id-2"}'
```
...
```cmd
set OPENLAYER_PRIMEAGFENT_MAPPING={"Flow Name 1":"pipeline-id-1","Flow Name 2":"pipeline-id-2"}
```
...
2. JSON mapping: `OPENLAYER_PRIMEAGFENT_MAPPING`
...
To disable the Openlayer integration, remove the `OPENLAYER_API_KEY` environment variable, and then restart Primeagent.
