...
Primeagent sends events to the Playground during each agent run: the input message, each tool call with its input and result, streaming tokens as they arrive, and the final answer.
Primeagent writes the completed message to chat history when the run finishes.
When using the **Structured Response** output, Primeagent does not send events to the Playground and does not write to chat history.
