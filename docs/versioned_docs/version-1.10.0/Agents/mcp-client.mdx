...
Register MCP servers, and then add **MCP Tools** from the <McpIcon /> **MCP** sidebar as described in [Connect to a non-Primeagent MCP server](#mcp-stdio-mode).
...
5. To configure headers for your MCP server, enter each header in the **Headers** fields as key-value pairs.
    ...
    If this server is nested inside a Primeagent flow that runs as an MCP server, you can propagate the incoming request's `x-api-key` or `Authorization` header directly to this server at runtime.
...
To leverage flows-as-tools, register your Primeagent MCP endpoint as a server first, and then add the **MCP Tools** component from the <McpIcon /> **MCP** sidebar.

1. Register the Primeagent MCP server.
...
2. In the **MCP URL** field, enter your Primeagent server's MCP endpoint.
   ...
   All flows available from the targeted server are treated as tools. For authentication and project URLs, see [Use Primeagent as an MCP server](/mcp-server).
...
To add a new MCP server, click <Icon name="Plus" aria-hidden="true"/> **Add MCP Server**, register the server, and then select the server on the **MCP Tools** component as described in [Connect to a non-Primeagent MCP server](#mcp-stdio-mode).

Click <Icon name="Ellipsis" aria-hidden="true"/> **More** to edit or delete an MCP server connection.

If your Primeagent server administrator has locked MCP server management, you will receive a locked message when you try to add or modify servers. For more information, see [Lock MCP server management](/mcp-server#restrict-mcp-server-management).

## Modify MCP server environment variables with the API {#mcp-api-tweaks}

You can modify MCP server environment variables at runtime when running flows through the [Primeagent API](/api-reference-api-examples) by tweaking the **MCP Tools** component.

You can include tweaks with any Primeagent API request that supports the `tweaks` parameter, such as POST requests to the `/run` or `/webhook` endpoints.
...
3. Send a POST request to the Primeagent server's `/run` endpoint, and include tweaks to the **MCP Tools** component.
    ...
    PRIMEAGFENT_SERVER_ADDRESS = "http://localhost:7860"
    FLOW_ID = "your-flow-id"
    PRIMEAGFENT_API_KEY = os.getenv("PRIMEAGFENT_API_KEY")
    MCP_TOOLS_COMPONENT_ID = "MCPTools-Bzahc"

    url = f"{PRIMEAGFENT_SERVER_ADDRESS}/api/v1/run/{FLOW_ID}?stream=false"
    headers = {
    ...
    ```typescript
    const PRIMEAGFENT_SERVER_ADDRESS = "http://localhost:7860";
    ...
    const url = `${PRIMEAGFENT_SERVER_ADDRESS}/api/v1/run/${FLOW_ID}?stream=false`;
    ...
    ```bash
    curl --request POST \
      --url "http://PRIMEAGFENT_SERVER_ADDRESS/api/v1/run/FLOW_ID?stream=false" \
    ...
    Replace `MCP_TOOLS_COMPONENT_ID`, `PRIMEAGFENT_API_KEY`, `PRIMEAGFENT_SERVER_ADDRESS`, and `FLOW_ID` with the actual values from your Primeagent deployment.

    Primeagent doesn't automatically discover or expose which environment variables your MCP server accepts from the **MCP Tools** component.
...
Enter a global variable name as the header value, and Primeagent resolves the global variable name to its actual value before making the MCP server request. Primeagent only passes the token value to your server; it doesn't validate tokens on behalf of your MCP server.
...
5. To manage MCP server connections for your Primeagent client, click <McpIcon /> **MCP servers**, and then click <Icon name="ArrowUpRight" aria-hidden="true"/> **Manage Servers**, or click your profile icon, select **Settings**, and then click **MCP Servers**.
...
8. Click **Create Server**.
   ...
   If the connection succeeds, Primeagent shows the number of tools exposed by the server.
...
12. Test your flow to make sure the agent uses your server to respond to queries. Open the **Playground**, and then enter a prompt that uses a tool that you connected through the **MCP Tools** component.
    ...
    Primeagent automatically resolves `TEST_BEARER_TOKEN` to its actual value before sending the request to the MCP server. When your MCP server receives the request, the `Authorization` header contains the resolved token value.

## Propagate `x-api-key` to nested MCP servers {#propagate-x-api-key}

When Primeagent runs as an MCP server and a flow contains an [**MCP Tools** component](/mcp-tools) that calls an external server, you can forward the outer client's `x-api-key` or `Authorization` header at runtime.

In the nested server's **Headers** config, set the key and value to the same header name, such as `x-api-key` and `x-api-key`.
Primeagent reads the matching header from the incoming request and substitutes it before calling the nested server.

If the incoming request does not include the configured header, the literal string is passed through unchanged.

## See also

- [Primeagent MCP Client](./primeagfent-mcp-client.mdx)
