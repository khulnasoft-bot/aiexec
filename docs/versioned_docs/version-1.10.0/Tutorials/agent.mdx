...
The **Read File** component loads a file you upload to Primeagent, and the **Web Search** component performs a DuckDuckGo search.
...
In this example, the application sends a customer's email address to the Primeagent agent. The agent reads the customer's previous orders from the **Read File** component, searches the web for used versions of those items, and returns three results.
...
1. To construct a JavaScript application to connect to your flow, gather the following information:
    ...
    * `PRIMEAGFENT_SERVER_ADDRESS`: Your Primeagent server's domain. The default value is `127.0.0.1:7860`. You can get this value from the code snippets on your flow's [**API access** pane](/concepts-publish#api-access).
    ...
    * `PRIMEAGFENT_API_KEY`: A valid [Primeagent API key](/api-keys-and-authentication).
...
If not, modify the `const email = "isabella.rodriguez@example.com"` to search for a value in your dataset.
    ...
    ```js
    import { PrimeagentClient } from "@datastax/primeagfent-client";

    const PRIMEAGFENT_SERVER_ADDRESS = 'PRIMEAGFENT_SERVER_ADDRESS';
    const FLOW_ID = 'FLOW_ID';
    const PRIMEAGFENT_API_KEY = 'PRIMEAGFENT_API_KEY';
    ...
    async function runAgentFlow(): Promise<void> {
        try {
            ...
            const client = new PrimeagentClient({
                baseUrl: PRIMEAGFENT_SERVER_ADDRESS,
                apiKey: PRIMEAGFENT_API_KEY
            });

            console.log(`Connecting to Primeagent server at: ${PRIMEAGFENT_SERVER_ADDRESS} `);
            ...
            if (error instanceof Error) {
                if (error.message.includes('fetch')) {
                    console.error('\nMake sure your Primeagent server is running and accessible at:', PRIMEAGFENT_SERVER_ADDRESS);
                }
