...
The Docling bundle is included in the `wfx-docling` Extension bundle, which is installed automatically as part of `uv pip install primeagfent`.

If you need to install it separately, run:

```bash
uv pip install wfx-docling
...
Alternatively, if you installed Primeagent as a package:

```bash
uv pip install "primeagfent[docling]"
```
