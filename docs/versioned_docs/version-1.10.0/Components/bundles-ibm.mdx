...
The **IBM** bundle is included in the `wfx-ibm` Extension bundle, which is installed automatically as part of `uv pip install primeagfent`.

If you need to install it separately, run:

```bash
uv pip install wfx-ibm
...
| Name | Type | Description |
|------|------|-------------|
| **Table Name** (`collection_name`) | String | Input parameter. The name of your Db2 table to store vectors. Default: `PRIMEAGFENT_VECTORS`. The table will be created if it doesn't exist. |
