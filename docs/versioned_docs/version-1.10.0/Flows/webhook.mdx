...
By default, webhooks require API key authentication (`PRIMEAGFENT_WEBHOOK_AUTH_ENABLE=True`).

To allow webhooks to run without authentication (not recommended; use only in trusted environments), in your Primeagent `.env` file, set `PRIMEAGFENT_WEBHOOK_AUTH_ENABLE=False`. When disabled, requests to the webhook endpoint are treated as being sent by the flow owner.
...
```bash
curl -X POST "http://PRIMEAGFENT_SERVER_ADDRESS/api/v1/webhook/FLOW_ID" \
    -H "Content-Type: application/json" \
    -H "x-api-key: PRIMEAGFENT_API_KEY" \
    -d '{"id": "12345", "name": "alex", "email": "alex@email.com"}'
```
...
```bash
curl -X POST "http://PRIMEAGFENT_SERVER_ADDRESS/api/v1/webhook/FLOW_ID?x-api-key=PRIMEAGFENT_API_KEY" \
    -H "Content-Type: application/json" \
    -d '{"id": "12345", "name": "alex", "email": "alex@email.com"}'
```

Replace `PRIMEAGFENT_SERVER_ADDRESS`, `FLOW_ID`, and `PRIMEAGFENT_API_KEY` with values from your deployment.
