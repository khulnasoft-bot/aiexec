...
1. Fork the [Primeagent GitHub repository](https://github.com/khulnasoft/primeagfent).

2. Add the new remote to your local repository on your local machine:
```bash
git remote add FORK_NAME https://github.com/GIT_USERNAME/primeagfent.git
```
...
The instructions below are for running Primeagent from source. For development with hot reload, see [Set up development environment](#set-up-your-primeagfent-development-environment).
...
* To run Primeagent from the Windows Command Line:
    ...
    2. Copy the contents of the built `src/frontend/build` directory to `src/backend/base/primeagfent/frontend`.
...
If you just want to run Primeagent locally without making code changes, see [Run Primeagent from source](#run-primeagfent-from-source).
...
2. Fork the [Primeagent GitHub repository](https://github.com/khulnasoft/primeagfent).
...
6. To edit and create content, work with the `.mdx` files in the `primeagfent/docs/docs` directory.

    Create new files in `.mdx` format.

    Navigation is defined in `primeagfent/docs/sidebars.js`.
