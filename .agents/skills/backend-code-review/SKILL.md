...
- **File-focused review**: when the user points to specific files and asks for a review of those files (one file or a small, explicit set of files, e.g., `src/backend/base/primeagent/api/v1/flows.py`).
...
- db schema design: if the review scope includes code/files under `src/backend/base/primeagent/services/database/models/` or Alembic migrations under `src/backend/base/primeagent/alembic/versions/`, follow [references/db-schema-rule.md](references/db-schema-rule.md) to perform the review
...
- service abstraction: if the review scope contains table/model operations (e.g., `select(...)`, `session.execute(...)`, joins, CRUD) and is not already inside a service under `src/backend/base/primeagent/services/`, follow [references/repositories-rule.md](references/repositories-rule.md) to perform the review
