...
- Key directories:
  - Routes: `src/backend/base/primeagfent/api/v1/`, `src/backend/base/primeagfent/api/v2/`
  - Services: `src/backend/base/primeagfent/services/`
  - Models: `src/backend/base/primeagfent/services/database/models/`
  - Helpers: `src/backend/base/primeagfent/helpers/`
  - Components: `src/backend/base/primeagfent/components/`
...
- Suggested fix: Move domain/business logic into the appropriate service under `src/backend/base/primeagfent/services/`. Keep route handlers thin and orchestration-focused.
- Example:
  - Bad:
    ```python
...
- Severity: critical
- Description: Routes may depend on services, and services may depend on models and domain abstractions. Reversing this direction (for example, a model or service importing from `primeagfent.api`) creates cycles and leaks transport concerns into domain code. The dependency flow must be: Routes -> Services -> Models (never reverse).
...
- Example:
  - Bad:
    ```python
    # src/backend/base/primeagfent/services/database/models/flow/model.py
    from primeagfent.api.v1.schemas import FlowListCreate  # Model importing from API layer
  ...
  - Good:
    ...
    # src/backend/base/primeagfent/api/v1/flows.py (route layer handles serialization)
...
- Category: maintainability
- Severity: critical
- Description: Modules under `src/backend/base/primeagfent/helpers/` should remain reusable, business-agnostic building blocks. They must not encode product/domain-specific rules, workflow orchestration, or business decisions. Helpers may contain thin wrappers for user lookups or data transformation but must not implement business policy.
- Suggested fix:
  - If business logic appears in `src/backend/base/primeagfent/helpers/`, extract it into the appropriate service under `src/backend/base/primeagfent/services/` and keep helpers focused on generic, cross-cutting utilities.
  - Keep helper dependencies clean: avoid importing service or route modules into helpers.
- Example:
  - Bad:
    ```python
    # src/backend/base/primeagfent/helpers/flow.py
  ...
  - Good:
    ```python
    # src/backend/base/primeagfent/helpers/flow.py (business-agnostic helper)
    ...
    # src/backend/base/primeagfent/services/flow_service.py (business logic stays in service)
    from primeagfent.helpers.flow import is_older_than_days
...
- Category: best practices
- Severity: critical
- Description: Service classes and model definitions must never import FastAPI-specific constructs such as `Request`, `Response`, `HTTPException`, `APIRouter`, or `Depends`. This keeps the domain layer transport-agnostic and testable without spinning up an HTTP server. Primeagent services inherit from `primeagfent.services.base.Service` and receive dependencies through their factory's `create()` method or via constructor injection, not through FastAPI's `Depends()`.
...
- Example:
  - Bad:
    ```python
    # src/backend/base/primeagfent/services/variable/service.py
  ...
  - Good:
    ...
    # src/backend/base/primeagfent/api/v1/variables.py (route translates to HTTP)
...
- Example:
  ...
  - Good:
    ...
    if TYPE_CHECKING:
        from primeagfent.services.database.models.user.model import User
