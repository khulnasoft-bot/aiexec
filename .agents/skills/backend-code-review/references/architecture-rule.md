...
- Key directories:
  - Routes: `src/backend/base/primeagent/api/v1/`, `src/backend/base/primeagent/api/v2/`
  - Services: `src/backend/base/primeagent/services/`
  - Models: `src/backend/base/primeagent/services/database/models/`
  - Helpers: `src/backend/base/primeagent/helpers/`
  - Components: `src/backend/base/primeagent/components/`
...
- Severity: critical
- Description: Route handlers (FastAPI endpoint functions) should parse input, delegate to a service, and return a serialized response. Business decisions inside route handlers make behavior hard to reuse, test, and maintain. Primeagent uses FastAPI with `Depends()` for dependency injection and async handlers throughout.
- Suggested fix: Move domain/business logic into the appropriate service under `src/backend/base/primeagent/services/`. Keep route handlers thin and orchestration-focused.
- Example:
  - Bad:
    ```python
...
- Severity: critical
- Description: Routes may depend on services, and services may depend on models and domain abstractions. Reversing this direction (for example, a model or service importing from `primeagent.api`) creates cycles and leaks transport concerns into domain code. The dependency flow must be: Routes -> Services -> Models (never reverse).
...
- Example:
  - Bad:
    ```python
    # src/backend/base/primeagent/services/database/models/flow/model.py
    from primeagent.api.v1.schemas import FlowListCreate  # Model importing from API layer
  ...
  - Good:
    ...
    # src/backend/base/primeagent/api/v1/flows.py (route layer handles serialization)
...
- Category: maintainability
- Severity: critical
- Description: Modules under `src/backend/base/primeagent/helpers/` should remain reusable, business-agnostic building blocks. They must not encode product/domain-specific rules, workflow orchestration, or business decisions. Helpers may contain thin wrappers for user lookups or data transformation but must not implement business policy.
- Suggested fix:
  - If business logic appears in `src/backend/base/primeagent/helpers/`, extract it into the appropriate service under `src/backend/base/primeagent/services/` and keep helpers focused on generic, cross-cutting utilities.
  - Keep helper dependencies clean: avoid importing service or route modules into helpers.
- Example:
  - Bad:
    ```python
    # src/backend/base/primeagent/helpers/flow.py
  ...
  - Good:
    ```python
    # src/backend/base/primeagent/helpers/flow.py (business-agnostic helper)
    ...
    # src/backend/base/primeagent/services/flow_service.py (business logic stays in service)
    from primeagent.helpers.flow import is_older_than_days
...
- Category: best practices
- Severity: critical
- Description: Service classes and model definitions must never import FastAPI-specific constructs such as `Request`, `Response`, `HTTPException`, `APIRouter`, or `Depends`. This keeps the domain layer transport-agnostic and testable without spinning up an HTTP server. Primeagent services inherit from `primeagent.services.base.Service` and receive dependencies through their factory's `create()` method or via constructor injection, not through FastAPI's `Depends()`.
...
- Example:
  - Bad:
    ```python
    # src/backend/base/primeagent/services/variable/service.py
  ...
  - Good:
    ...
    # src/backend/base/primeagent/api/v1/variables.py (route translates to HTTP)
...
- Category: best practices
- Severity: suggestion
- Description: Components under `src/backend/base/primeagent/components/` represent flow nodes and are instantiated dynamically by the graph engine. They should depend on services and models but no other component should import from them. Component class names are stable identifiers used to match components in saved flows; renaming a component class is a breaking change.
- Suggested fix: If shared logic exists between components, extract it into a helper or a base class under `src/backend/base/primeagent/custom/` rather than creating cross-component imports.
