...
# primeagent-base ships the core framework only.  The extension bundles
# (wfx-duckduckgo, wfx-arxiv, wfx-ibm, wfx-docling) are intentionally NOT
# installed in this image: they are dependencies of the full ``primeagent``
# distribution, not of the lean ``primeagent-base`` core, and we keep that
# boundary at the image layer too.  Consumers who want those components
# should use the ``primeagent`` image, or ``pip install`` the bundle (e.g.
# ``wfx-duckduckgo``) alongside primeagent-base.
RUN --mount=type=cache,target=/root/.cache/uv \
    RUSTFLAGS='--cfg reqwest_unstable' \
