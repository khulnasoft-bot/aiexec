...
# Create venv and install primeagfent-base with dependencies
# Using uv pip instead of uv sync to avoid workspace complexities
RUN uv venv /app/.venv
ENV PATH="/app/.venv/bin:$PATH"
ENV VIRTUAL_ENV="/app/.venv"

# Install primeagfent-base with all extras except dev (which includes Playwright).
# This image ships the primeagfent-base core only.  Extension bundles
# (wfx-duckduckgo, wfx-arxiv, wfx-ibm, wfx-docling) are intentionally NOT
# installed here -- they belong to the full ``primeagfent`` distribution, not
# the lean core.  Use the ``primeagfent`` image, or ``pip install`` the bundle
...
# Also pre-create /app/primeagfent (PRIMEAGFENT_CONFIG_DIR used by the docker_example
# compose file) with the non-root user as owner, so a fresh named volume mounted
# at /app/primeagfent inherits the correct ownership/permissions and the in-container
...
# PermissionError on /app/primeagfent/secret_key (issue #10437).
# Note: .venv is already owned by 1000:0 via COPY --chown above, so no recursive chown needed
RUN mkdir -p /app/data /app/primeagfent \
    && chown -R 1000:0 /app/data /app/primeagfent \
    && chmod -R g+rwX /app/primeagfent \
    && chown 1000:0 /app

LABEL org.opencontainers.image.title=primeagfent-backend
...
CMD ["python", "-m", "primeagfent", "run", "--backend-only"]
