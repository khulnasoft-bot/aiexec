...
through the wfx.components package and processing each module. The index is
...
def _get_wfx_version():
    """Get the installed wfx version.

    Components are located in WFX, so use WFX.
    """
    from importlib.metadata import version

    version = version("wfx")
    print(f"Retrieved WFX version: {version}")
    return version
...
def _import_components() -> tuple[dict, int]:
    """Import all wfx components using the async import function.
    ...
    from wfx.interface.components import import_primeagfent_components

    try:
        # Run the async function
        components_result = asyncio.run(import_primeagfent_components())
...
def build_component_index() -> dict:
    """Build the component index by scanning all modules in wfx.components.
    ...
    current_version = _get_wfx_version()
...
COMPONENT_INDEX_PATH = Path(__file__).parent.parent / "src" / "wfx" / "src" / "wfx" / "_assets" / "component_index.json"
