...
2. **Scans backend files**: Finds all `.py` files in `src/backend/base/primeagfent/components/` with `legacy = True`
...
cd src/backend && python -m pytest --cov=src/backend/base/primeagfent --cov-config=.coveragerc
```
...
- **Input**: `src/backend/base/primeagfent/components/**/*.py` (legacy components)
