"""Rename ``wfx-*`` bundle packages to their ``-nightly`` counterparts.

Bundles under ``src/bundles/*`` follow the same package-rename convention as
``primeagfent``, ``primeagfent-base``, ``wfx``, and ``primeagfent-sdk``: for nightly
...
its ``wfx`` runtime dep so it resolves against the renamed ``wfx-nightly``
...
# Matches the wfx dep specifier inside a bundle pyproject's dependencies list.
# Accepts the bundle default ("wfx>=X.Y.Z" with an optional upper bound),
# legacy ~=/==, and the already-rewritten "wfx-nightly==X.Y.Z" form (idempotent).
...
def rename_bundle_pyproject(pyproject_path: Path, wfx_version: str, dev_n: str) -> tuple[str, str, str] | None:
    ...
    - ``"wfx>=...,<..."`` dep    → ``"wfx-nightly==<wfx_version>"``
    ...
    entry_point_pattern = re.compile(
        rf'(\[project\.entry-points\."primeagfent\.extensions"\]\s*\n)'
        rf"{re.escape(base_name)}(?:-nightly)?"
        rf'(\s*=\s*"[^"]+")'
    )
    content = entry_point_pattern.sub(rf"\g<1>{nightly_name}\g<2>", content, count=1)

    # Rewrite the wfx dep regardless of which form it's in.
    content = _WFX_DEP_PATTERN.sub(f'"wfx-nightly=={wfx_version}"', content)
...
def update_bundles_for_nightly(wfx_tag: str) -> list[tuple[str, str, str]]:
...
def main() -> None:
    ...
    Usage:
        update_bundle_versions.py <wfx_tag>

    ``wfx_tag`` is the WFX nightly tag (e.g., ``v0.5.0.dev38``); its ``.devN``
    suffix is reused for all bundles so they share a single nightly cadence.
    """
    expected_args = 2
    if len(sys.argv) != expected_args:
        print("Usage: update_bundle_versions.py <wfx_tag>")
        sys.exit(1)
