...
def update_uv_dep(base_version: str) -> None:
    ...
    # For the main project, update the primeagfent-base dependency in the UV section
    # Updated pattern to handle PEP 440 version suffixes, extras (e.g., [complete]),
    # and both ~= and == version specifiers
    # Also handles both primeagfent-base and primeagfent-base-nightly names
    # Captures extras in group 3 to preserve them in the replacement
    pattern = re.compile(
        r'(dependencies\s*=\s*\[\s*\n\s*)("primeagfent-base(?:-nightly)?((?:\[[^\]]+\])?)(?:~=|==|>=)[\d.]+(?:\.(?:post|dev|a|b|rc)\d+)*")'
    )
    ...
    # Keep the canonical `primeagfent-base` name with an exact `==<dev>` pin.
    replacement = rf'\1"primeagfent-base{extras}=={base_version}"'
