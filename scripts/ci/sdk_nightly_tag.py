"""Generate the nightly tag for the canonical ``primeagfent-sdk`` package.

Mirrors ``wfx_nightly_tag.py`` but for the SDK -- the nightly publishes ``primeagfent-sdk==<base>.devN``
to the canonical ``primeagfent-sdk`` PyPI project, so the dev counter is computed against that
...
PYPI_SDK_URL = "https://pypi.org/pypi/primeagfent-sdk/json"
...
def create_sdk_tag() -> str:
    """Return the next ``primeagfent-sdk`` nightly tag (with a leading ``v``)."""
