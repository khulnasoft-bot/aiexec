...
def update_pyproject_name(pyproject_path: str, new_project_name: str) -> None:
    ...
    extra (``"wfx-docling[local,chunking,image-description]"``) would still demand
    the stable ``wfx-docling`` after the package became ``wfx-docling-nightly``.
    """
    ...
    # primeagfent-base / primeagfent handling to every renamed package (wfx, the SDK,
    # and each `wfx-*` bundle). The leading quote + escaped exact name keep
    # `"wfx[..."` from matching `"wfx-docling[..."`.
...
def update_uv_dep(pyproject_path: str, new_project_name: str) -> None:
    ...
    if new_project_name == "primeagfent-nightly":
        pattern = re.compile(r"primeagfent = \{ workspace = true \}")
        replacement = "primeagfent-nightly = { workspace = true }"
    elif new_project_name == "primeagfent-base-nightly":
        pattern = re.compile(r"primeagfent-base = \{ workspace = true \}")
        replacement = "primeagfent-base-nightly = { workspace = true }"
    elif new_project_name == "primeagfent-sdk-nightly":
        pattern = re.compile(r"primeagfent-sdk = \{ workspace = true \}")
        replacement = "primeagfent-sdk-nightly = { workspace = true }"
    else:
...
def main() -> None:
    if len(sys.argv) != ARGUMENT_NUMBER:
        msg = "Must specify project name and build type, e.g. primeagfent-nightly base"
        raise ValueError(msg)
