"""Generate the nightly tag for the canonical ``wfx`` package.

The nightly publishes ``wfx==<base>.devN`` to the canonical ``wfx`` PyPI project (not a separate
``wfx-nightly`` distribution), so the dev counter is computed against ``wfx``'s own release history.
``<base>`` is the in-development version from
``src/wfx/pyproject.toml`` (the latest ``release-*`` branch the nightly builds from) and ``N`` is
...
def _wfx_base_version() -> str:
    """Return the base_version (e.g. "1.11.0") from src/wfx/pyproject.toml."""
    wfx_pyproject_path = Path(__file__).parent.parent.parent / "src" / "wfx" / "pyproject.toml"
    pyproject_data = tomllib.loads(wfx_pyproject_path.read_text())
...
def create_wfx_tag() -> str:
    """Return the next ``wfx`` nightly tag (with a leading ``v``)."""
    base_version = _wfx_base_version()
    dev_numbers = _dev_numbers(PYPI_WFX_URL, base_version)
