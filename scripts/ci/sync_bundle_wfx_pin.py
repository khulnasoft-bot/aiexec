...
Pin form: ``wfx>=X.Y.0,<(X+1).0.0`` -- floored at the current minor line,
capped below the next wfx major.  The cap is a coarse install-time guard
against an untested wfx major; fine-grained BUNDLE_API compatibility is still
enforced at load time by each ``extension.json``'s ``wfx.compat`` list against
the running wfx's ``BUNDLE_API_VERSION`` (see
...
def wfx_floor_spec(version: str) -> str:
    ...
    ``"1.10.0"`` -> ``"wfx>=1.10.0,<2.0.0"`` (floor at the minor line start,
    cap below the next wfx major). A leading ``v`` is tolerated.
    ...
    return f"wfx>={major}.{minor}.0,<{major + 1}.0.0"
