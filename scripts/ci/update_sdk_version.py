"""Update the canonical ``primeagfent-sdk`` version for nightly builds.

The SDK keeps its canonical ``primeagfent-sdk`` name -- it is NOT renamed to ``primeagfent-sdk-nightly`` --
...
def update_sdk_for_nightly(sdk_tag: str):
    """Set the canonical ``primeagfent-sdk`` package version for a nightly build."""
    sdk_pyproject_path = "src/sdk/pyproject.toml"

    version = sdk_tag.lstrip("v")
    update_pyproject_version(sdk_pyproject_path, version)

    print(f"Updated primeagfent-sdk to nightly version {version}")
