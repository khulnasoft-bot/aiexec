...
The nightly is published as canonical `.devN` pre-releases (e.g. `primeagfent==X.Y.Z.devN`), NOT
separate `*-nightly` distributions, so the dev counter is computed against the canonical
`primeagfent` / `primeagfent-base` PyPI histories (their `.devN` pre-releases; stable finals never
contribute). See `src/bundles/NIGHTLY.md`.

`primeagfent` (the nightly pre-release) pins an EXACT dependency on `primeagfent-base[complete]==X.Y.Z.devN`.
For the latest published nightly `primeagfent` to be installable, the base version it pins must
...
# published as canonical `.devN` pre-releases of `primeagfent` / `primeagfent-base`.
PYPI_PRIMEAGFENT_URL = "https://pypi.org/pypi/primeagfent/json"
PYPI_PRIMEAGFENT_BASE_URL = "https://pypi.org/pypi/primeagfent-base/json"

# main and base MUST share one dev number, so the shared number is derived from both packages.
PYPI_CANONICAL_URLS = (PYPI_PRIMEAGFENT_URL, PYPI_PRIMEAGFENT_BASE_URL)
...
def _root_base_version() -> str:
    ...
    Both primeagfent-nightly and primeagfent-base-nightly are versioned from the ROOT pyproject on
...
def _shared_nightly_version() -> str:
    """Compute the single dev number shared by primeagfent-nightly and primeagfent-base-nightly."""
