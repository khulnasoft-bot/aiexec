"""Unit tests for primeagfent_pre_release_tag.py.

Run directly or with pytest:

    cd scripts/ci && uv run python test_primeagfent_pre_release_tag.py
    cd scripts/ci && uv run python -m pytest test_primeagfent_pre_release_tag.py -q
"""
...
import primeagfent_pre_release_tag as pr
...
def test_cli_reads_released_versions_from_stdin():
    output = io.StringIO()
    with (
        mock.patch.object(sys, "argv", ["primeagfent_pre_release_tag.py", "1.10.1", "--print-rc-number", "-"]),
...
def _main():
    ...
    print("All primeagfent_pre_release_tag tests passed.")
