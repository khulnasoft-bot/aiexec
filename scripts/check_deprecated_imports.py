...
This script scans all Python files in the wfx/components directory for
...
def main() -> int:
    ...
    """
    try:
        # Find the wfx components directory
        script_dir = Path(__file__).parent
        repo_root = script_dir.parent
        wfx_components = repo_root / "src" / "wfx" / "src" / "wfx" / "components"

        if not wfx_components.exists():
            print(f"Error: Components directory not found at {wfx_components}", file=sys.stderr)
            return 2

        # Check for deprecated imports
        deprecated_imports = check_deprecated_imports(wfx_components)
