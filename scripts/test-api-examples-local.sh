...
HOST="${PRIMEAGFENT_HOST:-127.0.0.1}"
PORT="${PRIMEAGFENT_PORT:-7860}"
SUITES="${SUITES:-curl,python,javascript}"
EXECUTE_MODE="${EXECUTE_MODE:-true}"

export PRIMEAGFENT_AUTO_LOGIN="${PRIMEAGFENT_AUTO_LOGIN:-true}"
# /api/v2/workflows (docs Python workflow examples) requires this. Always enable for this
# harness so a user-wide PRIMEAGFENT_DEVELOPER_API_ENABLED=false does not break the suite.
export PRIMEAGFENT_DEVELOPER_API_ENABLED=true

cleanup() {
  if [[ -f /tmp/primeagfent-server.pid ]]; then
    SERVER_PID="$(< /tmp/primeagfent-server.pid)"
    ...
    rm -f /tmp/primeagfent-server.pid
  fi
}
trap cleanup EXIT

# If PORT is already taken, Primeagent may bind to PORT+1 while this script still uses PORT for
...
if port_is_in_use "$HOST" "$PORT"; then
  PORT="$(pick_free_port)"
  echo "Port $REQUESTED_PORT was in use; using $PORT for this run (set PRIMEAGFENT_PORT to pin a port)."
fi

echo "Starting Primeagent on http://$HOST:$PORT (developer API enabled for /api/v2/workflows)"
# Set on the command line so the server process always sees it (macOS launcher/exec paths).
PRIMEAGFENT_DEVELOPER_API_ENABLED=true uv run primeagfent run --backend-only --host "$HOST" --port "$PORT" >/tmp/primeagfent-server.log 2>&1 &
echo $! >/tmp/primeagfent-server.pid

echo "Waiting for Primeagent readiness (DB + services via /health_check)..."
# /health can respond before the Primeagent app is fully up (uvicorn default).
for _ in {1..60}; do
...
if ! curl -sf "http://$HOST:$PORT/health_check" >/dev/null; then
  echo "Primeagent did not become ready in time."
  echo "See /tmp/primeagfent-server.log for details."
  exit 1
fi
...
user = os.environ.get("PRIMEAGFENT_SUPERUSER", "primeagfent")
password = os.environ.get("PRIMEAGFENT_SUPERUSER_PASSWORD", "primeagfent")
...
if [[ -z "$PRIMEAGFENT_API_KEY" ]]; then
  echo "Failed to create API key."
  exit 1
fi

export PRIMEAGFENT_URL="http://$HOST:$PORT"
export PRIMEAGFENT_SERVER_URL="http://$HOST:$PORT"
export PRIMEAGFENT_API_KEY
...
base_url = os.environ["PRIMEAGFENT_URL"]
api_key = os.environ["PRIMEAGFENT_API_KEY"]
...
try:
    ...
    project_zip_path = Path("/tmp/primeagfent-project-import.zip")
    try:
...
echo "Done. Server log: /tmp/primeagfent-server.log"
