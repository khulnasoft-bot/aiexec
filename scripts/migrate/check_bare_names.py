...
The extension migration table accepts three legacy reference forms:
    ...
    1. ``bare_class_name`` -- the unqualified Python identifier
       ...
       exactly one bundle folder under ``src/wfx/src/wfx/components/`` (or one
...
It intentionally has zero runtime dependency on the wfx package (CI may run
before wfx is importable): the AST walk uses the stdlib only, and the
...
TABLE_RELPATH = "src/wfx/src/wfx/extension/migration/migration_table.json"
TABLE_PATH = REPO_ROOT / TABLE_RELPATH
DEFAULT_COMPONENT_ROOTS = (
    REPO_ROOT / "src" / "wfx" / "src" / "wfx" / "components",
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Defaults to src/wfx/src/wfx/components and src/bundles."
        ),
    )
