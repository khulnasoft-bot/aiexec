...
(``src/backend/base`` and ``src/wfx/src``).  Relative imports are
...
SCAN_ROOTS: tuple[Path, ...] = (
    REPO_ROOT / "src" / "backend" / "base" / "primeagfent" / "api",
    REPO_ROOT / "src" / "wfx" / "src" / "wfx",
)

# Python package roots.  Used to resolve ``from <module> import <name>``
# back to a file path -- ``primeagfent.api.v1.extensions`` lives under
# ``src/backend/base/`` and ``wfx.extension.bundle_registry`` lives under
# ``src/wfx/src/``.
MODULE_ROOTS: tuple[Path, ...] = (
    REPO_ROOT / "src" / "backend" / "base",
    REPO_ROOT / "src" / "wfx" / "src",
)
