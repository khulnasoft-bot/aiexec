...
# Must match primeagent.services.variable.constants.CREDENTIAL_TYPE
CREDENTIAL_TYPE = "Credential"


def get_default_config_dir() -> Path:
    """Get the default Primeagent config directory using platformdirs."""
    return Path(user_cache_dir("primeagent", "primeagent"))
...
def ensure_valid_key(s: str) -> bytes:
    ...
    NOTE: This function is duplicated from primeagent.services.auth.utils.ensure_valid_key
...
def get_default_database_url(config_dir: Path) -> str | None:
    ...
    default_db = config_dir / "primeagent.db"
...
Examples:
  ...
  %(prog)s --database-url postgresql://user:pass@host/db --config-dir /etc/primeagent  # pragma: allowlist secret
  ...
  %(prog)s --old-key "current-key" --new-key "replacement-key"
    ...
    if not database_url:
        ...
        print(f"  No database found at {args.config_dir}/primeagent.db")
