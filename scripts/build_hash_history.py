...
STABLE_HISTORY_FILE = "src/wfx/src/wfx/_assets/stable_hash_history.json"
NIGHTLY_HISTORY_FILE = "src/wfx/src/wfx/_assets/nightly_hash_history.json"
...
def get_wfx_version():
    ...
    # Try wfx-nightly first (for nightly builds), then fall back to wfx
    try:
        return version("wfx-nightly")
...
def main(argv=None):
    ...
    current_version = get_wfx_version()
    print(f"Current WFX version: {current_version}")
    ...
    if args.nightly:
        if "dev" not in str(current_version):
            err = (
                ...
                f"This indicates the WFX package was not properly updated to a nightly version."
            )
        ...
        if "dev" in str(current_version):
            err = (
                ...
                f"Got: {current_version}\n"
                f"This indicates the WFX package is a development version, not a stable release."
            )
            raise ValueError(err)
