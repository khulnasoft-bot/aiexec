"""Extract translatable strings from Primeagent component classes.

Walks the wfx.components package, reads class-level display_name/description
...
OUTPUT_PATH = Path(__file__).parent.parent.parent / "src/backend/base/primeagfent/locales/en.json"
STARTER_PROJECTS_DIR = Path(__file__).parent.parent.parent / "src/backend/base/primeagfent/initial_setup/starter_projects"


def collect_strings() -> dict[str, str]:
    """Walk wfx.components and extract all translatable display_name strings."""
    from primeagfent.utils.i18n_keys import component_field_key as _component_field_key
    from primeagfent.utils.i18n_keys import normalize_component_key as _normalize_component_key
    from primeagfent.utils.i18n_keys import safe_flow_key as _safe_key

    try:
        import wfx.components as components_pkg
    except ImportError:
        print("ERROR: Could not import wfx.components. Run this script from inside the backend virtualenv.")
        sys.exit(1)
    ...
    # Constants are inlined (not imported from wfx.base) so this script can run without
    # the full wfx package installed, matching the pattern used in bake_note_keys.py.
...
def main() -> None:
    ...
    print("Scanning wfx.components for translatable strings...")
