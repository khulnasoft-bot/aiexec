...
``os.environ`` is process-global. A warm, long-lived serving process (``wfx serve``'s
worker pool, or the main Primeagent server) handles many callers' requests in one
...
This walks every component source file (stdlib AST only — zero dependency on the wfx
...
DEFAULT_ROOTS = (
    ...
    "src/backend/base/primeagfent/components",
    "src/bundles",
)
...
ALLOWLIST: dict[str, set[str]] = {
    ...
    "src/wfx/src/wfx/components/datastax/dotenv.py": {"load_dotenv"},
}
...
def main(argv: list[str] | None = None) -> int:
    ...
    print("       in a shared serving process such as `wfx serve`):\n")
