...
# Get current version
CURRENT_VERSION=$(grep '^version = ' src/wfx/pyproject.toml | cut -d'"' -f2)
print_info "Current WFX version: $CURRENT_VERSION"
...
if [ "$DRY_RUN" = true ]; then
    ...
    sed -i.bak "s/^version = \".*\"/version = \"$NEW_VERSION\"/" src/wfx/pyproject.toml
    rm src/wfx/pyproject.toml.bak
fi

# Update version in Dockerfiles if they have ARG WFX_VERSION
if grep -q "ARG WFX_VERSION" src/wfx/docker/Dockerfile 2>/dev/null; then
    if [ "$DRY_RUN" = true ]; then
        print_dry_run "Would update version in Dockerfiles to $NEW_VERSION"
    else
        print_info "Updating version in Dockerfiles..."
        sed -i.bak "s/ARG WFX_VERSION=.*/ARG WFX_VERSION=$NEW_VERSION/" src/wfx/docker/Dockerfile*
        rm src/wfx/docker/Dockerfile*.bak
    fi
fi
...
# Create git tag
TAG_NAME="wfx-v$NEW_VERSION"
if [ "$DRY_RUN" = true ]; then
    print_dry_run "Would create git tag: $TAG_NAME"
else
    print_info "Creating git tag: $TAG_NAME"
    git tag -a "$TAG_NAME" -m "WFX Release $NEW_VERSION"
fi
