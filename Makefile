.PHONY: all init format_backend format lint build run_backend dev help tests coverage clean_python_cache clean_npm_cache clean_frontend_build clean_all run_cli
...
#
# Usage:
#   $(call CLEAR_DIRS,dir1 dir2 dir3)
# Example:
#   $(call CLEAR_DIRS,src/frontend/build src/backend/base/aiexec/frontend)
...
help: ## show this help message
	...
	@echo 'Note: The Makefile includes Makefile.frontend for additional frontend targets.'
...
ifdef login
	...
	AIEXEC_AUTO_LOGIN=$(login) uv run uvicorn \
		...
		--host 0.0.0.0 \
		--port $(port) \
		$(if $(filter-out 1,$(or $(workers),1)),, --reload) \
		--env-file $(env) \
		--loop asyncio \
