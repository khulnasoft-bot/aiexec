"""Unit test for custom code execution in isolation (with real Primeagent code)."""
...
from primeagent_stepflow.exceptions import ExecutionError
from primeagent_stepflow.worker.custom_code_executor import (
    CustomCodeExecutor,
)


class TestCustomCodeExecutor:
    """Test CustomCodeExecutor functionality with real Primeagent component code."""
...
''',
    ...
    def chat_input_component_blob(self):
        """Real ChatInput component blob data from Primeagent."""
        return {
            "code": '''
from primeagent.base.io.chat import ChatComponent
from primeagent.inputs.inputs import BoolInput
from primeagent.io import DropdownInput, MessageTextInput, MultilineInput, Output
from primeagent.schema.message import Message
from primeagent.utils.constants import (
...
''',
    ...
    async def test_execute_simple_component(
        ...
        # Primeagent Message: result data is nested in ["result"] field
    ...
    async def test_execute_chat_input_component(
        ...
        # Should have Primeagent Message structure
        # Handle nested result structure from Primeagent Messages
        message_data = result_data["result"] if "result" in result_data else result_data
        assert "text" in message_data
        assert message_data["text"] == "What is Python?"
        assert message_data["sender"] == "User"
        assert message_data["sender_name"] == "Test User"
        assert message_data.get("__primeagent_type__") == "Message"
...
class TestCustomCodeExecutorDataFrameOutput:
    ...
    def pandas_dataframe_component_blob(self):
        """Component that returns a plain pandas DataFrame (not wfx DataFrame).
...
class DataFrameComponent(Component):
    ...
    def fetch_content(self) -> pd.DataFrame:
        """Return a plain pandas DataFrame (not wfx DataFrame)."""
        return pd.DataFrame([
...
''',
    ...
    async def test_plain_pandas_dataframe_serializes(
        ...
        assert output["__primeagent_type__"] == "DataFrame"
        assert "json_data" in output


class TestCustomCodeExecutorIntegration:
    """Integration tests with mock Primeagent imports."""

    @pytest.fixture
    def executor_with_mocks(self):
        """Create CustomCodeExecutor with mocked Primeagent imports."""
        # We'll test this without real Primeagent imports to avoid dependency issues
        return CustomCodeExecutor()
...
class TestCustomCodeExecutorWithRealPrimeagentComponents:
    """Test CustomCodeExecutor with real converted Primeagent workflow components."""
...
"""
        ...
        stepflow_workflow = converter.convert(primeagent_data)

        # Find steps that use /primeagent/custom_code
        custom_code_steps = [step for step in stepflow_workflow.steps if step.component == "/primeagent/custom_code"]
...
""",
    ...
    def basic_prompting_flow(self):
        ...
        flow_path = Path(__file__).parent.parent / "fixtures" / "primeagent" / "basic_prompting.json"
    ...
    def chat_input_component_data(self, basic_prompting_flow):
        """Extract ChatInput component data (wfx-based) from basic_prompting flow."""
        nodes = basic_prompting_flow["data"]["nodes"]
    ...
    def agent_component_blob(self):
        ...
        This tests the PlaceholderGraph.vertices fix for wfx Agent components.
        """
        import json
        import os
    ...
    async def test_agent_component(self, executor: CustomCodeExecutor, mock_context, agent_component_blob):
        ...
        1. Compiling the Agent component from wfx
        2. Instantiating the component
        3. Accessing the graph.vertices attribute (which would fail without the fix)

        The fix patches wfx.custom.custom_component.component.PlaceholderGraph
        ...
        # The wfx Agent component code accesses graph.vertices internally
    ...
    async def test_prompt_component(self, executor: CustomCodeExecutor, mock_context, prompt_component_data):
        ...
        has_message_marker = "__primeagent_type__" in output or "__class_name__" in output
    ...
    async def test_language_model_message_to_string_conversion(
        ...
        This test verifies the fix for Primeagent 1.6.4+ wfx components that
    ...
    @pytest.mark.asyncio
    async def test_chat_input_wfx_component(
        self, executor: CustomCodeExecutor, mock_context, chat_input_component_data
    ):
        """Test executing ChatInput component (wfx-based) from basic_prompting flow.

        This test verifies that:
        1. We can compile and execute an wfx-based ChatInput component
    ...
    async def test_chat_input_message_to_string_conversion(
        ...
        This is a specific test for the Primeagent 1.6.4+ fix where Message objects
        ...
        message_obj = {
            ...
            "__module_name__": "wfx.schema.message",
