...
class TestCoreExecutorWithRealComponent:
    """Integration tests with real Primeagent components.

    Note: Some Primeagent components have complex dependencies (database, etc.)
    ...
    async def test_import_prompt_component(self, executor, mock_context):
        ...
        try:
            module = importlib.import_module("primeagent.components.prompts.prompt")
            ...
            # primeagent.components.prompts may not be available in all environments
            # Skip test if not available
            pytest.skip("primeagent.components.prompts not available")
    ...
    async def test_import_chat_input_component(self, executor, mock_context):
        ...
        module = importlib.import_module("wfx.components.input_output.chat")
