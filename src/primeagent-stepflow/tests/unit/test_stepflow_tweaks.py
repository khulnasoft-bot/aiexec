...
from primeagent_stepflow.translation.stepflow_tweaks import (
...
class TestStepflowTweaks:
    ...
    def test_convert_tweaks_to_overrides_basic(self):
        ...
        assert "primeagent_LanguageModelComponent-kBOja" in overrides

        step_override = overrides["primeagent_LanguageModelComponent-kBOja"]
    ...
    def test_convert_tweaks_to_overrides_multiple_steps(self):
        ...
        assert "primeagent_Step1" in overrides
        assert "primeagent_Step2" in overrides

        # Check Step1
        assert overrides["primeagent_Step1"]["$type"] == "merge_patch"
        step1_input = overrides["primeagent_Step1"]["value"]["input"]["input"]
        assert step1_input["field1"] == "value1"

        # Check Step2
        assert overrides["primeagent_Step2"]["$type"] == "merge_patch"
        step2_input = overrides["primeagent_Step2"]["value"]["input"]["input"]
        assert step2_input["field2"] == "value2"
        assert step2_input["field3"] == 123


class TestStepflowTweaksIntegration:
    """Test integration with the full Primeagent conversion process."""
    ...
    def basic_prompting_flow_dict(self):
        ...
        fixture_path = Path(__file__).parent.parent / "fixtures" / "primeagent" / "basic_prompting.json"

        if fixture_path.exists():
            with open(fixture_path) as f:
                primeagent_data = json.load(f)
            # Convert to Flow object and get dict representation
            flow = converter.convert(primeagent_data)
            import msgspec
    ...
    def test_real_workflow_tweaks_application(self, basic_prompting_flow_dict):
        ...
        primeagent_step = None
        for step in modified_dict["steps"]:
            if step["id"] == "primeagent_LanguageModelComponent-kBOja" and (
                step["component"] == "/primeagent/custom_code" or step["component"].startswith("/primeagent/core/")
            ):
                primeagent_step = step
                break

        assert primeagent_step is not None, "LanguageModelComponent executor step not found"

        # Verify tweaks were applied
        input_section = primeagent_step.get("input", {}).get("input", {})
    ...
    def test_tweaks_preserve_existing_inputs(self, basic_prompting_flow_dict):
        ...
        # Find the step
        for step in modified_dict["steps"]:
            if step["id"] == "primeagent_LanguageModelComponent-kBOja":
    ...
    def test_add_astradb_tweaks(self, monkeypatch):
        ...
        expected = {
            "AstraDB-store-123": {
                ...
                "database_name": "primeagent-test",  # Default value
