"""Unit tests for PrimeagentType input/output handlers and DataFrame output handler.

Tests serialization (output) and deserialization (input) of Primeagent
...
from primeagent_stepflow.worker.handlers import (
...
# ---------------------------------------------------------------------------
# PrimeagentTypeOutputHandler — matches()
# ---------------------------------------------------------------------------


class TestPrimeagentTypeOutputHandlerMatches:
    def setup_method(self):
        self.handler = PrimeagentTypeOutputHandler()
...
# ---------------------------------------------------------------------------
# PrimeagentTypeOutputHandler — process()
# ---------------------------------------------------------------------------


class TestPrimeagentTypeOutputHandlerProcess:
    def setup_method(self):
        self.handler = PrimeagentTypeOutputHandler()
    ...
    async def test_serialize_message(self):
        ...
        assert result["__primeagent_type__"] == "Message"
    ...
    async def test_serialize_data(self):
        ...
        assert result["__primeagent_type__"] == "Data"
...
class TestDataFrameOutputHandlerMatches:
    ...
    def test_matches_wfx_dataframe(self):
...
class TestDataFrameOutputHandlerProcess:
    ...
    async def test_serialize_dataframe(self):
        ...
        assert result["__primeagent_type__"] == "DataFrame"
...
# ---------------------------------------------------------------------------
# PrimeagentTypeInputHandler — matches()
# ---------------------------------------------------------------------------


class TestPrimeagentTypeInputHandlerMatches:
    def setup_method(self):
        self.handler = PrimeagentTypeInputHandler()

    def test_matches_dict_with_marker(self):
        value = {"__primeagent_type__": "Message", "text": "hi"}
        assert self.handler.matches(template_field={}, value=value) is True

    def test_matches_list_with_marked_items(self):
        value = [
            {"__primeagent_type__": "Data", "text": "row1"},
            {"__primeagent_type__": "Data", "text": "row2"},
        ]
        assert self.handler.matches(template_field={}, value=value) is True
...
# ---------------------------------------------------------------------------
# PrimeagentTypeInputHandler — prepare() (Message)
# ---------------------------------------------------------------------------


class TestPrimeagentTypeInputHandlerMessage:
    def setup_method(self):
        self.handler = PrimeagentTypeInputHandler()

    @pytest.mark.asyncio
    async def test_deserialize_message(self):
        serialized = {"__primeagent_type__": "Message", "text": "hello"}
    ...
    async def test_deserialize_message_empty_text(self):
        serialized = {"__primeagent_type__": "Message", "text": ""}
        fields = {"msg": (serialized, {})}
        result = await self.handler.prepare(fields, None)
...
# ---------------------------------------------------------------------------
# PrimeagentTypeInputHandler — prepare() (Data)
# ---------------------------------------------------------------------------


class TestPrimeagentTypeInputHandlerData:
    def setup_method(self):
        self.handler = PrimeagentTypeInputHandler()

    @pytest.mark.asyncio
    async def test_deserialize_data(self):
        serialized = {"__primeagent_type__": "Data", "text": "some data"}
...
# ---------------------------------------------------------------------------
# PrimeagentTypeInputHandler — prepare() (DataFrame)
# ---------------------------------------------------------------------------


class TestPrimeagentTypeInputHandlerDataFrame:
    def setup_method(self):
        self.handler = PrimeagentTypeInputHandler()
...
# ---------------------------------------------------------------------------
# PrimeagentTypeInputHandler — prepare() (list handling)
# ---------------------------------------------------------------------------


class TestPrimeagentTypeInputHandlerLists:
    def setup_method(self):
        self.handler = PrimeagentTypeInputHandler()
    ...
    async def test_deserialize_list_of_messages(self):
        value = [
            {"__primeagent_type__": "Message", "text": "msg1"},
            {"__primeagent_type__": "Message", "text": "msg2"},
        ]
        fields = {"msgs": (value, {})}
        result = await self.handler.prepare(fields, None)
    ...
    async def test_deserialize_list_of_data(self):
        value = [
            {"__primeagent_type__": "Data", "text": "d1"},
            {"__primeagent_type__": "Data", "text": "d2"},
        ]
        fields = {"items": (value, {})}
        result = await self.handler.prepare(fields, None)
    ...
    async def test_deserialize_multiple_fields(self):
        ...
        fields = {
            "msg": ({"__primeagent_type__": "Message", "text": "hello"}, {}),
            "data": ({"__primeagent_type__": "Data", "text": "world"}, {}),
        }
        result = await self.handler.prepare(fields, None)
...
# ---------------------------------------------------------------------------
# PrimeagentTypeInputHandler — unknown / edge cases
# ---------------------------------------------------------------------------


class TestPrimeagentTypeInputHandlerEdgeCases:
    def setup_method(self):
        self.handler = PrimeagentTypeInputHandler()

    @pytest.mark.asyncio
    async def test_unknown_primeagent_type_returns_dict(self):
        """Unknown __primeagent_type__ values should return the raw dict."""
        serialized = {"__primeagent_type__": "UnknownType", "data": "test"}
        fields = {"field": (serialized, {})}
        result = await self.handler.prepare(fields, None)

        assert isinstance(result["field"], dict)
        assert result["field"]["__primeagent_type__"] == "UnknownType"

    @pytest.mark.asyncio
    async def test_empty_primeagent_type_returns_dict(self):
        """Empty __primeagent_type__ value should return the raw dict."""
        serialized = {"__primeagent_type__": "", "text": "test"}
        fields = {"field": (serialized, {})}
        result = await self.handler.prepare(fields, None)
...
class TestPrimeagentTypeRoundTrip:
    def setup_method(self):
        self.output_handler = PrimeagentTypeOutputHandler()
        self.input_handler = PrimeagentTypeInputHandler()
...
class TestDataFrameRoundTrip:
    ...
    deserialize with PrimeagentTypeInputHandler."""
    ...
    async def test_dataframe_round_trip(self):
        ...
        serialized = await self.output_handler.process(original)
        assert serialized["__primeagent_type__"] == "DataFrame"
...
class TestPandasDataFrameRoundTrip:
    ...
    DataFrames instead of wfx DataFrames, depending on the Primeagent
