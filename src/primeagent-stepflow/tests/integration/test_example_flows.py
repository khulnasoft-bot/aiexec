...
Tests the full end-to-end path used by the ``stepflow-primeagent run`` CLI:
PrimeagentConverter -> StepflowClient -> orchestrator -> primeagent worker.
...
def _find_poc_flow() -> Path:
    ...
    # here.parents[4] = primeagent repo root (e.g. /Users/.../primeagent)
    primeagent_repo = here.parents[4]
    candidate = primeagent_repo.parent / "stepflow" / "docs" / "static" / "files" / "2025-09-primeagent-poc-flow.json"
    if candidate.exists():
        return candidate
...
def test_poc_flow_translation(converter, poc_flow_data):
    ...
    for step in flow.steps:
        assert step.component.startswith(("/primeagent/", "/builtin/")), (
...
async def test_poc_flow_execution(runner, poc_flow_data):
    ...
    as ``stepflow-primeagent run --local``.
    """
