"""Custom code executor for Primeagent components.

Executes Primeagent components by compiling their code from blob storage.
...
class CustomCodeExecutor(BaseExecutor):
    """Executes Primeagent custom code components by compiling code from blobs.
    ...
    def _get_input_handlers(self) -> list[InputHandler]:
        ...
        conversion needed for Primeagent type transformations.
        """
        return [
            PrimeagentTypeInputHandler(),
            BaseModelInputHandler(),
            ...
            if blob_id not in self.compiled_components:
                try:
                    ...
                    raise ExecutionError(
                        f"No component code found for blob {blob_id}. All components must have real Primeagent code."
                    ) from e
    ...
    async def _compile_component(self, blob_data: dict[str, Any], blob_id: str | None = None) -> dict[str, Any]:
        ...
        with tracer.start_as_current_span(
            ...
            # Patch PlaceholderGraph for wfx compatibility
    ...
    def _patch_placeholder_graph(self) -> None:
        """Patch PlaceholderGraph to add vertices attribute for wfx compatibility.

        wfx 0.1.12+ Agent components access graph.vertices, but PlaceholderGraph
        doesn't have this attribute by default. This patch adds it.
        """
        from typing import NamedTuple

        class EnhancedPlaceholderGraph(NamedTuple):
            """Enhanced PlaceholderGraph with vertices for wfx compatibility."""
        ...
        try:
            from wfx.custom.custom_component import (
                component as wfx_component_module,
            )

            wfx_component_module.PlaceholderGraph = EnhancedPlaceholderGraph  # type: ignore[assignment,misc]
        except ImportError:
            pass
