# M1 dogfood checklist — `wfx-duckduckgo` pilot bundle
...
1. **Deserialize + build pipeline (automated)** — lives in
   [`src/wfx/tests/integration/extension/test_pilot_duckduckgo_upgrade.py`](../../wfx/tests/integration/extension/test_pilot_duckduckgo_upgrade.py)
   and covers:
   ...
   - The `wfx-duckduckgo` distribution is importable and ships its
...
| Date | _YYYY-MM-DD_ |
| Pre-migration Primeagent version | `1.9.x` (last release before the pilot landed) |
| Post-migration Primeagent version | this branch / `1.10.x` |
| Result | _pass / fail_ |
| Notes | _free text_ |
...
1. **Pre-migration save.**
   - [ ] `pip install primeagent==<pre-migration version>` in a clean venv
   - [ ] `primeagent run` and open the UI
...
3. **Upgrade.**
   - [ ] In a separate clean venv: `pip install primeagent==<this branch>`
         (which transitively pulls in `wfx-duckduckgo`)
   - [ ] Confirm `wfx extension list --format=json` shows
         `wfx-duckduckgo` at slot `@official`

4. **Post-migration load.**
   - [ ] `primeagent run`, open the saved flow JSON from step 1
