...
``ibm-db`` ships no linux/aarch64 wheel, so ``wfx-ibm`` has to load on that
...
_BUNDLE_MODULES = (
    "wfx_ibm.components.ibm",
    "wfx_ibm.components.ibm.db2vs",
    "wfx_ibm.components.ibm.db2_vector",
)
...
def test_db2vs_module_imports_without_ibm_db():
    ...
    module = importlib.import_module("wfx_ibm.components.ibm.db2vs")
...
def test_bundle_components_import_without_ibm_db():
    ...
    from wfx_ibm.components.ibm import (
...
def test_db2_vector_store_use_raises_clean_import_error():
    ...
    from wfx_ibm.components.ibm import DB2VectorStoreComponent
