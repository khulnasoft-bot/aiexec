...
# ibm-watsonx-ai and langchain-ibm are real runtime deps of the wfx-ibm bundle,
...
# ``ibm_watsonx_ai`` so the wfx workspace venv (which did not pull either
...
# ``EmbedTextParamsMetaNames.RETURN_OPTIONS`` through a dict key, because
# the component imports the real symbol at module-load time while the
...
# ``@patch("wfx_ibm.components.ibm.watsonx_embeddings.WatsonxEmbeddings")``.
...
class TestWatsonxEmbeddingsComponent:
    ...
    @patch("wfx.base.models.model_utils.requests.get")
    ...
    @patch("wfx_ibm.components.ibm.watsonx_embeddings.WatsonxEmbeddingsComponent.fetch_models")
    ...
    @patch("wfx_ibm.components.ibm.watsonx_embeddings.WatsonxEmbeddingsComponent.fetch_models")
    @patch("wfx_ibm.components.ibm.watsonx_embeddings.logger")
