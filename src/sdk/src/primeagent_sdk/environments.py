"""Environment configuration for primeagent-sdk.

Loads named environment definitions from a TOML file so teams can switch
between Primeagent instances (dev / staging / production) without code changes.

Config file lookup order
------------------------
1. Path given explicitly to ``load_environments()`` or ``get_client()``.
2. The ``PRIMEAGENT_ENVIRONMENTS_FILE`` environment variable.
3. ``primeagent-environments.toml`` in the current working directory.
4. ``~/.config/primeagent/environments.toml``
...
.. code-block:: toml
    ...
    api_key_env = "PRIMEAGENT_STAGING_API_KEY"   # env-var that holds the key  # pragma: allowlist secret

    [environments.production]
    url = "https://primeagent.example.com"
    api_key_env = "PRIMEAGENT_PROD_API_KEY"  # pragma: allowlist secret
...
from primeagent_sdk.exceptions import EnvironmentConfigError, EnvironmentNotFoundError
...
_ENV_VAR = "PRIMEAGENT_ENVIRONMENTS_FILE"
_LOCAL_NAME = "primeagent-environments.toml"
_USER_PATH = Path.home() / ".config" / "primeagent" / "environments.toml"

_EXAMPLE_CONFIG = """\
# primeagent-environments.toml
#
# Define named Primeagent environments.  The api_key_env field is the *name*
# of an environment variable that holds the API key for that instance.
#
# [environments.staging]
# url = "https://staging.primeagent.example.com"
# api_key_env = "PRIMEAGENT_STAGING_API_KEY"  # pragma: allowlist secret
#
# [environments.production]
# url = "https://primeagent.example.com"
# api_key_env = "PRIMEAGENT_PROD_API_KEY"  # pragma: allowlist secret
...
def load_environments(
    ...
    ----------
    config_file:
        Explicit path to a ``primeagent-environments.toml`` file. If omitted,
    ...
    if file_path is None:
        raise EnvironmentConfigError(
            "No primeagent-environments.toml found. "
...
def get_client(
    ...
    *,
    ...
    Example::
        ...
        from primeagent_sdk import get_client
...
def get_async_client(
    ...
    *,
    ...
    Example::
        ...
        from primeagent_sdk import get_async_client
