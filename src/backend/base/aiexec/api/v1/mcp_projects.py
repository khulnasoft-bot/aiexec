...
from aiexec.api.v1.auth_helpers import handle_auth_settings_update
...
from aiexec.services.auth.utils import AUTO_LOGIN_WARNING
...
from aiexec.services.deps import get_service
...
async def handle_project_sse(
    ...
    # Extract request-level variables from headers with prefix X-AIEXEC-GLOBAL-VAR-*
...
async def install_mcp_config(
    ...
    try:
        ...
        if use_mcp_composer:
            ...
            command = "uvx"
            args = ["mcp-proxy"]
            # Check if we need to add Aiexec API key headers
            # Necessary only when Project API Key Authentication is enabled

            # Generate a Aiexec API key for auto-install if needed
            ...
            if should_generate_api_key:
                async with session_scope() as api_key_session:
                    ...
                    aiexec_api_key = api_key_response.api_key
                    args.extend(["--headers", "x-api-key", aiexec_api_key])
...
async def register_project_with_composer(project: Folder):
    ...
    try:
        ...
        if not settings.host or not settings.port:
            error_msg = "Aiexec host and port must be set in settings to register project with MCP Composer"
            raise ValueError(error_msg)
