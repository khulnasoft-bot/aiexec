...
from aiexec.services.deps import get_queue_service, get_service, get_settings_service, get_telemetry_service
...
def warn_about_future_cors_changes(settings):
    ...
    if using_defaults:
        logger.warning(
            ...
            "Consider setting AIEXEC_CORS_ORIGINS for production deployments. "
    ...
    if settings.cors_origins == "*" and settings.cors_allow_credentials:
        logger.warning(
            ...
            "Specify exact origins in AIEXEC_CORS_ORIGINS to use credentials securely."
        )
    ...
    async def lifespan(_app: FastAPI):
        ...
        try:
            ...
            async def delayed_init_mcp_servers():
                await asyncio.sleep(
                    10.0
                )  # Increased delay to allow starter projects to be createdawait asyncio.sleep(3.0)
                current_time = asyncio.get_event_loop().time()
