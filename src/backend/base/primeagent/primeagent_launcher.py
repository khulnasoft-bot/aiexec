...
def main():
    """Launches primeagent with appropriate environment setup.
    ...
    if platform.system() == "Darwin":  # macOS
        ...
        from primeagent.__main__ import main as primeagent_main

        primeagent_main()


def _launch_with_exec():
    """Launch primeagent by replacing current process with properly configured environment.
    ...
    try:
        os.execv(sys.executable, [sys.executable, "-m", "primeagent.__main__", *sys.argv[1:]])  # noqa: S606
    except OSError as e:
        # If exec fails, we need to exit since the process replacement failed
        typer.echo(f"Failed to exec primeagent: {e}", file=sys.stderr)
        sys.exit(1)
