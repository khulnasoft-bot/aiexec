...
from wfx.services.deps import get_deployment_adapter
from wfx.services.interfaces import DeploymentServiceProtocol

from primeagent.api.v1.mappers.deployments.contracts import ProviderSnapshotBinding
from primeagent.api.v1.mappers.deployments.sync import (
...
from primeagent.services.database.models.flow_version.model import FlowVersion
...
from primeagent.services.database.utils import require_non_empty

if TYPE_CHECKING:
    from collections.abc import Sequence

    from primeagent.api.utils import DbSession
    from primeagent.services.database.models.flow_version_deployment_attachment.model import (
...
# ---------------------------------------------------------------------------
# Route helpers (moved from primeagent.api.v1.deployments)
# ---------------------------------------------------------------------------
...
def handle_adapter_errors(*, mapper: BaseDeploymentMapper | None = None):
    ...
    ``wfx.services.adapters.deployment.exceptions`` module.  Non-domain
...
async def resolve_adapter_mapper_from_provider_id(
    ...
    *,
    ...
    from primeagent.api.v1.mappers.deployments.registry import get_deployment_mapper
...
async def get_deployment_row_or_404(
    *,
    ...
    # registered and ``PRIMEAGENT_AUTHZ_ENABLED`` is on; otherwise it stays
