...
from wfx.base.data.utils import extract_text_from_bytes
from wfx.base.knowledge_bases.backends import BackendType, create_backend
from wfx.base.knowledge_bases.ingestion_sources import (
    FolderSource,
    SourceType,
    create_source,
    get_source_class,
...
from wfx.base.vectorstores.chroma_security import chroma_client_create_collection_kwargs
from wfx.log import logger

from primeagent.api.utils import CurrentActiveUser, ingestion_run_service, knowledge_base_service
from primeagent.api.utils.kb_helpers import KBAnalysisHelper, KBIngestionHelper, KBStorageHelper
from primeagent.api.utils.kb_metadata import parse_per_file_metadata, parse_user_metadata
from primeagent.api.v1.schemas import TaskResponse
from primeagent.schema.knowledge_base import (
    BulkDeleteRequest,
    ChunkInfo,
...
from primeagent.services.authorization import (
    KnowledgeBaseAction,
    ensure_knowledge_base_permission,
)
from primeagent.services.database.models.jobs.model import JobStatus, JobType
from primeagent.services.database.models.knowledge_base.model import KnowledgeBaseRecord
from primeagent.services.deps import get_job_service, get_settings_service, get_task_service
from primeagent.services.jobs import DuplicateJobError
from primeagent.services.jobs.service import JobService
from primeagent.services.memory_base.kb_path_helpers import validate_kb_path
from primeagent.services.task.service import TaskService
from primeagent.utils.kb_constants import (
...
async def _guard_kb_action(
    *,
    ...
    ``PRIMEAGENT_AUTHZ_ENABLED=true``, the helper falls back to scanning every
    ...
    """
    from primeagent.services.authorization.utils import (
        _auth_context,
        _coerce_action,
        _resolve_authz_domain,
    )
    from primeagent.services.database.models.user.crud import get_user_by_id
    from primeagent.services.deps import get_authorization_service, session_scope
...
def _validate_kb_path_containment(kb_user_path: Path, kb_path: Path, kb_name: str, username: str) -> None:
    ...
    :func:`primeagent.services.memory_base.kb_path_helpers.validate_kb_path`
...
async def _delete_remote_backend_collection(
    *,
    ...
    delete the KB from Primeagent's UI at all. Before this, the backend
...
async def ingest_folder_to_knowledge_base(
    ...
    try:
        ...
        from primeagent.api.utils.kb_metadata import (
