...
:class:`~wfx.extension.bundle_registry.BundleRegistry`.
...
``PRIMEAGENT_ENABLE_EXTENSION_RELOAD`` is off.
"""
...
from wfx.extension.bundle_registry import get_default_registry
from wfx.extension.errors import ExtensionError
from wfx.extension.reload import ReloadInProgressError, reload_bundle
from wfx.log.logger import logger
from wfx.services.deps import get_extension_events_service, get_settings_service

from primeagent.api.utils.core import CurrentActiveUser
...
def _require_extension_reload_enabled() -> None:
    ...
    The route is always mounted (see ``primeagent.api.router``) so that
    ``--env-file`` can opt in to it without forcing route registration
    to happen after the env file is loaded.  Operators on Mode B/C
    deployments leave ``PRIMEAGENT_ENABLE_EXTENSION_RELOAD`` unset; the
        ...
        raise _typed_http_exception(
            ...
            error=ExtensionError(
                ...
                message=(
                    ...
                    "Set PRIMEAGENT_ENABLE_EXTENSION_RELOAD=true to enable it on "
                    "a local-development install (Mode A)."
                ),
                hint=(
                    "Local development: ``wfx extension dev`` sets the flag "
                    "for you.  Self-hosted: export PRIMEAGENT_ENABLE_EXTENSION_RELOAD=true "
...
async def reload_extension_bundle(
    ...
    Returns the typed :class:`~wfx.extension.reload.ReloadResult` body on
        ...
        raise _typed_http_exception(
            ...
            error=ExtensionError(
                ...
                hint=(
                    ...
                    "distribution name); run ``wfx extension list`` to see the "
    ...
    if not result.ok:
        ...
        primary = (
            ...
            if result.errors
                ...
                location=f"{extension_id}/{bundle_name}",
                hint="Run `wfx extension validate` against the bundle source for details.",
            )
        )
        logger.warning(
