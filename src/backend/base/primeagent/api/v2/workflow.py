...
from primeagent.services.authorization import FlowAction, ensure_flow_permission
...
from primeagent.services.deps import get_job_service, get_memory_base_service, get_task_service
...
def _resolve_request_variables(
    ...
    values can be transported safely. The legacy ``X-PRIMEAGENT-GLOBAL-VAR-*``
    ...
    if legacy_globals:
        logger.warning(
            "X-PRIMEAGENT-GLOBAL-VAR-* headers are deprecated on /api/v2/workflows; "
            "send request-level globals in the JSON body instead. Header-based "
            "globals will be removed in a future Primeagent release."
        )
...
async def execute_workflow(
    ...
    **background** and **stream** can't be true at the same time.
    ...
    Args:
        ...
        http_request: The HTTP request, used to honor deprecated ``X-PRIMEAGENT-GLOBAL-VAR-*`` headers
...
async def execute_sync_workflow(
    ...
    # Unicode strings transport safely. The legacy X-PRIMEAGENT-GLOBAL-VAR-*
