...
from wfx.services.deps import injectable_session_scope, injectable_session_scope_readonly
...
def extract_global_variables_from_headers(headers, *, include_auth_headers: bool = False) -> dict[str, str]:
    ...
    By default, only headers with the ``X-PRIMEAGENT-GLOBAL-VAR-*`` prefix are
    ...
    ``/workflow``, ``x-api-key`` is Primeagent's own authentication key — exposing
    ...
    Example:
        headers = {"X-PRIMEAGENT-GLOBAL-VAR-API-KEY": "secret", "x-api-key": "mykey"}
