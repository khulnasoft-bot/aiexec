...
from wfx.base.models.model_metadata import MODEL_PROVIDER_METADATA, get_provider_param_mapping

import wfx
from primeagent.agentic.helpers.assistant_workspace import resolve_assistant_fs_root

# Resolves only from the monorepo root; inject_wfx_components_path rewrites it to
# an absolute path at runtime so packaged installs (Desktop, pip, Docker) work.
WFX_COMPONENTS_PATH_SENTINEL = "./src/wfx/src/wfx/components/"
...
def inject_wfx_components_path(flow_data: dict) -> dict:
    """Rewrite Directory nodes targeting bundled wfx components to an absolute path.

    The bundled PrimeagentAssistant flow hardcodes a relative path that only
    ...
    be a directory.", causing the Primeagent Assistant to fail with
    ...
    `path` value equals WFX_COMPONENTS_PATH_SENTINEL, replaces it with the
    absolute path derived from the installed wfx package.
    """
    absolute_path = str(Path(wfx.__file__).parent / "components")
    ...
    for node in flow_data.get("data", {}).get("nodes", []):
        ...
        if path_field and path_field.get("value") == WFX_COMPONENTS_PATH_SENTINEL:
...
def inject_assistant_fs_root(flow_data: dict) -> dict:
    ...
    The shipped PrimeagentAssistant flow leaves FileSystemTool.root_path empty
...
def load_and_prepare_flow(
    ...
    flow_data = inject_wfx_components_path(flow_data)
