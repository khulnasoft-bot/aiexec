...
from wfx.custom.validate import extract_class_name
...
class _ReturnChecker(ast.NodeVisitor):
    ...
    # Primeagent components commonly use async output methods
...
async def validate_component_runtime(code: str, user_id: str | None = None) -> str | None:
    ...
    - Wrong import paths (e.g., ``from wfx.base import Component`` instead of
    ...
    """
    try:
        from wfx.custom.custom_component.component import Component as ComponentClass
    ...
    try:
        ...
        if _RESERVED_OUTPUT_NAME in output_names:
            return ValidationResult(
                ...
                code=code,
                error=(
                    f"Output name {_RESERVED_OUTPUT_NAME!r} is reserved by Primeagent for the "
        ...
        if _RESERVED_OUTPUT_METHOD in output_methods:
            return ValidationResult(
                ...
                code=code,
                error=(
                    f"Output method {_RESERVED_OUTPUT_METHOD!r} is reserved by Primeagent for "
