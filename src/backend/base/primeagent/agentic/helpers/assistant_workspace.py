"""Resolve the FileSystem sandbox root for the Primeagent Assistant.

The shipped PrimeagentAssistant flow embeds a FileSystemTool whose `root_path`
...
that PR's contract:
    ...
    Env var:  PRIMEAGENT_FS_TOOL_BASE_DIR  (same name PR #13031 uses)
    Default:  ~/.primeagent/fs_tool/fs_sandbox  (same default PR #13031 uses)
...
Resolution order:
    ...
    2. PRIMEAGENT_FS_TOOL_BASE_DIR env var (after expanduser + strip), if set.
    3. ~/.primeagent/fs_tool/fs_sandbox.
...
BASE_DIR_ENV = "PRIMEAGENT_FS_TOOL_BASE_DIR"
# Sub-path under the user's home, mirrored from PR #13031's default.
DEFAULT_BASE_SUBPATH = Path(".primeagent") / "fs_tool" / "fs_sandbox"
...
ISOLATION_MODULE = "wfx.components.tools._filesystem_isolation"
