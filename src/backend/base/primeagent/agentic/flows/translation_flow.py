...
Usage:
    from primeagent.agentic.flows.translation_flow import get_graph
    graph = await get_graph(provider="OpenAI", model_name="gpt-4o-mini")
"""

from wfx.base.models.model_metadata import get_provider_param_mapping
...
Agent for Primeagent Assistant.
...
- "generate_component": User wants you to CREATE/BUILD/GENERATE/MODIFY a custom Primeagent component.
...
- "question": User is ASKING A QUESTION about Primeagent, seeking help with Primeagent, wants \
information about Primeagent features/components/flows, OR is just being conversational \
...
- "off_topic": The request is NOT about Primeagent AND is not a social pleasantry. It is a \
SUBSTANTIVE question/task about other tools, platforms, or general knowledge unrelated to Primeagent.
...
"question" (answer it briefly and warmly). off_topic is ONLY for substantive non-Primeagent topics.

IMPORTANT rules:
- "How to create a component" = question (asking for Primeagent guidance)
...
- General knowledge questions NOT related to Primeagent = off_topic
...
- If unsure whether it's about Primeagent, classify as "question" (not off_topic)
...
- "how do I create a file?" / "how to save files in Primeagent" = question (asking for guidance)
...
Examples:
Input: "como criar um componente no primeagent"
Output: {{"translation": "how to create a component in primeagent", "intent": "question"}}
...
Input: "how do I save a file in Primeagent?"
Output: {{"translation": "how do I save a file in Primeagent?", "intent": "question"}}
