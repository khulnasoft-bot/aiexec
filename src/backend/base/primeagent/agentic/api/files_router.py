...
- Path validation: a two-layer guard.
    ...
    2. **Sandbox boundary** is enforced by ``FileSystemToolComponent._validate_path``
       ...
       deny-list / hardlink / O_NOFOLLOW guards already proven in the wfx
       sandbox test suite.
...
from primeagent.api.utils.core import CurrentActiveUser  # noqa: TC001
...
async def get_file(
    *,
    ...
    # Deferred import: FileSystemToolComponent pulls a chunk of wfx — we don't
    ...
    from wfx.components.files_and_knowledge.filesystem import (
        BINARY_SNIFF_BYTES,
        MAX_FILE_SIZE_BYTES,
