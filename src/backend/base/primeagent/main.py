...
from primeagent.services.database.service import UnsupportedPostgreSQLVersionError
...
from primeagent.services.tracing.otel_fastapi_patch import patch_otel_fastapi_route_details
...
def cors_origins_contain_wildcard(origins) -> bool:
    ...
    `PRIMEAGENT_CORS_ORIGINS="*"` is parsed as the raw string on some Python versions
...
def get_lifespan(*, fix_migration=False, version=None):
    ...
    async def lifespan(_app: FastAPI):
        ...
        from primeagent.preload import PreloadStep, get_owned_temp_dirs, is_step_complete
        ...
        # early failure (e.g. an unresolvable PRIMEAGENT_DATABASE_URL) is masked by a
        ...
        try:
            ...
            if get_settings_service().settings.sentry_dsn:
                try:
                    ...
                    await logger.awarning(
                        "PRIMEAGENT_SENTRY_DSN is set but sentry-sdk is not installed; "
                    ...
                    try:
                        ...
                        await logger.awarning(f"Failed to initialize Sentry SDK (check PRIMEAGENT_SENTRY_DSN): {e}")
            ...
            try:
                from primeagent.services.deps import get_telemetry_writer_service
            ...
            # Locally-registered dev extensions (``wfx extension dev``) are
            ...
            async def refresh_models_dev_periodically() -> None:
                ...
                availability.
                """
                from wfx.base.models.models_dev_catalog import (
            ...
            # PRIMEAGENT_MODELS_DEV_REFRESH=false disables the live models.dev
            ...
            if os.getenv("PRIMEAGENT_MODELS_DEV_REFRESH", "true").lower() not in ("false", "0", "no"):
                models_dev_refresh_task = asyncio.create_task(refresh_models_dev_periodically())
            else:
                await logger.adebug("models.dev refresh disabled via PRIMEAGENT_MODELS_DEV_REFRESH")
            ...
            try:
                ...
                with shutdown_progress.step(2):
                    ...
                    try:
                        from primeagent.services.authorization.utils import drain_pending_audit_writes
...
def create_app():
    ...
    if cors_origins_contain_wildcard(origins):
        if allow_credentials:
            ...
            logger.warning(
                ...
                "PRIMEAGENT_CORS_ALLOW_CREDENTIALS=true; disabling credentials because a "
                ...
                "Set PRIMEAGENT_CORS_ORIGINS to explicit origins to keep credentials enabled."
            )
    ...
    async def set_locale(request: Request, call_next):
        ...
        if _supported_locales is None:
            from primeagent.utils.i18n import get_supported_locales
    ...
    async def rate_limit_exception_handler(request: Request, _exc: RateLimitExceeded):
        ...
        from primeagent.services.rate_limit.service import get_limiter_key
    ...
    from primeagent.services.rate_limit import get_rate_limiter
