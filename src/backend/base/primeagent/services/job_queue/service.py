...
_STREAM_PREFIX = "primeagent:queue:"
_OWNER_PREFIX = "primeagent:owner:"
...
_CANCEL_CHANNEL_PREFIX = "primeagent:cancel:"
...
_ACTIVITY_PREFIX = "primeagent:activity:"
...
_PUBLIC_JOB_PREFIX = "primeagent:public_job:"
...
class RedisQueueWrapper:
    ...
    * End-of-stream →  ``XADD key * event_id __sentinel__ data __sentinel__ ts <float>``
    ...
    # value can be driven from settings (PRIMEAGENT_REDIS_QUEUE_STARTUP_GRACE_S).
...
class RedisJobQueueService(JobQueueService):
    ...
    Configuration
    -------------
    Set ``PRIMEAGENT_JOB_QUEUE_TYPE=redis`` and, optionally:

    * ``PRIMEAGENT_REDIS_QUEUE_DB`` (default ``1``, separate from cache DB ``0``)
    * ``PRIMEAGENT_REDIS_QUEUE_URL`` (full URL, overrides host/port/db)
    * ``PRIMEAGENT_REDIS_QUEUE_HOST`` / ``PRIMEAGENT_REDIS_QUEUE_PORT``
    ...
    ``primeagent:cancel:*``. Any worker can publish a cancel signal via
    ...
    completion. See ``primeagent.api.build.create_flow_response``.
    """
    ...
    async def _check_connection(self) -> None:
        ...
        try:
            ...
            await logger.aerror(
                ...
                "Set PRIMEAGENT_JOB_QUEUE_TYPE=asyncio or start Redis before running Primeagent."
            )
    ...
    _CANCEL_MARKER_PREFIX = "primeagent:cancel-marker:"
    ...
    def get_queue_data(self, job_id: str) -> tuple[asyncio.Queue, EventManager, asyncio.Task | None, float | None]:  # type: ignore[override]
        ...
        * **Cross-worker path**: no local entry exists; a null ``EventManager`` and
          ...
          response's :func:`primeagent.api.build.create_flow_response.on_disconnect`
