...
from primeagent.services.database.constants import (
...
_MIGRATION_ADVISORY_LOCK_ID = 0x4C616E67666C6F77  # ASCII "Primeagent"
_MIGRATION_LOCK_DEFAULT_TIMEOUT_S = 300.0
_MIGRATION_LOCK_POLL_INTERVAL_S = 2.0


def _migration_lock_timeout_s() -> float:
    raw = os.getenv("PRIMEAGENT_MIGRATION_LOCK_TIMEOUT_S")
    if raw is None:
    ...
    try:
        ...
        logger.warning(
            "Ignoring invalid PRIMEAGENT_MIGRATION_LOCK_TIMEOUT_S=%r; falling back to %.0fs.",
            raw,
...
def _acquire_migration_lock_or_raise(conn, lock_id: int) -> None:
    ...
    msg = (
        ...
        "wait via PRIMEAGENT_MIGRATION_LOCK_TIMEOUT_S (seconds) if your migration "
...
def _postgres_migration_lock(database_url: str):
    ...
    No-op for non-PostgreSQL URLs. SQLite has no advisory locks (and Primeagent
...
def check_sqlite_database_path(database_url: str) -> None:
    ...
    ``PRIMEAGENT_DATABASE_URL`` are resolved by SQLAlchemy against the current
    ...
    is opaque, so surface where Primeagent actually tried to open the database and
    ...
    if db_path.is_absolute():
        msg += "Create the directory before starting Primeagent, or point PRIMEAGENT_DATABASE_URL at an existing path."
    else:
        msg += (
            f"The relative path '{db_path}' from PRIMEAGENT_DATABASE_URL was resolved against the current working "
            f"directory ('{Path.cwd()}'). Set PRIMEAGENT_DATABASE_URL to an absolute path "
            f"(e.g. 'sqlite:///{resolved}'), or create the directory before starting Primeagent."
        )
...
class DatabaseService(Service):
    ...
    def __init__(self, settings_service: SettingsService):
        ...
        if self.alembic_log_to_stdout:
            ...
            base_dir = Path(config_dir) if config_dir else primeagent_dir
    ...
    async def initialize_alembic_log_file(self):
        ...
        try:
            ...
            await logger.awarning(
                ...
                "Migration output falls back to stdout. Set PRIMEAGENT_ALEMBIC_LOG_FILE to a writable path "
                "or PRIMEAGENT_ALEMBIC_LOG_TO_STDOUT=true to silence this warning."
            )
    ...
    async def ensure_postgresql_version(self) -> None:
        ...
        Primeagent's schema uses UNIQUE NULLS DISTINCT, which is only supported in PostgreSQL 15+.
    ...
    def _open_alembic_log_buffer(self):
        ...
        try:
            ...
            logger.warning(
                ...
                "Falling back to stdout. Set PRIMEAGENT_ALEMBIC_LOG_FILE to a writable path "
                "or PRIMEAGENT_ALEMBIC_LOG_TO_STDOUT=true to silence this warning."
            )
            return nullcontext(sys.stdout)
