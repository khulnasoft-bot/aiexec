...
from primeagent.services.database.models.deployment.orm_guards import ensure_deployment_immutable_fields
...
async def get_deployment(
    ...
    *,
    ...
    """
    from primeagent.services.authorization.fetch import authorized_or_owner_scoped
...
def _deployment_metadata_changed_fields(update_item: DeploymentMetadataUpdate) -> list[str]:
    primeagent_data = update_item.primeagent_db_row
    changed_fields: list[str] = []
    if update_item.display_name != primeagent_data.display_name:
        changed_fields.append("display_name")
    if update_item.description != primeagent_data.description:
...
def _deployment_metadata_updates_cte(
    ...
    return (
        ...
        .data(
            [
                (
                    item.primeagent_db_row.id,
                    item.primeagent_db_row.user_id,
                    item.display_name,
                    item.description,
                )
                for item in updates
