...
async def setup_superuser(settings_service: SettingsService, session: AsyncSession) -> SetupSuperuserResult:
    if settings_service.auth_settings.AUTO_LOGIN:
        ...
        lock_file = Path(gettempdir()) / "primeagent_auto_login_superuser.lock"
        ...
        try:
            with lock:
                ...
                if super_user:  # Only initialize if user was created
                    ...
                    # Recover MCP server config files orphaned when DB was reset but PRIMEAGENT_CONFIG_DIR was kept.
...
async def migrate_orphaned_mcp_servers_config(
    ...
    via a ``File`` row. When Primeagent starts with a fresh database but the same
...
async def clean_authz_audit_log(settings_service: SettingsService, session: AsyncSession) -> int:
    ...
    from primeagent.services.database.models.auth import AuthzAuditLog
...
def register_all_service_factories() -> None:
    ...
    from primeagent.services.authorization import factory as authorization_factory
    ...
    from primeagent.services.telemetry_writer import factory as telemetry_writer_factory
    ...
    # ``override=True`` so Primeagent always has a default. A registered
    # authorization plugin replaces it by listing its class in
    # ``PRIMEAGENT_CONFIG_DIR/wfx.toml`` (config files use ``override=True`` via
    # ``_discover_from_config``). Plain entry-point discovery uses
    # ``override=False`` and would lose to this default — the supported
    # override path is the ``wfx.toml`` config, matching SSO.
    service_manager.register_service_class(
        ServiceType.AUTHORIZATION_SERVICE, PrimeagentAuthorizationService, override=True
    )
...
def register_builtin_adapters() -> None:
    ...
    try:
        import_module("primeagent.services.adapters.deployment.watsonx_orchestrate.register")
...
def register_builtin_deployment_mappers() -> None:
    ...
    try:
        import_module("primeagent.api.v1.mappers.deployments.watsonx_orchestrate")
    except ModuleNotFoundError as exc:
