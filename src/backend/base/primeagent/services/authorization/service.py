"""Primeagent authorization service (OSS pass-through; plugins enforce RBAC)."""
...
class PrimeagentAuthorizationService(BaseAuthorizationService):
    ...
    def __init__(self, settings_service: SettingsService) -> None:
        ...
        self.set_ready()
        logger.debug("Primeagent authorization service initialized")
        ...
        # plugin registered via ``wfx.toml``, ``enforce()`` returns True for
        ...
        if authz_enabled and not self.SUPPORTS_CROSS_USER_FETCH:
            logger.warning(
                "PRIMEAGENT_AUTHZ_ENABLED=true but the OSS pass-through authorization service is "
                ...
                "Register an authorization plugin via wfx.toml or set PRIMEAGENT_AUTHZ_ENABLED=false "
