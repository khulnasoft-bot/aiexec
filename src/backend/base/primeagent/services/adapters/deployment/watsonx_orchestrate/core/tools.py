...
from ibm_watsonx_orchestrate_core.types.tools.primeagent_tool import create_primeagent_tool
from wfx.log.logger import logger
from wfx.services.adapters.deployment.exceptions import (
    InvalidContentError,
    InvalidDeploymentOperationError,
)
from wfx.utils.flow_requirements import generate_requirements_from_flow
...
from primeagent.services.adapters.deployment.watsonx_orchestrate.payloads import (
    WatsonxFlowArtifactProviderData,
    WatsonxToolRefBinding,
    normalize_wxo_name,
)
from primeagent.services.adapters.deployment.watsonx_orchestrate.utils import (
    dedupe_list,
...
if TYPE_CHECKING:
    from ibm_watsonx_orchestrate_core.types.tools.primeagent_tool import PrimeagentTool
    from wfx.services.adapters.deployment.schema import BaseFlowArtifact, SnapshotItems, SnapshotListResult

    from primeagent.services.adapters.deployment.watsonx_orchestrate.types import WxOClient
...
def ensure_primeagent_connections_binding(tool_payload: dict[str, Any]) -> dict[str, str]:
    """Ensure ``binding.primeagent.connections`` exists in *tool_payload* and return the mutable dict.
    ...
    primeagent = _ensure_dict(binding, "primeagent")
    return _ensure_dict(primeagent, "connections")


def verify_primeagent_owned(tool: dict[str, Any], *, tool_id: str) -> None:
    """Raise ``InvalidContentError`` if the tool lacks ``binding.primeagent``.

    Call before any mutating operation on an existing tool to ensure
    Primeagent created it.  Tools created manually in the wxO console or
    by other integrations will not have this marker.
    """
    binding = tool.get("binding")
    if not isinstance(binding, dict) or "primeagent" not in binding:
        msg = (
            f"Cannot modify tool '{tool_id}': it does not have a Primeagent binding"
            " and may not be managed by Primeagent."
        )
        raise InvalidContentError(message=msg)


def extract_primeagent_connections_binding(tool_payload: dict[str, Any]) -> dict[str, str]:
    """Extract ``binding.primeagent.connections`` from a provider tool payload.
    ...
    if not isinstance(binding, dict):
        return {}
    primeagent = binding.get("primeagent")
    if not isinstance(primeagent, dict):
        return {}
    connections = primeagent.get("connections")
...
def extract_primeagent_artifact_from_zip(artifact_zip_bytes: bytes, *, snapshot_id: str) -> dict[str, Any]:
    """Read and parse the Primeagent flow JSON from a wxO snapshot artifact zip."""
    try:
...
def build_primeagent_artifact_bytes(
    *,
    tool: PrimeagentTool,
    ...
    wfx_requirement = _resolve_wfx_requirement()
    requirements = generate_requirements_from_flow(
        flow_definition,
        include_wfx=False,
        pin_versions=True,
    )
    requirements = [wfx_requirement, *requirements]
    requirements = dedupe_list(requirements)
    requirements_content = "\n".join(requirements) + "\n"
    logger.debug("build_primeagent_artifact_bytes: filename='%s', requirements=%s", filename, requirements)
...
def create_wxo_flow_tool(
    *,
    ...
    logger.debug(
        ...
        primeagent_flow_name=flow_name,
        flow_id=flow_id,
    ...
    if not flow_definition.get("last_tested_version"):
        ...
        if not detected_version:
            msg = "Unable to determine running Primeagent version for snapshot creation."
            raise InvalidContentError(message=msg)
        flow_definition["last_tested_version"] = detected_version

    tool: PrimeagentTool = create_primeagent_tool(
    ...
    (tool_payload.setdefault("binding", {}).setdefault("primeagent", {})["project_id"]) = project_id
    logger.debug(
        "create_wxo_flow_tool_payload",
        tool_name=tool_payload["name"],
        tool_display_name=tool_payload["display_name"],
        project_id=project_id,
        binding=tool_payload.get("binding", {}).get("primeagent"),
    )

    artifacts: bytes = build_primeagent_artifact_bytes(
        tool=tool,
...
async def process_raw_flows_with_app_id(
    ...
    """Create primeagent tools in wxO and connect them to the given app_id."""
...
def _resolve_wfx_requirement() -> str:
    """Pin wfx to the installed version, falling back to a minimum spec.

    If the ``WXO_WFX_REQUIREMENT_OVERRIDE`` environment variable is set, its
    value is used verbatim as the wfx requirement line (e.g.
    ``wfx-nightly==0.4.0.dev32``) instead of resolving from the installed
    package metadata.
    """
    override = os.environ.get("WXO_WFX_REQUIREMENT_OVERRIDE", "").strip()
    if override:
        logger.debug("Using wxO wfx requirement override: %s", override)
        return override
    try:
        return _pin_requirement_name("wfx")
        ...
        message = "Could not determine installed wfx version. Failing deployment."
...
async def verify_tools_by_ids(
    ...
    from wfx.services.adapters.deployment.schema import SnapshotItem, SnapshotListResult
    ...
    for tool in tools or []:
        if not isinstance(tool, dict) or not tool.get("id"):
            continue
        connections = extract_primeagent_connections_binding(tool)
