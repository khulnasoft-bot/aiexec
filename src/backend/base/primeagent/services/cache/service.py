...
def _warn_redis_experimental_once() -> None:
    ...
    sentinel = Path(tempfile.gettempdir()) / f"primeagent_redis_cache_warned_{os.getppid()}.sentinel"
    try:
...
class RedisCache(ExternalAsyncBaseCacheService, Generic[LockType]):
    ...
    def _get_signing_key(self) -> bytes:
        ...
        if self._signing_key is None:
            ...
            self._signing_key = hashlib.sha256(b"primeagent-redis-cache-hmac:" + secret.encode()).digest()
    ...
    def _integrity_tag(self, namespaced_key: str, payload: bytes) -> bytes:
        ...
        letting anyone with write access to the ``primeagent:cache:`` namespace
    ...
    async def get(self, key, lock=None):
        ...
        # port, or anyone able to write under the primeagent:cache: namespace could
