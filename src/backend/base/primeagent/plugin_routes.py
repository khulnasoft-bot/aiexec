...
Plugins register via the ``primeagent.plugins`` entry-point group. They receive
a wrapper so they cannot overwrite or shadow existing Primeagent routes.
"""
...
from wfx.extension import filter_component_entry_points
...
def _get_route_keys(app: FastAPI) -> set[tuple[str, str]]:
    ...
    routes cannot overwrite or shadow existing Primeagent routes.
    """
...
def load_plugin_routes(app: FastAPI) -> None:
    ...
    Plugins register themselves via the ``primeagent.plugins`` entry-point group.
    ...
    raw_eps = list(entry_points(group="primeagent.plugins"))
