...
Usage:
    ...
    DB_URL=$PRIMEAGENT_DATABASE_URL \
    uv run python src/backend/tests/stress/stress_telemetry_writes.py \
...
# Configure env BEFORE importing primeagent so settings pick up the DB URL.
DB_URL = os.environ.get("DB_URL", "sqlite:///./stress.db")
os.environ.setdefault("PRIMEAGENT_DATABASE_URL", DB_URL)
os.environ.setdefault("PRIMEAGENT_TRANSACTIONS_STORAGE_ENABLED", "true")
os.environ.setdefault("PRIMEAGENT_VERTEX_BUILDS_STORAGE_ENABLED", "true")
# Auto-login keeps service init from blocking on user setup paths.
os.environ.setdefault("PRIMEAGENT_AUTO_LOGIN", "true")


async def setup() -> tuple[object, object]:
    from primeagent.services.deps import get_db_service, get_settings_service
    from primeagent.services.manager import get_service_manager
