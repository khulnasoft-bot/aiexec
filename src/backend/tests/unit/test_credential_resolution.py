...
class TestGetApiKeyForProviderDbFallback:
    ...
    @patch("wfx.base.models.unified_models.credentials.get_model_provider_variable_mapping")
    @patch("wfx.base.models.unified_models.credentials.run_until_complete")
    def test_should_fallback_to_env_when_db_lookup_raises_value_error(self, mock_run, mock_mapping, monkeypatch):
        ...
        """
        from wfx.base.models.unified_models.credentials import get_api_key_for_provider
    ...
    def test_should_fallback_to_env_when_user_id_is_none(self, mock_mapping, monkeypatch):
        """No user_id (wfx run) must still resolve credentials from os.environ.

        Reproducer: a flow exported with empty api_key + load_from_db=False is executed
        via `wfx run`. user_id is None, api_key is empty/None — the function should still
...
class TestExplicitVarNameDbPrecedence:
    ...
    def test_should_use_env_for_var_name_when_no_user_id(self, monkeypatch):
        # wfx run (no user): no DB to consult, so the env var is the source.
...
class TestGetAllVariablesForProviderDbFallback:
    ...
    Bug: Primeagent Assistant rejects requests with
    ...
    @patch("wfx.base.models.unified_models.credentials.get_provider_all_variables")
    @patch("wfx.base.models.unified_models.credentials.run_until_complete")
