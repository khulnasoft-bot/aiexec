...
from primeagent.agentic.services.flow_preparation import (
    WFX_COMPONENTS_PATH_SENTINEL,
    available_model_providers,
    inject_assistant_fs_root,
    inject_model_into_flow,
    inject_wfx_components_path,
    load_and_prepare_flow,
)

import wfx

MODULE = "primeagent.agentic.services.flow_preparation"
...
class TestInjectWfxComponentsPath:
    """Tests for inject_wfx_components_path.

    Regression guard for the Primeagent Desktop bug where the PrimeagentAssistant
    flow embedded a relative path './src/wfx/src/wfx/components/' which only
    ...
    def test_should_rewrite_directory_path_to_absolute_wfx_components_when_path_matches_sentinel(self):
        flow_data = _make_directory_flow(WFX_COMPONENTS_PATH_SENTINEL)

        result = inject_wfx_components_path(flow_data)

        rewritten = result["data"]["nodes"][0]["data"]["node"]["template"]["path"]["value"]
        expected = str(Path(wfx.__file__).parent / "components")
        assert rewritten == expected
        # Must be absolute — the whole point of the fix.
        assert Path(rewritten).is_absolute()
        # The rewritten path must actually exist in the installed wfx package.
    ...
    def test_should_rewrite_sentinel_path_when_loading_assistant_flow(self, tmp_path):
        ...
        load_and_prepare_flow must apply the wfx path injection so that
        Desktop (and any non-monorepo CWD) can execute the assistant flow.
        """
        flow_data = _make_directory_flow(WFX_COMPONENTS_PATH_SENTINEL)
        flow_file = tmp_path / "PrimeagentAssistant.json"
        flow_file.write_text(json.dumps(flow_data))
