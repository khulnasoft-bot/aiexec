...
Thin wrapper that reuses Primeagent's existing CLI validator
...
from primeagent.agentic.services.flow_static_validation import (
    FlowValidationReport,
    validate_flow_spec,
)

MODULE = "primeagent.agentic.services.flow_static_validation"


def _fake_result(*, errors=(), warnings=()):
    from wfx.cli.validation.core import ValidationIssue, ValidationResult
...
class TestValidateFlowSpecRealReuse:
    def test_should_flag_an_unknown_component_type_via_the_real_validator(self):
        # No mock — proves the real wfx validator is actually wired.
        flow = {
