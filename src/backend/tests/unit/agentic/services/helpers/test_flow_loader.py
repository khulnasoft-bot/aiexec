...
class TestLoadGraphFromPython:
    ...
    async def test_should_load_graph_from_get_graph_function(self):
        ...
        with (
            ...
            patch(
                "primeagent.agentic.services.helpers.flow_loader.validate_flow_for_current_settings"
            ) as mock_validate,
        ):
            mock_spec = MagicMock()
    ...
    async def test_should_pass_provider_params_to_get_graph(self):
        ...
        with (
            ...
            patch("importlib.util.module_from_spec") as mock_module_from_spec,
            patch("primeagent.agentic.services.helpers.flow_loader._temporary_sys_path"),
            patch("primeagent.agentic.services.helpers.flow_loader.validate_flow_for_current_settings"),
...
class TestBugsAndEdgeCases:
    ...
    def test_resolve_flow_path_traversal_escape(self, tmp_path):
        ...
        with patch("primeagent.agentic.services.helpers.flow_loader.FLOWS_BASE_PATH", flows_dir):
            with pytest.raises(HTTPException) as exc_info:
