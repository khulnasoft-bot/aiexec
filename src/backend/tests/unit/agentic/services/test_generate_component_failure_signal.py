...
import pytest
from primeagent.agentic.services.component_events import (
...
async def test_should_emit_validation_failed_signal_when_generation_fails(monkeypatch, fresh_component_events):  # noqa: ARG001
    from wfx.mcp.flow_builder_tools.run_tools import GenerateComponent

    async def _failed_generation(**_kwargs):
        return {"validated": False, "validation_error": "Output method 'run' has no return statement"}

    monkeypatch.setattr(
        "primeagent.agentic.services.assistant_service.execute_flow_with_validation",
        _failed_generation,
    )
