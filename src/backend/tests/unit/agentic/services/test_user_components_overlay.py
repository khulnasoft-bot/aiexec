...
import pytest
import wfx.custom.utils as wfx_utils
...
SAMPLE_CODE = (
    ...
    "from wfx.io import FloatInput, Output\n"
...
def isolated_sandbox(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    from primeagent.agentic.services.user_components_context import reset_current_user_id
    from primeagent.agentic.services.user_components_overlay import _OVERLAY_ENTRY_CACHE
...
class TestOverlayEntryCaching:
    ...
    def test_overlay_built_once_then_rebuilt_when_file_changes(self, isolated_sandbox: Path) -> None:  # noqa: ARG002
        real_build = wfx_utils.build_custom_component_template

        register_user_component(user_id="user-alice", class_name="SumComponent", code=SAMPLE_CODE)

        with patch.object(wfx_utils, "build_custom_component_template", side_effect=real_build) as spy:
...
class TestRegistryOverlay:
    ...
    async def test_overlay_template_reflects_real_inputs_and_runs_with_config(
        ...
        code = (
            "from math import isqrt\n"
            "from wfx.custom import Component\n"
            "from wfx.io import IntInput, Output\n"
            "from wfx.schema import Message\n"
            "\n"
            "class PrimeChecker(Component):\n"
            "    display_name = 'PrimeChecker'\n"
    ...
    def test_should_silently_skip_invalid_files(self, isolated_sandbox: Path) -> None:  # noqa: ARG002
        ...
        from primeagent.agentic.services.user_components import (
