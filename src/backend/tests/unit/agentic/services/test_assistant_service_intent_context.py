...
MODULE = "primeagent.agentic.services.assistant_service"


@pytest.fixture
def fresh_buffer(monkeypatch):
    """Swap the module-level singleton with a fresh, empty buffer."""
    import primeagent.agentic.services.conversation_buffer as module
...
async def test_should_not_leak_working_flow_when_off_topic_after_canvas_seed(fresh_buffer):  # noqa: ARG001
    ...
    """
    from wfx.mcp.flow_builder_tools import get_working_flow, reset_working_flow

    reset_working_flow()

    async def fake_summary(_flow_id, **_kwargs):  # accepts user_id kwarg from production (I2)
        from wfx.mcp.flow_builder_tools import init_working_flow
