...
import pytest
from primeagent.agentic.services.conversation_buffer import ConversationTurn
...
class TestIntentRoutingRealLLM:
    ...
    async def test_pt_canvas_question_is_not_false_off_topic(self):
        ...
        assert result.intent != "off_topic", f"Primeagent-related request misrouted to off_topic ({result.intent})"
...
class TestLegacyFilterReal:
    ...
    def test_real_registry_search_excludes_legacy_but_describe_resolves(self):
        from wfx.graph.flow_builder.builder import load_local_registry
        from wfx.mcp.registry import describe_component, search_registry
    ...
    def test_real_registry_search_includes_beta(self):
        from wfx.graph.flow_builder.builder import load_local_registry
        from wfx.mcp.registry import search_registry
...
class TestRuntimeValidationReal:
    ...
    async def test_rejects_output_returning_non_dict_from_input(self):
        ...
        from primeagent.agentic.helpers.validation import validate_component_runtime
...
class TestUserComponentOverlayReal:
    ...
    def test_registered_component_is_discoverable_by_class_name(self, tmp_path, monkeypatch):
        monkeypatch.setenv("PRIMEAGENT_FS_TOOL_BASE_DIR", str(tmp_path))
        from primeagent.agentic.services.user_components import register_user_component
        from primeagent.agentic.services.user_components_overlay import (
...
class TestRunFlowIntentRealLLM:
    ...
    async def test_edit_continuation_signal_routes_as_flow_request(self):
        ...
        from primeagent.agentic.services.flow_types import EDIT_CONTINUATION_INPUT
...
class TestRealEngineRunReturnsResult:
    ...
    async def test_real_minimal_flow_runs_and_returns_result(self):
        ...
        from wfx.graph.flow_builder.builder import build_flow_from_spec, load_local_registry
