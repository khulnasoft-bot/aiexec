"""The Primeagent Assistant must be callable from external MCP clients.

User report (Discord, 2026-06-11): "i can't call the assistant by mcp."
Verified live (2026-06-12): the ``primeagent-agentic`` MCP server exposes 18
...
RUNNER_MODULE = "primeagent.agentic.utils.assistant_runner"
...
EVENTS_TEXT_ONLY = [
    ...
    {"event": "complete", "data": {"result": "Primeagent is a visual flow builder."}},
]
...
class TestRunAssistantAndPersist:
    ...
    async def test_should_create_a_new_flow_when_no_flow_id_is_given(self):
        from primeagent.agentic.utils.assistant_runner import run_assistant_and_persist
    ...
    async def test_should_not_touch_the_flow_when_assistant_only_answers_text(self):
        ...
        with (
            ...
            result = await run_assistant_and_persist(
                session=session,
                user_id=user_id,
                instruction="What is Primeagent?",
                flow_id=str(flow.id),
            )

        assert flow.data is original_data
        assert result["flow_changed"] is False
        assert result["result"] == "Primeagent is a visual flow builder."
...
class TestRunAssistantAppliesProposedFieldEdits:
        ...
        return [
            {
                ...
                "field": "system_prompt",
                "old_value": "You are a Primeagent Agent.",
    ...
    async def test_should_persist_proposed_edit_when_working_flow_omits_it(self):
        from primeagent.agentic.utils import assistant_runner

        user_id = uuid4()
        flow = SimpleNamespace(
            id=uuid4(), name="My Flow", data=self._agent_data("You are a Primeagent Agent."), user_id=user_id
        )
        session = AsyncMock()
        session.get = AsyncMock(return_value=flow)
        ...
        working_without_edit = {"data": self._agent_data("You are a Primeagent Agent.")}
