...
def mock_tracers():
    with (
        ...
        patch(
            "primeagent.services.tracing.service._get_native_tracer",
            return_value=MockTracer,
        ),
        patch(
            "primeagent.services.tracing.service._get_openlayer_tracer",
            return_value=MockTracer,
        ),
    ):
        yield
...
async def test_start_tracers_forwards_tracing_user_id_to_langfuse(tracing_service):
    ...
    the authenticated Primeagent user (backwards compat) and exposes the override
