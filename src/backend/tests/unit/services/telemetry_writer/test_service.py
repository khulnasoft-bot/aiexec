...
we don't have to bring up the entire primeagent service manager — instead each
...
import pytest
from primeagent.services.database.models.transactions.model import TransactionBase, TransactionTable
from primeagent.services.database.models.vertex_builds.model import VertexBuildBase, VertexBuildTable
from primeagent.services.telemetry_writer.service import (
...
def _find_dead_pid(start: int = 99_999, max_checks: int = 50_000) -> int:
    from primeagent.services.telemetry_writer.service import _pid_alive
...
async def test_teardown_spills_remaining_buffer(tmp_path: Path) -> None:
    ...
    # going through start() (which needs the full primeagent service stack).
    writer._started = True
    writer._shutdown_event = asyncio.Event()
    writer._shutdown_event.set()
    ...
    fallback = Path(tempfile.gettempdir()) / "primeagent_telemetry_outbox"
