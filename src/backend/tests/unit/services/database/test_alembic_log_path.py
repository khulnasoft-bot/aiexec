...
Regression coverage for https://github.com/khulnasoft/primeagent/issues/11143:
...
def _make_service(
    *,
    ...
    with patch("primeagent.services.database.service.create_async_engine", return_value=MagicMock()):
...
class TestAlembicLogPathResolution:
    def test_relative_path_resolves_under_config_dir_not_package(self, tmp_path):
        ...
        It must NOT resolve into the installed primeagent package directory, which
...
class TestInitializeAlembicLogFile:
    ...
    async def test_oserror_is_swallowed(self, tmp_path):
        ...
        service = _make_service(config_dir=str(tmp_path), alembic_log_file="alembic.log")
        with patch("primeagent.services.database.service.anyio.Path") as mock_anyio_path:
