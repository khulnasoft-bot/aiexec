...
``PRIMEAGENT_DATABASE_URL`` pointing at a missing subdirectory used to crash with
...
Issue: https://github.com/khulnasoft/primeagent/issues/13634
"""

from pathlib import Path

import pytest
from primeagent.services.database.service import (
...
class TestGetSqliteDatabaseFilePath:
    ...
    @pytest.mark.parametrize(
        "url",
        [
            "postgresql+psycopg://localhost:5432/primeagent",
            "postgresql://localhost/primeagent",
            "mysql://localhost/primeagent",
        ],
    )
    ...
    def test_relative_path_is_returned_verbatim(self):
        ...
        assert get_sqlite_database_file_path("sqlite:///db/primeagent.db") == Path("db/primeagent.db")

    def test_relative_dot_path_is_returned_verbatim(self):
        assert get_sqlite_database_file_path("sqlite:///./primeagent.db") == Path("./primeagent.db")

    def test_absolute_path_is_returned(self):
        assert get_sqlite_database_file_path("sqlite:////var/data/primeagent.db") == Path("/var/data/primeagent.db")

    def test_sanitized_async_driver_is_recognized(self):
        # ``_sanitize_database_url`` rewrites ``sqlite`` -> ``sqlite+aiosqlite``.
        assert get_sqlite_database_file_path("sqlite+aiosqlite:///db/primeagent.db") == Path("db/primeagent.db")
...
class TestCheckSqliteDatabasePath:
    ...
    def test_no_raise_when_absolute_parent_exists(self, tmp_path):
        url = f"sqlite:///{tmp_path / 'primeagent.db'}"
        check_sqlite_database_path(url)

    def test_no_raise_for_relative_existing_parent(self, tmp_path, monkeypatch):
        # The documented default ``sqlite:///./primeagent.db`` resolves its parent
        # to the CWD, which always exists -- it must keep working.
        monkeypatch.chdir(tmp_path)
        check_sqlite_database_path("sqlite:///./primeagent.db")

    def test_raises_for_absolute_missing_parent(self, tmp_path):
        missing = tmp_path / "does_not_exist" / "primeagent.db"
    ...
    def test_raises_for_relative_missing_subdirectory(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        with pytest.raises(ValueError, match="parent directory") as exc_info:
            check_sqlite_database_path("sqlite:///db/primeagent.db")
        message = str(exc_info.value)
...
class TestInitializeDatabaseWiring:
    ...
    async def test_relative_missing_dir_fails_fast_instead_of_opaque_error(self, tmp_path, monkeypatch):
        from primeagent.services.database import utils as db_utils

        monkeypatch.chdir(tmp_path)

        class _StubDatabaseService:
            database_url = "sqlite+aiosqlite:///db/primeagent.db"
        ...
        monkeypatch.setattr("primeagent.services.deps.get_db_service", lambda: _StubDatabaseService())
