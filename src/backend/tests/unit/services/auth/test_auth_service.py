...
async def test_authenticate_with_credentials_auto_login_skip_returns_superuser(
    ...
    with (
        ...
        patch("primeagent.services.auth.service.logger") as mock_logger,
    ):
...
async def test_authenticate_with_credentials_auto_login_skip_missing_superuser_raises(
    ...
    from primeagent.services.auth.exceptions import InvalidCredentialsError
...
async def test_auto_login_longterm_token_is_short_lived_with_refresh(
    ...
    with (
        patch("primeagent.services.auth.service.get_user_by_username", new=AsyncMock(return_value=superuser)),
        patch("primeagent.services.auth.service.update_user_last_login_at", new=AsyncMock()),
    ):
...
def test_add_padding_no_extra_chars_when_divisible_by_4():
    ...
    from primeagent.services.auth.utils import add_base64_padding
...
def test_ensure_fernet_key_with_44_char_key():
    ...
    from primeagent.services.auth.utils import ensure_fernet_key
...
async def test_ws_api_key_security_auto_login_skip_rejects_missing_superuser(
    ...
    with (
        patch("primeagent.services.auth.service.session_scope", _mock_session_scope),
        patch(
            "primeagent.services.auth.service.get_user_by_username",
            new=AsyncMock(return_value=None),
        ),
