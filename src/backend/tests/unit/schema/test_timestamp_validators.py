"""Unit tests for timestamp validator functions in both primeagent and wfx schemas."""

from datetime import datetime, timezone

import primeagent.schema.validators as lf_validators
import pytest
import wfx.schema.validators as wfx_validators
...
VALIDATOR_MODULES = [
    pytest.param(lf_validators, id="primeagent"),
    pytest.param(wfx_validators, id="wfx"),
]
...
class TestEncoderCompatibility:
    ...
    def test_encode_datetime_parseable_by_primeagent_str_to_timestamp(self):
        from primeagent.schema.encoders import encode_datetime
    ...
    def test_encode_datetime_parseable_by_wfx_str_to_timestamp(self):
        from primeagent.schema.encoders import encode_datetime

        dt = _utc(2024, 8, 20, 14, 0, 0, 654321)
        encoded = encode_datetime(dt)
        recovered = wfx_validators.str_to_timestamp(encoded)
        assert recovered == dt
