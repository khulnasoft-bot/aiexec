...
def migrate_module():
    ...
    repo_root = test_file.parents[5]  # Goes to primeagent repo root
...
class TestDatabaseMigrationUnit:
    ...
    def test_migrate_user_store_api_key(self, migrate_module, sqlite_db, old_key, new_key):
        ...
        user_id = str(uuid4())
        original_value = "primeagent-store-api-key"
...
class TestKeyFileManagement:
    ...
    def test_get_config_dir_default(self, migrate_module, monkeypatch):
        ...
        monkeypatch.delenv("PRIMEAGENT_CONFIG_DIR", raising=False)
        result = migrate_module.get_config_dir()
        expected = Path(user_cache_dir("primeagent", "primeagent"))
        assert result == expected

    def test_get_config_dir_from_env(self, migrate_module, monkeypatch):
        """Test config directory from environment variable."""
        monkeypatch.setenv("PRIMEAGENT_CONFIG_DIR", "/custom/config")
        result = migrate_module.get_config_dir()
...
class TestMigrationWithRealDatabase:
    """Integration tests using real Primeagent database fixtures."""
...
class TestMigrationCompatibility:
    """Test that migration script is compatible with Primeagent's encryption."""

    def test_script_encryption_matches_primeagent(self, migrate_module):
        """Verify migration script produces same results as Primeagent's auth utils."""
        ...
        # Encrypt with Primeagent
        primeagent_encrypted = auth_utils.encrypt_api_key(plaintext, settings_service)

        # Decrypt with migration script
        script_decrypted = migrate_module.decrypt_with_key(primeagent_encrypted, secret_key)
        ...
        # Decrypt with Primeagent
        primeagent_decrypted = auth_utils.decrypt_api_key(script_encrypted, settings_service)
        assert primeagent_decrypted == plaintext
