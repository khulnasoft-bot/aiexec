...
class TestRemoveSqliteLockFiles:
    ...
    def test_should_remove_all_lock_files(self, kb_dir):
        from primeagent.api.utils.kb_helpers import _remove_sqlite_lock_files
...
class TestTruncateSqliteFiles:
    ...
    def test_should_truncate_sqlite_files_to_zero(self, kb_dir):
        from primeagent.api.utils.kb_helpers import _truncate_sqlite_files
...
class TestReleaseChromaResources:
    ...
    def test_should_clear_registry_entry_for_path(self, kb_dir):
        ...
        from primeagent.api.utils.kb_helpers import KBStorageHelper
    ...
    @patch("primeagent.api.utils.kb_helpers.gc.collect")
...
class TestDeleteStorage:
    ...
    @patch("primeagent.api.utils.kb_helpers.KBStorageHelper.get_fresh_chroma_client", new=MagicMock())
    @patch("primeagent.api.utils.kb_helpers.Chroma", new=MagicMock())
    @patch("primeagent.api.utils.kb_helpers.time.sleep", new=MagicMock())
    ...
    def test_should_retry_and_succeed_on_second_attempt(self, kb_dir):
        ...
        with patch("primeagent.api.utils.kb_helpers.shutil.rmtree", side_effect=rmtree_fails_once):
            result = KBStorageHelper.delete_storage(kb_dir, "test_kb")
    ...
    def test_should_write_sentinel_when_all_retries_fail(self, kb_dir):
        ...
        """
        from primeagent.api.utils.kb_helpers import KB_DELETED_SENTINEL, KBStorageHelper

        with patch(
            "primeagent.api.utils.kb_helpers.shutil.rmtree",
    ...
    @patch("primeagent.api.utils.kb_helpers.KBStorageHelper.get_fresh_chroma_client", new=MagicMock())
    @patch("primeagent.api.utils.kb_helpers.Chroma", new=MagicMock())
    @patch("primeagent.api.utils.kb_helpers.time.sleep")
    def test_should_use_exponential_backoff_on_retries(self, mock_sleep, kb_dir):
        ...
        with patch("primeagent.api.utils.kb_helpers.shutil.rmtree", side_effect=rmtree_fails_three_times):
            result = KBStorageHelper.delete_storage(kb_dir, "test_kb")
    ...
    @patch("primeagent.api.utils.kb_helpers.KBStorageHelper.get_fresh_chroma_client")
    @patch("primeagent.api.utils.kb_helpers.Chroma")
    @patch("primeagent.api.utils.kb_helpers.time.sleep", new=MagicMock())
...
class TestDeleteEndpoint:
    ...
    @patch("primeagent.api.utils.kb_helpers.KBStorageHelper.delete_storage", return_value=False)
    @patch("primeagent.api.v1.knowledge_bases.KBStorageHelper.get_root_path")
...
class TestBulkDeleteEndpoint:
    ...
    @patch("primeagent.api.utils.kb_helpers.KBStorageHelper.delete_storage")
    @patch("primeagent.api.v1.knowledge_bases.KBStorageHelper.get_root_path")
    ...
    @patch("primeagent.api.utils.kb_helpers.KBStorageHelper.delete_storage", new=MagicMock(return_value=True))
    @patch("primeagent.api.v1.knowledge_bases.KBStorageHelper.get_root_path")
...
# ===========================================================================
# Unit tests: wfx get_knowledge_bases filter parity
# ===========================================================================


class TestWfxSentinelStringInSync:
    """The wfx package inlines the sentinel filename string.

    wfx is published independently of primeagent and cannot import
    ``KB_DELETED_SENTINEL`` from primeagent.api.utils.kb_helpers without
    ...
    def test_sentinel_constant_matches_wfx_literal(self):
        from primeagent.api.utils.kb_helpers import KB_DELETED_SENTINEL

        # The wfx-side literal is intentionally inlined as the string
        # ".kb_deleted" inside ``wfx.base.knowledge_bases.knowledge_base_utils``;
