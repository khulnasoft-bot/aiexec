...
allowing ``--env-file PRIMEAGENT_ENABLE_EXTENSION_RELOAD=true`` to opt
...
from primeagent.api.v1.extensions import _require_extension_reload_enabled
...
def test_guard_raises_404_when_disabled(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    """
    monkeypatch.setattr(
        "primeagent.api.v1.extensions.get_settings_service",
    ...
    assert "PRIMEAGENT_ENABLE_EXTENSION_RELOAD" in detail["message"]
    assert detail["hint"]
...
def test_route_is_mounted_unconditionally() -> None:
    ...
    Regression: an earlier implementation read ``PRIMEAGENT_ENABLE_EXTENSION_RELOAD``
    at import time, which meant ``--env-file`` could not turn the route on
    because ``primeagent.__main__`` imports the router before ``load_dotenv``
...
def test_no_runtime_mutation_routes() -> None:
    ...
    Fix hint:
      ...
      Mode A (local dev): use ``wfx extension dev`` which sets
      PRIMEAGENT_ENABLE_EXTENSION_RELOAD=true.
    """
    from primeagent.api.router import router
