...
from primeagent.api.v1.mappers.deployments import get_mapper
from primeagent.api.v1.mappers.deployments.base import (
...
try:
    from primeagent.services.adapters.deployment.watsonx_orchestrate import (
...
from wfx.services.adapters.payload import AdapterPayloadValidationError, PayloadSlot
...
def test_watsonx_mapper_create_model_uses_adapter_result_description() -> None:
    ...
    result = DeploymentCreateResult(
        id="provider-id",
        type=DeploymentType.AGENT,
        name="primeagent_OK_AGENT_6_cea1e533",
        description="Primeagent deployment OK AGENT 6",
        provider_result={
    ...
    assert deployment.description == "Primeagent deployment OK AGENT 6"
...
async def test_watsonx_mapper_translates_create_bind_into_raw_tool_payload() -> None:
    ...
    assert raw_provider_data["tool_name"].startswith("primeagent_Flow_A_")
...
async def test_watsonx_mapper_translates_create_bind_with_tool_display_name_override() -> None:
    ...
    assert raw_provider_data["tool_name"].startswith("primeagent_My_Create_Tool_")
    assert raw_tool_name == "Flow A"
...
def test_watsonx_mapper_trusts_top_level_deployment_id() -> None:
    ...
    from wfx.services.adapters.deployment.schema import ExecutionStatusResult
...
def test_wxo_mapper_verify_credentials_create_filters_non_credential_fields() -> None:
    ...
    from primeagent.api.v1.schemas.deployments import DeploymentProviderAccountCreateRequest
...
def test_wxo_mapper_update_allows_name_changes_only_for_non_credential_fields() -> None:
    ...
    from primeagent.api.v1.schemas.deployments import DeploymentProviderAccountUpdateRequest
...
def test_wxo_mapper_resolve_verify_credentials_rejects_extra_fields() -> None:
    ...
    from primeagent.services.adapters.deployment.watsonx_orchestrate.payloads import PAYLOAD_SCHEMAS
    from wfx.services.adapters.payload import AdapterPayloadValidationError
...
async def test_watsonx_mapper_tool_display_name_override_generates_valid_technical_name() -> None:
    ...
    assert raw_payload["provider_data"]["source_ref"] == str(flow_version_id)
    assert raw_payload["provider_data"]["tool_name"].startswith("primeagent_valid_name_")
...
def test_watsonx_created_tool_includes_tool_id() -> None:
    ...
    from primeagent.api.v1.mappers.deployments.watsonx_orchestrate.payloads import WatsonxApiCreatedTool
