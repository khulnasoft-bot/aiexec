...
import pytest
from primeagent.api.utils.core import extract_global_variables_from_headers
from primeagent.api.v1 import mcp_utils
from primeagent.helpers import flow as flow_helpers
...
class TestExtractGlobalVariablesFromHeaders:
    ...
    ``x-api-key`` is Primeagent's own auth key and must not leak into the graph
    context.
    """

    def test_primeagent_global_var_prefix_still_extracted(self):
        """Regression guard: ``X-PRIMEAGENT-GLOBAL-VAR-*`` extraction is preserved."""
        headers = {
            "X-PRIMEAGENT-GLOBAL-VAR-API-KEY": "secret-value",
            "X-PRIMEAGENT-GLOBAL-VAR-DB-URL": "postgres://host/db",
    ...
    def test_auth_headers_not_extracted_by_default(self):
        ...
        headers = {
            "x-api-key": "primeagent-auth-key",
            "authorization": "Bearer token",
            "X-PRIMEAGENT-GLOBAL-VAR-MY-VAR": "value",
        }
    ...
    def test_both_categories_extracted_together(self):
        """``X-PRIMEAGENT-GLOBAL-VAR-*`` and auth headers coexist when opted in."""
        headers = {
            "X-PRIMEAGENT-GLOBAL-VAR-API-KEY": "global-secret",
...
def test_json_schema_from_flow_includes_optional_session_id(monkeypatch):
    ...
    import wfx.graph.graph.base as graph_base_module
