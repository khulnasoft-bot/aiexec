...
Covers the fix for https://github.com/khulnasoft/primeagent/issues/9797 where MCP
SSE transport breaks when Primeagent sits behind a reverse proxy that adds a URL
prefix (basePath).
"""

import pytest
from mcp.server.sse import SseServerTransport
from primeagent.api.v1.mcp_projects import get_project_sse, project_sse_transports
...
class TestForwardedPrefixMiddleware:
    ...
    async def _captured_downstream_root_path(self, *, configured_root_path, headers=None):
        ...
        from primeagent.main import get_settings_service
    ...
    async def test_middleware_sets_root_path_when_enabled(self):
        ...
        root_path = await self._captured_downstream_root_path(
            configured_root_path="/enabled",
            headers={"X-Forwarded-Prefix": "/primeagent"},
        )

        assert root_path == "/primeagent"
...
class TestRootPathSetting:
    ...
    def test_root_path_can_be_set(self, monkeypatch):
        ...
        monkeypatch.setenv("PRIMEAGENT_ROOT_PATH", "/basePath")
        s = Settings()
