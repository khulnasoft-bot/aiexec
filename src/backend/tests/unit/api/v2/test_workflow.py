...
from primeagent.services.database.models.jobs.model import Job, JobType
...
class TestWorkflowSyncExecution:
    ...
    async def test_sync_execution_uses_body_globals_with_unicode(
        ...
        try:
            ...
            with (
                patch("primeagent.api.v2.workflow.Graph.from_payload", return_value=graph) as mock_from_payload,
                patch("primeagent.api.v2.workflow.get_job_service", return_value=mock_job_service),
                patch("primeagent.api.v2.workflow.get_task_service", return_value=mock_task_service),
            ):
                headers = {"x-api-key": created_api_key.api_key}
                response = await client.post("api/v2/workflows", json=request_data, headers=headers)
    ...
    async def test_sync_execution_honors_legacy_global_variable_headers_with_deprecation(
        ...
        """V2 workflows still read X-PRIMEAGENT-GLOBAL-VAR-* headers for one release.
        ...
        try:
            ...
            with (
                ...
                patch("primeagent.api.v2.workflow.get_job_service", return_value=mock_job_service),
                patch("primeagent.api.v2.workflow.get_task_service", return_value=mock_task_service),
                patch("primeagent.api.v2.workflow.logger.warning") as mock_warning,
            ):
                headers = {
                    "x-api-key": created_api_key.api_key,
                    "X-PRIMEAGENT-GLOBAL-VAR-FILENAME": "header-loses.pdf",
                    "X-PRIMEAGENT-GLOBAL-VAR-OWNER_NAME": "header-only",
                }
                response = await client.post("api/v2/workflows", json=request_data, headers=headers)
    ...
    async def test_sync_execution_rejects_oversized_globals_value(
        ...
        from wfx.schema.workflow import GLOBAL_VALUE_MAX_LEN
...
class TestWorkflowIDORProtection:
    ...
    @pytest.fixture
    def mock_settings_dev_api_enabled(self):
        with patch("primeagent.api.v2.workflow.get_settings_service") as mock_get_settings:
            mock_service = MagicMock()
            mock_settings = MagicMock()
            mock_settings.developer_api_enabled = True
            mock_service.settings = mock_settings
    ...
    async def test_background_job_stores_user_id_and_blocks_cross_user_access(
        ...
        try:
            ...
            with patch("primeagent.api.v2.workflow.get_task_service") as mock_task_svc:
                mock_task_service = MagicMock()
                mock_task_service.fire_and_forget_task = AsyncMock()
