...
class TestAPIRequestSSRFProtection:
    ...
    async def test_ssrf_protection_disabled_allows_all_urls(self, component):
        ...
        with (
            patch.dict(os.environ, {"PRIMEAGENT_SSRF_PROTECTION_ENABLED": "false"}),
            respx.mock,
        ):
    ...
    async def test_ssrf_protection_allows_public_urls(self, component):
        ...
        public_urls = [
            ...
            "https://api.github.com/repos/khulnasoft/primeagent",
    ...
    async def test_ssrf_allowlist_ip_address(self, component):
        ...
        with (
            patch.dict(
                ...
                {
                    "PRIMEAGENT_SSRF_PROTECTION_ENABLED": "true",
                    "PRIMEAGENT_SSRF_ALLOWED_HOSTS": "192.168.1.100",
                },
            ),
            respx.mock,
        ):
    ...
    async def test_ssrf_allowlist_multiple_entries(self, component):
        ...
        with (
            patch.dict(
                ...
                {
                    "PRIMEAGENT_SSRF_PROTECTION_ENABLED": "true",
                    "PRIMEAGENT_SSRF_ALLOWED_HOSTS": "localhost,192.168.1.0/24,internal.local",
                },
            ),
            respx.mock,
        ):
    ...
    async def test_dns_pinning_is_used_when_protection_enabled(self, component):
        ...
        with (
            patch.dict(os.environ, {"PRIMEAGENT_SSRF_PROTECTION_ENABLED": "true"}),
            patch("wfx.components.data_source.api_request.create_ssrf_protected_client") as mock_create_client,
            respx.mock,
        ):
...
class TestAPIRequestRedirectSSRFProtection:
    ...
    async def test_redirect_to_internal_allowed_when_protection_disabled(self, component):
        ...
        with patch.dict(os.environ, {"PRIMEAGENT_SSRF_PROTECTION_ENABLED": "false"}):
            result = await component.make_api_request()
