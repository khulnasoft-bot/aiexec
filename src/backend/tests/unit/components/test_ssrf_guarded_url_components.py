...
import pytest
from wfx.components.deepseek.deepseek import DEEPSEEK_MODELS, DeepSeekModelComponent
from wfx.components.glean.glean_search_api import GleanAPIWrapper
from wfx.components.homeassistant.home_assistant_control import HomeAssistantControl
from wfx.components.homeassistant.list_home_assistant_states import ListHomeAssistantStates
from wfx.components.huggingface.huggingface_inference_api import HuggingFaceInferenceAPIEmbeddingsComponent
from wfx.components.litellm.litellm_proxy import LiteLLMProxyComponent
from wfx.components.lmstudio.lmstudioembeddings import LMStudioEmbeddingsComponent
from wfx.components.lmstudio.lmstudiomodel import LMStudioModelComponent
...
from wfx.components.xai.xai import XAI_DEFAULT_MODELS, XAIModelComponent
...
def enable_ssrf_protection(monkeypatch):
    monkeypatch.setenv("PRIMEAGENT_SSRF_PROTECTION_ENABLED", "true")
    monkeypatch.delenv("PRIMEAGENT_SSRF_ALLOWED_HOSTS", raising=False)
...
def test_lmstudio_model_build_blocks_metadata_url_before_openai_client():
    ...
    with (
        patch("wfx.components.lmstudio.lmstudiomodel.ChatOpenAI") as mock_chat_openai,
...
def test_lmstudio_embeddings_build_blocks_metadata_url_before_sdk_client():
    ...
    with (
        patch("wfx.components.lmstudio.lmstudioembeddings.NVIDIAEmbeddings", create=True) as mock_embeddings,
        pytest.raises(ValueError, match="SSRF Protection"),
    ):
...
def test_xai_build_blocks_metadata_url_before_openai_client():
    ...
    with (
        patch("wfx.components.xai.xai.ChatOpenAI") as mock_chat_openai,
        pytest.raises(ValueError, match="SSRF Protection"),
    ):
        component.build_model()
...
def test_ollama_embeddings_build_blocks_metadata_url_before_sdk_client():
    ...
    with (
        patch("wfx.components.ollama.ollama_embeddings.OllamaEmbeddings") as mock_embeddings,
        pytest.raises(ValueError, match="SSRF Protection"),
    ):
        component.build_embeddings()
...
def test_ollama_build_blocks_metadata_url_before_sdk_client():
    ...
    with (
        patch("wfx.components.ollama.ollama.ChatOllama") as mock_chat_ollama,
        pytest.raises(ValueError, match="SSRF Protection"),
    ):
        component.build_model()
...
def test_litellm_build_blocks_metadata_url_before_httpx_and_openai_client():
    ...
    with (
        ...
        patch("wfx.components.litellm.litellm_proxy.ChatOpenAI") as mock_chat_openai,
        pytest.raises(ValueError, match="SSRF Protection"),
    ):
        component.build_model()
