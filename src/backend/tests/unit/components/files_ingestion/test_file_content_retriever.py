...
import pandas as pd
import pytest
from wfx.components.files_ingestion.file_content_retriever import FileContentRetrieverComponent
...
class TestFileContentRetrieverComponent(ComponentTestBaseWithoutClient):
    ...
    def test_retrieve_content_with_dataframe_input(self, component_class):
        ...
        primeagent_df = DataFrame(df)
        primeagent_df.attrs["source_file_path"] = "data.csv"

        component = component_class()
        component.set_attributes({"file_data": [primeagent_df]})
    ...
    def test_retrieve_content_mixed_data_and_dataframe(self, component_class):
        ...
        component = component_class()
        component.set_attributes({"file_data": [primeagent_df, data_obj]})
    ...
    def test_as_dataframe_returns_existing_dataframe(self, component_class):
        ...
        assert result is primeagent_df
        assert len(result) == 3
