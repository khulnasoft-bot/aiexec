...
import pytest
from wfx.components.weaviate.weaviate import WeaviateVectorStoreComponent
...
def test_connect_client_cloud(mocker):
    ...
    mock_auth = mocker.patch("wfx.components.weaviate.weaviate.AuthApiKey", return_value="AUTH")
    component = WeaviateVectorStoreComponent(
...
def test_build_vector_store_without_documents(mocker):
    ...
    mock_store_cls = mocker.patch("wfx.components.weaviate.weaviate.WeaviateVectorStore", return_value=MagicMock())
...
def test_build_vector_store_with_documents_uses_from_documents(mocker):
    mock_client = MagicMock()
    mocker.patch.object(WeaviateVectorStoreComponent, "_connect_client", return_value=mock_client)
    mock_store_cls = mocker.patch("wfx.components.weaviate.weaviate.WeaviateVectorStore")
    fake_embedding = MagicMock()
