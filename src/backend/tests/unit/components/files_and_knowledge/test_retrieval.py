...
``wfx.base.models.unified_models.get_embeddings`` (same as ingestion)
...
from wfx.base.knowledge_bases.backends import ChromaLocalBackend
...
class TestKnowledgeBaseComponent(ComponentTestBaseWithClient):
    ...
    def mock_knowledge_base_path(self, tmp_path):
        ...
        with patch("wfx.components.files_and_knowledge._kb_paths._KNOWLEDGE_BASES_ROOT_PATH", tmp_path):
            yield
    ...
    async def test_get_knowledge_bases_db_first_hides_stale_disk_dirs(self, tmp_path, active_user):
        ...
        management page. Before the fix, the wfx helper scanned the disk
    ...
    def test_resolve_model_selection_falls_back_to_catalog_lookup(self, component_class, default_kwargs):
        ...
        with patch(
            "wfx.components.files_and_knowledge.knowledge.get_embedding_model_options",
    ...
    async def test_retrieve_data_missing_user_id_raises(self, component_class, default_kwargs):
        ...
        with patch("wfx.components.files_and_knowledge.knowledge.session_scope") as mock_session_scope:
    ...
    async def test_retrieve_data_missing_user_record_raises(self, component_class, default_kwargs):
        component = component_class(**default_kwargs)
        with (
            patch("wfx.components.files_and_knowledge.knowledge.session_scope") as mock_session_scope,
            patch(
    ...
    async def test_retrieve_data_routes_query_with_scores(
        ...
        with (
            ...
            patch(
                "wfx.components.files_and_knowledge.knowledge._get_knowledge_bases_root_path",
                return_value=Path(default_kwargs["kb_root_path"]),
            ),
            patch(
                "wfx.components.files_and_knowledge.knowledge.get_embeddings",
                return_value=MagicMock(),
            ),
            patch(
                "wfx.components.files_and_knowledge.knowledge.create_backend",
...
class TestEmbeddingMatchKey:
    ...
    def test_prefers_id_when_present(self):
        from wfx.components.files_and_knowledge.knowledge import _embedding_match_key
...
class TestMetadataFilterHelpers:
    ...
    def test_parse_returns_empty_when_unset(self):
        from wfx.components.files_and_knowledge.retrieval import _parse_metadata_filter
    ...
    def test_chunk_match_requires_every_key(self):
        ...
        from wfx.components.files_and_knowledge.retrieval import _chunk_matches_filter
