...
import numpy as np
import pytest
from wfx.components.files_and_knowledge import _kb_paths
from wfx.components.files_and_knowledge.memory_retrieval import (
    MemoryBaseComponent,
    _coerce_uuid,
...
def _patched_session_scope(db) -> object:
    return patch(
        "wfx.components.files_and_knowledge.memory_retrieval.session_scope",
...
class TestLoadKbMetadata:
    ...
    def test_no_api_key_skips_decrypt(self, tmp_path: Path):
        ...
        with patch("wfx.components.files_and_knowledge._kb_paths.decrypt_api_key") as decrypt:
    ...
    def test_decrypt_success(self, tmp_path: Path):
        ...
        (tmp_path / "embedding_metadata.json").write_text(json.dumps(payload))
        with patch(
            "wfx.components.files_and_knowledge._kb_paths.decrypt_api_key",
            return_value="plain",
        ):
            result = _kb_paths.load_kb_metadata(tmp_path, log_label="x")
...
class TestRootPathCache:
    def test_reset_cache_picks_up_new_setting(self, tmp_path: Path):
        ...
        with patch("wfx.components.files_and_knowledge._kb_paths.get_settings_service") as gs:
...
class TestMemoryBaseRetrievalInvariants:
    ...
    async def test_owner_not_found_raises(self):
        ...
        with (
            ...
            patch(
                "wfx.components.files_and_knowledge.memory_retrieval.get_user_by_id",
    ...
    async def test_missing_metadata_raises(self):
        ...
        with (
            ...
            patch(
                "wfx.components.files_and_knowledge.memory_retrieval.get_knowledge_bases_root_path",
                return_value=Path(),
            ),
            patch(
                "wfx.components.files_and_knowledge.memory_retrieval.validate_kb_path",
                return_value=None,
            ),
            patch(
                "wfx.components.files_and_knowledge.memory_retrieval.load_kb_metadata",
                return_value={},
            ),
...
class TestMemoryBaseRetrievalBehavior:
    ...
    def _enter_full_chain(stack: contextlib.ExitStack, *, db, fake_chroma, owner, metadata):
        for cm in (
            ...
            patch(
                "wfx.components.files_and_knowledge.memory_retrieval.KBIngestionHelper.build_embeddings",
                new=AsyncMock(return_value=MagicMock()),
            ),
            patch(
                "wfx.components.files_and_knowledge.memory_retrieval.Chroma",
