...
class TestGetVersionInfo:
    ...
    def test_get_version_info_canonical_dev_is_nightly(self, mock_metadata):
        """A canonical ``primeagent`` install at a ``.dev`` version is labeled a nightly.

        The nightly now publishes as ``primeagent==X.Y.Z.devN`` (the canonical name matches first),
