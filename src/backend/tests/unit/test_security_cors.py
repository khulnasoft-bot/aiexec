...
class TestCORSConfiguration:
    ...
    def test_wildcard_with_credentials_allowed_current_behavior(self):
        ...
        with (
            ...
            settings = Settings()
            # pydantic-settings parses PRIMEAGENT_CORS_ORIGINS="*" as the raw string
    ...
    @patch("primeagent.main.add_sentry_middleware")  # Mock Sentry setup
    ...
    def test_cors_origins_contain_wildcard(self, origins, expected):
        ...
        from primeagent.main import cors_origins_contain_wildcard
    ...
    def test_cors_list_wildcard_credentials_disabled_and_warned(
        ...
        This is the gap the string-only check missed: ``PRIMEAGENT_CORS_ORIGINS="https://app.com,*"``
...
class TestRefreshTokenSecurity:
    ...
    async def test_refresh_token_valid_flow(self):
        ...
        # Create a primeagent AuthService instance (not wfx) with mocked settings
        mock_settings_service = MagicMock()
        auth_service = AuthService(mock_settings_service)

        with (
            patch("primeagent.services.auth.utils._auth_service", return_value=auth_service),
            patch("primeagent.services.auth.service.jwt.decode") as mock_decode,
        ):
