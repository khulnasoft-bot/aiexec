...
_SCRIPT_LOCATION = _WORKSPACE_ROOT / "src/backend/base/primeagent/alembic"
...
def _pg_url() -> str | None:
    ...
    url = os.environ.get("PRIMEAGENT_TEST_DATABASE_URI")
    if url is not None:
...
def db_url(request):
    ...
    if request.param == "sqlite":
        ...
        base_url = _pg_url()
        if base_url is None:
            pytest.skip("PRIMEAGENT_TEST_DATABASE_URI not set")
        ...
        try:
            result = subprocess.run(  # noqa: S603
                [
                    ...
                    "origin/main",
                    "--",
                    "src/backend/base/primeagent/alembic/versions/",
                ],
                capture_output=True,
                text=True,
                check=True,
