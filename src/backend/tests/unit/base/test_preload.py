"""Unit tests for primeagent.preload module.
...
import pytest
from primeagent.preload import (
    _STATE,
    PreloadStep,
    _run_master_preload,
    get_owned_temp_dirs,
...
def _preload_env(
    *,
    ...
    with ExitStack() as stack:
        stack.enter_context(patch("primeagent.preload.logger", logger_mock))
        stack.enter_context(
            patch("primeagent.services.utils.initialize_services", new_callable=AsyncMock),
        )
        stack.enter_context(
            patch("primeagent.services.deps.get_settings_service", return_value=settings_service),
        )
        stack.enter_context(
            patch("primeagent.services.deps.get_telemetry_service", return_value=MagicMock()),
        )
        stack.enter_context(patch("primeagent.services.deps.get_db_service", return_value=db_service))
        stack.enter_context(patch("primeagent.services.deps.get_service", return_value=cache_service))
        stack.enter_context(
            patch("primeagent.services.deps.session_scope", return_value=_async_cm(AsyncMock())),
        )
        stack.enter_context(patch("primeagent.initial_setup.setup.copy_profile_pictures", copy_pics))
        stack.enter_context(
            patch("primeagent.initial_setup.setup.create_or_update_starter_projects", create_starter),
        )
        stack.enter_context(
            patch("primeagent.initial_setup.setup.load_flows_from_directory", load_flows),
        )
        stack.enter_context(patch("primeagent.main.load_bundles_with_error_handling", load_bundles))
        stack.enter_context(
            patch("wfx.interface.components.get_and_cache_all_types_dict", get_and_cache),
        )
        stack.enter_context(
            patch(
                "primeagent.api.utils.mcp.agentic_mcp.initialize_agentic_global_variables",
                init_agentic,
            ),
        )
        stack.enter_context(
            patch(
                "primeagent.api.utils.mcp.agentic_mcp.auto_configure_agentic_mcp_server",
                auto_config,
            ),
        )
        ...
        stack.enter_context(patch("wfx.interface.components.component_cache", types_mock))
...
@patch("primeagent.preload.asyncio.run")
...
@patch("primeagent.preload.logger")
@patch("primeagent.preload.asyncio.run")
