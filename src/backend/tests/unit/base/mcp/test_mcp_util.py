...
class TestMCPSessionManager:
    ...
    async def test_concurrent_get_session_same_server_reuses_one_session(self, session_manager):
        ...
        https://github.com/khulnasoft/primeagent/issues/9860 where two MCPTools
...
class TestUpdateToolsPerToolResilience:
    ...
    async def test_one_bad_tool_does_not_drop_the_other_tools_issue_11229(self):
        ...
        from wfx.schema.json_schema import create_input_schema_from_json_schema as real_converter
        ...
        with (
            patch("wfx.base.mcp.util.create_input_schema_from_json_schema", side_effect=selective_converter),
            patch("wfx.base.mcp.util.logger") as mock_logger,
        ):
    ...
    async def test_resilience_under_many_mixed_tools(self):
        ...
        with patch("wfx.base.mcp.util.create_input_schema_from_json_schema", side_effect=selective_converter):
    ...
    def mcp_tool(self, test_schema, mock_client):
        ...
        class MCPStructuredTool(StructuredTool):
            ...
            def _run(self, *args, config: RunnableConfig, run_manager=None, **kwargs):
                from wfx.base.mcp.util import _convert_mcp_result
            ...
            def _convert_parameters(self, input_dict):
                ...
                from wfx.base.agents.utils import maybe_unflatten_dict
...
class TestNormalizeArgumentsForMcp:
    ...
    def test_primeagent_data_to_dict_when_dict_expected(self):
        """Test Primeagent Data/JSON connected to a dict MCP parameter unwraps to its payload."""
        from pydantic import BaseModel, Field
...
class TestStripNoneRecursive:
    ...
    def test_should_remove_none_from_flat_dict(self):
        ...
        from wfx.base.mcp.util import _strip_none_recursive
...
class TestStreamableHttpTransportPolicy:
    ...
    async def test_streamable_success_sse_not_invoked(self):
        ...
        try:
            ...
            with (
                patch("wfx.base.mcp.util.ClientSession", return_value=fake),
    ...
    async def test_streamable_404_triggers_sse(self):
        ...
        try:
            ...
            with (
                patch("wfx.base.mcp.util.ClientSession", side_effect=client_session_factory),
                patch("mcp.client.streamable_http.streamablehttp_client", stream_cm),
