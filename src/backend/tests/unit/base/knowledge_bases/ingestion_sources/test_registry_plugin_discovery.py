...
* Third-party entry-point registration under ``wfx.ingestion_source.adapters``.
* TOML config-file registration under ``[ingestion_source.adapters]`` in
  ``wfx.toml`` (and the ``[tool.wfx.ingestion_source.adapters]`` fallback
  in ``pyproject.toml``).
...
import pytest
from wfx.base.knowledge_bases.ingestion_sources import registry as ingestion_registry
...
from wfx.services.adapters import registry as adapter_registry_mod
...
async def _reset_registry(monkeypatch, tmp_path):
    ...
    ``wfx.base.knowledge_bases.ingestion_sources.__init__``. That
    ...
    import wfx.base.knowledge_bases.ingestion_sources.registry as reg
...
def _reinstall_builtin_sources() -> None:
    ...
    ``wfx.base.knowledge_bases.ingestion_sources.__init__`` at import
    ...
    """
    from wfx.base.knowledge_bases.ingestion_sources.file_upload import FileUploadSource
    from wfx.base.knowledge_bases.ingestion_sources.folder import FolderSource
...
def _make_ensure_discovered(config_dir):
    ...
    from wfx.base.knowledge_bases.ingestion_sources import registry as reg
...
def test_entry_point_registers_plugin_source(monkeypatch):
    ...
    ``wfx.ingestion_source.adapters`` should register lazily when
...
def test_wfx_toml_registers_plugin_source(tmp_path, monkeypatch):
    """``wfx.toml`` entries register with ``override=True``.

    ``[ingestion_source.adapters]`` in ``wfx.toml`` should register the
