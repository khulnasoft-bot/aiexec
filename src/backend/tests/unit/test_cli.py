...
import pytest
import typer
from primeagent.__main__ import (
...
# ---------------------------------------------------------------------------
# Platform routing for `primeagent run` startup.
#
...
def test_clamp_uvicorn_workers_caps_to_one_on_direct_uvicorn(system):
    ...
    # primeagent uses loguru, not stdlib logging — patch the logger method
    # directly rather than relying on caplog.
    with patch("primeagent.__main__.logger") as mock_logger:
        assert clamp_uvicorn_workers(4, system=system) == 1
    mock_logger.warning.assert_called_once()
    # The warning should mention what we clamped and why, so users can
    # diagnose `primeagent run --workers N` regressing to a single worker.
...
def test_ensure_multi_worker_safe_refuses_multiple_workers():
    ...
    with (
        patch("primeagent.__main__.get_settings_service", return_value=_settings_service_with_queue("asyncio")),
...
def test_ensure_multi_worker_safe_error_lists_workarounds():
    ...
    assert "PRIMEAGENT_JOB_QUEUE_TYPE=redis" in msg
...
def test_ensure_multi_worker_safe_allows_redis_queue():
    ...
    with patch("primeagent.__main__.get_settings_service", return_value=_settings_service_with_queue("redis")):
