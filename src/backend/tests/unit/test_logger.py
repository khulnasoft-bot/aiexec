...
class TestConfigure:
    ...
    def test_configure_routes_loguru_messages_to_log_file(self):
        """Test configure() routes Loguru messages through Primeagent logging."""
        with tempfile.TemporaryDirectory() as tmp_dir:
            log_file_path = Path(tmp_dir) / "primeagent.log"
...
class TestProductionObservability:
    ...
    def test_per_logger_level_overrides_via_env(self, monkeypatch):
        monkeypatch.setenv("PRIMEAGENT_LOG_LEVELS", "noisy.lib=WARNING,other=ERROR")
        configure(log_env="container", log_level="DEBUG", cache=False)
    ...
    def test_extra_redact_keys_via_env(self, capsys, monkeypatch):
        monkeypatch.setenv("PRIMEAGENT_LOG_REDACT_KEYS", "session_id,internal_key")
        configure(log_env="container", log_level="DEBUG", cache=False)
    ...
    def test_traceback_locals_enabled_via_opt_in(self, capsys, monkeypatch):
        # PRIMEAGENT_LOG_TRACE_LOCALS=true is the explicit opt-in for local
        # debugging. Verifies the opt-in actually flips the safe default.
        monkeypatch.setenv("PRIMEAGENT_LOG_TRACE_LOCALS", "true")
        configure(log_env="container", log_level="DEBUG", cache=False)
    ...
    def test_intercept_handler_not_installed_in_pretty_mode(self, monkeypatch):
        ...
        monkeypatch.setenv("PRIMEAGENT_PRETTY_LOGS", "true")
    ...
    def test_service_info_from_env_appears_in_records(self, capsys, monkeypatch):
        monkeypatch.setenv("PRIMEAGENT_SERVICE_NAME", "wfx-runner")
        monkeypatch.setenv("PRIMEAGENT_VERSION", "1.2.3")
        monkeypatch.setenv("PRIMEAGENT_ENVIRONMENT", "staging")
        configure(log_env="container", log_level="DEBUG", cache=False)
        log = structlog.get_logger("svc.env")
        records = self._emit_and_parse(capsys, lambda: log.info("hi"))
        rec = records[-1]
        assert rec["service"] == "wfx-runner"
    ...
    def test_malformed_log_levels_emits_warning(self, monkeypatch):
        ...
        monkeypatch.setenv(
            "PRIMEAGENT_LOG_LEVELS",
            "sqlalchemy.engine=WARN,good=INFO,broken,=NOLEVEL,empty=,a=NOTALEVEL",
        )
        with pytest.warns(UserWarning, match="PRIMEAGENT_LOG_LEVELS"):
            configure(log_env="container", log_level="DEBUG", cache=False)
...
class TestFileModeStdlibUnification:
    ...
    Regression coverage for the bug where ``PRIMEAGENT_LOG_ENV=container`` plus
    ``PRIMEAGENT_LOG_FILE`` skipped the stdlib path entirely: uvicorn, sqlalchemy,
    ...
    def test_container_file_mode_stdlib_logs_are_json_with_logger_name(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            assert sa[0]["service"] == "primeagent"
    ...
    def test_container_file_mode_app_logs_still_json_and_redacted(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            structlog.get_logger("primeagent.api").info(
                "incoming",
                api_key="sk-do-not-leak",  # pragma: allowlist secret
            )

            records = self._read_records(log_file_path)
            app = [r for r in records if r.get("logger") == "primeagent.api"]
...
class TestConfigureEarlyReturnFingerprint:
    ...
    def test_same_level_new_output_file_takes_effect(self):
        """Same level, new output_file must reconfigure (the wfx.run.base path).

        wfx.run.base calls configure(log_level=..., output_file=sys.stderr) at
