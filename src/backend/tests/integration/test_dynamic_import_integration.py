...
class TestDynamicImportIntegration:
    ...
    def test_getenvvar_component_removed(self):
        """Test that the removed GetEnvVar component cannot be imported from wfx datastax."""
        import importlib

        import wfx.components.datastax as wfx_datastax

        with pytest.raises(AttributeError):
            _ = wfx_datastax.GetEnvVar

        assert not hasattr(wfx_datastax, "GetEnvVar"), "GetEnvVar should have been removed from wfx.components.datastax"

        with pytest.raises((ImportError, ModuleNotFoundError)):
            importlib.import_module("wfx.components.datastax.getenvvar")

    def test_python_code_structured_tool_removed(self):
        """Test that the removed PythonCodeStructuredTool component cannot be imported from wfx tools.
        ...
        ``wfx.utils.flow_validation.CODE_EXECUTION_COMPONENT_TYPES``) so stored code in
        any saved flow that still references it cannot execute on that path.
        """
        import importlib

        import wfx.components.tools as wfx_tools

        with pytest.raises(AttributeError):
            _ = wfx_tools.PythonCodeStructuredTool

        assert not hasattr(wfx_tools, "PythonCodeStructuredTool"), (
            "PythonCodeStructuredTool should have been removed from wfx.components.tools"
        )

        with pytest.raises((ImportError, ModuleNotFoundError)):
            importlib.import_module("wfx.components.tools.python_code_structured_tool")
