...
from primeagent.services.database.models.vertex_builds.crud import delete_vertex_builds_by_flow_id_unchecked
...
def disable_rate_limiting():
    ...
    os.environ["PRIMEAGENT_RATE_LIMIT_ENABLED"] = "false"
    yield
    os.environ.pop("PRIMEAGENT_RATE_LIMIT_ENABLED", None)
...
def disable_models_dev_refresh():
    ...
    """
    os.environ["PRIMEAGENT_MODELS_DEV_REFRESH"] = "false"
    yield
    os.environ.pop("PRIMEAGENT_MODELS_DEV_REFRESH", None)
...
def pytest_runtest_setup(item):
    ...
    if item.get_closest_marker("api_key_required") and not _has_nonempty_env("OPENAI_API_KEY"):
        pytest.skip("OPENAI_API_KEY is not set or is empty")
...
def disable_telemetry_writer(monkeypatch):
    ...
    monkeypatch.setenv("PRIMEAGENT_TELEMETRY_WRITER_ENABLED", "false")
    yield
    monkeypatch.undo()
