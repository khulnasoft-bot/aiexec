...
export type UtilityStoreType = {
   ...
   * ``settings.enable_extension_reload``) so a ``primeagent run`` started
   * with ``PRIMEAGENT_ENABLE_EXTENSION_RELOAD=true`` (or via ``--env-file``,
   * or by ``wfx extension dev``) lights up the Reload button without a
