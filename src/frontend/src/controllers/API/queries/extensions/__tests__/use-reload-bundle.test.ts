...
describe("useReloadBundle", () => {
  ...
  it("posts to /extensions/{id}/bundles/{name}/reload and returns the body", async () => {
    ...
    await mutation.mutate({ extensionId: "wfx-openai", bundleName: "openai" });

    expect(mockApiPost).toHaveBeenCalledWith(
      "/api/v1/extensions/wfx-openai/bundles/openai/reload",
    );
  ...
  it("rethrows reload-in-progress with a parseable shape", async () => {
    ...
    await expect(
      mutation.mutate({ extensionId: "wfx-openai", bundleName: "openai" }),
