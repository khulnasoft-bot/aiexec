...
test("user should be able to edit name and description of a node", {
...
test("user should be able to edit name and description of a node with inspect panel disabled", {
  tag: ["@release", "@workspace"],
}, async ({ page }) => {
  const randomName = Math.random().toString(36).substring(2, 15);
  const randomDescription = Math.random().toString(36).substring(2, 15);
