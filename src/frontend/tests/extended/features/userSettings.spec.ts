...
test("should see general profile gradient", {
...
test("should interact with global variables", {
  ...
  await page.getByText("Save Variable", { exact: true }).dispatchEvent("click");
  ...
  await page.getByPlaceholder(TEXTS.placeholderVariableName).fill(randomName2);
  ...
  await page.getByPlaceholder(TEXTS.placeholderVariableName).fill(randomName3);
...
test("should interact with API Keys", { tag: ["@release", "@api"] }, async ({
  page,
}) => {
  await awaitBootstrapTest(page, {
    skipModal: true,
...
test("should navigate back to flow from global variables", {
