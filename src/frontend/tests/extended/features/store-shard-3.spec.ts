...
test("should order the visualization (requires store API key)", {
  tag: ["@release"],
}, async ({ page, context }) => {
  test.skip();
  ...
  await newPageStore.getByTestId("sidebar-nav-Primeagent Store").click();
...
test("should filter by type (requires store API key)", {
  tag: ["@release"],
}, async ({ page, context }) => {
  test.skip();
  skipIfMissing.storeApiKey();
