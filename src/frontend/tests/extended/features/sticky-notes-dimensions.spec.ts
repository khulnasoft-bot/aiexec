...
test("sticky notes should have consistent 280x140px dimensions", {
...
test("sticky notes should maintain size with content", {
  tag: ["@release", "@workspace"],
}, async ({ page }) => {
  await openBlankFlow(page);
...
test("sticky notes should have larger readable text", {
  tag: ["@release", "@workspace"],
}, async ({ page }) => {
  await openBlankFlow(page);
...
test("sticky notes should handle overflow with scrollbars", {
  tag: ["@release", "@workspace"],
}, async ({ page }) => {
  await openBlankFlow(page);
  ...
  expect(["auto", "scroll", "hidden"].includes(contentOverflow.overflowY)).toBe(
    true,
  );
...
test("sticky notes should respect resize constraints", {
  tag: ["@release", "@workspace"],
}, async ({ page }) => {
  await openBlankFlow(page);
