...
test.describe("Output Modal Copy Button", () => {
  test("user should be able to copy text output from component output modal", {
    ...
    await expect(copyButton.locator('[data-testid="icon-Copy"]')).toBeVisible();
  });

  test("copy button should work with JSON output from API Request component", {
    tag: ["@release", "@workspace"],
  }, async ({ page }) => {
    await openBlankFlow(page);
