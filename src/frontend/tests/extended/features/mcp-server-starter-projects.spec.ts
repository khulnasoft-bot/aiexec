...
test("user must be able to see starter projects for mcp servers", {
  ...
  expect(await page.getByText("lf-new_project", { exact: true }).count()).toBe(
    1,
  );
  expect(
...
test("user must not be able to add duplicate mcp servers from starter projects", {
  tag: ["@release", "@workspace", "@components"],
}, async ({ page }) => {
  await awaitBootstrapTest(page);
  ...
  const numberOfErrors = await page.getByText("Server already exists.").count();
