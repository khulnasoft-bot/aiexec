...
test("should navigate to files page and show empty state", {
...
test("should upload file using upload button", {
  tag: ["@release", "@components"],
}, async ({ page }) => {
...
test("should upload file using drag and drop", {
  ...
  // Trigger drag events
  await page.dispatchEvent('[data-testid="drag-wrap-component"]', "dragover", {
    dataTransfer,
  });
...
test("should upload multiple files with different types", {
  tag: ["@release", "@components"],
}, async ({ page }) => {
  const fileNames = {
...
test("should search uploaded files", {
  tag: ["@release", "@components"],
}, async ({ page }) => {
  const fileNames = {
    txt: generateRandomFilename(),
  ...
  expect(await page.getByText(fileNames.txt + ".txt").isVisible()).toBeFalsy();
...
test("should handle bulk actions for multiple files", {
  tag: ["@release", "@components"],
}, async ({ page }) => {
  const fileNames = {
    txt: generateRandomFilename(),
