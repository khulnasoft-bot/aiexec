...
test("STDIO MCP server fields should persist after saving and editing", {
  ...
  expect(await page.getByTestId("stdio-env-value-0").last().inputValue()).toBe(
    testEnvValue1,
  );
  expect(await page.getByTestId("stdio-env-key-1").last().inputValue()).toBe(
    testEnvKey2,
  );
  expect(await page.getByTestId("stdio-env-value-1").last().inputValue()).toBe(
    testEnvValue2,
  );
...
test("HTTP/SSE MCP server fields should persist after saving and editing", {
  ...
  // Verify all fields persisted correctly
  expect(await page.getByTestId("http-name-input").inputValue()).toBe(testName);
