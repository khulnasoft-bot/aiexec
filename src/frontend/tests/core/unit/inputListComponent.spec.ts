...
test("InputListComponent", { tag: ["@release", "@workspace"] }, async ({
  page,
}) => {
  ...
  expect(await page.getByTestId("inputlist_str_edit_urls_0").inputValue()).toBe(
    "test1 test1 test1 test1",
  );

  expect(await page.getByTestId("inputlist_str_edit_urls_1").inputValue()).toBe(
    "",
  );
