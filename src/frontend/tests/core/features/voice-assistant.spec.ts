...
test.skip("should able to see and interact with voice assistant", {
...
test.skip("user should not be able to see voice button if voice mode is not available", {
  tag: ["@release", "@workspace", "@api"],
}, async ({ page, request }) => {
  await page.route("**/api/v1/config", (route) => {
    route.fulfill({
...
test.skip("user should be able to see voice button if voice mode is available", {
  tag: ["@release", "@workspace", "@api"],
}, async ({ page, request }) => {
  await page.route("**/api/v1/config", (route) => {
    route.fulfill({
