...
test.skip(
  process.env.PRIMEAGENT_FEATURE_WXO_DEPLOYMENTS !== "true",
  "Requires PRIMEAGENT_FEATURE_WXO_DEPLOYMENTS=true",
);
...
test("Renders Deployments tab with sub-tab toggles", {
...
test("Deployments empty state shows create button when no providers exist", {
  tag: ["@release", "@workspace", "@api"],
}, async ({ page }) => {
  await page.route("**/api/v1/deployments/providers*", (route) => {
    route.fulfill({
...
test("Providers empty state shows add provider button when switching to providers tab", {
  tag: ["@release", "@workspace", "@api"],
}, async ({ page }) => {
  await page.route("**/api/v1/deployments/providers*", (route) => {
    route.fulfill({
...
test("Deployment list renders a row for each deployment", {
  tag: ["@release", "@workspace", "@api"],
}, async ({ page }) => {
  await page.route("**/api/v1/deployments*", (route) => {
    route.fulfill({
...
test("Provider list renders a row for each provider", {
  tag: ["@release", "@workspace", "@api"],
}, async ({ page }) => {
  await page.route("**/api/v1/deployments*", (route) => {
    route.fulfill({
...
test("Delete deployment opens type-to-confirm dialog", {
  tag: ["@release", "@workspace", "@api"],
}, async ({ page }) => {
  await page.route("**/api/v1/deployments/providers*", (route) => {
    route.fulfill({
  ...
  await expect(page.getByTestId("input-type-to-confirm-delete")).toBeVisible();
});

test("Delete deployment button is disabled until deployment name is typed", {
  tag: ["@release", "@workspace", "@api"],
}, async ({ page }) => {
  await page.route("**/api/v1/deployments/providers*", (route) => {
    route.fulfill({
  ...
  await page.getByTestId("input-type-to-confirm-delete").fill(DEPLOYMENT.name);
  await expect(deleteBtn).toBeEnabled();
});

test("Delete deployment — typing correct name and confirming calls DELETE", {
  tag: ["@release", "@workspace", "@api"],
}, async ({ page }) => {
  await page.route("**/api/v1/deployments/providers*", (route) => {
    route.fulfill({
...
test("Cancel delete deployment dismisses dialog without calling DELETE", {
  tag: ["@release", "@workspace", "@api"],
}, async ({ page }) => {
...
test("Names filter is passed to API when fetching deployments", {
  tag: ["@release", "@workspace", "@api"],
}, async ({ page }) => {
