...
// ---------------------------------------------------------------------------
test("deployment-create: review step does not block deploy when provider has matching tool names", {
  tag: ["@deployment", "@workspace"],
}, async ({ page }) => {
...
// ---------------------------------------------------------------------------
test("deployment-create: editing tool display name on review with provider duplicates", {
  tag: ["@deployment", "@workspace"],
}, async ({ page }) => {
  await openDeploymentStepper(page, SNAPSHOTS_DUPLICATE_MOCK);
  await goToStepReview(page);
...
test("deployment-create: review step allows numeric flow names as tool display names", {
  tag: ["@deployment", "@workspace"],
}, async ({ page }) => {
  await openDeploymentStepper(page, SNAPSHOTS_EMPTY_MOCK, [
