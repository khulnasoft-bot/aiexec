...
test("shareable playground: bot messages display token usage", {
...
test("regular playground: Finished In with token display still works (regression)", {
  tag: ["@release", "@workspace", "@api"],
}, async ({ page }) => {
  skipIfMissing.openAiKey();
  loadDotenvIfLocal(__dirname);
  ...
  await page.waitForFunction(
    () =>
      document.querySelectorAll('[data-testid="div-chat-message"]').length >= 2,
