...
test("Empty state shows Add Environment button", {
...
test("New Environment button opens modal with save disabled on empty form", {
  tag: ["@release", "@workspace", "@api"],
}, async ({ page }) => {
  await page.route("**/api/v1/deployments/providers*", (route) => {
    route.fulfill({
...
test("Save button becomes enabled after filling all required fields", {
  tag: ["@release", "@workspace", "@api"],
}, async ({ page }) => {
  await page.route("**/api/v1/deployments/providers*", (route) => {
    route.fulfill({
...
test("Save calls POST and modal closes", {
  tag: ["@release", "@workspace", "@api"],
}, async ({ page }) => {
  ...
  await expect(page.getByTestId("add-provider-modal-title")).not.toBeVisible();
});

test("Delete triggers confirmation dialog then calls DELETE", {
  tag: ["@release", "@workspace", "@api"],
}, async ({ page }) => {
  await page.route("**/api/v1/deployments*", (route) => {
    route.fulfill({
...
test("Cancel delete dismisses confirmation without calling DELETE", {
  tag: ["@release", "@workspace", "@api"],
}, async ({ page }) => {
