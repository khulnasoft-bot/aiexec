...
test("deleting a folder should update the folder list immediately", {
...
test("deleting one folder should not affect other folders", {
  tag: ["@release", "@api", "@folder"],
}, async ({ page }) => {
  await awaitBootstrapTest(page);
...
test("creating a new folder after deletion should work correctly", {
  tag: ["@release", "@api", "@folder"],
}, async ({ page }) => {
  await awaitBootstrapTest(page);
...
test("creating a flow after deleting all folders should create a default folder", {
  tag: ["@release", "@api", "@folder"],
}, async ({ page }) => {
