...
test("manual scenarios A–G prerequisite — Agent exposes structured_response output handle alongside response", {
...
test("manual scenarios A/D/E/F/G setup — inspection panel exposes Output Schema and Output Format Instructions", {
  tag: ["@release", "@workspace", "@components"],
}, async ({ page }) => {
  test.skip(
    !process?.env?.OPENAI_API_KEY,
  ...
  await expect(page.locator('//*[@id="showformat_instructions"]')).toBeVisible({
    timeout: 10000,
  });
});

test("scenarios A–C wiring — switching the agent dropdown to Structured Response keeps the downstream edge", {
  tag: ["@release", "@workspace", "@components"],
}, async ({ page }) => {
  test.skip(
    !process?.env?.OPENAI_API_KEY,
