...
test("Test button opens modal with chat interface", {
...
test("Send message calls POST run with correct body", {
  tag: ["@release", "@workspace", "@api"],
}, async ({ page }) => {
...
test("Response appears in chat after polling completes", {
  tag: ["@release", "@workspace", "@api"],
}, async ({ page }) => {
  let runCallCount = 0;
...
test("Input is disabled during polling and re-enabled after response", {
  tag: ["@release", "@workspace", "@api"],
}, async ({ page }) => {
  let runCallCount = 0;
...
test("Multi-turn: second message includes thread_id from first response", {
  tag: ["@release", "@workspace", "@api"],
}, async ({ page }) => {
  let runCallCount = 0;
...
test("Close and reopen modal resets chat history", {
  tag: ["@release", "@workspace", "@api"],
}, async ({ page }) => {
  let runCallCount = 0;
