...
// ---------------------------------------------------------------------------
test("Agent component shows the 7-section default template when dropped on canvas", {
...
// ---------------------------------------------------------------------------
test("Tool Calling Agent component shows the 7-section default template when dropped on canvas", {
  tag: ["@release", "@workspace", "@agents"],
}, async ({ page }) => {
  await awaitBootstrapTest(page);
...
// ---------------------------------------------------------------------------
test("runtime substitution injects today's date into the agent's effective prompt", {
...
// ---------------------------------------------------------------------------
test("custom prompt without placeholders passes through untouched", {
  tag: ["@release", "@agents", "@api"],
}, async ({ page }) => {
  test.skip(
    !process?.env?.OPENAI_API_KEY,
  ...
  expect(reply).not.toContain("primeagent agent");
});
...
// ---------------------------------------------------------------------------
test("custom prompt with placeholder opts in to runtime substitution", {
  tag: ["@release", "@agents", "@api"],
}, async ({ page }) => {
  test.skip(
    !process?.env?.OPENAI_API_KEY,
