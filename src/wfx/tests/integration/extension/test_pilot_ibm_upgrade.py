...
that ship in ``wfx-ibm``: ``DB2VectorStoreComponent``,
...
the three IBM components from the pre-extraction Primeagent:
    ...
    3. The wfx-ibm distribution is importable AND ships the manifest in
...
def test_short_import_path_flow_upgrades(migration_table, class_name, _legacy_module, expected_target) -> None:
    ...
    flow = _saved_flow(_saved_flow_node(f"ibm-short-{class_name}", f"wfx.components.ibm.{class_name}"))
...
@pytest.mark.integration
def test_wfx_ibm_distribution_is_importable() -> None:
    ...
    (wfx's own venv does not list wfx-ibm as a dep); the primeagent
    ...
    try:
        ...
        pytest.skip("wfx-ibm not installed in this test environment")
...
@pytest.mark.integration
def test_wfx_ibm_ships_manifest() -> None:
    ...
    """
    try:
        dist = importlib_metadata.distribution("wfx-ibm")
    ...
    if _is_editable_install(dist):
        ...
        package_dir = Path(wfx_ibm.__file__).parent
        manifest_path = package_dir / "extension.json"
        assert manifest_path.is_file(), (
            f"wfx-ibm source tree at {package_dir} does not ship "
        ...
        assert manifests, (
            "wfx-ibm distribution does not ship extension.json in its "
