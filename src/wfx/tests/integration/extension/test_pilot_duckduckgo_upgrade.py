...
``DuckDuckGoSearchComponent`` from the pre-extraction Primeagent:

    1. A saved flow uses the legacy bare class name ``DuckDuckGoSearchComponent``
       (the form Primeagent serialized for years before the bundle move).
    ...
    4. The wfx-duckduckgo distribution is importable and ships the manifest
       in a location ``importlib.metadata.files`` can discover.

This covers the *deserialize-side* half of the M1 proof gate plus the
build-pipeline runtime contract: a saved flow from pre-migration Primeagent
...
swap: standing up a pre-migration Primeagent release, saving a flow,
...
def test_legacy_bare_name_flow_upgrades(migration_table) -> None:
    ...
    Primeagent used for years before the bundle move) must rewrite to
...
def test_legacy_import_path_flow_upgrades(migration_table) -> None:
    ...
    Some Primeagent versions serialized the full module path instead of the
...
def test_short_import_path_flow_upgrades(migration_table) -> None:
    ...
    Catches flows that referenced ``wfx.components.duckduckgo.DuckDuckGoSearchComponent``
    via the package-level re-export rather than the file-level dotted path.
    """
    from wfx.extension.migration.rewrite import migrate_flow_payload

    flow = _saved_flow(_saved_flow_node("ddg-3", "wfx.components.duckduckgo.DuckDuckGoSearchComponent"))
...
@pytest.mark.integration
def test_wfx_duckduckgo_distribution_is_importable() -> None:
    ...
    (wfx's own venv does not list wfx-duckduckgo as a dep); the primeagent
    workspace venv pulls it in transitively from primeagent's pyproject.
    """
    try:
        from wfx_duckduckgo import DuckDuckGoSearchComponent
    except ImportError:
        pytest.skip("wfx-duckduckgo not installed in this test environment")
...
def test_pilot_loads_and_resolves_to_runtime_class() -> None:
    ...
    resolves to a Component class that:
        ...
        2. Comes from the bundle's own source file (not a shadow copy
           ...
           Object identity vs. ``from wfx_duckduckgo import ...`` does
           NOT hold -- the loader stages bundle modules under
           ``_wfx_ext.<slot>.<bundle>`` so the registered class is a
    ...
    """
    try:
        import wfx_duckduckgo
    except ImportError:
        pytest.skip("wfx-duckduckgo not installed in this test environment")

    from wfx.extension import SLOT_OFFICIAL, load_extension
    from wfx_duckduckgo import DuckDuckGoSearchComponent as ExpectedClass
    ...
    package_dir = Path(wfx_duckduckgo.__file__).parent
    bundle_root = package_dir

    result = load_extension(bundle_root, slot=SLOT_OFFICIAL, distribution="wfx-duckduckgo")
    ...
    # modules under its own ``_wfx_ext.<slot>.<bundle>`` namespace, so the
    # class object the registry holds is a different ``type`` instance
    # than ``from wfx_duckduckgo import ...`` resolves to -- even though
...
def test_pilot_build_pipeline_runs_against_stub_wrapper() -> None:
    ...
    package_dir = Path(wfx_duckduckgo.__file__).parent
    result = load_extension(package_dir, slot=SLOT_OFFICIAL, distribution="wfx-duckduckgo")
    assert result.ok, [e.code for e in result.errors]
    ...
    # because the loaded class (built under ``_wfx_ext.<slot>.<bundle>``)
...
@pytest.mark.integration
def test_wfx_duckduckgo_ships_manifest() -> None:
    ...
    """
    try:
        dist = importlib_metadata.distribution("wfx-duckduckgo")
    ...
    if _is_editable_install(dist):
        ...
        assert manifest_path.is_file(), (
            f"wfx-duckduckgo source tree at {package_dir} does not ship "
        ...
        assert manifests, (
            "wfx-duckduckgo distribution does not ship extension.json in its "
    ...
    assert manifest["id"] == "wfx-duckduckgo"
