...
``pip install`` three local bundles and verify ``wfx extension list``
...
def test_pip_install_three_bundles_visible_at_official(tmp_path: Path) -> None:
    ...
    project_specs = [
        ("wfx-test-openai", "wfx_test_openai", "wfx-test-openai", "openai", "1.0.0"),
        ("wfx-test-anthropic", "wfx_test_anthropic", "wfx-test-anthropic", "anthropic", "0.4.1"),
        ("wfx-test-qdrant", "wfx_test_qdrant", "wfx-test-qdrant", "qdrant", "2.5.0"),
    ]
    ...
    assert ids == ["wfx-test-anthropic", "wfx-test-openai", "wfx-test-qdrant"]
...
def test_pip_install_seed_directory_three_bundles(tmp_path: Path) -> None:
    ...
    for bundle in ("openai", "anthropic", "qdrant"):
        sub = seed / f"wfx_test_{bundle}"
        sub.mkdir()
        (sub / bundle).mkdir()
        (sub / "extension.json").write_text(
            _EXTENSION_JSON_TEMPLATE.format(
                extension_id=f"wfx-test-{bundle}",
                version="1.0.0",
                name=f"wfx-test-{bundle}",
                bundle_name=bundle,
            ),
            encoding="utf-8",
        )

    from wfx.extension import discover_seed_extensions
