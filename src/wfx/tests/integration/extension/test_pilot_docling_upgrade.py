"""Integration tests for migrating in-tree Docling components to wfx-docling."""
...
def test_legacy_import_path_flow_upgrades(migration_table, class_name: str, module_name: str) -> None:
    ...
    target = f"ext:docling:{class_name}@official"
    flow = _saved_flow(f"wfx.components.docling.{module_name}.{class_name}")
...
def test_short_import_path_flow_upgrades(migration_table, class_name: str, _module_name: str) -> None:
    ...
    target = f"ext:docling:{class_name}@official"
    flow = _saved_flow(f"wfx.components.docling.{class_name}")
...
@pytest.mark.integration
def test_wfx_docling_distribution_is_importable() -> None:
    try:
        import wfx_docling
    except ImportError:
        pytest.skip("wfx-docling not installed in this test environment")

    for class_name, _module_name in COMPONENTS:
        assert getattr(wfx_docling, class_name).__name__ == class_name
...
def test_bundle_loads_and_resolves_runtime_classes() -> None:
    ...
    result = load_extension(Path(wfx_docling.__file__).parent, slot=SLOT_OFFICIAL, distribution="wfx-docling")
