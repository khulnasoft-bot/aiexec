...
from wfx.run.base import RunError, _materialize_flow_dict, output_error, run_flow
...
class TestRunFlowJsonInput:
    ...
    async def test_valid_json_creates_temp_file_and_loads_graph(self):
        ...
        with (
            patch("wfx.load.aload_flow_from_json") as mock_load,
            patch("wfx.run.base.validate_global_variables_for_env") as mock_validate,
            patch("wfx.run.base.extract_structured_result") as mock_extract,
        ):
...
class TestRunFlowSessionId:
    ...
    async def test_session_id_autogenerated_when_not_provided(self, tmp_path):
        ...
        with (
            patch("wfx.run.base.find_graph_variable") as mock_find,
            patch("wfx.run.base.load_graph_from_script") as mock_load,
            patch("wfx.run.base.validate_global_variables_for_env") as mock_validate,
            patch("wfx.run.base.extract_structured_result") as mock_extract,
        ):
...
class TestRunFlowSessionIdPropagation:
    """Session_id must reach Memory/MessageHistory components on the wfx run path.

    wfx run uses ``graph.async_start`` (not ``graph.arun``), so it bypasses the
    ...
    ``primeagent/api/utils/flow_utils.build_graph_from_data`` does for the playground.
    """
    ...
    def _patches():
        return (
            patch("wfx.run.base.find_graph_variable"),
            patch("wfx.run.base.load_graph_from_script"),
            patch("wfx.run.base.validate_global_variables_for_env"),
            patch("wfx.run.base.extract_structured_result"),
        )
    ...
    async def test_session_id_does_not_overwrite_hardcoded_vertex_value(self, tmp_path):
        ...
        Matches Primeagent's playground behavior: ``build_graph_from_data`` only writes
...
class TestRunFlowUserId:
    """user_id auto-generation on the wfx run path.
    ...
    wfx run has no notion of authenticated users, but the precheck still has to
    ...
    for variable scoping in wfx (env vars are process-global) but exists to keep
...
class TestRunFlowFallbackToEnvVars:
    ...
    Without this, a primeagent ``DatabaseVariableService`` registered alongside
    ``database_service`` would raise ``variable not found`` for any
    ``load_from_db=True`` field whose user_id has no Variable row (e.g., the
    ceremonial UUID wfx auto-generates). The flag tells
    ``loading.update_params_with_load_from_db_fields`` to fall back to
    ``os.environ`` when the DB lookup misses — same behavior as the primeagent
    ...
    async def test_respects_settings_when_disabled(self, tmp_path):
        """When PRIMEAGENT_FALLBACK_TO_ENV_VAR=false, the flag plumbs through as False."""
        script_path = tmp_path / "test.py"
        script_path.write_text("graph = None")
        captured: dict = {}
        mock_graph = self._mock_graph_capturing_kwargs(captured)
        ...
        with (
            ...
            patch("wfx.run.base.validate_global_variables_for_env") as mock_validate,
            patch("wfx.run.base.extract_structured_result") as mock_extract,
            patch("wfx.run._defaults.get_settings_service", return_value=mock_settings),
        ):
            mock_find.return_value = {"line_number": 1, "type": "Graph", "source_line": "graph = Graph(...)"}
            mock_load.return_value = mock_graph
            mock_validate.return_value = []
...
class TestRunFlowOutputFormats:
    ...
    async def test_result_format_extracts_text(self, tmp_path, mock_successful_execution):
        ...
        with (
            ...
            patch("wfx.run.base.load_graph_from_script") as mock_load,
            patch("wfx.run.base.validate_global_variables_for_env") as mock_validate,
            patch("wfx.run.base.extract_text_from_result") as mock_extract_text,
        ):
            mock_find.return_value = {"line_number": 1, "type": "Graph", "source_line": "graph = ..."}
            mock_load.return_value = mock_successful_execution
            mock_validate.return_value = []
...
class TestRunFlowVerbosity:
    ...
    async def test_verbose_false_configures_critical_logging(self, tmp_path):
        ...
        log_module = sys.modules["wfx.log.logger"]
...
class TestRunFlowInputValueHandling:
    ...
    async def test_input_value_takes_precedence(self, tmp_path):
        ...
        with (
            ...
            patch("wfx.run.base.validate_global_variables_for_env") as mock_validate,
            patch("wfx.run.base.extract_structured_result") as mock_extract,
            patch("wfx.run.base.InputValueRequest") as mock_input_request,
        ):
            mock_find.return_value = {"line_number": 1, "type": "Graph", "source_line": "graph = ..."}
            mock_load.return_value = mock_graph
            mock_validate.return_value = []
...
class TestRunFlowJsonFileExecution:
    ...
    async def test_json_file_calls_aload_flow_from_json(self, simple_json_flow):
        ...
        with (
            patch("wfx.load.aload_flow_from_json") as mock_load_json,
            patch("wfx.run.base.validate_global_variables_for_env") as mock_validate,
            patch("wfx.run.base.extract_structured_result") as mock_extract,
        ):
...
from wfx.template.field.base import Output, Input
...
class TestMaterializeFlowDict:
    ...
    Exported Primeagent flows look like ``{"name": ..., "data": {<graph>}}``; the inner graph is
...
class TestUpgradeFlowOption:
    ...
    async def test_upgrade_flow_check_rejects_outdated_inline_json(self):
        ...
        with (
            patch("wfx.upgrade.cli_gate._load_bundled_registry", return_value=self._registry()),
