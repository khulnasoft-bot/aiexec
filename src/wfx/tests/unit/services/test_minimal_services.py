"""Tests for minimal service implementations in WFX."""
...
class TestTelemetryService:
    ...
    async def test_enqueue_skipped_when_do_not_track(self, telemetry):
        ...
        from wfx.services.telemetry.schema import MCPToolPayload
...
class TestVariableService:
    ...
    async def test_get_variable_absorbs_extra_kwargs(self, variables):
        """Extra kwargs are accepted for compatibility with primeagent call signature."""
        os.environ["WFX_KWARG_TEST"] = "value-from-env"
        try:
            value = await variables.get_variable(
                user_id="random-uuid", name="WFX_KWARG_TEST", field="value", session=None
            )
            assert value == "value-from-env"
        finally:
            del os.environ["WFX_KWARG_TEST"]
    ...
    @pytest.mark.asyncio
    async def test_get_variable_from_primeagent_request_variables(self, variables):
        """Test request-scoped variables are read from PRIMEAGENT_REQUEST_VARIABLES."""
        os.environ["PRIMEAGENT_REQUEST_VARIABLES"] = '{"runtime_token":"abc123","normal_key":"value1"}'
        try:
            assert await variables.get_variable("runtime_token") == "abc123"
            assert await variables.get_variable("normal_key") == "value1"
        finally:
            del os.environ["PRIMEAGENT_REQUEST_VARIABLES"]

    async def test_global_alias_from_request_scope(self, variables):
        """x-primeagent-global-var-* aliases resolve from request-scoped variables."""
        from wfx.services.variable.request_scope import activate_request_variables, reset_request_variables

        token = activate_request_variables({"x-primeagent-global-var-access-token": "alias-token"})
        try:
    ...
    async def test_request_scope_alias_overrides_env(self, variables):
        """A request-scoped x-primeagent-global-var-* alias wins over an env var of the same name.
    ...
    @pytest.mark.asyncio
    async def test_primeagent_request_variables_invalid_json_falls_back(self, variables):
        """Test invalid request variable JSON does not break env fallback."""
        os.environ["PRIMEAGENT_REQUEST_VARIABLES"] = "{not-json"
    ...
    async def test_strict_no_bearer_token_from_request_access_token(self, variables):
        ...
        os.environ["PRIMEAGENT_REQUEST_VARIABLES"] = '{"wxo_github_access_token":"request-token"}'
        try:
    ...
    async def test_strict_no_token_access_token_alias_resolution(self, variables):
        ...
        os.environ["PRIMEAGENT_REQUEST_VARIABLES"] = '{"access_token":"request-access"}'
        try:
    ...
    async def test_strict_no_prefixed_token_alias_resolution(self, variables):
        ...
        os.environ["PRIMEAGENT_REQUEST_VARIABLES"] = '{"my_app_access_token":"prefixed-token"}'
        try:
    ...
    async def test_global_var_alias_resolution(self, variables):
        """Test x-primeagent-global-var-* alias lookup."""
        os.environ["x-primeagent-global-var-access-token"] = "global-token"  # noqa: SIM112
        try:
            assert await variables.get_variable("access_token") == "global-token"
            assert await variables.get_variable("token") is None
        finally:
            del os.environ["x-primeagent-global-var-access-token"]  # noqa: SIM112
