"""Isolation guarantees for request-scoped variables (wfx serve concurrency).

These tests verify the two properties that matter for running multiple flows in a
single ``wfx serve`` process:
...
1. **Per-request isolation** - one request's ``global_vars`` must never be visible
   ...
   :mod:`wfx.services.variable.request_scope`: each asyncio task gets its own
...
async def test_scoped_value_used_without_consulting_env_for_that_name():
    ...
    with (
        ...
        patch("wfx.services.variable.service.os.getenv") as mock_getenv,
    ):
        try:
...
async def test_no_env_fallback_suppresses_primeagent_request_variables_env():
    """Env fallback disabled + no active scope must not leak the process PRIMEAGENT_REQUEST_VARIABLES blob.

    Regression: _get_request_variables() read os.getenv("PRIMEAGENT_REQUEST_VARIABLES")
    ...
    flag_token = activate_no_env_fallback(disabled=True)
    with patch.dict(os.environ, {"PRIMEAGENT_REQUEST_VARIABLES": '{"leaked_token": "SHOULD-NOT-LEAK"}'}):
        try:
...
async def test_primeagent_request_variables_env_blob_used_when_fallback_enabled():
    ...
    with patch.dict(os.environ, {"PRIMEAGENT_REQUEST_VARIABLES": blob}):
        try:
...
async def test_no_env_fallback_flag_is_isolated_per_request():
    ...
    async def strict_request() -> None:
        # Active (non-None) scope so resolution never reads PRIMEAGENT_REQUEST_VARIABLES.
