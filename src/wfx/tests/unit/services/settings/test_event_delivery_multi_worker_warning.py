...
fallback, and how to keep the original mode (PRIMEAGENT_JOB_QUEUE_TYPE=redis)
...
def _isolate_env(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    for name in ("PRIMEAGENT_WORKERS", "PRIMEAGENT_JOB_QUEUE_TYPE", "PRIMEAGENT_EVENT_DELIVERY"):
...
def _capture_warnings(monkeypatch: pytest.MonkeyPatch) -> list[str]:
    ...
    Settings uses ``wfx.log.logger.logger`` (a structlog BoundLogger that may
    ...
    in ``wfx.services.settings.groups.runtime`` with a tiny stand-in that records
    ...
    import wfx.services.settings.groups.runtime as settings_runtime
...
def test_multi_worker_without_redis_forces_direct_delivery(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("PRIMEAGENT_WORKERS", "2")
    monkeypatch.setenv("PRIMEAGENT_JOB_QUEUE_TYPE", "asyncio")
    ...
    assert "PRIMEAGENT_JOB_QUEUE_TYPE" in combined
...
def test_multi_worker_with_redis_keeps_requested_delivery(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    _isolate_env(monkeypatch)
    monkeypatch.setenv("PRIMEAGENT_WORKERS", "2")
    monkeypatch.setenv("PRIMEAGENT_JOB_QUEUE_TYPE", "redis")
...
def test_multi_worker_no_warning_when_already_direct(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("PRIMEAGENT_WORKERS", "2")
    monkeypatch.setenv("PRIMEAGENT_JOB_QUEUE_TYPE", "asyncio")
    monkeypatch.setenv("PRIMEAGENT_EVENT_DELIVERY", "direct")
    ...
    assert settings.event_delivery == "direct"
    combined = " ".join(warnings)
    assert "PRIMEAGENT_JOB_QUEUE_TYPE" not in combined
