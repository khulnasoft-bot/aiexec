...
def _clear_env(monkeypatch):
    for var in (
        "PRIMEAGENT_ALLOW_CUSTOM_COMPONENTS",
        "PRIMEAGENT_ALLOW_COMPONENTS_PATHS_OVERRIDE",
        "PRIMEAGENT_COMPONENTS_PATH",
        "PRIMEAGENT_COMPONENTS_INDEX_PATH",
    ):
...
def test_env_paths_bypass_when_override_true(monkeypatch):
    ...
    with tempfile.TemporaryDirectory() as tmp:
        monkeypatch.setenv("PRIMEAGENT_ALLOW_CUSTOM_COMPONENTS", "false")
        monkeypatch.setenv("PRIMEAGENT_ALLOW_COMPONENTS_PATHS_OVERRIDE", "true")
        monkeypatch.setenv("PRIMEAGENT_COMPONENTS_PATH", tmp)
        monkeypatch.setenv("PRIMEAGENT_COMPONENTS_INDEX_PATH", "/nonexistent/index.json")
...
def test_env_paths_stripped_when_override_false(monkeypatch):
    ...
    with tempfile.TemporaryDirectory() as tmp:
        monkeypatch.setenv("PRIMEAGENT_ALLOW_CUSTOM_COMPONENTS", "false")
        monkeypatch.setenv("PRIMEAGENT_ALLOW_COMPONENTS_PATHS_OVERRIDE", "false")
        monkeypatch.setenv("PRIMEAGENT_COMPONENTS_PATH", tmp)
        monkeypatch.setenv("PRIMEAGENT_COMPONENTS_INDEX_PATH", "/nonexistent/index.json")
...
def test_override_false_has_no_effect_when_custom_allowed(monkeypatch):
    ...
    _clear_env(monkeypatch)
    with tempfile.TemporaryDirectory() as tmp:
        monkeypatch.setenv("PRIMEAGENT_ALLOW_CUSTOM_COMPONENTS", "true")
        monkeypatch.setenv("PRIMEAGENT_ALLOW_COMPONENTS_PATHS_OVERRIDE", "false")
        monkeypatch.setenv("PRIMEAGENT_COMPONENTS_PATH", tmp)
        monkeypatch.setenv("PRIMEAGENT_COMPONENTS_INDEX_PATH", "/nonexistent/index.json")
...
def test_index_path_preserved_when_env_var_unset(monkeypatch):
    """The index path is only stripped when it came from PRIMEAGENT_COMPONENTS_INDEX_PATH.
...
def test_multi_path_env_var_stripped_when_override_false(monkeypatch):
    """Comma-separated PRIMEAGENT_COMPONENTS_PATH entries are each stripped when override is off."""
    _clear_env(monkeypatch)
    with tempfile.TemporaryDirectory() as tmp1, tempfile.TemporaryDirectory() as tmp2:
        monkeypatch.setenv("PRIMEAGENT_ALLOW_CUSTOM_COMPONENTS", "false")
        monkeypatch.setenv("PRIMEAGENT_ALLOW_COMPONENTS_PATHS_OVERRIDE", "false")
        monkeypatch.setenv("PRIMEAGENT_COMPONENTS_PATH", f"{tmp1},{tmp2}")
