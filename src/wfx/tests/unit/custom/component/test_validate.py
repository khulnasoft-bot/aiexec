...
class TestGetModuleFallbacks:
    ...
    def test_primeagent_falls_back_to_wfx(self):
        result = _get_module_fallbacks("primeagent.custom")
        assert result == ["primeagent.custom", "wfx.custom"]

    def test_primeagent_deep_path(self):
        result = _get_module_fallbacks("primeagent.custom.validate")
        assert result == ["primeagent.custom.validate", "wfx.custom.validate"]
    ...
    def test_bare_primeagent_no_fallback(self):
        assert _get_module_fallbacks("primeagent") == ["primeagent"]
    ...
    def test_original_always_first(self):
        ...
        for name in ["primeagent.custom", "langchain.agents", "requests"]:
