"""Unit tests for wfx.testing — the flow_runner pytest plugin.

All tests mock ``_run_sync`` / ``_run_async`` so no real Primeagent instance is needed.
"""
...
class TestLocalFlowRunner:
    def test_success_result(self):
        with patch("wfx.testing.runners._run_sync", return_value=_SUCCESS_RAW) as mock_run:
    ...
    def test_error_result(self):
        with patch("wfx.testing.runners._run_sync", return_value=_ERROR_RAW):
            runner = LocalFlowRunner()
    ...
    def test_env_file_loaded(self, tmp_path):
        ...
        with (
            patch("wfx.testing.runners._run_sync", return_value=_SUCCESS_RAW),
            patch("wfx.testing.runners._load_dotenv") as mock_load,
        ):
...
class TestAsyncLocalFlowRunner:
    async def test_success_result(self):
        with patch("wfx.testing.runners._run_async", new_callable=AsyncMock, return_value=_SUCCESS_RAW):
    ...
    async def test_error_result(self):
        with patch("wfx.testing.runners._run_async", new_callable=AsyncMock, return_value=_ERROR_RAW):
            runner = AsyncLocalFlowRunner()
    ...
    async def test_default_timeout_used(self):
        with patch("wfx.testing.runners._run_async", new_callable=AsyncMock, return_value=_SUCCESS_RAW) as mock_run:
    ...
    async def test_env_file_loaded(self, tmp_path):
        ...
        env_file.write_text("MY_VAR=1\n")
        with (
            patch("wfx.testing.runners._run_async", new_callable=AsyncMock, return_value=_SUCCESS_RAW),
            patch("wfx.testing.runners._load_dotenv") as mock_load,
        ):
...
class TestGetMarkerArg:
    def test_returns_arg_when_marker_present(self):
        ...
        result = _get_marker_arg(request, "wfx_env_file")
...
class TestResolveRunnerConfig:
    def _make_request(
        ...
        *,
        ...
        def get_closest_marker(name: str):
            if name == "wfx_env_file" and env_file_marker is not None:
                m = MagicMock()
                m.args = [env_file_marker]
                return m
            if name == "wfx_timeout" and timeout_marker is not None:
                m = MagicMock()
        ...
        def getoption(name, default=None):
            mapping = {
                "wfx_env_file": cli_env_file,
                "wfx_timeout": cli_timeout,
                "wfx_flow_dir": cli_flow_dir,
            }
    ...
    def test_marker_takes_precedence_over_cli(self, monkeypatch):
        monkeypatch.delenv("WFX_ENV_FILE", raising=False)
        monkeypatch.delenv("WFX_TIMEOUT", raising=False)
        monkeypatch.delenv("WFX_FLOW_DIR", raising=False)
    ...
    def test_env_var_used_as_fallback(self, monkeypatch):
        monkeypatch.setenv("WFX_ENV_FILE", ".env.envvar")
        monkeypatch.setenv("WFX_TIMEOUT", "42")
        monkeypatch.delenv("WFX_FLOW_DIR", raising=False)
        req = self._make_request()
        env_file, timeout, _ = _resolve_runner_config(req)
    ...
    def test_flow_dir_env_var_fallback(self, monkeypatch, tmp_path):
        monkeypatch.delenv("WFX_ENV_FILE", raising=False)
        monkeypatch.delenv("WFX_TIMEOUT", raising=False)
        monkeypatch.setenv("WFX_FLOW_DIR", str(tmp_path))
        req = self._make_request()
        _, _, base_dir = _resolve_runner_config(req)
        assert base_dir == tmp_path
    ...
    def test_invalid_timeout_env_var_ignored(self, monkeypatch):
        monkeypatch.delenv("WFX_ENV_FILE", raising=False)
        monkeypatch.setenv("WFX_TIMEOUT", "not-a-number")
        monkeypatch.delenv("WFX_FLOW_DIR", raising=False)
        req = self._make_request()
...
class TestFixtures:
    ...
    def test_flow_runner_fixture_runs_flow(self, flow_runner):
        with patch("wfx.testing.runners._run_sync", return_value=_SUCCESS_RAW):
