...
async def test_should_call_create_agent_with_resolved_llm_and_tools_when_create_agent_runnable_invoked() -> None:
    ...
    with (
        ...
        patch("wfx.components.models_and_agents.agent.create_agent", side_effect=_capture_create_agent),
    ):
...
async def test_should_attach_single_tool_call_middleware_when_llm_is_watsonx() -> None:
    ...
    """
    from wfx.components.models_and_agents.agent_helpers.single_tool_call_middleware import (
...
async def test_should_attach_watsonx_placeholder_middleware_when_llm_is_watsonx() -> None:
    ...
    """
    from wfx.components.models_and_agents.agent_helpers.placeholder_corrective_middleware import (
...
async def test_should_attach_token_usage_handler_and_set_usage_on_result_properties() -> None:
    ...
    """
    from wfx.schema.message import Message as _Msg
    ...
    with (
        ...
        patch(
            "wfx.components.models_and_agents.agent.TokenUsageCallbackHandler",
            return_value=handler_instance,
        ),
        patch(
            "wfx.components.models_and_agents.agent.process_agent_events",
...
async def test_should_pass_event_manager_on_token_callback_to_process_agent_events() -> None:
    ...
    Primeagent runtime), `process_agent_events` must receive a callback that pipes each
    ...
    with (
        patch.object(type(component), "_get_shared_callbacks", return_value=[]),
        patch("wfx.components.models_and_agents.agent.process_agent_events", side_effect=_capture_process),
    ):
...
async def test_should_delete_stored_message_and_emit_remove_event_on_exception_with_message_error() -> None:
    ...
    """
    from wfx.base.agents.events import ExceptionWithMessageError as _ExcWithMsg
    ...
    with (
        patch.object(type(component), "_get_shared_callbacks", return_value=[]),
        patch.object(type(component), "_send_message_event", new=send_event),
        patch("wfx.components.models_and_agents.agent.delete_message", new=delete_message_mock),
        patch(
            "wfx.components.models_and_agents.agent.process_agent_events",
...
async def test_should_invoke_graph_astream_events_with_messages_input_when_run_agent_called() -> None:
    ...
    with (
        patch("wfx.components.models_and_agents.agent.process_agent_events", new=AsyncMock(return_value=fake_result)),
        patch.object(type(component), "_get_shared_callbacks", return_value=[]),
    ):
        result = await component.run_agent(fake_graph)
...
async def test_should_skip_eager_bind_tools_validation_when_no_tools_attached() -> None:
    ...
    with (
        ...
        patch("wfx.components.models_and_agents.agent.create_agent", return_value=MagicMock()),
    ):
...
async def test_should_not_mutate_shared_base_inputs_when_overriding_info_text() -> None:
    ...
    from wfx.components.models_and_agents.agent import _agent_base_inputs
...
async def test_should_drop_verbose_input_from_agent_component() -> None:
    ...
    """
    from wfx.base.agents.agent import LCToolsAgentComponent
    from wfx.components.models_and_agents.agent import AgentComponent, _agent_base_inputs
...
async def test_should_not_require_verbose_in_update_build_config() -> None:
    ...
    with (
        patch(
            "wfx.components.models_and_agents.agent.handle_model_input_update",
...
async def test_should_complete_run_agent_synchronously_before_returning_so_downstream_agents_see_final_state() -> None:
    ...
    wall time was ~207s. Investigation found:
      ...
      - `wfx/graph/graph/base.py` runs vertices in topological layers; each
...
async def test_should_pass_stream_true_to_get_llm_when_self_stream_toggle_is_false() -> None:
    ...
    with patch("wfx.components.models_and_agents.agent.get_llm", side_effect=_capture_get_llm):
        component._get_llm()
