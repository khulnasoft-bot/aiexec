"""Unit tests for wfx.base.composio.safe_provider.
...
class TestSanitizeSchema:
    ...
    def _get_fn(self):
        from wfx.base.composio.safe_provider import _sanitize_schema
...
class TestPatchIdentifierSubstitutionOnce:
    ...
    def _run_patch(self, fake_mod):
        ...
        with (
            ...
            patch("wfx.base.composio.safe_provider._COMPOSIO_AVAILABLE", new=True),
            patch("wfx.base.composio.safe_provider._composio_lc_provider", fake_mod),
        ):
            from wfx.base.composio import safe_provider

            # Reset sentinel so the patch runs fresh
            if hasattr(fake_mod, "_wfx_identifier_patched"):
                del fake_mod._wfx_identifier_patched
    ...
    def test_sets_patched_sentinel(self):
        ...
        assert getattr(fake_mod, "_wfx_identifier_patched", False)

    def test_idempotent(self):
        fake_mod = _make_fake_lc_provider()
        patched_once = self._run_patch(fake_mod)
        # Mark as already patched and call again
        fake_mod._wfx_identifier_patched = True
        with (
            patch("wfx.base.composio.safe_provider._COMPOSIO_AVAILABLE", new=True),
            patch("wfx.base.composio.safe_provider._composio_lc_provider", fake_mod),
        ):
    ...
    def test_skips_when_composio_unavailable(self):
        fake_mod = _make_fake_lc_provider()
        original = fake_mod._substitute_reserved_python_keywords
        with patch("wfx.base.composio.safe_provider._COMPOSIO_AVAILABLE", new=False):
            from wfx.base.composio import safe_provider
