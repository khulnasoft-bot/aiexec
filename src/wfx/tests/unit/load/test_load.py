...
from wfx.utils.flow_validation import collect_component_hash_lookups
...
async def test_aload_flow_from_json_fail_closed_when_hashes_are_unavailable():
    ...
    with (
        patch("wfx.load.load.update_settings", new=AsyncMock()),
        patch(
        ...
        patch("wfx.graph.graph.base.Graph.add_nodes_and_edges") as mock_add_nodes_and_edges,
        pytest.raises(
            ValueError,
