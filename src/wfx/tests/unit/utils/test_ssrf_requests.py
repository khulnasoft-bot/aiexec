...
from wfx.utils.ssrf_requests import ssrf_safe_get
...
class TestSSRFSafeGet:
    ...
    def test_cross_host_redirect_strips_credential_headers(self):
        ...
        assert mock_get.call_args_list[1].kwargs["headers"] == {"User-Agent": "primeagent-test"}
    ...
    def test_same_host_redirect_keeps_headers(self):
        ...
        headers = {"Authorization": "Bearer secret-token", "User-Agent": "primeagent-test"}
        with (
            patch.dict(os.environ, {"PRIMEAGENT_SSRF_PROTECTION_ENABLED": "true"}),
            patch("socket.getaddrinfo", side_effect=_resolve_public),
            patch("requests.get", side_effect=responses) as mock_get,
