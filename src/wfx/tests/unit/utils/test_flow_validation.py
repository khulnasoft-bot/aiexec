"""Unit tests for WFX flow validation helpers."""

from types import SimpleNamespace
from unittest.mock import AsyncMock, patch

import pytest
from wfx.utils.flow_validation import (
...
async def test_ensure_component_hash_lookups_loaded_surfaces_loader_failures(monkeypatch):
    ...
    monkeypatch.setattr("wfx.services.deps.get_settings_service", lambda: settings_service)
    monkeypatch.setattr(component_cache, "type_to_current_hash", None)

    with (
        patch(
            "wfx.interface.components.get_and_cache_all_types_dict",
...
def test_substitute_trusted_node_code_replaces_known_and_blocks_unknown():
    from wfx.utils import flow_validation as fv
...
async def test_prepare_public_flow_build_substitutes_trusted_code(monkeypatch):
    ...
    monkeypatch.setattr("wfx.services.deps.get_settings_service", lambda: _public_settings())
