...
request-scoped variables (injected via wfx serve ``global_vars``) must still
...
def variable_service_registered():
    ...
    Standalone wfx has no VariableService factory, so ``get_variable_service()``
...
# ---------------------------------------------------------------------------
# get_api_key_for_provider — env gating (no VariableService, == standalone wfx)
# ---------------------------------------------------------------------------
...
def test_env_if_allowed_gates_connection_config(monkeypatch):
    ...
    from wfx.base.models.unified_models.instantiation import _env_if_allowed
