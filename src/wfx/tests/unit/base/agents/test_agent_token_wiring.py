...
class TestAgentTokenCallbackWiring:
    ...
    async def test_token_usage_stored_on_component_when_handler_returns_usage(self):
        ...
        with (
            patch("wfx.base.agents.agent.TokenUsageCallbackHandler") as mock_handler_cls,
            patch("wfx.base.agents.agent.process_agent_events", new_callable=AsyncMock, return_value=result_message),
        ):
