"""Unit tests for the shared --upgrade-flow gate (wfx.upgrade.cli_gate)."""
...
import pytest
from wfx.upgrade.cli_gate import UpgradeFlowError, UpgradeFlowMode, _load_bundled_registry, apply_upgrade_gate
...
# that `wfx upgrade` reads. An empty registry classifies EVERY node as `blocked`, so every flow
...
def test_check_defaults_to_bundled_registry_for_clean_flow():
    ...
    'blocked'. Passing all_types_dict=None must load the bundled index (like `wfx upgrade`) and
...
def test_missing_bundled_registry_raises_upgrade_flow_error():
    ...
    """
    with (
        patch("wfx.interface.components._read_component_index", return_value=None),
