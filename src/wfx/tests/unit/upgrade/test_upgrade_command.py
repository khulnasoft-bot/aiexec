...
from wfx.cli.upgrade import upgrade_command
...
def test_upgrade_outer_envelope_flow_finds_nodes(tmp_path, capsys):
    """Wfx upgrade must detect nodes in Primeagent's exported outer-envelope format.

    Primeagent exports flows as {"name": "...", "data": {"nodes": [...], "edges": []}}.
...
def test_upgrade_outer_envelope_write_updates_inner_nodes(tmp_path):
    ...
    # The envelope must be preserved on write so the flow remains re-importable into Primeagent.
...
def test_load_registry_from_index_raises_when_empty(monkeypatch):
    ...
    from wfx.cli.upgrade import load_registry_from_index

    monkeypatch.setattr("wfx.interface.components._read_component_index", lambda _: None)
    with pytest.raises(typer.Exit) as exc_info:
