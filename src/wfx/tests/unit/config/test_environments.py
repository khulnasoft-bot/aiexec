"""Unit tests for wfx.config.environments — environment resolution.
...
from wfx.config.environments import (
    ConfigError,
...
class TestPrimeagentEnvironment:
    def test_fields_stored(self):
        e = PrimeagentEnvironment(name="staging", url="https://x.com", api_key="key123")  # pragma: allowlist secret
    ...
    def test_api_key_may_be_none(self):
        e = PrimeagentEnvironment(name="local", url="http://localhost:7860", api_key=None)
...
class TestFindConfigFile:
    ...
    def test_finds_wfx_yaml_in_cwd(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        p = _write(tmp_path, ".wfx/environments.yaml", _MINIMAL_YAML)
        assert _find_config_file(None) == p

    def test_finds_wfx_yml_in_cwd(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        p = _write(tmp_path, ".wfx/environments.yml", _MINIMAL_YAML)
        assert _find_config_file(None) == p

    def test_walks_up_to_parent(self, tmp_path, monkeypatch):
        parent_yaml = _write(tmp_path, ".wfx/environments.yaml", _MINIMAL_YAML)
    ...
    def test_stops_at_git_boundary(self, tmp_path, monkeypatch):
        ...
        _write(tmp_path, ".wfx/environments.yaml", _MINIMAL_YAML)
    ...
    def test_toml_fallback_when_no_yaml(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        p = _write(tmp_path, "primeagent-environments.toml", _MINIMAL_TOML)
        assert _find_config_file(None) == p
...
class TestResolveConfigMode:
    ...
    def test_no_default_and_no_env_name_raises(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        _write(tmp_path, ".wfx/environments.yaml", _NO_DEFAULT_YAML)
    ...
    def test_toml_file_also_works(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        monkeypatch.setenv("MY_LOCAL_KEY", "local-secret")  # pragma: allowlist secret
        _write(tmp_path, "primeagent-environments.toml", _MINIMAL_TOML)
        result = resolve_environment("local")
        assert result.url == "http://localhost:7860"
        assert result.api_key == "local-secret"  # pragma: allowlist secret
...
class TestResolveNoConfigFallbacks:
    def test_primeagent_url_env_var_used_as_fallback(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        (tmp_path / ".git").mkdir()  # stop file walk here
        monkeypatch.setenv("PRIMEAGENT_URL", "http://fallback:7860")
        monkeypatch.setenv("PRIMEAGENT_API_KEY", "fallback-key")  # pragma: allowlist secret
        result = resolve_environment(None)
        assert result.url == "http://fallback:7860"
        assert result.api_key == "fallback-key"  # pragma: allowlist secret
        assert result.name == "__env__"

    def test_wfx_url_env_var_used_as_fallback(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        (tmp_path / ".git").mkdir()
        monkeypatch.delenv("PRIMEAGENT_URL", raising=False)
        monkeypatch.setenv("WFX_URL", "http://wfx-fallback:7860")
        monkeypatch.setenv("WFX_API_KEY", "wfx-key")  # pragma: allowlist secret
        result = resolve_environment(None)
        assert result.url == "http://wfx-fallback:7860"
        assert result.api_key == "wfx-key"  # pragma: allowlist secret

    def test_named_env_without_config_raises_clear_error(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        (tmp_path / ".git").mkdir()
        monkeypatch.delenv("PRIMEAGENT_URL", raising=False)
        monkeypatch.delenv("WFX_URL", raising=False)
...
class TestErrorMessages:
    ...
    def test_no_config_message_suggests_init(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("PRIMEAGENT_URL", raising=False)
        monkeypatch.delenv("WFX_URL", raising=False)
        with pytest.raises(ConfigError, match="wfx init"):
            resolve_environment("staging")
