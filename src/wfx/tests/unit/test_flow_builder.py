"""Tests for wfx.graph.flow_builder — pure flow-building utilities.

All tests use synthetic registries (plain dicts) — no wfx component loading,
no I/O, no network.
"""
...
from wfx.graph.flow_builder.connect import _custom_stringify, _scaped_json_stringify
...
class TestConnect:
    ...
    def test_add_connection_dedupes_against_ui_saved_edges(self):
        ...
        from older Primeagent versions use the `xy-edge__` id prefix
...
# pattern as real Primeagent components: tool_mode=True on an INPUT, plain
...
class TestConnectComponentAsTool:
    ...
    def test_synthesized_tool_output_matches_canonical_primeagent_shape(self):
        ...
        # Required keys present in every real Primeagent tool output entry.
        required_keys = {
            "allows_loop",
            "cache",
            "display_name",
...
class TestLayout:
    ...
    def test_layout_uses_spacing_constants(self):
        from wfx.graph.flow_builder.layout import LAYER_SPACING_X
    ...
    def test_layout_y_spacing_multiple_at_same_layer(self):
        ...
        from wfx.graph.flow_builder.layout import NODE_SPACING_Y
    ...
    def test_assign_layers_directly(self):
        ...
        from wfx.graph.flow_builder.layout import _assign_layers
...
class TestCustomStringifyExtended:
    ...
    def test_scaped_round_trips_handle_format(self):
        """A source handle dict should produce a string that matches Primeagent's format."""
        handle = {
            "dataType": "ChatInput",
            "id": "ChatInput-abc12",
            "name": "message",
