...
class TestLoginSessionIsolation:
    async def test_login_closes_only_session_client(self):
        ...
        with patch.object(srv, "PrimeagentClient", return_value=_make_mock_client()):
    ...
    async def test_login_no_close_when_no_session_client(self):
        ...
        with patch.object(srv, "PrimeagentClient", return_value=new_client):
            await srv.login("user", "pass", "http://localhost:7860")
...
class TestCreateFlowFromSpecValidation:
    async def test_raises_on_validation_failure(self):
        ...
        with (
            ...
            patch("wfx.mcp.server.validate_flow", new_callable=AsyncMock, return_value=validation_result),
    ...
    async def test_skips_validation_when_disabled(self):
        ...
        with (
            ...
            patch("wfx.mcp.server.create_flow", new_callable=AsyncMock, return_value=created),
            patch("wfx.mcp.server.add_component", new_callable=AsyncMock, return_value={"id": "X-1"}),
            patch("wfx.mcp.server.validate_flow", new_callable=AsyncMock) as mock_validate,
...
class TestUpdateFlowFromSpecRollback:
    async def test_restores_original_data_on_failure(self):
        ...
        with (
            patch("wfx.mcp.server._get_client", return_value=mock_client),
            patch("wfx.mcp.server._get_flow", new_callable=AsyncMock, return_value=flow),
            patch("wfx.mcp.server._get_registry", new_callable=AsyncMock, return_value=MOCK_REGISTRY),
            patch("wfx.mcp.server.parse_flow_spec", return_value=parsed),
            patch("wfx.mcp.server.validate_spec_references"),
            patch("wfx.mcp.server.add_component", new_callable=AsyncMock, side_effect=RuntimeError("Server error")),
    ...
    async def test_emits_settle_event_on_rollback(self):
        ...
        with (
            ...
            patch("wfx.mcp.server.parse_flow_spec", return_value=parsed),
            patch("wfx.mcp.server.validate_spec_references"),
            patch("wfx.mcp.server.add_component", new_callable=AsyncMock, side_effect=RuntimeError("fail")),
    ...
    async def test_success_does_not_rollback(self):
        ...
        with (
            ...
            patch("wfx.mcp.server.get_flow_info", new_callable=AsyncMock, return_value={"id": "flow-123"}),
        ):
