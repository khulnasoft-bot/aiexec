...
def _patch_server(mock_client, mock_registry, mock_flow):
    ...
    def ctx():
        with (
            _patch("wfx.mcp.server._get_client", return_value=mock_client),
            _patch("wfx.mcp.server._get_registry", new_callable=AsyncMock, return_value=mock_registry),
            _patch("wfx.mcp.server._get_flow", new_callable=AsyncMock, return_value=mock_flow),
            _patch("wfx.mcp.server._patch_flow", new_callable=AsyncMock, return_value=mock_flow),
        ):
            yield
...
async def _call_add_component(flow_id, _flow):
    from wfx.mcp.server import add_component

    with (
        patch("wfx.mcp.server.fb_add_component", return_value={"id": "X-1", "display_name": "X"}),
        patch("wfx.mcp.server.layout_flow"),
    ):
        await add_component(flow_id, "ChatInput")


async def _call_remove_component(flow_id, _flow):
    from wfx.mcp.server import remove_component

    with patch("wfx.mcp.server.fb_remove_component"), patch("wfx.mcp.server.layout_flow"):
        await remove_component(flow_id, "X-1")


async def _call_configure_component(flow_id, flow):
    from wfx.mcp.server import configure_component

    flow["data"]["nodes"] = [_node("X-1")]
    with patch("wfx.mcp.server.fb_configure"), patch("wfx.mcp.server.needs_server_update", return_value=False):
        await configure_component(flow_id, "X-1", {"input_value": "hello"})


async def _call_connect_components(flow_id, _flow):
    from wfx.mcp.server import connect_components

    with patch("wfx.mcp.server.fb_add_connection"), patch("wfx.mcp.server.layout_flow"):
        await connect_components(flow_id, "A-1", "message", "B-1", "input_value")


async def _call_disconnect_components(flow_id, _flow):
    from wfx.mcp.server import disconnect_components

    with patch("wfx.mcp.server.fb_remove_connection", return_value=1), patch("wfx.mcp.server.layout_flow"):
        await disconnect_components(flow_id, "A-1", "B-1")


async def _call_freeze_component(flow_id, flow):
    from wfx.mcp.server import freeze_component
...
async def _call_unfreeze_component(flow_id, flow):
    from wfx.mcp.server import unfreeze_component
...
async def _call_layout_flow_tool(flow_id, _flow):
    from wfx.mcp.server import layout_flow_tool

    with patch("wfx.mcp.server.layout_flow"):
        await layout_flow_tool(flow_id)


async def _call_update_flow_from_spec(flow_id, _flow):
    from wfx.mcp.server import update_flow_from_spec

    parsed = {"name": "T", "description": "", "nodes": [{"id": "A", "type": "ChatInput"}], "edges": [], "config": {}}
    with (
        patch("wfx.mcp.server.parse_flow_spec", return_value=parsed),
        patch("wfx.mcp.server.validate_spec_references"),
        patch("wfx.mcp.server.add_component", new_callable=AsyncMock, return_value={"id": "X-1"}),
        patch("wfx.mcp.server._create_prompt_template_vars", new_callable=AsyncMock),
        patch("wfx.mcp.server.get_flow_info", new_callable=AsyncMock, return_value={"id": flow_id}),
    ):
...
class TestCreateFlowFromSpecEmitsSettled:
    async def test_emits_flow_settled_after_batch(self, mock_client, mock_registry):
        from wfx.mcp.server import create_flow_from_spec
        ...
        with (
            patch("wfx.mcp.server._get_client", return_value=mock_client),
            patch("wfx.mcp.server._get_registry", new_callable=AsyncMock, return_value=mock_registry),
            patch("wfx.mcp.server.parse_flow_spec", return_value=parsed),
            patch("wfx.mcp.server.validate_spec_references"),
            patch("wfx.mcp.server.create_flow", new_callable=AsyncMock, return_value=created),
            patch("wfx.mcp.server.add_component", new_callable=AsyncMock, return_value={"id": "X-1"}),
            patch("wfx.mcp.server.validate_flow", new_callable=AsyncMock, return_value={"valid": True}),
            patch("wfx.mcp.server.get_flow_info", new_callable=AsyncMock, return_value={"id": "flow-new"}),
        ):
...
class TestNotifyDone:
    async def test_emits_flow_settled_and_returns_ok(self, mock_client):
        from wfx.mcp.server import notify_done

        with patch("wfx.mcp.server._get_client", return_value=mock_client):
...
class TestCreateFlowFromSpecRollback:
    async def test_emits_settle_on_rollback(self, mock_client, mock_registry):
        ...
        with (
            ...
            patch("wfx.mcp.server.validate_spec_references"),
            patch("wfx.mcp.server.create_flow", new_callable=AsyncMock, return_value=created),
            patch("wfx.mcp.server.add_component", new_callable=AsyncMock, side_effect=RuntimeError("Build failed")),
            patch("wfx.mcp.server.delete_flow", new_callable=AsyncMock),
