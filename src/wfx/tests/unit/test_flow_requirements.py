...
import pytest
from wfx.utils.flow_requirements import (
    MODULE_EXTRA_DEPS,
    _detect_providers_from_template,
    _extract_component_requirements,
    _extract_imports,
    _get_import_to_dist_map,
    _get_wfx_provided_imports,
    _get_wfx_transitive_dists,
    _import_to_package,
    _pin_version,
...
def _find_starter_projects_dir() -> Path:
    ...
    for parent in current.parents:
        candidate = parent / "src" / "backend" / "base" / "primeagent" / "initial_setup" / "starter_projects"
        if candidate.is_dir():
            return candidate
...
"""
    ...
    def test_wfx_imports(self):
        code = "from wfx.schema.message import Message"
        result = _extract_imports(code)
        assert "wfx" in result
...
class TestDynamicResolution:
    ...
    def test_wfx_transitive_dists_includes_wfx(self):
        dists = _get_wfx_transitive_dists()
        assert "wfx" in dists

    def test_wfx_transitive_dists_includes_langchain(self):
        dists = _get_wfx_transitive_dists()
        assert "langchain" in dists
        assert "langchain-core" in dists

    def test_wfx_transitive_dists_includes_pydantic(self):
        dists = _get_wfx_transitive_dists()
        assert "pydantic" in dists

    def test_wfx_provided_imports_includes_expected(self):
        provided = _get_wfx_provided_imports()
        ...
        for imp in resolvable:
            assert imp in provided, f"{imp} should be provided by wfx"

    def test_wfx_provided_imports_excludes_optional(self):
        """Packages not in wfx's dep tree should NOT be in provided."""
        provided = _get_wfx_provided_imports()
        # langchain-openai is an optional provider, not a core wfx dep
...
class TestExtractComponentRequirements:
    def test_wfx_only_component(self):
        code = """
...
"""
    ...
    def test_wfx_provided_filtered(self):
        """Imports that are transitively provided by wfx should not appear as requirements.
        ...
        if not resolvable:
            pytest.skip("None of the test imports are wfx-provided in this environment")
...
"""
    ...
    def test_module_extra_deps(self):
        ...
        whether it's transitively provided by wfx, but the extra runtime deps
...
"""
    ...
    def test_primeagent_imports_filtered(self):
        """Components with primeagent imports should NOT list primeagent as a dep.

        wfx provides the primeagent interfaces at runtime, so primeagent/primeagent_base
        should be filtered out just like wfx itself.
        """
        code = """
from primeagent.custom import Component
from primeagent.io import MessageTextInput
"""
        node = _make_node("LegacyComponent", code)
        packages, _ = _extract_component_requirements(node)
        assert "primeagent" not in packages
        assert "primeagent-base" not in packages
...
class TestGenerateRequirementsFromFlow:
    ...
    def test_include_wfx_false(self):
        flow = _make_flow()
        result = generate_requirements_from_flow(flow, include_wfx=False, pin_versions=False)
        assert "wfx" not in result

    def test_custom_wfx_package_name(self):
        flow = _make_flow()
        result = generate_requirements_from_flow(flow, wfx_package="wfx-nightly", pin_versions=False)
        assert "wfx-nightly" in result
        assert "wfx" not in result
    ...
    def test_results_sorted(self):
        ...
        result = generate_requirements_from_flow(flow, pin_versions=False)
        # wfx should be first, then sorted extras
        assert result[0] == "wfx"
        extras = result[1:]
...
class TestVersionPinning:
    def test_pin_version_installed_package(self):
        ...
        result = _pin_version("wfx")
        assert result.startswith("wfx==")
    ...
    def test_pin_versions_true_by_default(self):
        ...
        assert result[0].startswith("wfx==")
    ...
    def test_pinned_output_includes_versions_for_installed_deps(self):
        ...
        # wfx is installed, so it should be pinned
        assert result[0].startswith("wfx==")
...
class TestStarterProjects:
    ...
    def test_basic_prompting_includes_wfx(self, basic_prompting_flow):
        """Basic Prompting (ships with no pre-selected model) should still need wfx.
    ...
    def test_simple_agent_requirements(self, simple_agent_flow):
        """Simple Agent requires wfx and no longer drags in langchain-community.
        ...
        include ``wfx`` but never ``langchain-community`` (regardless of whether
        wfx happens to provide it transitively in a given environment).
        """
    ...
    def test_basic_prompting_from_file(self):
        ...
        generated requirements should include ``wfx`` but none of the
    ...
    def test_wfx_nightly_package_name(self, basic_prompting_flow):
        """Test specifying wfx-nightly as the package name."""
        result = generate_requirements_from_flow(
            basic_prompting_flow,
            wfx_package="wfx-nightly",
            pin_versions=False,
        )
        assert result[0] == "wfx-nightly"
        assert "wfx" not in result
...
class TestResolveProviderPackages:
    ...
    def _skip_if_provider_not_loaded(provider_name: str):
        ...
        """
        try:
            from wfx.base.models.model_input_constants import MODEL_PROVIDERS_DICT
        except ImportError:
...
class TestResolveEmbeddingProviderPackages:
    ...
    def test_all_embedding_providers_resolve(self):
        ...
        from wfx.base.models.unified_models import EMBEDDING_PROVIDER_CLASS_MAPPING
...
class TestErrorHandling:
    ...
    def test_generate_requirements_from_file_wrong_structure(self, tmp_path):
        """A valid JSON file that isn't a flow should still produce a result (just wfx)."""
...
# ===================================================================
# Typer CLI tests: wfx requirements
# ===================================================================


class TestTyperRequirementsCommand:
    """Tests for the typer-based ``wfx requirements`` CLI command."""
    ...
    def flow_file(self, tmp_path):
        flow = _make_flow(_make_node("Simple", "import wfx"))
    ...
    def test_output_flag_writes_file(self, runner, app, flow_file, tmp_path):
        ...
        assert result.exit_code == 0
        assert out.exists()
        assert "wfx" in out.read_text(encoding="utf-8")
        assert "Requirements written to" in result.output

    def test_no_wfx_flag(self, runner, app, flow_file):
        result = runner.invoke(app, ["requirements", str(flow_file), "--no-wfx", "--no-pin"])
        assert result.exit_code == 0
        # With --no-wfx and only wfx imports, no packages should appear after header
        lines = [line for line in result.output.strip().split("\n") if line and not line.startswith("#")]
        assert "wfx" not in lines
    ...
    def test_wfx_package_flag(self, runner, app, flow_file):
        result = runner.invoke(app, ["requirements", str(flow_file), "--wfx-package", "wfx-nightly", "--no-pin"])
        assert result.exit_code == 0
        assert "wfx-nightly" in result.output
        # Should not contain bare "wfx" as a separate line
        lines = [line for line in result.output.strip().split("\n") if line and not line.startswith("#")]
        assert "wfx" not in lines
