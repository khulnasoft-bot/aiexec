...
from wfx.cli.serve_app import FlowMeta, FlowRegistry, StreamRequest, create_multi_serve_app
...
class TestMultiServeStreaming:
    ...
    async def test_stream_blocks_custom_components_when_disabled(
        ...
        async with (
            ...
            with (
                ...
                patch(
                    "wfx.cli.serve_app.run_flow_generator_for_serve",
                    new=AsyncMock(),
                ) as mock_run_flow,
            ):
                response = await client.post(
    ...
    async def test_stream_setup_error_returns_valid_json(self, mock_graphs, mock_metas, monkeypatch):
        ...
        async with (
            ...
            with patch(
                "wfx.cli.serve_app.validate_flow_for_current_settings",
