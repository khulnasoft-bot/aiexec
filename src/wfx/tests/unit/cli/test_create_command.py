"""Unit tests for ``wfx create`` and the ``--example`` seeding in ``wfx init``.
...
from wfx.cli.init import init_command
...
class TestCreateCommand:
    ...
    def test_raises_exit1_when_no_templates_dir(self, tmp_path):
        ...
        with patch("wfx.cli.create._FLOWS_TEMPLATE_DIR", fake_dir), pytest.raises(typer.Exit):
...
class TestPrintTemplates:
    ...
    def test_prints_nothing_when_no_templates(self, tmp_path):
        with patch("wfx.cli.create._FLOWS_TEMPLATE_DIR", tmp_path / "empty"):
            print_templates()


# ---------------------------------------------------------------------------
# CLI integration — wfx create
# ---------------------------------------------------------------------------
