"""Unit tests for wfx export -- export_command and helpers.
...
def _make_sdk_mock(client_mock: MagicMock | None = None) -> MagicMock:
    """Return a mock primeagent_sdk module wired up for export tests."""
...
def _run_export(
    ...
    *,
    ...
    with (
        patch("wfx.cli.export.load_sdk", return_value=mock),
...
class TestWriteFlow:
    def test_in_place_writes_to_source_path(self, tmp_path):
        from wfx.cli.export import _write_flow
...
class TestExportCommandRemoteFlowId:
    def test_missing_env_and_target_raises_exit_1(self):
        ...
        with patch("wfx.cli.export.load_sdk", return_value=sdk), pytest.raises(typer.Exit) as exc_info:
            export_command(
                flow_paths=[],
                output=None,
                output_dir=None,
    ...
    def test_config_error_raises_exit_1(self, tmp_path):
        ...
        sdk = _make_sdk_mock()
        with (
            patch("wfx.cli.export.load_sdk", return_value=sdk),
...
class TestExportCommandRemoteProject:
    ...
    def test_project_config_error_raises_exit_1(self, tmp_path):
        ...
        with (
            patch("wfx.cli.export.load_sdk", return_value=sdk),
            patch("wfx.config.resolve_environment", side_effect=ConfigError("cfg error")),
            pytest.raises(typer.Exit) as exc_info,
        ):
            export_command(
                flow_paths=[],
    ...
    def test_project_target_used_without_env_name(self, tmp_path):
        ...
        with (
            patch("wfx.cli.export.load_sdk", return_value=sdk),
            patch("wfx.config.resolve_environment", return_value=env_cfg),
        ):
            export_command(
                flow_paths=[],
                output=None,
