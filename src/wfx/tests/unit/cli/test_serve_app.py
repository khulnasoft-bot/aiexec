...
from wfx.interface.components import component_cache
...
class TestFlowRegistry:
    ...
    def test_duplicate_add_raises_without_overwrite(self):
        from wfx.cli.serve_app import FlowAlreadyRegisteredError
    ...
    def test_warm_from_store_skips_unloadable_flows(self, tmp_path):
        ...
        with patch("wfx.cli.serve_app.load_flow_from_json", side_effect=fake_load):
    ...
    def test_registry_get_misses_to_store(self):
        ...
        with patch("wfx.cli.serve_app.load_flow_from_json", side_effect=AssertionError("should use cache")):
    ...
    def test_overwrite_aliased_flow_removes_old_store_file_and_alias(self):
        ...
        with patch("wfx.cli.serve_app.load_flow_from_json", return_value=new_graph):
    ...
    def test_remove_by_uuid_clears_both_aliases(self):
        ...
        assert result is True
        with patch("wfx.cli.serve_app.load_flow_from_json", side_effect=AssertionError("store already deleted")):
...
class TestCreateServeAppFactory:
    ...
    def test_create_serve_app_empty_start(self):
        ...
        from wfx.cli.serve_app import create_serve_app

        with patch.dict(os.environ, {"PRIMEAGENT_API_KEY": "test-key"}, clear=False):  # pragma: allowlist secret
            # Remove startup paths env if present
            env = {k: v for k, v in os.environ.items() if not k.startswith("WFX_SERVE_")}
            with patch.dict(os.environ, env, clear=True):
                os.environ["PRIMEAGENT_API_KEY"] = "test-key"  # pragma: allowlist secret
    ...
    def test_create_serve_app_startup_paths_cleaned_from_env_by_caller(self):
        """WFX_SERVE_STARTUP_PATHS must be consumed by create_serve_app() but not deleted.
        ...
        from wfx.cli.serve_app import _SERVE_STARTUP_PATHS_ENV, create_serve_app

        env_override = {
            "PRIMEAGENT_API_KEY": "test-key",  # pragma: allowlist secret
    ...
    def test_create_serve_app_with_flow_dir_skips_startup_paths_uses_store(self, tmp_path):
        ...
        with (
            ...
            patch("wfx.cli.commands.build_registry_from_paths") as mock_brfp,
            patch("wfx.cli.serve_app.load_flow_from_json", return_value=mock_graph),
        ):
            app = create_serve_app()
    ...
    def test_create_serve_app_without_flow_dir_loads_startup_paths_from_files(self, tmp_path):
        ...
        env_override = {
            "PRIMEAGENT_API_KEY": "test-key",  # pragma: allowlist secret
            # No WFX_SERVE_FLOW_DIR — no flow_dir
    ...
    def test_create_serve_app_startup_path_load_error_propagates(self, tmp_path):
        ...
        env_override = {
            ...
            # No WFX_SERVE_FLOW_DIR — triggers the ThreadPoolExecutor path
        }

        with (
            patch.dict(os.environ, env_override),
            patch(
                "wfx.cli.commands.load_flow_from_json",
...
class TestServeAppEndpoints:
    ...
    def test_run_endpoint_blocks_custom_components_when_disabled(
        ...
        with (
            patch.dict(os.environ, {"PRIMEAGENT_API_KEY": "test-api-key"}),  # pragma: allowlist secret
            patch(
                "wfx.services.deps.get_settings_service",
                return_value=_make_settings_service(allow_custom_components=False),
            ),
            patch(
                "wfx.utils.flow_validation.ensure_component_hash_lookups_loaded",
            ...
            patch(
                "wfx.cli.serve_app.execute_graph_with_capture",
    ...
    def test_run_endpoint_forwards_session_id(self, app_client):
        ...
        with (
            patch.dict(os.environ, {"PRIMEAGENT_API_KEY": "test-api-key"}),  # pragma: allowlist secret
            patch("wfx.cli.serve_app.execute_graph_with_capture", mock_execute_capture),
        ):
    ...
    def test_run_endpoint_global_vars_do_not_mutate_registry_graph(self, real_graph_with_async, monkeypatch):
        ...
        with (
            patch.dict(os.environ, {"PRIMEAGENT_API_KEY": "test-api-key"}),  # pragma: allowlist secret
            patch("wfx.cli.serve_app.execute_graph_with_capture", mock_execute_noop),
            TestClient(app) as client,
        ):
            client.post(
                "/flows/00000000-0000-0000-0000-000000000001/run",
...
class TestUploadEndpoint:
    ...
    def full_export(self):
        """Full Primeagent export JSON as exported from the UI (name/data/... at top level).
    ...
    def test_upload_full_export_as_body(self, app_with_empty_registry, full_export):
        """Uploading a Primeagent export JSON directly as the body must succeed.
    ...
    def test_upload_invalid_flow_data_returns_422(self, app_with_empty_registry):
        with patch("wfx.cli.serve_app.load_flow_from_json", side_effect=ValueError("bad flow")):
            response = app_with_empty_registry.post(
                "/flows/upload/",
    ...
    def test_run_flow_failure_result_returns_500(self, app_with_empty_registry, valid_flow_data):
        ...
        with (
            patch("wfx.cli.serve_app.execute_graph_with_capture", return_value=([], "")),
            patch(
                "wfx.cli.serve_app.extract_result_data",
