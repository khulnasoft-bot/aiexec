"""Unit tests for wfx status -- status_command and helpers.
...
# ---------------------------------------------------------------------------
# Fake exception class — avoids importing primeagent_sdk in isolation mode
# ---------------------------------------------------------------------------


class _FakePrimeagentNotFoundError(Exception):
    """Stand-in for primeagent_sdk.exceptions.PrimeagentNotFoundError in unit tests."""
...
def _fake_env_config(url: str = _BASE_URL, api_key: str = _API_KEY, name: str = "test-env") -> MagicMock:
    """Return a MagicMock that looks like an wfx EnvConfig."""
    cfg = MagicMock()
    cfg.url = url
    cfg.api_key = api_key
    cfg.name = name
...
def _fake_remote_flow(flow_id: UUID = _FLOW_ID, flow_dict: dict | None = None) -> MagicMock:
    """Return a MagicMock that looks like a primeagent_sdk Flow model."""
    remote = MagicMock()
    remote.id = flow_id
...
def _make_sdk_triple(
    ...
    return _identity_normalize, _json_flow_to_json, client_cls, _FakePrimeagentNotFoundError
...
def _run_status(
    ...
    *,
    ...
    """
    from wfx.cli.status import status_command
    ...
    with (
        patch("wfx.cli.status._load_sdk", return_value=triple),
        patch("wfx.config.resolve_environment", return_value=cfg),
    ):
        status_command(
            dir_path=dir_path,
...
class TestFlowStatus:
    def test_required_fields(self):
        from wfx.cli.status import FlowStatus
...
class TestFlowHash:
    def test_returns_12_char_string(self):
        from wfx.cli.status import _flow_hash
...
class TestCollectFiles:
    def test_dir_path_returns_json_files(self, tmp_path):
        from wfx.cli.status import _collect_files
...
class TestRenderTable:
    ...
    def _capture_render(statuses, env_label):
        ...
        from wfx.cli.status import _render_table

        buf = io.StringIO()
        fake_console = Console(file=buf, width=200, no_color=True)
        with patch("wfx.cli.status.console", fake_console):
    ...
    def test_no_id_status_contains_detail(self, tmp_path):
        ...
        statuses = [
            FlowStatus(
                ...
                detail="run wfx export first",
            )
        ]
        output = self._capture_render(statuses, "test-env")
        assert "OrphanFlow" in output
        assert "no id" in output.lower()
        assert "run wfx export first" in output
...
class TestStatusCommandSynced:
    ...
    def test_client_constructed_with_env_url_and_key(self, tmp_path):
        ...
        with (
            patch("wfx.cli.status._load_sdk", return_value=(normalize_fn, to_json_fn, client_cls, not_found_error)),
            patch("wfx.config.resolve_environment", return_value=cfg),
        ):
            from wfx.cli.status import status_command
    ...
    def test_client_is_closed_after_requests_finish(self, tmp_path):
        ...
        triple = _identity_normalize, _json_flow_to_json, MagicMock(return_value=client), _FakePrimeagentNotFoundError
...
class TestStatusCommandNew:
    def test_not_found_on_remote_gives_new_status(self, tmp_path):
        """Remote raises PrimeagentNotFoundError → status 'new' → exits 1."""
        p = _write_flow(tmp_path, "flow.json")
        client = _make_client_mock(get_flow_side_effect=_FakePrimeagentNotFoundError("not found"))
        triple = _make_sdk_triple(client)
...
class TestStatusCommandConfigError:
    def test_config_error_exits_1(self, tmp_path):
        ...
        with (
            patch("wfx.cli.status._load_sdk", return_value=triple),
            patch("wfx.config.resolve_environment", side_effect=ConfigError("bad config")),
