"""Simple tests for WFX serve command focusing on CLI functionality."""
...
# Tests that invoke `serve` with valid-looking input may hang while primeagent
...
def test_cli_imports():
    ...
    from wfx.__main__ import app, main
...
def test_serve_command_missing_api_key():
    """Serve command must exit 1 and mention PRIMEAGENT_API_KEY when no key is set."""
    from wfx.__main__ import app
...
def test_serve_command_with_flow_json():
    ...
    env = {"PRIMEAGENT_API_KEY": "test-key"}  # pragma: allowlist secret
