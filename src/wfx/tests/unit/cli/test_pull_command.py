"""Unit tests for wfx pull -- pull_command and helpers.
...
def _fake_flow_obj(
    ...
    """Return a MagicMock that looks like a primeagent_sdk Flow object."""
    flow = MagicMock()
    flow.id = flow_id
    flow.name = name
...
def _fake_project(
    ...
    """Return a MagicMock that looks like a primeagent_sdk Project object."""
    proj = MagicMock()
    proj.name = name
    proj.id = project_id
...
def _run_pull(
    *,
    ...
    with (
        patch("wfx.cli.pull.load_sdk", return_value=mock_sdk),
        patch("wfx.config.resolve_environment", return_value=mock_cfg),
    ):
        pull_command(
            env=env,
...
class TestSafeFilename:
    def test_alphanumeric_passthrough(self):
        from wfx.cli.common import safe_filename as _safe_filename
...
class TestWriteFlow:
    def test_status_created_when_file_does_not_exist(self, tmp_path: Path):
        from wfx.cli.pull import _write_flow
    ...
    def test_logs_debug_when_write_fails(self, tmp_path: Path):
        ...
        with patch("wfx.cli.pull.logger") as mock_logger:
            _write_flow(flow, sdk=sdk, dest_dir=tmp_path, strip_secrets=False, indent=2)
...
class TestPullResult:
    def test_created_is_ok(self, tmp_path: Path):
        from wfx.cli.pull import PullResult
...
class TestPullCommandErrorHandling:
    def test_config_error_exits_1(self, tmp_path: Path):
        ...
        sdk = _make_sdk_mock()
        with (
            patch("wfx.cli.pull.load_sdk", return_value=sdk),
    ...
    def test_sdk_not_installed_raises_bad_parameter(self, tmp_path: Path):
        ...
        with (
            patch("wfx.cli.pull.load_sdk", side_effect=typer.BadParameter("primeagent-sdk is required")),
...
class TestRenderResults:
    def test_does_not_crash_with_mixed_statuses(self, tmp_path: Path):
        from wfx.cli.pull import PullResult, _render_results
    ...
    def test_does_not_crash_with_empty_results(self):
        from wfx.cli.pull import _render_results
