...
from wfx.cli.runtime_variables import build_request_variables_from_global_vars
from wfx.services.variable.service import VariableService


def test_build_request_variables_parses_primeagent_request_variables():
    merged = {"access_token": "from-json", "USER_ID": "u1"}
    global_vars = {
        "PRIMEAGENT_REQUEST_VARIABLES": json.dumps(merged),
        "access_token": "from-raw-override",
        "x-primeagent-global-var-access-token": "alias-value",
    }
    ...
    assert flat["x-primeagent-global-var-access-token"] == "alias-value"


def test_build_request_variables_keeps_raw_keys_when_blob_is_malformed():
    """A malformed PRIMEAGENT_REQUEST_VARIABLES blob is skipped; raw keys still merge."""
    global_vars = {
        "PRIMEAGENT_REQUEST_VARIABLES": "{not valid json",
...
def test_build_request_variables_ignores_non_dict_blob():
    ...
    global_vars = {
        "PRIMEAGENT_REQUEST_VARIABLES": json.dumps(["a", "b"]),
        "access_token": "raw-tok",
    }
    flat = build_request_variables_from_global_vars(global_vars)
    assert flat == {"access_token": "raw-tok"}
...
def test_build_request_variables_coerces_structured_blob_values():
    ...
    global_vars = {
        "PRIMEAGENT_REQUEST_VARIABLES": json.dumps(
...
def test_normalize_parsed_variables_coercion_rules():
    ...
    from wfx.services.variable.request_scope import normalize_parsed_variables
    ...
    token = activate_request_variables(
        {
            ...
            "x-primeagent-global-var-wxo-github-access-token": "tok-123",
        }
    )
    try:
