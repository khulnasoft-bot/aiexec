...
def test_create_multi_serve_app_unknown_flow_id_returns_404(mock_graph, test_flow_meta):
    from wfx.cli.serve_app import FlowRegistry
...
def test_serve_command_json_file():
    ...
    try:
        ...
        with (
            patch("wfx.cli.commands.load_flow_from_json") as mock_load,
...
class TestBuildRegistryFromDirectory:
    def test_loads_all_json_files(self, tmp_path):
        ...
        from wfx.cli.commands import build_registry_from_directory
        ...
        with patch("wfx.cli.commands.load_flow_from_json", return_value=mock_graph):
    ...
    def test_failed_file_raises_with_filename(self, tmp_path):
        ...
        with (
            patch("wfx.cli.commands.load_flow_from_json", side_effect=ValueError("corrupt")),
...
class TestBuildRegistryFromPaths:
    def test_loads_explicit_paths(self, tmp_path):
        ...
        from wfx.cli.commands import build_registry_from_paths
    ...
    def test_failed_path_raises_with_filename(self, tmp_path):
        ...
        with (
            patch("wfx.cli.commands.load_flow_from_json", side_effect=ValueError("oops")),
            pytest.raises(ValueError, match=r"bad\.json"),
        ):
    ...
    def test_same_filename_in_different_dirs_gets_distinct_ids(self, tmp_path):
        """Regression: wfx serve a/flow.json b/flow.json must not collide on ID."""
        import asyncio
...
class TestServeCommandMultiFlow:
    def test_serve_command_with_directory(self, tmp_path):
        ...
        with (
            patch("wfx.cli.commands.load_flow_from_json", return_value=mock_graph),
            patch("wfx.cli.commands.uvicorn.run"),
            patch.dict(os.environ, {"PRIMEAGENT_API_KEY": "test-key"}),  # pragma: allowlist secret
        ):
            import typer
            from typer.testing import CliRunner
...
class TestPythonScriptServe:
    def test_load_graph_and_meta_dispatches_to_script_loader_for_py(self, tmp_path):
        ...
        from wfx.cli.commands import _load_graph_and_meta
        ...
        with (
            patch("wfx.cli.commands.load_graph_from_script", new=AsyncMock(return_value=mock_graph)) as mock_script,
            patch("wfx.cli.commands.find_graph_variable", return_value={"source": "x", "type": "Graph", "line": 1}),
            patch("wfx.cli.commands.load_flow_from_json") as mock_json,
        ):
    ...
    def test_serve_command_accepts_py_file(self, tmp_path):
        """Wfx serve my_script.py must not be rejected as an unsupported file type."""
        from wfx.cli.commands import serve_command
        ...
        with (
            patch("wfx.cli.commands.load_graph_from_script", new=AsyncMock(return_value=mock_graph)),
            patch("wfx.cli.commands.find_graph_variable", return_value={"type": "assignment", "line": 1}),
            patch("wfx.cli.commands.uvicorn.run"),
            patch.dict(os.environ, {"PRIMEAGENT_API_KEY": "test-key"}),  # pragma: allowlist secret
        ):
            import typer
...
def test_build_registry_from_paths_passes_raw_json_to_store():
    ...
    from wfx.cli.commands import build_registry_from_paths
    from wfx.cli.flow_store import NullFlowStore
...
def test_startup_scan_store_flows_accessible_lazily(tmp_path):
    ...
    from wfx.cli.commands import build_registry_from_paths
    from wfx.cli.flow_store import FilesystemFlowStore
    ...
    with patch("wfx.cli.serve_app.load_flow_from_json", return_value=mock_graph):
...
def test_serve_command_passes_workers_to_uvicorn():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with (
            patch.dict(os.environ, {"PRIMEAGENT_API_KEY": "test-key"}),  # pragma: allowlist secret
            patch("wfx.cli.commands.load_flow_from_json", return_value=mock_graph),
            patch("wfx.cli.commands.uvicorn.run") as mock_run,
        ):
            serve_command(
                script_paths=[str(p)],
                host="127.0.0.1",
    ...
    assert call_args[0] == "wfx.cli.serve_app:create_serve_app"
...
def test_serve_command_sets_startup_paths_env_for_multi_worker(tmp_path):
    """serve_command must set WFX_SERVE_STARTUP_PATHS so each worker can reload flows."""
    import json
    import os
    from unittest.mock import MagicMock, patch

    from wfx.cli.commands import serve_command
    from wfx.cli.serve_app import _SERVE_STARTUP_PATHS_ENV
    ...
    with (
        patch.dict(os.environ, {"PRIMEAGENT_API_KEY": "test-key"}),  # pragma: allowlist secret
        patch("wfx.cli.commands.load_flow_from_json", return_value=mock_graph),
        patch("wfx.cli.commands.uvicorn.run", side_effect=capture_env),
    ):
        serve_command(
            script_paths=[str(p)],
            host="127.0.0.1",
    ...
    assert _SERVE_STARTUP_PATHS_ENV in captured_env, "WFX_SERVE_STARTUP_PATHS must be set before uvicorn.run()"
    ...
    assert _SERVE_STARTUP_PATHS_ENV not in os.environ, "WFX_SERVE_STARTUP_PATHS must be cleaned up after server exits"


def test_serve_command_does_not_set_startup_paths_when_flow_dir_set(tmp_path):
    """When --flow-dir is set, WFX_SERVE_STARTUP_PATHS must be empty.
...
def test_serve_command_allows_py_with_multiple_workers_no_flow_dir(tmp_path):
    ...
    Each worker reloads the .py via WFX_SERVE_STARTUP_PATHS since there is no
    ...
    # WFX_SERVE_STARTUP_PATHS must contain the .py path so workers can reload it
    assert _SERVE_STARTUP_PATHS_ENV in captured_env, "WFX_SERVE_STARTUP_PATHS must be set"
    paths = json.loads(captured_env[_SERVE_STARTUP_PATHS_ENV])
...
# ---------------------------------------------------------------------------
# --upgrade-flow gate parity with `wfx run`
#
# `serve` and `run` share wfx.upgrade.cli_gate.apply_upgrade_gate; these tests
...
# serve: flows load via wfx.cli.commands.load_flow_from_json (which receives the
...
def _upgrade_capturing_loader(captured: dict):
    """Side effect for a patched wfx.cli.commands.load_flow_from_json.
...
def test_serve_upgrade_flow_check_aborts_on_incompatible():
    ...
    with (
        patch("wfx.upgrade.cli_gate._load_bundled_registry", return_value=_upgrade_registry()),
        patch("wfx.cli.commands.uvicorn.run") as mock_uvicorn,
        patch.dict(os.environ, {"PRIMEAGENT_API_KEY": "test-key"}),  # pragma: allowlist secret
    ):
...
def test_serve_upgrade_flow_safe_blocked_aborts():
    ...
    with (
        patch("wfx.upgrade.cli_gate._load_bundled_registry", return_value={}),
        patch("wfx.cli.commands.uvicorn.run") as mock_uvicorn,
        patch.dict(os.environ, {"PRIMEAGENT_API_KEY": "test-key"}),  # pragma: allowlist secret
    ):
...
def test_serve_upgrade_flow_safe_proceeds_to_serve():
    ...
    with (
        patch("wfx.upgrade.cli_gate._load_bundled_registry", return_value=_upgrade_registry()),
        patch("wfx.cli.commands.load_flow_from_json", side_effect=_upgrade_capturing_loader(captured)),
        patch("wfx.cli.commands.uvicorn.run") as mock_uvicorn,
        patch.dict(os.environ, {"PRIMEAGENT_API_KEY": "test-key"}),  # pragma: allowlist secret
    ):
        args = ["--flow-json", _upgrade_flow_json(), "--upgrade-flow", "safe"]
