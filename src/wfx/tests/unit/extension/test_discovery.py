...
from wfx.extension.manifest import ManifestSource
...
def _write_pyproject_extension(root: Path, manifest: dict[str, object]) -> Path:
    """Drop a pyproject.toml that declares ``[tool.primeagent.extension]``.
    ...
    bundle_name = manifest["bundles"][0]["name"]  # type: ignore[index]
    (root / bundle_name).mkdir(exist_ok=True)
    body = "[tool.primeagent.extension]\n"
    body += f'id = "{manifest["id"]}"\n'
    body += f'version = "{manifest["version"]}"\n'
    body += f'name = "{manifest["name"]}"\n'
    body += "\n"
    body += "[tool.primeagent.extension.wfx]\n"
    body += 'compat = ["1"]\n'
    body += "\n"
    body += "[[tool.primeagent.extension.bundles]]\n"
...
def fake_installed_distributions(tmp_path: Path) -> list[_FakeDistribution]:
    ...
    for index, bundle in enumerate(_BUNDLE_NAMES):
        ext_root = tmp_path / f"site-packages/wfx_{bundle}"
        ext_id = f"wfx-{bundle}"
...
def seed_dir_with_three_bundles(tmp_path: Path) -> Path:
    ...
    seed.mkdir()
    for index, bundle in enumerate(_BUNDLE_NAMES):
        sub = seed / f"wfx_{bundle}"
        manifest = _manifest(extension_id=f"wfx-{bundle}", bundle_name=bundle, version=f"2.{index}.0")
...
def test_discover_installed_finds_three_distributions(
    ...
    assert ids == ["wfx-anthropic", "wfx-openai", "wfx-qdrant"]
...
def test_discover_installed_accepts_pyproject_form(
    ...
    assert by_id["wfx-anthropic"].manifest.kind == "pyproject.toml"
    assert by_id["wfx-openai"].manifest.kind == "extension.json"
...
def test_discover_installed_emits_typed_error_on_malformed_manifest(tmp_path: Path) -> None:
    ...
    """
    bad_root = tmp_path / "site-packages/wfx_broken"
    bad_root.mkdir(parents=True)
    (bad_root / "extension.json").write_text("{ this is not json", encoding="utf-8")
    bad = _FakeDistribution(name="wfx-broken", root=bad_root, manifest_relative="extension.json")

    good_root = tmp_path / "site-packages/wfx_openai"
    _write_extension_json(good_root, _manifest("wfx-openai", "openai"))
    good = _FakeDistribution(name="wfx-openai", root=good_root, manifest_relative="extension.json")

    extensions, errors = discover_installed_extensions(distributions=[bad, good])

    assert len(errors) == 1
    assert errors[0].code == "manifest-unreadable"
    assert "wfx-broken" in errors[0].content if errors[0].content else True
    assert len(extensions) == 1
    assert extensions[0].extension_id == "wfx-openai"
...
def test_discover_installed_emits_typed_error_on_schema_failure(tmp_path: Path) -> None:
    ...
    root = tmp_path / "site-packages/wfx_bad_schema"
    root.mkdir(parents=True)
    # Missing ``wfx`` field -- pydantic will reject this.
    (root / "extension.json").write_text(
        json.dumps({"id": "wfx-x", "version": "1.0.0", "name": "X", "bundles": []}),
        encoding="utf-8",
    )
    dist = _FakeDistribution(name="wfx-x", root=root, manifest_relative="extension.json")
...
def test_canonicalize_distribution_normalizes_per_pep503() -> None:
    assert canonicalize_distribution("WFX_OpenAI") == "wfx-openai"
    assert canonicalize_distribution("wfx.openai") == "wfx-openai"
    assert canonicalize_distribution("wfx--OpenAI") == "wfx-openai"
...
class _EditableDistribution(importlib_metadata.Distribution):
    ...
    listed in RECORD), and the ``primeagent.extensions`` entry-point is
...
def test_discover_installed_falls_back_to_entry_point_for_editable_install(
    ...
    """Editable installs are discovered via the ``primeagent.extensions`` entry-point.
    ...
    _make_editable_package(
        site,
        module_name="wfx_editable_bundle",
        manifest=_manifest("wfx-editable-bundle", "bundle"),
    )
    ...
    sys.modules.pop("wfx_editable_bundle", None)

    dist = _EditableDistribution(
        name="wfx-editable-bundle",
        module_name="wfx_editable_bundle",
    )
    ...
    assert len(extensions) == 1
    ext = extensions[0]
    assert ext.extension_id == "wfx-editable-bundle"
    assert ext.source_kind == "installed"
    assert ext.bundle_name == "bundle"
    assert ext.manifest.kind == "extension.json"
    assert ext.extension_root == site / "wfx_editable_bundle"
...
def test_discover_installed_entry_point_fallback_handles_missing_module(
    ...
    # No package on sys.path corresponds to ``wfx_nonexistent``.
    monkeypatch.syspath_prepend(str(tmp_path))
    import sys

    sys.modules.pop("wfx_nonexistent_bundle", None)

    dist = _EditableDistribution(
        name="wfx-nonexistent-bundle",
        module_name="wfx_nonexistent_bundle",
    )
...
def test_discover_installed_entry_point_fallback_skipped_when_files_have_manifest(
    ...
    """
    site = tmp_path / "site-packages/wfx_wheelish"
    _write_extension_json(site, _manifest("wfx-wheelish", "wheelish"))
...
def test_discover_seed_supports_pathsep_multiple_roots(tmp_path: Path) -> None:
    """``$PRIMEAGENT_SEED_DIR`` accepts multiple roots joined by ``os.pathsep``."""
    root_a = tmp_path / "a"
    root_b = tmp_path / "b"
    root_a.mkdir()
    root_b.mkdir()
    _write_extension_json(root_a / "wfx_openai", _manifest("wfx-openai", "openai"))
    _write_extension_json(root_b / "wfx_anthropic", _manifest("wfx-anthropic", "anthropic"))

    composite = f"{root_a}{os.pathsep}{root_b}"
    extensions, errors = discover_seed_extensions(seed_dir_env=composite)

    assert errors == []
    assert {ext.extension_id for ext in extensions} == {"wfx-openai", "wfx-anthropic"}
...
def test_discover_seed_skips_directories_without_manifest(tmp_path: Path) -> None:
    ...
    _write_extension_json(seed / "wfx_openai", _manifest("wfx-openai", "openai"))

    extensions, errors = discover_seed_extensions(seed_dir_env=str(seed))

    assert errors == []
    assert [ext.extension_id for ext in extensions] == ["wfx-openai"]
...
def test_discover_seed_skips_dot_directories(tmp_path: Path) -> None:
    ...
    hidden = seed / ".cache"
    hidden.mkdir()
    _write_extension_json(hidden, _manifest("wfx-hidden", "hidden"))
    _write_extension_json(seed / "wfx_visible", _manifest("wfx-visible", "visible"))

    extensions, _ = discover_seed_extensions(seed_dir_env=str(seed))
    ids = {ext.extension_id for ext in extensions}
    assert ids == {"wfx-visible"}
...
def test_default_seed_dir_constant_is_opt_primeagent_bundles() -> None:
    """Pinning the documented default so docs and code agree."""
    assert Path("/opt/primeagent/bundles") == DEFAULT_SEED_DIR
...
def test_discover_all_merges_installed_and_seed(
    ...
    seed = tmp_path / "seed"
    _write_extension_json(seed / "wfx_seed_only", _manifest("wfx-seed-only", "seed_only"))
    ...
    assert any(ext.extension_id == "wfx-seed-only" for ext in extensions)
...
def test_discover_all_emits_seed_bundle_shadowed_when_ids_collide(
    ...
    """
    # ``fake_installed_distributions`` ships ``wfx-openai``; the seed dir
    # below carries the same id from a different on-disk source.
    seed = tmp_path / "seed"
    shadow_root = seed / "wfx_openai_shadow"
    _write_extension_json(shadow_root, _manifest("wfx-openai", "openai", version="9.9.9"))
    fresh_root = seed / "wfx_only_seed"
    _write_extension_json(fresh_root, _manifest("wfx-only-seed", "only_seed"))
    ...
    assert extension_ids.count("wfx-openai") == 1
    assert any(ext.extension_id == "wfx-openai" and ext.source_kind == "installed" for ext in extensions)
    assert any(ext.extension_id == "wfx-only-seed" for ext in extensions)

    shadow_errors = [err for err in errors if err.code == "seed-bundle-shadowed"]
    assert len(shadow_errors) == 1
    [err] = shadow_errors
    assert err.content == "wfx-openai"
