"""Tests for ``wfx.extension.validate.validate_extension``.
...
import pytest
from wfx.extension.errors import ERROR_CODES
from wfx.extension.validate import validate_extension
...
def test_execute_imports_runs_in_subprocess_and_isolates_state(tmp_path: Path) -> None:
    """Subprocess runs the bundle but does not inherit primeagent server state.

    Two assertions:

    1. A canary written by the bundle's import-time code DOES appear (proving
       the subprocess actually ran the module).
    2. The subprocess does NOT inherit PRIMEAGENT_* env vars from the parent.
    """
    canary = tmp_path / "canary.txt"
    ...
    src = (
        ...
        "from pathlib import Path\n"
        f"Path({str(canary)!r}).write_text('triggered', encoding='utf-8')\n"
        f"# Should be empty since PRIMEAGENT_* is filtered out:\n"
        f"_state = os.environ.get('PRIMEAGENT_DATABASE_URL', '')\n"
        f"if _state:\n"
    ...
    os.environ["PRIMEAGENT_DATABASE_URL"] = "sqlite:///parent-state"
    try:
        report = validate_extension(tmp_path, execute_imports=True)
    finally:
        os.environ.pop("PRIMEAGENT_DATABASE_URL", None)
    ...
    # ...but did not inherit primeagent server state:
    assert not secret_canary.exists(), "--execute-imports leaked PRIMEAGENT_* env vars into the bundle subprocess"
