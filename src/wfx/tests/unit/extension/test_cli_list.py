"""End-to-end CLI tests for ``wfx extension list``.
...
unit tests for :mod:`wfx.extension.discovery` already exercise the
...
def test_list_text_output_with_three_seed_bundles(runner: CliRunner, tmp_path: Path) -> None:
    ...
    for index, bundle in enumerate(("openai", "anthropic", "qdrant")):
        _seed_subdir(seed, f"wfx-{bundle}", bundle, version=f"1.{index}.0")
    ...
    for bundle in ("wfx-openai", "wfx-anthropic", "wfx-qdrant"):
        assert bundle in stdout
...
def test_list_json_output_with_three_seed_bundles(runner: CliRunner, tmp_path: Path) -> None:
    ...
    for bundle in ("openai", "anthropic", "qdrant"):
        _seed_subdir(seed, f"wfx-{bundle}", bundle)
...
def test_list_reports_malformed_manifest(runner: CliRunner, tmp_path: Path) -> None:
    ...
    _seed_subdir(seed, "wfx-openai", "openai")

    result = runner.invoke(app, ["extension", "list", "--seed-dir", str(seed)])
    assert result.exit_code == 1
    assert "wfx-openai" in result.stdout
...
def test_list_text_output_includes_interpreter_info(runner: CliRunner, tmp_path: Path) -> None:
    """Operators hit the wrong-venv footgun when ``wfx`` and ``primeagent run`` diverge.
...
def test_list_exits_zero_when_only_warning_is_seed_bundle_shadowed(
    ...
    Any CI script doing ``wfx extension list && ...`` would otherwise break
    ...
    real_discover_all = __import__("wfx.extension", fromlist=["discover_all_extensions"]).discover_all_extensions
    ...
    # The CLI command imports ``discover_all_extensions`` from ``wfx.extension``
    # at call time, so patching that name suffices.
    monkeypatch.setattr("wfx.extension.discover_all_extensions", _shadowed_discovery)
