...
import pytest
from wfx.extension import errors as errors_mod
from wfx.extension import reload as reload_mod
from wfx.extension.bundle_registry import BundleRegistry
from wfx.extension.errors import ExtensionError
from wfx.extension.reload import ReloadInProgressError, ReloadResult, reload_bundle
...
def _scrub_synthetic_modules() -> Iterable[None]:
    ...
    yield
    for name in [m for m in sys.modules if m.startswith(("_wfx_ext.", "__reload_staging__."))]:
...
def test_reload_publishes_namespaced_module_in_sys_modules(tmp_path: Path) -> None:
    ...
    assert component.module_name.startswith("_wfx_ext.")
...
def test_emit_uses_user_keyspace_and_full_payload(
    ...
    * P2 -- payload is the full ReloadResult envelope so the frontend can read
    ...
    monkeypatch.setattr(
        "wfx.services.deps.get_extension_events_service",
        lambda: _FakeSvc(),
    )
...
def test_extra_slot_reload_uses_bundle_directory(tmp_path: Path) -> None:
    ...
    PRIMEAGENT_COMPONENTS_PATH entry.
    """
...
def test_swap_retags_class_module_to_prod_namespace(tmp_path: Path) -> None:
    ...
    assert component.klass.__module__.startswith("_wfx_ext.")
...
def _isolated_post_swap_hooks() -> Iterable[None]:
    ...
    When the full wfx test suite runs, primeagent startup may have already
    registered ``_post_reload_refresh_cache`` -- which then raises on these
    synthetic Component stubs (they don't inherit from the real wfx
...
def test_swap_rollback_on_mid_rename_baseexception_restores_sys_modules() -> None:
    ...
    in :func:`wfx.extension.reload_swap.swap_sys_modules` must fully
    ...
    * ``module.__name__`` mutations that landed in earlier iterations
    ...
    from wfx.extension import reload_swap
    from wfx.extension.bundle_registry import BundleRecord
    from wfx.extension.loader import SLOT_OFFICIAL, LoadedComponent

    target_ns = "_wfx_ext.rollback_pilot"
    ...
    # Cleanup (the autouse fixture handles ``_wfx_ext.*`` and
