...
from wfx.extension.discovery import DiscoveredExtension
from wfx.extension.errors import ExtensionError
from wfx.extension.manifest import ExtensionManifest, ManifestSource
...
def test_register_installed_pins_extension_at_official_slot(tmp_path: Path) -> None:
    discovered = _make_discovered(
        extension_id="wfx-openai",
        bundle_name="openai",
        source_kind="installed",
        source="wfx-openai",
        root=tmp_path / "wfx_openai",
    )
    registry = ExtensionRegistry()
    registered = registry.register_installed(discovered)

    assert registered.extension_id == "wfx-openai"
...
def test_register_seed_pins_extension_at_official_slot(tmp_path: Path) -> None:
    discovered = _make_discovered(
        extension_id="wfx-anthropic",
        bundle_name="anthropic",
        source_kind="seed",
        source=str(tmp_path / "seed/wfx_anthropic"),
        root=tmp_path / "seed/wfx_anthropic",
    )
    registry = ExtensionRegistry()
...
def test_registration_validates_source_kind(tmp_path: Path) -> None:
    ...
    seed_record = _make_discovered(
        ...
        bundle_name="xx",
        source_kind="seed",
        source=str(tmp_path / "seed/wfx_xx"),
        root=tmp_path / "seed/wfx_xx",
    )
    installed_record = _make_discovered(
        ...
        source_kind="installed",
        source="wfx-yy",
        root=tmp_path / "site-packages/wfx_yy",
    )
    registry = ExtensionRegistry()
...
def test_duplicate_extension_id_raises_typed_error(tmp_path: Path) -> None:
    a = _make_discovered(
        ...
        source_kind="installed",
        source="wfx-openai",
        root=tmp_path / "site-packages/wfx_openai",
    )
    b = _make_discovered(
        extension_id="wfx-openai",
        bundle_name="openai",
        source_kind="seed",
        source=str(tmp_path / "seed/wfx_openai"),
        root=tmp_path / "seed/wfx_openai",
    )
    registry = ExtensionRegistry()
    ...
    assert exc_info.value.error.content == "wfx-openai"
...
def test_installed_extension_is_immutable(tmp_path: Path, verb: str) -> None:
    ...
    with pytest.raises(ExtensionImmutableError) as exc_info:
        _invoke_mutation_verb(registry, verb, "wfx-openai")
...
def test_seed_extension_is_immutable(tmp_path: Path, verb: str) -> None:
    ...
    seed_root = tmp_path / "seed/wfx_anthropic"
    discovered = _make_discovered(
        extension_id="wfx-anthropic",
        bundle_name="anthropic",
        source_kind="seed",
    ...
    with pytest.raises(ExtensionImmutableError) as exc_info:
        _invoke_mutation_verb(registry, verb, "wfx-anthropic")
    assert exc_info.value.error.code == "seed-directory-immutable"
    assert exc_info.value.error.content == "wfx-anthropic"
...
def test_mark_loaded_replaces_load_status(tmp_path: Path) -> None:
    ...
    updated = registry.mark_loaded("wfx-openai")
    assert updated.load_status is LoadStatus.LOADED
    assert registry.get("wfx-openai") == updated
...
def test_mark_failed_attaches_typed_error(tmp_path: Path) -> None:
    ...
    error = ExtensionError(
        ...
        location=str(tmp_path / "wfx_openai/openai"),
        content="openai",
    )
    updated = registry.mark_failed("wfx-openai", error=error)
...
def test_build_registry_from_discovery_pins_three(tmp_path: Path) -> None:
    discovered = [
        ...
        _make_discovered(
            ...
            bundle_name="anthropic",
            source_kind="installed",
            source="wfx-anthropic",
            root=tmp_path / "site-packages/wfx_anthropic",
        ),
        _make_discovered(
            extension_id="wfx-qdrant",
            bundle_name="qdrant",
            source_kind="seed",
            source=str(tmp_path / "seed/wfx_qdrant"),
            root=tmp_path / "seed/wfx_qdrant",
        ),
    ]
...
def test_build_registry_from_discovery_collects_duplicate_errors(tmp_path: Path) -> None:
    ...
    registry, errors = build_registry_from_discovery(discovered)
    assert {ext.extension_id for ext in registry.list_extensions()} == {"wfx-openai"}
    assert len(errors) == 1
