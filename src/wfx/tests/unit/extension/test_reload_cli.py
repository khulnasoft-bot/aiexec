"""Tests for the reload CLI: HTTP client + ``wfx extension reload``."""

from __future__ import annotations

import pytest
from wfx.cli._extension_reload_client import (
    DEFAULT_TARGET,
    ReloadHttpResponse,
    reload_via_http,
    resolve_api_key,
...
def test_resolve_target_prefers_explicit(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("PRIMEAGENT_HOST", "http://from-env")
    assert resolve_target("http://explicit/") == "http://explicit"


def test_resolve_target_uses_primeagent_host(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("PRIMEAGENT_HOST", "http://from-env/")
    monkeypatch.delenv("PRIMEAGENT_SERVER_URL", raising=False)
...
def patched_httpx(monkeypatch: pytest.MonkeyPatch) -> dict[str, _StubClient | None]:
    ...
    def _factory_for(response: _StubResponse) -> None:
        ...
        monkeypatch.setattr(
            "wfx.cli._extension_reload_client.httpx.Client",
...
def test_reload_via_http_success(patched_httpx: dict, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert captured["url"] == "http://server/api/v1/extensions/wfx-pilot/bundles/pilot/reload"
