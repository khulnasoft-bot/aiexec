"""End-to-end CLI tests for ``wfx extension validate`` and ``wfx extension schema``."""
...
def test_validate_json_output(runner: CliRunner, good_extension: Path) -> None:
    ...
    assert payload["manifest"]["id"] == "wfx-openai"
...
def isolated_dev_registry(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    state_dir.mkdir()
    monkeypatch.setenv("PRIMEAGENT_DEV_EXTENSIONS_DIR", str(state_dir))
    return state_dir
...
# ---------------------------------------------------------------------------
# dev launch env: contract for the env vars handed to ``primeagent run``
# ---------------------------------------------------------------------------


def test_dev_launch_env_enables_reload_and_eager_loading() -> None:
    """The launched primeagent inherits flags that make 'edit -> Reload' work.

    Without ``PRIMEAGENT_ENABLE_EXTENSION_RELOAD=true`` the backend reload
    ...
    showing the Reload button.  Without ``PRIMEAGENT_LAZY_LOAD_COMPONENTS=false``
    dev components miss the palette's 5-second budget.
    """
    from wfx.cli._extension_commands import _build_dev_launch_env

    env = _build_dev_launch_env({})

    assert env["PRIMEAGENT_LAZY_LOAD_COMPONENTS"] == "false"
    assert env["PRIMEAGENT_ENABLE_EXTENSION_RELOAD"] == "true"


def test_dev_launch_env_overrides_author_lazy_loading() -> None:
    """``PRIMEAGENT_LAZY_LOAD_COMPONENTS=true`` in the author shell must not win.

    The dev workflow always wants eager loading; an author whose shell
    exports lazy loading for normal primeagent use should not silently
    lose dev components from the palette.
    """
    from wfx.cli._extension_commands import _build_dev_launch_env

    env = _build_dev_launch_env({"PRIMEAGENT_LAZY_LOAD_COMPONENTS": "true"})
    assert env["PRIMEAGENT_LAZY_LOAD_COMPONENTS"] == "false"
...
def test_dev_launch_env_respects_author_reload_off_path() -> None:
    ...
    env = _build_dev_launch_env({"PRIMEAGENT_ENABLE_EXTENSION_RELOAD": "false"})
    assert env["PRIMEAGENT_ENABLE_EXTENSION_RELOAD"] == "false"
...
def test_reload_all_rejects_extra_args(runner: CliRunner) -> None:
    ...
    result = runner.invoke(app, ["extension", "reload", "--all", "wfx-pilot"])
    assert result.exit_code == 2
    msg = result.stderr or result.stdout
    assert "--all" in msg
...
def test_reload_id_not_in_local_discovery_errors_clean(runner: CliRunner, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from wfx.cli import _extension_commands as commands

    monkeypatch.setattr(
        "wfx.extension.discover_all_extensions",
        lambda: ([], []),
    )
    ...
    result = runner.invoke(app, ["extension", "reload", "wfx-not-installed"])
    assert result.exit_code == 1
    msg = result.stderr or result.stdout
    assert "wfx-not-installed" in msg
