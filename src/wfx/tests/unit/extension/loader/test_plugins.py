"""Tests for manifest-first precedence over ``primeagent.plugins``.
...
from wfx.extension.loader._plugins import canonicalize_distribution
...
def test_finds_manifest_shipping_distribution(tmp_path: Path) -> None:
    dist = make_installed_extension(tmp_path, "wfx-pilot")
    roots = installed_extension_roots(distributions=[dist])
    assert "wfx-pilot" in roots
    assert (roots["wfx-pilot"] / "extension.json").is_file()


def test_canonicalizes_distribution_name(tmp_path: Path) -> None:
    """PEP-503: ``Wfx_Pilot`` and ``wfx-pilot`` are the same distribution."""
    dist = make_installed_extension(tmp_path, "wfx-pilot")
    # Override the metadata Name to test the canonicalize path.
    dist._name = "WFX_Pilot"
    roots = installed_extension_roots(distributions=[dist])
    assert list(roots) == ["wfx-pilot"]
...
def test_finds_pyproject_only_distribution(tmp_path: Path) -> None:
    """A distribution shipping pyproject.toml with [tool.primeagent.extension] is found."""
    dist = make_installed_pyproject_extension(tmp_path, "wfx-pyproject")
    roots = installed_extension_roots(distributions=[dist])
    assert "wfx-pyproject" in roots
    assert (roots["wfx-pyproject"] / "pyproject.toml").is_file()


def test_pyproject_without_extension_section_is_ignored(tmp_path: Path) -> None:
    """A pyproject.toml without [tool.primeagent.extension] is NOT treated as a manifest.
...
def test_malformed_pyproject_section_surfaces_manifest_invalid(tmp_path: Path) -> None:
    """A pyproject with [tool.primeagent.extension] missing required fields surfaces manifest-invalid.
    ...
    """
    from wfx.extension import load_installed_extensions

    dist = make_installed_pyproject_malformed_extension(tmp_path, "wfx-bad-pyproject")
    ...
    assert "wfx-bad-pyproject" in owners
    ...
    assert result.distribution == "wfx-bad-pyproject"
...
def test_malformed_pyproject_section_suppresses_legacy_component_entry_point(tmp_path: Path) -> None:
    ...
    its legacy ``primeagent.plugins`` component entry-points must be
...
def test_extension_json_wins_over_pyproject(tmp_path: Path) -> None:
    ...
    dist = FakeDist(
        name="wfx-both",
        root=tmp_path,
        files=[Path("wfx-both") / "extension.json", Path("wfx-both") / "pyproject.toml"],
    )
    roots = installed_extension_roots(distributions=[dist])
    # Both forms point at the same root directory, but the manifest path
    # used for resolution should be the JSON file.
    assert "wfx-both" in roots
    assert (roots["wfx-both"] / "extension.json").is_file()
...
def test_pyproject_only_extension_loads_at_official_slot(tmp_path: Path) -> None:
    ...
    Without this path the AC's "extension.json or [tool.primeagent.extension]"
    ...
    bundle_dir = tmp_path / "wfx-pyproject" / "components"
    ...
    assert result.distribution == "wfx-pyproject"
    assert result.extension_id == "wfx-pyproject"
    # Bundle name comes from the pyproject manifest section.
    assert result.bundle == "wfx_pyproject"
...
def test_pyproject_only_distribution_suppresses_legacy_component_entry_point(tmp_path: Path) -> None:
    ...
    A legacy ``primeagent.plugins`` component entry-point on a pyproject-form
    ...
    owners = manifest_owning_distributions(distributions=[dist])
    assert "wfx-pyproject" in owners
    kept, skipped = filter_component_entry_points([component_ep], skip=owners)
...
def test_owning_distributions_is_set_of_canonical_names(tmp_path: Path) -> None:
    ...
    assert owners == frozenset({"wfx-pilot"})
...
def test_skips_manifest_shipping_distribution(tmp_path: Path) -> None:
    ...
    """
    manifest_dist = make_installed_extension(tmp_path, "wfx-pilot")
...
def test_keeps_entry_points_without_distribution() -> None:
    ...
    kept, skipped = filter_plugin_entry_points([ep], skip=frozenset({"wfx-pilot"}))
    assert kept == [ep]
    assert skipped == []
...
def test_uses_real_distributions_by_default(tmp_path: Path) -> None:
    ...
    Primeagent itself does not ship an extension manifest today, but if a
...
def test_canonicalize_distribution_matches_pep503() -> None:
    assert canonicalize_distribution("Wfx-Pilot") == "wfx-pilot"
    assert canonicalize_distribution("wfx_pilot") == "wfx-pilot"
    assert canonicalize_distribution("WFX...PILOT") == "wfx-pilot"
...
def test_component_filter_keeps_components_on_non_manifest_distribution(tmp_path: Path) -> None:
    ...
    The legacy ``primeagent.plugins`` path is still the registration channel
