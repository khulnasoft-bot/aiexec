...
piece -- it strips the loader's ``_wfx_ext.<slot>...`` modules from
...
def component_source(class_name: str = "PilotThing", *, with_build: bool = True) -> str:
    ...
    subclass check fires without importing the real wfx Component (which
...
def make_installed_pyproject_extension(parent: Path, distribution_name: str) -> FakeDist:
    ...
    ``[tool.primeagent.extension]`` discovery path. ``files`` points at a
...
def make_installed_pyproject_no_extension(parent: Path, distribution_name: str) -> FakeDist:
    """Fake distribution shipping pyproject.toml WITHOUT [tool.primeagent.extension].
...
def make_installed_pyproject_malformed_extension(
    ...
    """Fake distribution shipping pyproject with [tool.primeagent.extension] missing required fields.
...
[tool.primeagent.extension]
# Intentionally missing id/version/wfx/bundles -- pydantic must reject.
...
def _scrub_synthetic_modules() -> Iterable[None]:
    ...
    The loader installs each bundle module under ``_wfx_ext.<slot>.<bundle>...``
    ...
    """
    yield
    for name in [m for m in sys.modules if m.startswith("_wfx_ext.")]:
