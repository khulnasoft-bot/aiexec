...
``$PRIMEAGENT_SEED_DIR`` (or the default ``/opt/primeagent/bundles/``) that
...
def test_seed_default_absent_is_silent_no_op(tmp_path: Path) -> None:
    """Mode A: $PRIMEAGENT_SEED_DIR unset and the default doesn't exist -> []."""
...
def test_seed_directory_loads_each_subdirectory(tmp_path: Path) -> None:
    ...
    seed_root.mkdir()
    _seed_subdir(seed_root, "wfx-alpha", bundle_name="alpha")
    _seed_subdir(seed_root, "wfx-bravo", bundle_name="bravo")
...
def test_seed_skips_subdirectories_without_a_manifest(tmp_path: Path) -> None:
    ...
    seed_root = tmp_path / "bundles"
    seed_root.mkdir()
    _seed_subdir(seed_root, "wfx-alpha")
    # README, scripts, etc.
...
def test_seed_results_are_deterministic(tmp_path: Path) -> None:
    ...
    seed_root = tmp_path / "bundles"
    seed_root.mkdir()
    _seed_subdir(seed_root, "wfx-charlie", bundle_name="charlie")
    _seed_subdir(seed_root, "wfx-alpha", bundle_name="alpha")
    _seed_subdir(seed_root, "wfx-bravo", bundle_name="bravo")
...
def test_pathsep_split_loads_multiple_seed_roots(tmp_path: Path) -> None:
    """``$PRIMEAGENT_SEED_DIR`` is pathsep-separated for multiple roots."""
    root_a = tmp_path / "a"
    root_b = tmp_path / "b"
    root_a.mkdir()
    root_b.mkdir()
    _seed_subdir(root_a, "wfx-a", bundle_name="aa")
    _seed_subdir(root_b, "wfx-b", bundle_name="bb")
...
def test_default_seed_dir_used_when_env_unset(tmp_path: Path) -> None:
    ...
    seed_root.mkdir()
    _seed_subdir(seed_root, "wfx-default", bundle_name="defaultbundle")
