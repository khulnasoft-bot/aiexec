"""Tests for ``discover_inline_bundles`` (PRIMEAGENT_COMPONENTS_PATH).
...
def test_respects_bundle_json(tmp_path: Path) -> None:
    ...
    (bundle_dir / "bundle.json").write_text(json.dumps({"id": "wfx-alpha", "version": "9.9.9"}), encoding="utf-8")
    results = discover_inline_bundles([parent])
    assert results[0].extension_id == "wfx-alpha"
...
def test_non_existent_path_emits_inline_path_missing(tmp_path: Path) -> None:
    ...
    A typo in PRIMEAGENT_COMPONENTS_PATH would otherwise yield zero
...
def test_unreadable_path_emits_inline_path_unreadable(tmp_path: Path, monkeypatch) -> None:
    ...
    Permission-denied on a configured PRIMEAGENT_COMPONENTS_PATH entry is a
