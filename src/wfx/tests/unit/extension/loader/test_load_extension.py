...
def test_single_bundle_registers_component(tmp_path: Path) -> None:
    ...
    assert result.extension_id == "wfx-pilot"
...
def test_distribution_passed_through(tmp_path: Path) -> None:
    root = make_extension(tmp_path)
    result = load_extension(root, distribution="wfx-pilot")
    assert result.ok
    assert result.components[0].distribution == "wfx-pilot"
...
def test_runtime_multi_bundle_check(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    """
    from wfx.extension.loader import _orchestrator
...
def test_version_constraint_unsatisfied(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    """A manifest declaring wfx.compat without our BUNDLE_API_VERSION is rejected.
    ...
    so a bundle declaring only ``["2"]`` against a wfx that ships
    ...
    forged = ExtensionManifest.model_construct(
        ...
        version="1.2.3",
        name="Pilot",
        wfx=WfxCompat(compat=["999"]),  # well-formed but does not include BUNDLE_API_VERSION
...
def test_re_imported_class_is_skipped_via_module_filter(tmp_path: Path) -> None:
    ...
    coincidentally failed because the loader's synthetic ``_wfx_ext.*``
    ...
    :func:`wfx.extension.loader._detection.collect_component_classes`.
    """
    ...
    files = {
        ...
        "z_alias.py": (
            ...
            "    if name.startswith('_wfx_ext.') and name.endswith('primary')\n"
            ")\n"
