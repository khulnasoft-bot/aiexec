...
def test_loads_each_installed_extension_once(tmp_path: Path) -> None:
    ...
    dist_a = make_installed_extension(tmp_path, "wfx-alpha")
    dist_b = make_installed_extension(tmp_path, "wfx-bravo")
    _populate_bundle(tmp_path, "wfx-alpha")
    _populate_bundle(tmp_path, "wfx-bravo")

    results = load_installed_extensions(distributions=[dist_a, dist_b])
    by_dist = {r.distribution: r for r in results}
    assert set(by_dist) == {"wfx-alpha", "wfx-bravo"}
...
def test_duplicate_distribution_emits_warning(tmp_path: Path) -> None:
    ...
    result = results[0]
    assert result.distribution == "wfx-pilot"
...
def test_editable_install_discovered_via_direct_url(tmp_path: Path) -> None:
    ...
    the source path.  Without this fallback, an ``wfx-duckduckgo`` linked
    ...
    dist = _EditableDist("wfx-pilot", project_root)
...
def test_results_are_lexicographically_ordered(tmp_path: Path) -> None:
    ...
    for name in ("wfx-zeta", "wfx-alpha", "wfx-mike"):
        make_installed_extension(tmp_path, name)
        _populate_bundle(tmp_path, name)
    dists = [
        FakeDist(name="wfx-zeta", root=tmp_path, files=[Path("wfx-zeta") / "extension.json"]),
        FakeDist(name="wfx-alpha", root=tmp_path, files=[Path("wfx-alpha") / "extension.json"]),
        FakeDist(name="wfx-mike", root=tmp_path, files=[Path("wfx-mike") / "extension.json"]),
    ]
    results = load_installed_extensions(distributions=dists)
    assert [r.distribution for r in results] == ["wfx-alpha", "wfx-mike", "wfx-zeta"]
