...
def test_table_lookup_helpers() -> None:
    ...
    imp = MigrationEntry(
        import_path="primeagent.components.utilities.MergeDataComponent",
        target="ext:utilities:MergeDataComponent@official",
        added_in="1.10.0",
    )
    legacy = MigrationEntry(
    ...
    assert table.lookup_import_path("primeagent.components.utilities.MergeDataComponent") is imp
    ...
    assert sorted(table.all_known_legacy_values()) == sorted(
        [
            "MergeDataComponent",
            "primeagent.components.utilities.MergeDataComponent",
