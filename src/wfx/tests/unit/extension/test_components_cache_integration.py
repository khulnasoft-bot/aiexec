...
inherit from the real ``wfx.custom.custom_component.component.Component``
...
def test_decorate_template_stamps_required_fields() -> None:
    ...
    decorated = _decorate_template_with_extension(
        template,
        extension_id="wfx-pilot",
        bundle="pilot",
        extension_version="1.2.3",
        namespaced_id="ext:pilot:PilotThing@official",
    )
    assert decorated["extension"] == "wfx-pilot"
...
async def test_inline_bundle_components_decorated_with_extension_metadata(tmp_path: Path) -> None:
    ...
    with patch("wfx.interface.components.create_component_template", side_effect=_stub_template):
        result = await import_extension_components(settings_service)
...
async def test_components_path_empty_string_does_not_crash(monkeypatch) -> None:
    ...
    monkeypatch.setenv("PRIMEAGENT_COMPONENTS_PATH", os.pathsep)
    settings_service = _FakeSettingsService(components_path=[])
...
async def test_seed_directory_bundle_loads_at_official_slot(tmp_path: Path, monkeypatch) -> None:
    """A subdirectory under ``$PRIMEAGENT_SEED_DIR`` registers at @official.
    ...
    bundles into ``/opt/primeagent/bundles/`` (the Mode B/C alternative to
    ``pip install``) sees the docs say it works while no components actually
    appear in the palette.
    """
    import json

    from wfx.extension.bundle_registry import BundleRegistry

    seed_root = tmp_path / "seed-bundles"
    extension_root = seed_root / "wfx-pilot"
    ...
    monkeypatch.setenv("PRIMEAGENT_SEED_DIR", str(seed_root))

    fresh_registry = BundleRegistry()
    settings_service = _FakeSettingsService(components_path=[])

    with (
        patch("wfx.interface.components.create_component_template", side_effect=_stub_template),
        patch("wfx.interface.components.get_default_registry", return_value=fresh_registry),
    ):
        result = await import_extension_components(settings_service)
    ...
    assert template["extension"] == "wfx-pilot"
    ...
    assert record.extension_id == "wfx-pilot"
...
async def test_seed_bundle_shadowed_by_installed_emits_typed_warning(tmp_path: Path, monkeypatch) -> None:
    ...
    from wfx.extension.loader._types import LoadedComponent, LoadResult
    ...
    installed_loaded = LoadedComponent(
        ...
        module_name="_wfx_ext.official.pilot.thing",
        file_path=tmp_path / "installed_thing.py",
        distribution="wfx-pilot",
    )
    ...
    installed_result.extension_id = "wfx-pilot"
    ...
    with (
        patch("wfx.interface.components.create_component_template", side_effect=_stub_template),
        patch("wfx.interface.components.get_default_registry", return_value=fresh_registry),
        patch("wfx.interface.components.load_installed_extensions", return_value=[installed_result]),
        patch("wfx.interface.components._emit_extension_diagnostics", side_effect=_capture_diagnostics),
    ):
    ...
    assert record.distribution == "wfx-pilot"
...
async def test_seed_bundle_shadows_dev_emits_generic_bundle_shadowed(tmp_path: Path, monkeypatch) -> None:
    ...
    seed_root = tmp_path / "seed-bundles"
    seed_extension_root = seed_root / "wfx-pilot"
    ...
    dev_result.extension_id = "wfx-pilot"
    ...
    with (
        patch("wfx.interface.components.create_component_template", side_effect=_stub_template),
        patch("wfx.interface.components.get_default_registry", return_value=fresh_registry),
        patch("wfx.interface.components.load_dev_extensions", return_value=[dev_result]),
        patch("wfx.interface.components._emit_extension_diagnostics", side_effect=_capture_diagnostics),
    ):
        await import_extension_components(settings_service)
...
async def test_import_extension_components_populates_bundle_registry(tmp_path: Path) -> None:
    ...
    nothing populates.
    """
    from wfx.extension.bundle_registry import BundleRegistry, get_default_registry
...
def test_post_swap_hook_refreshes_component_cache(tmp_path: Path) -> None:
    ...
    """
    from wfx.extension.bundle_registry import BundleRecord
    from wfx.extension.loader._types import LoadedComponent
    from wfx.interface.components import (
        component_cache,
    ...
    loaded = LoadedComponent(
        extension_id="wfx-gamma",
        extension_version="1.0.0",
        bundle="gamma",
        class_name="GammaThing",
        slot="official",
        klass=_GammaThing,
        module_name="_wfx_ext.official.gamma.thing",
...
def test_post_swap_hook_invalidates_hash_lookups(tmp_path: Path) -> None:
    ...
    loaded = LoadedComponent(
        extension_id="wfx-delta",
        extension_version="1.0.0",
        bundle="delta",
        class_name="DeltaThing",
        slot="official",
        klass=_DeltaThing,
        module_name="_wfx_ext.official.delta.thing",
        file_path=tmp_path / "thing.py",
        distribution=None,
    )
    record = BundleRecord(
...
def test_refresh_cache_preserves_entry_on_total_failure(tmp_path: Path) -> None:
    ...
    pre_existing = {"ext:zeta:OldThing@official": {"display_name": "old", "extension": "wfx-zeta"}}
    ...
    loaded = LoadedComponent(
        extension_id="wfx-zeta",
        extension_version="1.0.0",
        bundle="zeta",
        class_name="ZetaThing",
        slot="official",
        klass=_ZetaThing,
        module_name="_wfx_ext.official.zeta.thing",
        file_path=tmp_path / "thing.py",
        distribution=None,
    )
    record = BundleRecord(
    ...
    with (
        patch("wfx.interface.components.create_component_template", side_effect=_always_raises),
...
def test_refresh_cache_writes_partial_on_partial_failure(tmp_path: Path) -> None:
    ...
    good = LoadedComponent(
        extension_id="wfx-eta",
        extension_version="1.0.0",
        bundle="eta",
        class_name="Good",
        slot="official",
        klass=_Good,
        module_name="_wfx_ext.official.eta.good",
    ...
    bad = LoadedComponent(
        ...
        slot="official",
        klass=_Bad,
        module_name="_wfx_ext.official.eta.bad",
    ...
    with patch("wfx.interface.components.create_component_template", side_effect=_template):
        refresh_bundle_cache_from_record(record)
...
async def test_dev_extension_components_loaded_via_official_slot(
    ...
    """Dev extensions registered via ``wfx extension dev`` must enter the @official pathway.

    Earlier wiring appended their bundle directories to PRIMEAGENT_COMPONENTS_PATH,
    ...
    from wfx.extension import register_dev_extension
    from wfx.extension.bundle_registry import BundleRegistry
    ...
    monkeypatch.setenv("PRIMEAGENT_DEV_EXTENSIONS_DIR", str(tmp_path / "registry"))
    ...
    assert result["devbundle"][expected_id]["extension"] == "wfx-dev-fixture"
