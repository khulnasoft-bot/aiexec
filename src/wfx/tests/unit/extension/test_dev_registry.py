"""Tests for ``wfx.extension.dev_registry``.
...
def isolated_state_dir(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    ``PRIMEAGENT_DEV_EXTENSIONS_DIR`` to a tmp_path subdirectory.
    """
...
def test_register_is_idempotent_and_refreshes_timestamp(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    import wfx.extension.dev_registry as dr
...
def test_dev_extension_component_paths_forwards_all_warnings(
    ...
    from wfx.extension import dev_registry as dr
...
def test_state_dir_env_var_takes_precedence(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("PRIMEAGENT_DEV_EXTENSIONS_DIR", str(override))
    assert state_file_path().parent == override
    # The override is honored even when PRIMEAGENT_CONFIG_DIR is also set.
    monkeypatch.setenv("PRIMEAGENT_CONFIG_DIR", str(tmp_path / "ignore-me"))
    assert state_file_path().parent == override


def test_config_dir_fallback(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("PRIMEAGENT_DEV_EXTENSIONS_DIR", raising=False)
    cfg = tmp_path / "cfg"
    monkeypatch.setenv("PRIMEAGENT_CONFIG_DIR", str(cfg))
