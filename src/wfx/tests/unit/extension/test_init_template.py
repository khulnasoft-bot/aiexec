"""Tests for ``wfx.extension.init_template``.
...
def test_manifest_carries_schema_pointer(tmp_path: Path) -> None:
    ...
    assert "schemas.prime.khulnasoft.com/extension/v1.json" in payload
...
def test_init_template_regexes_match_manifest_schema() -> None:
    ...
    """
    from wfx.extension import manifest as manifest_mod
    from wfx.extension.init_template import _BUNDLE_NAME_RE, _EXTENSION_ID_RE
...
def test_scaffolded_pyproject_is_pip_installable(tmp_path: Path) -> None:
    ...
    assert data["project"]["name"] == "wfx-my-ext"
    assert data["project"]["version"] == "0.1.0"
    # The [tool.primeagent.extension] section points back at the manifest so
    # the installed wheel is discoverable by wfx.extension.discovery.
    assert data["tool"]["primeagent"]["extension"]["manifest"] == "extension.json"
