...
class TestFieldTypingLazyImports:
    ...
    def test_input_import(self):
        ...
        from wfx.field_typing import Input
        from wfx.template.field.base import Input as DirectInput

        assert Input is DirectInput

    def test_output_import(self):
        """Test that Output can be imported from field_typing."""
        from wfx.field_typing import Output
        from wfx.template.field.base import Output as DirectOutput
    ...
    def test_range_spec_import(self):
        ...
        from wfx.field_typing.range_spec import RangeSpec as DirectRangeSpec
    ...
    def test_invalid_attribute_raises_error(self):
        ...
        import wfx.field_typing

        with pytest.raises(AttributeError, match="has no attribute 'NonExistentType'"):
            _ = wfx.field_typing.NonExistentType

    def test_all_exports_are_accessible(self):
        """Test that all items in __all__ are accessible."""
        import wfx.field_typing

        for name in wfx.field_typing.__all__:
            attr = getattr(wfx.field_typing, name)
    ...
    def test_repeated_access_returns_same_object(self):
        ...
        from wfx.field_typing import Data as Data1
        from wfx.field_typing import Data as Data2
    ...
    def test_constants_match_direct_imports(self):
        ...
        from wfx.field_typing import Data, Document, Object
        from wfx.field_typing.constants import Data as DirectData
        from wfx.field_typing.constants import Document as DirectDocument
        from wfx.field_typing.constants import Object as DirectObject
...
class TestFieldTypingNativeDependencyFailure:
    ...
    OSError raised by `torch._load_dll_libraries()` propagates upward through:
        ...
        wfx.field_typing.constants line 9 (`from langchain_classic.agents import AgentExecutor`)

    `wfx.field_typing.constants` already wraps its langchain imports in a
    ...
    def test_should_use_stub_classes_when_langchain_import_raises_oserror(self, monkeypatch):
        """Bug: `primeagent --version` crashes on fresh Windows because wfx fails to import.

        Cause: `c10.dll` raises `OSError: [WinError 126]` and the existing fallback
        only catches `ImportError`. After fix: stub classes are used and wfx loads
        ...
        monkeypatch.delitem(sys.modules, "wfx.field_typing.constants", raising=False)
        monkeypatch.delitem(sys.modules, "wfx.field_typing", raising=False)

        # Act — must not raise (this is the regression guard)
        from wfx.field_typing import constants

        # Assert — the in-module stub class is what's exposed, not the real one
        assert constants.AgentExecutor is not None
        assert constants.AgentExecutor.__module__ == "wfx.field_typing.constants"
...
class TestFieldTypingModuleStructure:
    ...
    def test_all_list_exists(self):
        ...
        assert hasattr(wfx.field_typing, "__all__")
        assert isinstance(wfx.field_typing.__all__, list)
        assert len(wfx.field_typing.__all__) > 0
    ...
    def test_getattr_function_exists(self):
        ...
        assert hasattr(wfx.field_typing, "Input") or "Input" in wfx.field_typing.__all__

    def test_constants_names_set_exists(self):
        """Test that _CONSTANTS_NAMES is defined."""
        import wfx.field_typing

        assert hasattr(wfx.field_typing, "_CONSTANTS_NAMES")
        assert isinstance(wfx.field_typing._CONSTANTS_NAMES, set)
