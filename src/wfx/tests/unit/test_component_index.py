...
class TestParseDevMode:
    ...
    def test_dev_mode_not_set(self, monkeypatch):
        """Test default behavior when WFX_DEV is not set."""
        monkeypatch.delenv("WFX_DEV", raising=False)
        enabled, modules = _parse_dev_mode()
        assert enabled is False
        assert modules is None

    def test_dev_mode_enabled_with_1(self, monkeypatch):
        """Test dev mode enabled with WFX_DEV=1."""
        monkeypatch.setenv("WFX_DEV", "1")
        enabled, modules = _parse_dev_mode()
        assert enabled is True
        assert modules is None  # Load all modules

    def test_dev_mode_enabled_with_true(self, monkeypatch):
        """Test dev mode enabled with WFX_DEV=true."""
        monkeypatch.setenv("WFX_DEV", "true")
        enabled, modules = _parse_dev_mode()
        assert enabled is True
        assert modules is None

    def test_dev_mode_enabled_with_yes(self, monkeypatch):
        """Test dev mode enabled with WFX_DEV=yes."""
        monkeypatch.setenv("WFX_DEV", "yes")
        enabled, modules = _parse_dev_mode()
        assert enabled is True
        assert modules is None

    def test_dev_mode_disabled_with_0(self, monkeypatch):
        """Test dev mode disabled with WFX_DEV=0."""
        monkeypatch.setenv("WFX_DEV", "0")
        enabled, modules = _parse_dev_mode()
        assert enabled is False
        assert modules is None

    def test_dev_mode_disabled_with_false(self, monkeypatch):
        """Test dev mode disabled with WFX_DEV=false."""
        monkeypatch.setenv("WFX_DEV", "false")
        enabled, modules = _parse_dev_mode()
        assert enabled is False
        assert modules is None
    ...
    def test_dev_mode_single_module(self, monkeypatch):
        ...
        monkeypatch.setenv("WFX_DEV", "mistral")
        enabled, modules = _parse_dev_mode()
        assert enabled is True
    ...
    def test_dev_mode_multiple_modules(self, monkeypatch):
        ...
        monkeypatch.setenv("WFX_DEV", "mistral,openai,anthropic")
        enabled, modules = _parse_dev_mode()
        assert enabled is True
    ...
    def test_dev_mode_modules_case_normalized(self, monkeypatch):
        ...
        monkeypatch.setenv("WFX_DEV", "Mistral,OpenAI")
        enabled, modules = _parse_dev_mode()
        assert enabled is True
...
class TestReadComponentIndex:
    ...
    def test_read_index_file_not_found(self):
        ...
        with patch("wfx.interface.components.Path") as mock_path_class:
    ...
    def test_read_index_valid(self, tmp_path):
        ...
        with (
            patch("wfx.interface.components.inspect.getfile") as mock_getfile,
            patch("importlib.metadata.version") as mock_version,
        ):
            mock_getfile.return_value = str(tmp_path / "wfx" / "__init__.py")
            mock_version.return_value = "0.1.12"

            # Create the directory structure
            (tmp_path / "wfx" / "_assets").mkdir(parents=True)
            (tmp_path / "wfx" / "_assets" / "component_index.json").write_bytes(
    ...
    def test_read_index_invalid_sha256(self, tmp_path):
        ...
        with (
            ...
            (tmp_path / "wfx" / "_assets").mkdir(parents=True)
            (tmp_path / "wfx" / "_assets" / "component_index.json").write_bytes(orjson.dumps(index))
    ...
    def test_read_index_package_not_found(self, tmp_path):
        """Test reading index when wfx package metadata is unavailable (e.g. Docker workspace install)."""
        ...
        with (
            ...
            mock_getfile.return_value = str(tmp_path / "wfx" / "__init__.py")
            mock_version.side_effect = PackageNotFoundError("wfx")
...
class TestSaveGeneratedIndex:
    ...
    def test_save_generated_index(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("wfx.interface.components._get_cache_path", lambda: cache_file)
...
class TestImportPrimeagentComponents:
    ...
    async def test_import_with_dev_mode(self, monkeypatch):
        ...
        with patch("wfx.interface.components._process_single_module") as mock_process:
            mock_process.return_value = ("category1", {"comp1": {"template": {}}})

            with (
                patch("wfx.interface.components.pkgutil.walk_packages") as mock_walk,
                patch("wfx.interface.components._save_generated_index") as mock_save,
            ):
    ...
    async def test_import_with_builtin_index(self, monkeypatch):
        ...
        with (
            patch("wfx.interface.components._read_component_index") as mock_read,
            patch("importlib.metadata.version", return_value="0.1.12"),
        ):
    ...
    async def test_import_with_missing_index_creates_cache(self, tmp_path, monkeypatch):
        ...
        with (
            patch("wfx.interface.components._read_component_index") as mock_read,
            patch("wfx.interface.components._process_single_module") as mock_process,
            patch("wfx.interface.components.pkgutil.walk_packages") as mock_walk,
            patch("importlib.metadata.version", return_value="0.1.12"),
        ):
