"""Tests for wfx.processing.process.apply_tweaks.

These mirror the primeagent-side regression tests in
``src/backend/tests/unit/test_process.py`` so the two copies of ``apply_tweaks``
(``primeagent.processing.process`` used by the API and ``wfx.processing.process``)
...
from wfx.processing.process import apply_tweaks
from wfx.utils.flow_validation import CODE_EXECUTION_COMPONENT_TYPES, CODE_EXECUTION_FIELD_NAMES
...
def test_apply_tweaks_blocks_code_named_field():
    ...
    with patch("wfx.processing.process.logger") as mock_logger:
...
# CODE_EXECUTION_FIELD_NAMES (see wfx/utils/flow_validation.py). "code" is the
...
def test_every_code_execution_type_has_registered_code_fields():
    ...
    for component_type, code_fields in _EXPECTED_CODE_FIELDS_BY_TYPE.items():
        ...
        assert not missing, (
            ...
            f"guard would not block. Add them to CODE_EXECUTION_FIELD_NAMES in wfx/utils/flow_validation.py."
        )
