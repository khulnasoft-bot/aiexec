...
from wfx.extension.bundle_registry import BundleRecord, get_default_registry
from wfx.extension.reload import register_post_swap_hook
from wfx.log.logger import logger
from wfx.utils.flow_validation import collect_code_by_hash, collect_component_hash_lookups
...
def _post_reload_refresh_cache(record: BundleRecord) -> None:
    ...
    needing to thread it through ``wfx.extension.reload``.  Concrete logic
...
def _read_component_index(custom_path: str | None = None) -> dict | None:
    ...
    try:
        ...
        try:
            ...
            # wfx may be importable but lack dist-info metadata. Skip version check.
            logger.debug("Could not determine installed wfx version (no package metadata); skipping version check")
...
def _warm_circular_imports() -> None:
    ...
    cleanly when first imported from a single thread, but the wfx policy modules reach
...
def _components_path_extension_paths(settings_service: "SettingsService") -> list[Path]:
    ...
    Primeagent components path is excluded (it is not an inline-bundle root).
...
# distribution = explicit, packaged install) to least (PRIMEAGENT_COMPONENTS_PATH
...
async def import_extension_components(
    ...
    Two sources feed this:
        ...
        - Subfolders of every ``PRIMEAGENT_COMPONENTS_PATH`` entry (parsed
    ...
    # ``$PRIMEAGENT_SEED_DIR`` (or the default ``/opt/primeagent/bundles``)
    ...
    # When neither $PRIMEAGENT_SEED_DIR is set nor /opt/primeagent/bundles
    # exists this is a no-op, so it costs nothing in Mode A.
    seed_results = load_seed_extensions()
    # Dev extensions registered via ``wfx extension dev`` ship the same v0
    ...
    # approach that appended their bundle directories to PRIMEAGENT_COMPONENTS_PATH,
    ...
    # dev (`wfx extension dev`) > inline (PRIMEAGENT_COMPONENTS_PATH) -- and the
    ...
    for result in all_results:
        ...
        for loaded in result.components:
            try:
                ...
                # doesn't actually inherit from the wfx Component base will
...
def refresh_bundle_cache_from_record(record: "BundleRecord") -> None:
    ...
    The reload pipeline updates :class:`~wfx.extension.bundle_registry.BundleRegistry`
    ...
    server restart.  Called by :func:`wfx.extension.reload.reload_bundle`
    in Stage 5.
