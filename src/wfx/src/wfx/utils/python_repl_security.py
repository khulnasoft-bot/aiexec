...
access is authentication: do not expose an instance with ``PRIMEAGENT_AUTO_LOGIN=true`` on
...
def ensure_code_execution_enabled() -> None:
    ...
    locks a deployment down with ``PRIMEAGENT_ALLOW_CUSTOM_COMPONENTS=false``,
    ...
    * ``ImportError`` importing the services layer means there is no settings
      stack at all (e.g. a stripped-down wfx embedding). That context is local
    ...
    * A ``None`` settings service means the stack is registered but failed to
    ...
    try:
        ...
        # Services layer absent (e.g. stripped-down wfx embed) -> local/trusted, allow.
    ...
    if not getattr(settings_service.settings, "allow_custom_components", True):
        msg = (
            ...
            "Set PRIMEAGENT_ALLOW_CUSTOM_COMPONENTS=true to enable this component."
        )
        raise CodeExecutionDisabledError(msg)
