"""Generate requirements.txt from a Primeagent flow JSON.

Analyzes a flow's component code and configuration to determine the minimal
set of PyPI packages needed to run that flow on a standalone WFX runner.

Uses ``importlib.metadata`` to dynamically resolve import names to PyPI
distribution names and to compute the transitive dependency tree of ``wfx``,
...
* **System-level dependencies** — Native libraries required by Python packages
  ...
  ``requirements.txt``.
* **``--wfx-package`` and transitive filtering** — The ``wfx_package`` parameter
  controls only the output name (e.g. ``wfx-nightly``).  The "already provided
  by wfx" filter always resolves against the ``wfx`` distribution installed in
...
# Import names that are internal to the wfx/primeagent runtime and should
# never appear as separate requirements.
_INTERNAL_IMPORT_NAMES: frozenset[str] = frozenset({"wfx", "primeagent", "primeagent_base"})
...
@lru_cache(maxsize=1)
def _get_wfx_transitive_dists() -> frozenset[str]:
    """Compute the full transitive closure of distributions provided by wfx.
...
@lru_cache(maxsize=1)
def _get_wfx_provided_imports() -> frozenset[str]:
    """Build the set of import names transitively provided by wfx.
...
def _resolve_provider_packages(provider_name: str) -> set[str]:
    ...
    Parent classes (e.g. ``LCModelComponent``) are all part of wfx, so any
    imports they introduce are already in wfx's transitive dependency tree
    ...
    wfx_provided = _get_wfx_provided_imports()
...
def _resolve_embedding_provider_packages(provider_name: str) -> set[str]:
    ...
    the ``LanguageModelComponent``: its code field only imports from ``wfx``
    ...
    # Use install_hint if provided (handles internal module paths like wfx.base.models.*)
    if install_hint:
        return {install_hint}
        ...
        if source and isinstance(source, str):
            ...
            for imp in imports:
                ...
                if imp in STDLIB_MODULES:
                    continue
                # Skip wfx / primeagent internal imports - wfx provides these
                ...
                # even if the import itself is provided by wfx, because the
                # extras may not be.
                if imp in MODULE_EXTRA_DEPS:
                    for extra in MODULE_EXTRA_DEPS[imp]:
                        packages.add(extra)

                # Skip imports already provided by wfx
                if imp in wfx_provided:
                    continue
...
def generate_requirements_from_flow(
    ...
    *,
    wfx_package: str = "wfx",
    include_wfx: bool = True,
    pin_versions: bool = True,
) -> list[str]:
    """Generate a requirements list from a Primeagent flow JSON.

    Args:
        flow: Parsed Primeagent flow JSON (dict).
        wfx_package: Name of the WFX package to include (e.g. ``"wfx"`` or
            ``"wfx-nightly"``).
        include_wfx: Whether to include the WFX package itself.
...
def generate_requirements_txt(
    ...
    *,
    ...
    """Generate requirements.txt content from a Primeagent flow JSON.

    Args:
        flow: Parsed Primeagent flow JSON (dict).
        wfx_package: Name of the WFX package to include.
        include_wfx: Whether to include the WFX package itself.
    ...
    reqs = generate_requirements_from_flow(
        flow,
        wfx_package=wfx_package,
        include_wfx=include_wfx,
        pin_versions=pin_versions,
    )
    lines = [
        "# Auto-generated requirements for Primeagent flow",
...
def generate_requirements_from_file(
    ...
    *,
    ...
    Args:
        flow_path: Path to a Primeagent flow JSON file.
        wfx_package: Name of the WFX package to include.
        include_wfx: Whether to include the WFX package itself.
        pin_versions: If True, pin each package to the currently installed
            version.
