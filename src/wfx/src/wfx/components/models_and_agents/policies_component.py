...
from wfx.base.models import LCModelComponent
from wfx.base.models.unified_models import (
    get_language_model_options,
    get_llm,
    update_model_options_in_build_config,
)
from wfx.components.models_and_agents.policies.module_utils import unload_module
from wfx.field_typing import LanguageModel, Tool
from wfx.io import (
    BoolInput,
    HandleInput,
    ModelInput,
...
_TOOLGUARD_INSTALL_HINT = (
    ...
    "Install the optional extra: `pip install 'primeagent-base[toolguard]'`."
)
...
class PoliciesComponent(LCModelComponent):
    ...
    `toolguard` is an optional extra (`primeagent-base[toolguard]`); imports happen
...
Powered by [ALTK ToolGuard](https://github.com/AgentToolkit/toolguard )"""
    ...
    def _import_toolguard():
        ...
        try:
            ...
            from wfx.components.models_and_agents.policies.guard_sync_utils import sync_generated_guard_code_inputs
            from wfx.components.models_and_agents.policies.guarded_tool import GuardedTool
            from wfx.components.models_and_agents.policies.llm_wrapper import LangchainModelWrapper
    ...
    def _code_execution_allowed() -> bool:
        ...
        where the settings layer cannot be imported at all (wfx used as a bare
        library), which is a local/trusted context.
        """
        try:
            from wfx.services.deps import get_settings_service
        except ImportError:
            # No settings layer at all (wfx used as a bare library) -> local/trusted.
            return True
        ...
        if self.enabled:
            ...
            if not self._code_execution_allowed():
                msg = (
                    ...
                    "allow_custom_components is False. Set PRIMEAGENT_ALLOW_CUSTOM_COMPONENTS=true "
