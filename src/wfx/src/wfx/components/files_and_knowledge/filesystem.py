...
from wfx.components.files_and_knowledge._filesystem_isolation import (
    IsolationConfig,
    load_isolation_config,
)
from wfx.components.files_and_knowledge._filesystem_namespace import (
    compute_user_namespace,
    load_or_create_pepper,
)
from wfx.custom.custom_component.component import Component
from wfx.inputs.inputs import BoolInput, StrInput
...
class FileSystemToolComponent(Component):
    ...
    inputs = [
        StrInput(
            ...
            info=(
                ...
                "BASE_DIR is operator-controlled via PRIMEAGENT_FS_TOOL_BASE_DIR."
            ),
        ),
        BoolInput(
            name="read_only",
        ...
        # and the backend gate in `src/wfx/src/wfx/template/utils.py` (toggle
    ...
    def _resolve_user_id(self) -> str | None:
        ...
        Why we filter ``"none"`` / ``"null"``: Primeagent's ``PlaceholderGraph``
    ...
    def _shared_root(self, *, config: IsolationConfig) -> Path:
        ...
        try:
            ...
            msg = (
                ...
                f"Check that PRIMEAGENT_FS_TOOL_BASE_DIR ({config.base_dir}) is writable "
                f"by the Primeagent process user."
            )
    ...
    def _isolated_user_root(self, *, config: IsolationConfig, user_id: str) -> Path:
        ...
        try:
            ...
            msg = (
                ...
                f"{exc.strerror or exc}. "
                f"Check that PRIMEAGENT_FS_TOOL_PEPPER_PATH points to a writable location."
            )
            raise PermissionError(msg) from exc
