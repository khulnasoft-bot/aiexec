...
_dynamic_imports = {
    ...
    # so existing flows / imports referencing wfx.components.tools keep working.
...
def __getattr__(attr_name: str) -> Any:
    ...
    # moved to wfx.components.files_and_knowledge.
    if attr_name in {"filesystem", "_filesystem_isolation", "_filesystem_namespace"}:
        from importlib import import_module

        result = import_module(f"wfx.components.files_and_knowledge.{attr_name}")
    ...
    try:
        with warnings.catch_warnings():
            ...
            if isinstance(mapping, tuple):
                ...
                result = import_mod(attr_name, module_name, f"wfx.components.{package}")
            else:
