...
from wfx.inputs.inputs import BoolInput, DataInput, DropdownInput, MessageTextInput, MultilineInput
from wfx.io import HandleInput, IntInput
from wfx.schema.message import Message
from wfx.template.field.base import RangeSpec
...
class OpenDsStarAgentRunnable(Runnable):
    ...
    async def astream_events(
        ...
        **_kwargs: Any,
        ...
        """Async stream events in LangChain format for Primeagent compatibility.

        This converts LangGraph's stream output to LangChain's astream_events format
        so it works with Primeagent's process_agent_events().
        """
        # Extract the input message
        if isinstance(input_value, dict) and "input" in input_value:
            query = input_value["input"]
...
class OpenDsStarAgentComponent(ToolCallingAgentComponent):
    ...
    async def message_response(self) -> Message:
        ...
        # Clean up tools list - remove empty strings or None which Primeagent
    ...
    async def run_agent(self, agent: Runnable) -> Message:
        ...
        from wfx.schema.content_types import TextContent
        from wfx.schema.message import Message
            ...
            while True:
                ...
                if len(trajectory) > last_trajectory_len:
                    ...
                    for event in new_events:
                        ...
                        from wfx.schema.content_types import CodeContent, ToolContent
    ...
    def _add_media_from_text(self, agent_message: Message, text: str) -> str:
        ...
        from wfx.schema.content_types import MediaContent
    ...
    def validate_tool_names(self) -> None:
        ...
        # Clean up tools list - remove empty strings or None which Primeagent sometimes passes
    ...
    def create_agent_runnable(self) -> Runnable:
        ...
        # Clean up empty strings or None which Primeagent sometimes passes when the input is functionally empty
