"""Offline, non-executing validator for a Primeagent Extension.

``validate_extension`` is the function backing the ``wfx extension validate``
CLI.  It performs four passes:

    1. **Manifest discovery + schema validation**
       (:func:`~wfx.extension.manifest.load_manifest`).
    ...
    4. **(opt-in)** ``--execute-imports``: forks a subprocess with a temporary
       Primeagent state dir, a strict env allowlist, and no inherited server
...
from wfx.extension._paths import is_within
...
def _discover_manifest_data(
    ...
    return (
        ...
        ExtensionError(
            ...
            message=(f"No extension.json or [tool.primeagent.extension] entry found in {root}."),
            location=str(root),
            hint=(
                "Create an extension.json at the extension root or add a "
                "[tool.primeagent.extension] section to pyproject.toml. See "
...
def _has_build_method(node: ast.ClassDef) -> bool:
    ...
    Primeagent Components reach the runtime in two shapes:
...
def _build_probe_env(tmp_path: Path) -> dict[str, str]:
    ...
    PRIMEAGENT_CONFIG_DIR are pinned to the throwaway temp directory so a
    misbehaving bundle cannot read or pollute the developer's real
    Primeagent state.  Cloud credentials (AWS_*, OPENAI_API_KEY, GITHUB_TOKEN,
    ...
    env["PRIMEAGENT_CONFIG_DIR"] = str(tmp_path / "config")
...
for py in sorted(bundle_root.rglob("*.py")):
    ...
    spec = importlib.util.spec_from_file_location(
        f"_wfx_extension_probe.{{rel.with_suffix('').as_posix().replace('/', '.')}}",
        py,
    )
...
def _run_execute_imports(
    ...
    """
    with tempfile.TemporaryDirectory(prefix="wfx-extension-probe-") as tmp:
        tmp_path = Path(tmp)
