...
Where :mod:`wfx.extension.discovery` *finds* manifest-shipping packages
and seed directories, this module *registers* them: it owns the in-memory
state that ``wfx extension list`` (and, downstream, the loader and the
...
milestone.  Two source kinds reach the registry:
    ...
    * **seed** -- an immediate subdirectory of a seed root (default
      ``/opt/primeagent/bundles``, overridable via ``$PRIMEAGENT_SEED_DIR``).
...
:class:`~wfx.extension.errors.ExtensionError` with code
...
if TYPE_CHECKING:
    ...
    from wfx.extension.discovery import DiscoveredExtension, SourceKind
...
class LoadStatus(str, Enum):
    """Lifecycle state surfaced through ``wfx extension list``.
...
class ExtensionImmutableError(RuntimeError):
    ...
    Wraps a typed :class:`~wfx.extension.errors.ExtensionError` so HTTP
...
class ExtensionRegistry:
    ...
    def list_extensions(self) -> list[Extension]:
        ...
        ``wfx extension list`` produces stable output even when the
    ...
    def _refuse_mutation(self, extension_id: str, *, verb: str) -> None:
        ...
        if extension.source_kind == "installed":
            ...
            hint = (
                ...
                "(remove the package from the image / lockfile) and restart Primeagent. "
            ...
            hint = (
                ...
                "restart Primeagent. Runtime mutation of seed Extensions is intentionally not supported."
            )
