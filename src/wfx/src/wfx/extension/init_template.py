"""Scaffolder for ``wfx extension init``.
...
from wfx.extension.manifest import _EXTENSION_ID_RE, EXTENSION_SCHEMA_URL
...
from wfx.extension.manifest import BUNDLE_NAME_RE as _BUNDLE_NAME_RE
...
class InitOptions:
    ...
    display_name: str
    """Human-readable display name shown in Primeagent's palette."""
...
class InitResult:
    ...
    Mirrors the loader's :class:`~wfx.extension.LoadResult` shape so the
...
def _manifest_payload(options: InitOptions) -> dict:
    ...
    return {
        ...
        "description": (
            f"Auto-generated by `wfx extension init`. Replace this description "
...
Replace ``build`` with your real logic.  See the Primeagent Component docs
at https://docs.prime.khulnasoft.com/components for input/output options.
"""
...
def _test_source(options: InitOptions) -> str:
    ...
    a constructed instance.  It does NOT depend on Primeagent's full graph
...
A Primeagent Extension generated by `wfx extension init`.
...
> **Prerequisite**: `primeagent` (which brings `wfx`) must be installed in
> your active environment before running any of the commands below or
> executing the generated `pytest` suite.  Install it with
> `pip install primeagent` or your package manager of choice.

```bash
# Validate the manifest + bundle without executing imports
wfx extension validate .

# Run the bundled smoke tests (requires primeagent / wfx)
pytest

# Launch a Primeagent dev server with this extension loaded
wfx extension dev .
```

After `wfx extension dev`, edit any file under `components/` and click
**Reload** in the Primeagent palette to pick up the change.
...
def _pyproject_payload(options: InitOptions) -> str:
    ...
    ``pip install -e .`` after ``wfx extension init``; without this file
    that step fails with ``Neither 'setup.py' nor 'pyproject.toml' found``.

    The ``[tool.primeagent.extension]`` entry mirrors the ``extension.json``
    ...
    """
    distribution_name = f"wfx-{options.extension_id}".replace("_", "-")
...
version = "0.1.0"
description = "{options.display_name} -- a Primeagent Extension."
...
[project.entry-points."primeagent.extensions"]
