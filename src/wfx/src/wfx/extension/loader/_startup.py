...
from wfx.extension.loader._orchestrator import load_extension
from wfx.extension.loader._plugins import _resolve_distribution_roots
from wfx.extension.loader._types import SLOT_OFFICIAL, LoadResult
...
def load_installed_extensions(
    ...
    both land at @official under ``_wfx_ext.official.<name>.*`` and silently
...
def load_seed_extensions(
    *,
    ...
    going through pip.  Default location is ``/opt/primeagent/bundles``;
    override via ``$PRIMEAGENT_SEED_DIR``.  Each immediate subdirectory that
    ships a v0 manifest becomes one Extension at the @official slot.

    Args:
        seed_dir_env: Test seam.  ``None`` reads ``$PRIMEAGENT_SEED_DIR``
            from the live environment; pass an explicit string to bypass
            ``os.environ``.
        default_seed_dir: Test seam.  ``None`` uses the discovery layer's
            default (``/opt/primeagent/bundles``); pass an explicit ``Path``
    ...
    Returns:
        ...
        determinism.
    """
    from wfx.extension.discovery import DEFAULT_SEED_DIR, discover_seed_extensions
