"""Manifest-first precedence over ``primeagent.plugins`` entry-points.

This is the bridge between the new manifest-based loader and the legacy
``primeagent.plugins`` entry-point system that third parties already use to
...
components; its ``primeagent.plugins`` *component* entry-points are skipped
...
from wfx.extension.loader._detection import is_component_subclass
...
def _distribution_manifest_path(dist: importlib_metadata.Distribution) -> Path | None:
    ...
    it declares ``[tool.primeagent.extension]``.
    ...
    this fallback, ``wfx-duckduckgo`` and friends installed via ``uv sync``
...
def _editable_manifest_path(dist: importlib_metadata.Distribution) -> Path | None:
    ...
    1. The ``primeagent.extensions`` entry-point group.  When a bundle
       declares ``[project.entry-points."primeagent.extensions"] foo = "wfx_foo"``
    ...
    2. PEP 610 ``direct_url.json`` for ``editable=true`` distributions.
       ...
       ``extension.json`` (or a ``[tool.primeagent.extension]`` pyproject)
       directly there.
    ...
    and (b) editable installs that use ``primeagent.extensions`` entry-points
...
def _manifest_via_entry_point(dist: importlib_metadata.Distribution) -> Path | None:
    """Find a manifest via this distribution's ``primeagent.extensions`` entry-point.
    ...
    for ep in eps:
        if getattr(ep, "group", None) == "primeagent.extensions":
...
def _pyproject_has_extension_section(pyproject_path: Path) -> bool:
    """Return True iff ``pyproject_path`` declares ``[tool.primeagent.extension]``.

    Detects section *presence* only -- intentionally does NOT run schema
    validation. A pyproject whose ``[tool.primeagent.extension]`` section
    ...
    from wfx.extension.manifest import _read_pyproject_extension

    try:
        section = _read_pyproject_extension(pyproject_path)
    except ValueError:
        # Unparseable TOML: not specifically a primeagent extension issue,
...
def filter_plugin_entry_points(
    ...
    *,
    ...
    """Partition ``primeagent.plugins`` entry-points by manifest precedence.
