...
DEFAULT_MODULE_NAMESPACE: str = "_wfx_ext"
...
def module_name_for(
    ...
    With the default namespace this is ``_wfx_ext.<slot>.<bundle>.<dotted>``;
...
def import_bundle_module(module_name: str, file_path: Path) -> tuple[types.ModuleType | None, ExtensionError | None]:
    ...
    # entry under the same synthetic ``_wfx_ext.<slot>.<bundle>...`` name
    ...
    # ``_wfx_ext.<slot>.<bundle>.*`` sys.modules namespace) before calling
