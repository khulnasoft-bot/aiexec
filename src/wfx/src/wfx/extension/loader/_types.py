...
"""Slot for loose PRIMEAGENT_COMPONENTS_PATH directories (ad-hoc local dev)."""
...
class LoadedComponent:
    ...
    folders from ``PRIMEAGENT_COMPONENTS_PATH``, never pip-installed. The
    ...
    *before* the package gets installed (the ``extension dev`` flow);
    ...
    package; ``None`` for inline PRIMEAGENT_COMPONENTS_PATH bundles AND for
    ...
    Used by :func:`wfx.extension.reload._diff` to detect in-class edits
