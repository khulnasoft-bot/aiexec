...
two public entry points the rest of Primeagent consumes:
    ...
    - :func:`discover_inline_bundles` for ``PRIMEAGENT_COMPONENTS_PATH`` --
...
from wfx.extension.loader._detection import collect_component_classes
from wfx.extension.loader._discovery import (
    DEFAULT_MODULE_NAMESPACE,
    import_bundle_module,
    iter_bundle_python_files,
    module_name_for,
)
from wfx.extension.loader._types import (
    SLOT_EXTRA,
    SLOT_OFFICIAL,
    SLOT_VALUES,
    LoadedComponent,
...
def _load_bundle_directory(
    *,
    ...
    ``sys.modules``; see :func:`wfx.extension.loader._discovery.module_name_for`.
    """
    ...
    # ``_wfx_ext.<slot>.<bundle>.{a,b}``.  This is deliberate: the reload
    # pipeline (wfx.extension.reload) re-stages the whole bundle under
    ...
    *,
    ...
    Args:
        ...
        distribution: PEP-503 canonical name of the distribution this
            ...
            extensions (e.g. ``primeagent extension dev`` against a working
            tree before pip install).
        module_namespace: Top-level package name used when registering bundle
            modules in ``sys.modules``.  Defaults to ``_wfx_ext`` for normal
    ...
    Single-load-per-process contract:
        ...
        ``_wfx_ext.<slot>.<bundle>.*`` namespace before re-invoking this
    ...
    try:
        ...
        result.errors.append(
            ExtensionError(
                ...
                hint=(
                    ...
                    "[tool.primeagent.extension] section to pyproject.toml."
                ),
            )
        )
        return result
        ...
        result.errors.append(
            ExtensionError(
                ...
                message=str(exc),
                location=str(root_path),
                hint="Run `wfx extension validate` for a detailed report.",
            )
        )
        return result
    ...
    # Version-constraint check: the manifest's wfx.compat list must include
    # this wfx package's BUNDLE_API_VERSION.  A bundle declaring only
    # ``["2"]`` against a wfx that ships ``BUNDLE_API_VERSION=1`` would
    ...
    declared = list(manifest.wfx.compat)
    if str(BUNDLE_API_VERSION) not in declared:
        result.errors.append(
            ExtensionError(
                code="version-constraint-unsatisfied",
                message=(
                    f"Extension {manifest.id!r} declares wfx.compat={declared!r}, "
                    f"which does not include this wfx package's "
                    f"BUNDLE_API_VERSION={BUNDLE_API_VERSION!s}."
                ),
                location=str(source.path),
                content=str(declared),
                hint=(
                    f"Update the manifest's wfx.compat to include "
                    f'"{BUNDLE_API_VERSION!s}", or install a Primeagent whose '
...
# ---------------------------------------------------------------------------
# Public entry point: discover_inline_bundles (PRIMEAGENT_COMPONENTS_PATH)
# ---------------------------------------------------------------------------
...
def _validate_inline_bundle_id(
    ...
    *,
    ...
    """
    from wfx.extension.manifest import _EXTENSION_ID_RE
...
def _read_inline_bundle_json(
    ...
    *,
    ...
    """Read optional ``bundle.json`` from a PRIMEAGENT_COMPONENTS_PATH bundle.
...
def load_inline_bundle(
    ...
    *,
    ...
    PRIMEAGENT_COMPONENTS_PATH entry.  Used by the reload pipeline so a
...
def discover_inline_bundles(
    ...
    """Discover inline bundles from PRIMEAGENT_COMPONENTS_PATH at the @extra slot.
        ...
        if not path_obj.is_dir():
            ...
            missing_result.warnings.append(
                ExtensionError(
                    code="inline-path-missing",
                    message=(
                        f"PRIMEAGENT_COMPONENTS_PATH entry {path_obj} does not exist or is not a directory; skipped."
                    ),
                    location=str(path_obj),
                    content=str(path_obj),
                    hint=(
                        "Remove the path from PRIMEAGENT_COMPONENTS_PATH or create the "
        ...
        try:
            ...
            unreadable_result.errors.append(
                ExtensionError(
                    ...
                    location=str(path_obj),
                    content=str(path_obj),
                    hint=("Check filesystem permissions on the path or remove it from PRIMEAGENT_COMPONENTS_PATH."),
                )
            )
            ...
            if name in seen_names:
                ...
                result.warnings.append(
                    ExtensionError(
                        ...
                        content=name,
                        hint=("Rename one of the bundle directories or remove it from PRIMEAGENT_COMPONENTS_PATH."),
                    )
                )
                results.append(result)
                continue
...
# :mod:`wfx.extension.loader._startup` (extracted to keep this orchestration
