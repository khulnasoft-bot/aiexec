...
in the real ``wfx.custom.custom_component.component.Component`` -- that
...
def is_component_subclass(obj: object) -> bool:
    ...
    here (the loader is part of wfx; importing the rich Component class
    pulls in graph/, vertex/, etc., which are not needed for registration).

    The Primeagent base classes themselves (``Component``, ``CustomComponent``,
