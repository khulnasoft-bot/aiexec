"""Single-Bundle loader for the Primeagent Extension System.

This package turns a directory tree on disk (an Extension or a loose
PRIMEAGENT_COMPONENTS_PATH entry) into a list of :class:`LoadedComponent`
...
Two entry points exist:
    ...
    1. :func:`load_extension` -- given a directory containing a v0 manifest
       (extension.json or [tool.primeagent.extension] in pyproject.toml), walk
    ...
    2. :func:`discover_inline_bundles` -- given a list of PRIMEAGENT_COMPONENTS_PATH
...
The loader's contract with the rest of the system is:
    ...
    - Failures are typed.  Every error path produces an
      :class:`~wfx.extension.errors.ExtensionError`; nothing raises across
...
Internal layout (all underscore-prefixed; not part of the public surface):
    ...
    - ``_plugins``     -- manifest-first precedence over ``primeagent.plugins``;
...
from wfx.extension.loader._discovery import DEFAULT_MODULE_NAMESPACE
from wfx.extension.loader._orchestrator import (
    discover_inline_bundles,
    load_extension,
    load_inline_bundle,
)
from wfx.extension.loader._plugins import (
    filter_component_entry_points,
    filter_plugin_entry_points,
    installed_extension_roots,
    manifest_owning_distributions,
)
from wfx.extension.loader._startup import (
