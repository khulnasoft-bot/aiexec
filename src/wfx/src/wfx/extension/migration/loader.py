...
table file, shipped at :data:`MIGRATION_TABLE_PATH` inside the wfx package.
...
from wfx.extension.migration.schema import MigrationTable
...
# wfx wheel.  Resolving via __file__ is intentional: any other discovery
...
def _read_json(path: Path) -> tuple[dict | None, ExtensionError | None]:
    ...
    try:
        ...
        return None, ExtensionError(
            ...
            location=str(path),
            hint=(
                "The migration table ships with wfx. If it is missing, "
                "your wfx install is corrupt; reinstall the package."
            ),
        )
        ...
        return None, ExtensionError(
            ...
            location=str(path),
            hint="Check filesystem permissions on the wfx install location.",
        )
