...
from wfx.extension.migration.schema import (
    _NAMESPACED_ID_RE,
    MigrationEntry,
    MigrationTable,
)
...
def _component_type_aliases(components: Mapping[str, Any]) -> set[str]:
    ...
    from wfx.utils.component_aliases import get_component_type_aliases
...
def _known_current_types_from_cache() -> set[str]:
    ...
    components_module = sys.modules.get("wfx.interface.components")
...
# Saved-flow node shape (current Primeagent): the canonical legacy reference
    ...
    *,
    ...
    if table is None:
        ...
        if load_error is not None or loaded_table is None:
            ...
            from wfx.extension.migration.loader import empty_table
