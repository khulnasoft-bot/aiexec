"""Local dev-extension registry for ``wfx extension dev``.

When an author types ``wfx extension dev <path>``, we don't ship the
Extension via pip; instead we record the absolute path in a small JSON
state file, then hand off to ``primeagent run``.  At server startup, the
loader reads this state file and registers each surviving directory at
the ``official`` slot via :func:`wfx.extension.loader.load_extension`.
Paths that have moved or been deleted surface as
``local-extension-missing`` warnings rather than aborting startup.

State file shape (kebab-case path under the primeagent user-cache dir)::
...
Scope notes:
    ...
    - This module only writes / reads / lists the state file; the actual
      registration is consumed by Primeagent's startup hook.  The runtime
    ...
    - Installed-package discovery keeps its own primitives and is
      ...
      Primeagent startup merges in the same step.
"""
...
from wfx.extension.loader import load_extension
from wfx.extension.loader._types import LoadResult
...
def _default_state_dir() -> Path:
    ...
    Resolution order:
        1. ``PRIMEAGENT_DEV_EXTENSIONS_DIR`` (test seam + override).
        2. ``PRIMEAGENT_CONFIG_DIR/extensions`` if ``PRIMEAGENT_CONFIG_DIR`` is set.
        3. ``platformdirs.user_cache_dir("primeagent", "primeagent")/extensions``.

    Created lazily on first write.
    """
    override = os.environ.get("PRIMEAGENT_DEV_EXTENSIONS_DIR")
    if override:
        return Path(override)

    config_dir = os.environ.get("PRIMEAGENT_CONFIG_DIR")
    if config_dir:
        return Path(config_dir) / "extensions"

    from platformdirs import user_cache_dir

    return Path(user_cache_dir("primeagent", "primeagent")) / "extensions"
...
def _read_state(state_path: Path) -> list[DevExtensionEntry]:
    ...
    * **File present but unreadable** (permission error etc.) -- log a
      ...
      extension on the next ``primeagent run`` with no diagnostic.  A
    ...
    * **File present but corrupt JSON / wrong shape** -- log a WARNING
    ...
    try:
        ...
        logger.warning(
            ...
            "treating as empty registry. Re-run `wfx extension dev <path>` "
...
def _write_state(state_path: Path, entries: Iterable[DevExtensionEntry]) -> None:
    ...
    ``dev_extensions.json`` gets code loaded at the next ``primeagent run``;
        ...
        if not entry.path.is_dir():
            ...
            result.warnings.append(
                ExtensionError(
                    ...
                    hint=(
                        "Restore the directory or run `wfx extension dev <path>` "
...
def dev_extension_component_paths(*, state_dir: Path | None = None) -> tuple[list[Path], list[ExtensionError]]:
    ...
    paths, IDE integrations).  The Primeagent startup path itself uses
    ...
    for result in load_dev_extensions(state_dir=state_dir):
        ...
        # caller (Primeagent's lifespan hook) is the only thing that turns
        ...
        if result.source_path is None:
            ...
            errors.append(
                ExtensionError(
                    ...
                    location="<unknown>",
                    hint=("File a bug against wfx.extension.loader: LoadResult had components but no source_path."),
                )
            )
            continue
