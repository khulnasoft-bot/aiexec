"""Production-install discovery for the Primeagent Extension System.
...
This module owns the two production install sources for Modes A, B, and C:
    ...
    1. **Installed Python distributions** -- the primary path. ``pip install
       primeagent wfx-<provider>`` puts a manifest-shipping distribution on
       ``importlib.metadata.distributions()``; we walk that iterator at
       server startup and surface every distribution that ships an
       ``extension.json`` or a ``[tool.primeagent.extension]`` section in its
       ``pyproject.toml``.
    ...
    2. **Seed directories** -- an optional filesystem source for Docker
       ...
       Defaults to ``/opt/primeagent/bundles/`` when present; the
       ``PRIMEAGENT_SEED_DIR`` environment variable overrides this with one
...
never touches the registry.  The :mod:`wfx.extension.registry` service is
...
Errors are surfaced as :class:`~wfx.extension.errors.ExtensionError`
...
from wfx.extension._paths import SKIP_DIR_NAMES, is_within
from wfx.extension.errors import ExtensionError
...
DEFAULT_SEED_DIR: Path = Path("/opt/primeagent/bundles")
"""Default filesystem seed directory used when ``PRIMEAGENT_SEED_DIR`` is unset.
...
SEED_DIR_ENV_VAR: str = "PRIMEAGENT_SEED_DIR"
...
``;`` on Windows), mirroring ``PRIMEAGENT_COMPONENTS_PATH``."""
...
class DiscoveredExtension:
    ...
    def slot(self) -> Literal["official"]:
        ...
        ``PRIMEAGENT_COMPONENTS_PATH`` folders; nothing in this module
        produces it.
        """
        return "official"
...
def _distribution_manifest_path_via_entry_points(
    ...
    ``primeagent.extensions`` entry-point points at the package that ships the
    ...
    try:
        selected = list(eps.select(group="primeagent.extensions"))
    except AttributeError:
        # Older importlib.metadata returns a plain tuple of EntryPoint with
        # no .select() helper; filter manually.
        selected = [ep for ep in eps if getattr(ep, "group", None) == "primeagent.extensions"]
...
def _pyproject_declares_extension(pyproject_path: Path) -> bool:
    """Return ``True`` iff *pyproject_path* contains ``[tool.primeagent.extension]``.
    ...
    try:
        ...
        # Unparseable TOML: not specifically a Primeagent concern, treat as
        # "not a manifest-shipping package" and keep scanning.
        return False
    except TypeError:
        # [tool.primeagent.extension] exists but isn't a table.  The author
...
def _build_installed_record(
    ...
    try:
        ...
        return None, ExtensionError(
            ...
            location=str(manifest_path),
            content=canonical,
            hint="Run `wfx extension validate` against the package source to see the per-field detail.",
        )
...
def discover_installed_extensions(
    ...
    Returns:
        ...
        :class:`~wfx.extension.errors.ExtensionError` per package whose
    ...
    :class:`~wfx.extension.registry.ExtensionRegistry`.
    """
    ...
    *,
    ...
    Behavior:
        ...
        * If ``seed_dir_env`` is unset/empty, fall back to ``default``.
          A missing default path is silently skipped: ``/opt/primeagent/
            ...
            if not candidate.is_dir():
                errors.append(
                    ExtensionError(
                        ...
                        content=stripped,
                        hint=(
                            "Set PRIMEAGENT_SEED_DIR to a directory that exists, "
                            "or unset it to fall back to /opt/primeagent/bundles."
                        ),
                    )
                )
                continue
...
def _iter_seed_subdirectories(seed_root: Path) -> Iterator[Path]:
    ...
    (see wfx.extension._paths.is_within and loader._discovery.iter_bundle_python_files).
...
def _build_seed_record(seed_subdir: Path) -> tuple[DiscoveredExtension | None, ExtensionError | None]:
    ...
    try:
        ...
        return None, ExtensionError(
            ...
            content=seed_subdir.name,
            hint="Ship an extension.json or a pyproject.toml with [tool.primeagent.extension].",
        )
    except (ValueError, TypeError, OSError) as exc:
        return None, ExtensionError(
            code="manifest-invalid",
            ...
            content=seed_subdir.name,
            hint="Run `wfx extension validate` against the seed subdirectory to see per-field detail.",
        )
...
def discover_seed_extensions(
    *,
    ...
    Args:
        seed_dir_env: Explicit override for ``$PRIMEAGENT_SEED_DIR``.  Pass
        ...
        default: Path checked when ``seed_dir_env`` is empty.  Defaults to
            :data:`DEFAULT_SEED_DIR` (``/opt/primeagent/bundles``).  Pass
