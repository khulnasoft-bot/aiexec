...
A Primeagent Extension is the distribution unit that gets pip-installed.  In v0
it ships exactly one Bundle (a named group of components) plus a manifest at
the distribution root.  The manifest tells Primeagent:
...
Manifest source forms (both supported):
    ...
    2. ``[tool.primeagent.extension]`` in ``pyproject.toml``.
...
if sys.version_info >= (3, 11):
    ...
    import tomli as tomllib  # 3.10 fallback (wfx already depends on tomli)
...
"""The integer version of the BUNDLE_API.md contract that this wfx package
implements.  Manifests declare the contract versions they support via
``wfx.compat`` (a list of stringified integers); a manifest that does not
...
EXTENSION_SCHEMA_URL: str = f"https://schemas.prime.khulnasoft.com/extension/v{SCHEMA_VERSION}.json"
...
"""Manifest ``wfx.compat`` entries are stringified positive integers (no leading
zeros) that name a frozen BUNDLE_API.md revision.  v0 only knows ``"1"``."""


class WfxCompat(BaseModel):
...
class ExtensionManifest(BaseModel):
    """The v0 Primeagent Extension manifest.

    Required fields:
        - id, version, name, bundles, wfx
    Optional:
    ...
    name: StrictStr = Field(
        ...
        min_length=1,
        max_length=200,
        description="Human-readable display name shown in Primeagent.",
    )
...
def _read_pyproject_extension(path: Path) -> dict[str, Any] | None:
    """Read ``[tool.primeagent.extension]`` from a pyproject.toml.
    ...
    if not isinstance(tool, dict):
        return None
    primeagent = tool.get("primeagent")
    if not isinstance(primeagent, dict):
        return None
    section = primeagent.get("extension")
    if section is None:
        return None
    if not isinstance(section, dict):
        msg = "[tool.primeagent.extension] must be a TOML table"
        raise TypeError(msg)
    return section
...
def load_manifest(root: Path | str) -> ManifestSource:
    ...
    Discovery order: ``extension.json`` first, then ``[tool.primeagent.extension]``
    ...
    msg = (
        ...
        f"'extension.json' or a [tool.primeagent.extension] section in 'pyproject.toml'."
    )
    raise FileNotFoundError(msg)
