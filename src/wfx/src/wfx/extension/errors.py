"""Typed errors for the Primeagent Extension System.
...
DOCS_BASE = "https://docs.prime.khulnasoft.com/extensions/errors"
    ...
    def __post_init__(self) -> None:
        if self.code not in ERROR_CODES:
            msg = (
                ...
                f"Add it to wfx.extension.errors.ERROR_CODES and add a "
...
_BRANCH_TEMPLATES: dict[str, str] = {
    "manifest-not-found": ("No extension.json or [tool.primeagent.extension] entry found in {location}."),
    ...
    "version-constraint-unsatisfied": (
        "Manifest at {location} declares wfx.compat={content!r}, which does not "
        "include this wfx package's BUNDLE_API_VERSION; refusing to load."
    ),
    ...
    "duplicate-inline-bundle": (
        "Inline bundle name {content!r} appears in multiple PRIMEAGENT_COMPONENTS_PATH entries; "
    ...
    "inline-path-missing": (
        "PRIMEAGENT_COMPONENTS_PATH entry {content!r} does not exist or is not a directory; skipped."
    ),
    "inline-path-unreadable": ("PRIMEAGENT_COMPONENTS_PATH entry {content!r} could not be enumerated: {message}"),
    "bundle-json-invalid": (
    ...
    "extension-reload-disabled": (
        ...
        "Set PRIMEAGENT_ENABLE_EXTENSION_RELOAD=true to enable it on a local-development install (Mode A)."
    ),
    "extension-events-keyspace-forbidden": (
