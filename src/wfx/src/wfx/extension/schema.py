...
The published schema lives at :data:`~wfx.extension.manifest.EXTENSION_SCHEMA_URL`
(``schemas.prime.khulnasoft.com/extension/v1.json``).  This module is the source of
truth for what gets uploaded there.

Two consumers:

    1. ``wfx extension schema`` (CLI, this PR) -- writes the schema to disk so
...
def _public_deferred_names() -> list[str]:
    ...
    Resolving here keeps :mod:`wfx.extension.manifest` the single source of
...
def build_schema() -> dict[str, Any]:
    ...
    schema["title"] = "Primeagent Extension Manifest (v1)"
    ...
    schema["description"] = (
        "Schema for the v0 Primeagent Extension manifest. "
        ...
        "See https://docs.prime.khulnasoft.com/extensions-manifest for the field-by-field reference "
        "and https://docs.prime.khulnasoft.com/extensions-author-guide for the author's guide."
    )
    return schema
