...
:class:`~wfx.extension.registry.BundleRegistry`.
...
1. **Stage 1 -- parallel load** into a per-reload staging namespace
   ...
   ``sys.modules`` without colliding with the live ``_wfx_ext.*`` entries.
...
3. **Stage 3 -- atomic swap** under the registry's write lock.  This is
   ...
   ``_wfx_ext.*`` namespace; the new :class:`BundleRecord` is installed.
...
from wfx.extension.bundle_registry import (
    BundleRecord,
    BundleRegistry,
...
from wfx.extension.reload_swap import (
...
# ---------------------------------------------------------------------------
#
# Layers above wfx (the primeagent component cache; eventually the events
# service) need to react to a successful reload.  ``wfx`` cannot import
# ``primeagent``, so we expose a tiny registration API: ``primeagent`` calls
...
def register_post_swap_hook(hook: Callable[[BundleRecord], None]) -> None:
    ...
    canonical use case is the primeagent component cache, which needs to
            ...
            errors=[
                ExtensionError(
                    ...
                    hint=(
                        ...
                        "`wfx extension dev <path>`), then reload via "
                        "`wfx extension reload <extension-id> --bundle <name>`."
                    ),
                )
            ],
            previous=None,
...
def _run_pipeline_body(
    *,
    ...
    # the default primeagent verbosity.
    logger.info(
...
# _drop_staging_modules) live in :mod:`wfx.extension.reload_swap`.  They
...
def _emit_bundle_reload_event(result: ReloadResult, *, user_id: str | None = None) -> None:
    ...
    """
    try:
        from wfx.services.deps import get_extension_events_service
