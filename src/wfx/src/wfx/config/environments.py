"""wfx environment configuration — resolve Primeagent instance URL and API key.

Config file lookup order
------------------------
1.  Explicit path given via ``--environments-file`` / ``environments_file`` parameter.
2.  ``.wfx/environments.yaml`` in the current working directory, then each
    parent directory up to the first ``.git`` boundary (project root discovery).
3.  ``~/.wfx/environments.yaml`` (user-level config).
4.  ``primeagent-environments.toml`` in the current working directory
    (backward-compatible with the primeagent-sdk TOML format).
...
.. code-block:: yaml
    ...
    environments:
      local:
        ...
        api_key_env: PRIMEAGENT_LOCAL_API_KEY
...
TOML format is also accepted (``primeagent-environments.toml`` or any ``.toml``
...
class ConfigError(Exception):
    ...
    will simply be ``None`` on the returned :class:`PrimeagentEnvironment`.
...
@dataclass
class PrimeagentEnvironment:
    """A fully-resolved Primeagent target instance.

    Attributes:
        name:    Human-readable label (environment name or ``"__inline__"``).
        url:     Base URL of the Primeagent instance.
...
_TOML_FALLBACK = "primeagent-environments.toml"
_WFX_DIR = ".wfx"
...
def _find_config_file(override: Path | None) -> Path | None:
    ...
    # Walk up from cwd looking for .wfx/environments.yaml
    cwd = Path.cwd()
    for directory in (cwd, *cwd.parents):
        for name in _YAML_NAMES:
            candidate = directory / _WFX_DIR / name
    ...
    # User-level YAML
    for name in _YAML_NAMES:
        user_yaml = Path.home() / _WFX_DIR / name
        if user_yaml.is_file():
            return user_yaml

    # Backward-compat: primeagent-environments.toml in cwd
...
def _parse_env_block(name: str, block: Any, config_path: Path) -> PrimeagentEnvironment:
    ...
    return PrimeagentEnvironment(name=name, url=url, api_key=api_key)


def _load_config(path: Path) -> tuple[dict[str, PrimeagentEnvironment], str | None]:
    ...
    envs: dict[str, PrimeagentEnvironment] = {}
...
def resolve_environment(
    ...
    *,
    ...
    """Resolve an environment name (or inline flags) to a :class:`PrimeagentEnvironment`.
    ...
    3. **Env-var fallback** — if no config file exists and no *env* was
       requested, fall back to ``PRIMEAGENT_URL`` / ``PRIMEAGENT_API_KEY`` (or
       ``WFX_URL`` / ``WFX_API_KEY``) env vars before raising.
    ...
    if target is not None:
        ...
        return PrimeagentEnvironment(name=name, url=target, api_key=api_key)
    ...
    if config_path is None:
        ...
        lf_url = os.environ.get("PRIMEAGENT_URL") or os.environ.get("WFX_URL")
        if lf_url and env is None:
            lf_key = api_key or os.environ.get("PRIMEAGENT_API_KEY") or os.environ.get("WFX_API_KEY")
            return PrimeagentEnvironment(name="__env__", url=lf_url, api_key=lf_key)

        if env is not None:
            msg = (
                f"Environment {env!r} requested but no config file was found.\n"
                f"  • Create .wfx/environments.yaml in your project root, or\n"
                f"  • Pass --target <url> [--api-key <key>] for inline configuration.\n"
                f"  • Run 'wfx init' to scaffold a project with a config template."
            )
            raise ConfigError(msg)
        ...
        msg = (
            ...
            "Options:\n"
            "  • wfx <cmd> --env <name>              (requires .wfx/environments.yaml)\n"
            "  • wfx <cmd> --target <url>             (inline, no config file needed)\n"
            "  • export PRIMEAGENT_URL=<url>            (env-var fallback)\n"
            "  • wfx init                             (scaffold a project with a template)"
        )
        raise ConfigError(msg)
    ...
    if api_key is not None:
        resolved = PrimeagentEnvironment(name=resolved.name, url=resolved.url, api_key=api_key)
