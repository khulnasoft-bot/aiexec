...
class ComponentsSettings(BaseModel):
    ...
    permitted to execute, even when PRIMEAGENT_ALLOW_CUSTOM_COMPONENTS is False.
    """
    ...
    explicit opt-in.  Set ``PRIMEAGENT_ENABLE_EXTENSION_RELOAD=true`` in your
    ...
    def set_components_path(cls, value):
        ...
        """
        env_value = os.getenv("PRIMEAGENT_COMPONENTS_PATH")
        if env_value:
            ...
            for raw_entry in env_value.split(os.pathsep):
                ...
                if not Path(entry).exists():
                    # Surface at warning so a typo in PRIMEAGENT_COMPONENTS_PATH
    ...
    def _enforce_components_paths_override(self):
        ...
        matching built-in templates. By default ``PRIMEAGENT_COMPONENTS_PATH`` and
        ``PRIMEAGENT_COMPONENTS_INDEX_PATH`` still contribute to that trust set (an
        ...
        env_components_path = os.getenv("PRIMEAGENT_COMPONENTS_PATH")
        if env_components_path:
            ...
            # would re-read PRIMEAGENT_COMPONENTS_PATH and append the paths again.
            ...
            if stripped_any:
                logger.warning(
                    "Ignoring PRIMEAGENT_COMPONENTS_PATH=%s: "
                    "PRIMEAGENT_ALLOW_CUSTOM_COMPONENTS=False and "
                    "PRIMEAGENT_ALLOW_COMPONENTS_PATHS_OVERRIDE=False.",
                    env_components_path,
                )
        ...
        env_components_index_path = os.getenv("PRIMEAGENT_COMPONENTS_INDEX_PATH")
        if env_components_index_path and self.components_index_path == env_components_index_path:
            logger.warning(
                "Ignoring PRIMEAGENT_COMPONENTS_INDEX_PATH=%s: "
                "PRIMEAGENT_ALLOW_CUSTOM_COMPONENTS=False and "
                "PRIMEAGENT_ALLOW_COMPONENTS_PATHS_OVERRIDE=False.",
