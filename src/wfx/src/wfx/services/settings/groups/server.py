...
class ServerSettings(BaseModel):
    ...
    prefix (e.g. '/primeagent').  When set, the MCP SSE transport includes this
    prefix in the POST-back URL so clients can reach the correct endpoint.
    Can also be set via the PRIMEAGENT_ROOT_PATH environment variable."""
    ...
    def validate_runtime_port(cls, value):
        ...
        Kubernetes auto-creates env vars like PRIMEAGENT_RUNTIME_PORT=tcp://<ip>:<port>
        for services, which collides with the PRIMEAGENT_ env prefix. Extract the port
