"""Minimal variable service for wfx package with in-memory storage and environment fallback."""
...
from wfx.services.variable.request_scope import (
...
class VariableService(Service):
    ...
    This is a lightweight implementation for WFX that maintains in-memory
    ...
    def _normalize_global_var_key(name: str) -> str:
        return f"x-primeagent-global-var-{name.lower().replace('_', '-')}"

    @staticmethod
    def _get_request_variables() -> dict[str, str]:
        """Request-scoped variables from serve ContextVar or PRIMEAGENT_REQUEST_VARIABLES env."""
        active = get_active_request_variables()
        if active is not None:
            return active

        # PRIMEAGENT_REQUEST_VARIABLES is a process-wide env var, so reading it is an
        ...
        raw = os.getenv("PRIMEAGENT_REQUEST_VARIABLES")
        if not raw:
            return {}
        try:
            parsed = json.loads(raw)
        except json.JSONDecodeError:
            logger.debug("Invalid PRIMEAGENT_REQUEST_VARIABLES JSON; skipping request-scoped lookup")
            return {}
        if not isinstance(parsed, dict):
            logger.debug("PRIMEAGENT_REQUEST_VARIABLES must be a JSON object; skipping request-scoped lookup")
            return {}
    ...
    async def get_variable(self, name: str, **kwargs) -> str | None:  # noqa: ARG002
        ...
        Resolution order (first match wins):
          ...
          2. Request-scoped exact name (serve ContextVar or ``PRIMEAGENT_REQUEST_VARIABLES``).
          3. Request-scoped ``x-primeagent-global-var-*`` alias.
          4. Environment variable (exact name).
          5. Environment variable (``x-primeagent-global-var-*`` alias).
        ...
        or the ``x-primeagent-global-var-*`` form — always beats a same-named variable left
