"""Request-scoped variables for wfx serve (and other in-process runners).
...
lookup table in a ContextVar so :class:`~wfx.services.variable.service.VariableService`
can resolve the same names as the ``wfx run`` subprocess path.
...
def activate_request_variables(variables: dict[str, str] | None) -> contextvars.Token[dict[str, str] | None]:
    ...
    ``PRIMEAGENT_REQUEST_VARIABLES`` env var. An empty ``{}`` is an *active* empty
