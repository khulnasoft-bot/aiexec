...
The CLI's ``wfx run`` (``run_flow``) and ``wfx serve``/``execute_graph_with_capture``
both need to:
...
3. Propagate ``session_id`` to ``Memory``/``MessageHistory`` vertices the way
   ``primeagent.api.utils.flow_utils.build_graph_from_data`` does — preserving
   any value already pinned in the flow JSON.
4. Resolve ``fallback_to_env_vars`` from settings (mirrors the primeagent API
   path in ``processing.process.run_graph_internal``).

Both call sites used to inline these steps; consolidating them here prevents
behavior drift between ``wfx run`` and ``wfx serve``.
"""
    ...
    *,
    ...
    if not user_id:
        ...
        logger.debug(
            ...
            "wfx's variable service is env-backed, so user_id is not used for variable scoping."
        )
    ...
    # Mirror primeagent's build_graph_from_data: only write when raw_params has no
...
def resolve_fallback_to_env_vars() -> bool:
    ...
    Mirrors the primeagent API path (``processing.process.run_graph_internal``):
