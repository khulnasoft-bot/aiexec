"""Core run functionality for executing Primeagent graphs."""
...
from wfx.run._defaults import apply_run_defaults, resolve_fallback_to_env_vars, validate_provided_id
from wfx.schema.schema import InputValueRequest
from wfx.utils.flow_envelope import split_flow_envelope
...
async def run_flow(
    ...
    *,
    ...
    This function analyzes and executes either a Python script containing a Primeagent graph,
    ...
    # --- upgrade compatibility gate (shared with `wfx serve`) ---
    if upgrade_flow and flow_dict is not None:
        ...
        # `wfx upgrade` uses). component_cache.all_types_dict is empty here because it is
    ...
    try:
        ...
        # don't fail. See wfx.run._defaults.apply_run_defaults for the full rationale.
    ...
    try:
        ...
        # See wfx.run._defaults.resolve_fallback_to_env_vars for why this flag is
        # plumbed through (mirrors primeagent's API path so load_from_db variables
