...
# Database imports removed - wfx should be lightweight
...
async def emit_vertex_build_event(
    *,
    ...
    try:
        ...
        from primeagent.services.event_manager import webhook_event_manager
        ...
        pass  # primeagent not available (standalone wfx usage)
...
async def log_transaction(
    ...
    Uses the pluggable TransactionService to log transactions. When running within primeagent,
    the concrete TransactionService implementation persists to the database.
    When running standalone (wfx only), transactions are not persisted.
    ...
    try:
        ...
        from wfx.services.deps import get_transaction_service
...
def _try_enqueue_via_telemetry_writer(vertex_build) -> bool:
    """Best-effort handoff to the primeagent telemetry writer.

    Returns ``True`` when the row was accepted by the writer (caller should
    skip the direct DB write). Returns ``False`` when primeagent isn't installed,
...
async def log_vertex_build(
    *,
    ...
    When running within primeagent, it will use primeagent's database service to persist the build.
    When running standalone (wfx only), it will only log debug messages.
    """
    try:
        # Try to use primeagent's services if available (when running within primeagent)
        try:
            from primeagent.services.deps import get_db_service as primeagent_get_db_service
            from primeagent.services.deps import get_settings_service as primeagent_get_settings_service

            settings_service = primeagent_get_settings_service()
            ...
            from primeagent.services.database.models.vertex_builds.model import VertexBuildBase
            ...
            db_service = primeagent_get_db_service()
            ...
            # Fallback for standalone wfx usage (without primeagent)
            ...
            # Log basic vertex build info - concrete implementation is in primeagent
...
def has_chat_output(vertices: dict[Vertex, int]):
    from wfx.graph.schema import InterfaceComponentTypes
