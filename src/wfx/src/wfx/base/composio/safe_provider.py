...
def _patch_file_helper_once() -> None:
    ...
    the ``_wfx_safe_patched`` sentinel; safe across repeated imports / pytest
    process reuse.
    """
    if not _COMPOSIO_AVAILABLE or getattr(FileHelper, "_wfx_safe_patched", False):
        return
    ...
    FileHelper._wfx_safe_patched = True  # noqa: SLF001
...
def _patch_pydantic_builder_once() -> None:
    ...
    Idempotent via the ``_wfx_safe_patched`` sentinel on the module.
    """
    if not _COMPOSIO_AVAILABLE or getattr(_composio_shared, "_wfx_safe_patched", False):
        return
    ...
    _composio_shared._wfx_safe_patched = True  # noqa: SLF001
...
def _patch_identifier_substitution_once() -> None:
    ...
    Idempotent via the _wfx_identifier_patched sentinel.
    """
    if not _COMPOSIO_AVAILABLE or getattr(_composio_lc_provider, "_wfx_identifier_patched", False):
        return
    ...
    _composio_lc_provider._wfx_identifier_patched = True  # noqa: SLF001
