...
Override scope:
    ...
    * Live-fetched providers (Ollama, IBM WatsonX, OpenRouter) are *not*
      ...
      :mod:`wfx.base.models.model_utils` runs **after** the catalog is
...
Cache layout:
    ...
    * Disk snapshot at ``<primeagent-cache>/models_dev/snapshot.json`` for
      offline cold starts.
...
from wfx.base.models.model_metadata import create_model_metadata
...
# Translation: models.dev provider slug -> Primeagent display name. Only
# providers Primeagent already knows how to instantiate (have an entry in
...
def _snapshot_dir() -> Path:
    ...
    """
    cache_root = Path(user_cache_dir("primeagent", "primeagent"))
...
def _translate_model_entry(
    ...
    *,
    ...
    """Convert one models.dev model entry to Primeagent's ``ModelMetadata`` shape.
