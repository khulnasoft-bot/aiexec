...
from wfx.services.variable.request_scope import is_env_fallback_disabled
...
def get_api_key_for_provider(user_id: UUID | str | None, provider: str, api_key: Any = None) -> str | None:
    ...
    # key). Env is the fallback for the no-user (wfx run) / no-DB-value case.
    ...
    # Fall through to os.environ regardless so wfx run (no user_id) can still pick
...
def _env_value_for(var_key: str) -> str | None:
    """Read a provider key from the environment, accepting a PRIMEAGENT_ alias.

    Provider keys are conventionally bare (``GOOGLE_API_KEY``), but some .env
    templates prefix everything with ``PRIMEAGENT_`` (matching how Primeagent reads
    its own settings). Accept ``PRIMEAGENT_<VAR>`` as a fallback so e.g.
    ``PRIMEAGENT_GOOGLE_API_KEY`` enables Gemini exactly like ``GOOGLE_API_KEY``.
    ...
    if value and value.strip():
        return value
    prefixed = os.environ.get(f"PRIMEAGENT_{var_key}")
