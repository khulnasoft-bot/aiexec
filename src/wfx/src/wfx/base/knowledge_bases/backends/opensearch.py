...
Primeagent Knowledge Bases can target a self-hosted or managed
OpenSearch cluster (AWS OpenSearch Service, Aiven, OpenSearch
Project, etc.) alongside the other native DB connectors. URL and
auth credentials are resolved through Primeagent's ``variable_service``
...
* ``url_variable`` — name of the Primeagent variable holding the
...
Primeagent installs without OpenSearch deps keep working.
"""
...
class OpenSearchBackend(BaseVectorStoreBackend):
    """OpenSearch k-NN as a Primeagent KB backend."""
    ...
    async def _resolve_secrets(self) -> None:
        ...
        if not url:
            msg = (
                f"OpenSearchBackend needs the {url_variable!r} Primeagent variable "
    ...
    async def similarity_search(
        ...
        *,
        ...
        key, which is what every Primeagent callsite that doesn't pass a
