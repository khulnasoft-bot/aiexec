...
async def get_knowledge_bases(kb_root: Path, user_id: UUID | str) -> list[str]:
    ...
    # Lazy imports: primeagent's DB models aren't part of the wfx
    # standalone install, and wfx's validate-rewrite layer can't
    # substitute ``wfx.services.database.models.user.crud`` (no such
    # module). Deferring the imports to call time keeps this module
    # importable under ``wfx run <starter>.json``, which is exercised
    ...
    # sentinel check: wfx must stay importable without the primeagent API
    # package, so we cannot reach for KBStorageHelper. The string literal
    # is kept in lockstep with KB_DELETED_SENTINEL in
    # src/backend/base/primeagent/api/utils/kb_helpers.py via a unit test
