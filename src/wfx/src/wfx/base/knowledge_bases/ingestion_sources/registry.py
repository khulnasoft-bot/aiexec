...
Thin wrapper over :class:`wfx.services.adapters.registry.AdapterRegistry`
...
* **Entry point** under the ``wfx.ingestion_source.adapters`` group::

      [project.entry-points."wfx.ingestion_source.adapters"]
      box = "primeagent_box.source:BoxSource"

* **Config file** — ``wfx.toml`` (preferred) or ``pyproject.toml``::
...
from wfx.base.knowledge_bases.ingestion_sources.base import KBIngestionSource, SourceType
from wfx.services.adapters.registry import get_adapter_registry
from wfx.services.adapters.schema import AdapterType

if TYPE_CHECKING:
    from uuid import UUID

    from wfx.services.adapters.registry import AdapterRegistry
...
def _ensure_discovered() -> None:
    ...
    Mirrors the pattern in :func:`wfx.services.deps.get_deployment_adapter`:
    ...
    with _discovery_lock:
        ...
        # wfx is used as a plain library (e.g. from tests) — the
        # deployment path does the same.
        from wfx.services.config_discovery import resolve_config_dir
