...
1. They need **credentials** — resolved from Primeagent's
...
from wfx.base.knowledge_bases.ingestion_sources.base import KBIngestionSource
...
class KBConnectorSource(KBIngestionSource):
    ...
    async def resolve_secret(self, variable_name: str) -> str | None:
        """Return the value of a Primeagent variable, or ``None`` if absent.

        Order of resolution:

        1. Primeagent's ``variable_service`` scoped to ``self.user_id``.
    ...
    async def resolve_required_secret(self, variable_name: str) -> str:
        ...
        if not value:
            msg = (
                ...
                "configured. Set it via Primeagent's variable settings or as "
    ...
    async def _lookup_variable(self, user_uuid: UUID, variable_name: str) -> str | None:
        """Look up ``variable_name`` through Primeagent's variable service.
...
class OAuthConnectorBase(KBConnectorSource):
    ...
    * The connector mints a short-lived access token on every list /
    ...
    async def _refresh_access_token(self) -> str:
        ...
        those deps keeps Primeagent's install footprint tight.
        """
        import httpx
