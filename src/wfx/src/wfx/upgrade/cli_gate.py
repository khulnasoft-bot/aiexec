...
Both ``wfx run`` and ``wfx serve`` accept ``--upgrade-flow`` with identical semantics:
...
from wfx.upgrade.checker import build_registry_lookup, check_flow_compatibility
...
def _load_bundled_registry() -> dict[str, Any]:
    ...
    This is the same source ``wfx upgrade`` reads, and the only registry that is reliably
    ...
    """
    from wfx.interface.components import _read_component_index
    ...
    if not blob or "entries" not in blob:
        msg = (
            ...
            "compatibility cannot be checked. Try reinstalling wfx "
            "(e.g. `pip install --force-reinstall wfx`)."
        )
...
def apply_upgrade_gate(
    ...
    Args:
        ...
        all_types_dict: The component registry to check against. When ``None`` (the default the
            ...
            source ``wfx upgrade`` reads. Tests pass an explicit registry to control classification.
