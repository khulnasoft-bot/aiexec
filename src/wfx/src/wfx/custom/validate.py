...
def _get_module_fallbacks(module_name: str) -> list[str]:
    ...
    Handles primeagent -> wfx and langchain -> langchain_classic remapping at the
    ...
    """
    names = [module_name]
    if module_name.startswith("primeagent."):
        names.append(module_name.replace("primeagent.", "wfx.", 1))
    if module_name.startswith("langchain."):
