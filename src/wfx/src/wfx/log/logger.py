...
def _get_service_info() -> dict[str, str]:
    ...
    service = os.getenv("PRIMEAGENT_SERVICE_NAME", "primeagent")
    version = os.getenv("PRIMEAGENT_VERSION", "")
    environment = os.getenv("PRIMEAGENT_ENVIRONMENT", "")
...
def add_otel_trace_context(_logger: Any, _method_name: str, event_dict: dict[str, Any]) -> dict[str, Any]:
    ...
    OpenTelemetry is optional in wfx, so the import is resolved once at module
...
def _apply_logger_level_overrides() -> None:
    """Apply ``PRIMEAGENT_LOG_LEVELS`` env var: ``name=LEVEL,name=LEVEL,...``.
    ...
    """
    raw = os.getenv("PRIMEAGENT_LOG_LEVELS", "").strip()
    if not raw:
        return
    for pair in raw.split(","):
        ...
        if "=" not in entry:
            warnings.warn(
                f"PRIMEAGENT_LOG_LEVELS: ignoring {entry!r} (expected 'name=LEVEL')",
                stacklevel=2,
            )
            continue
        ...
        if not name:
            warnings.warn(
                f"PRIMEAGENT_LOG_LEVELS: ignoring {entry!r} (empty logger name)",
                stacklevel=2,
            )
            continue
        ...
        if numeric is None:
            warnings.warn(
                f"PRIMEAGENT_LOG_LEVELS: ignoring {entry!r} (unknown level {level_str!r}, "
...
def _forward_loguru_message(message) -> None:
    """Forward Loguru messages through Primeagent's configured structlog pipeline."""
...
def setup_loguru_logger(log_level: str, *, enqueue: bool = False) -> None:
    """Route Loguru's default logger through Primeagent logging."""
...
def setup_log_file(log_file: Path, *, max_bytes: int, formatter: logging.Formatter | None = None) -> None:
    """Set up Primeagent's rotating file handler.
...
def configure(
    *,
    ...
    if log_level is None and os.getenv("PRIMEAGENT_LOG_LEVEL", "").upper() in VALID_LOG_LEVELS:
    ...
    # rebuilds from scratch. Env-only toggles (e.g. PRIMEAGENT_PRETTY_LOGS) are not
    ...
    extra_redact = frozenset(
        k.strip().lower() for k in os.getenv("PRIMEAGENT_LOG_REDACT_KEYS", "").split(",") if k.strip()
    )
    ...
    # PRIMEAGENT_LOG_TRACE_LOCALS=true when you need it for local debugging.
    show_locals = os.getenv("PRIMEAGENT_LOG_TRACE_LOCALS", "false").lower() == "true"
    ...
    json_mode = log_env.lower() in ("container", "container_json") or (
        not log_env and os.getenv("PRIMEAGENT_PRETTY_LOGS", "true").lower() != "true"
    )
