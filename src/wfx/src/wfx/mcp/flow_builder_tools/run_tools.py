...
Each is decoupled from ``primeagent`` via late imports (``wfx`` ships without
...
from wfx.graph.flow_builder.builder import build_flow_from_spec
...
class RunFlow(Component):
    ...
    Decoupled from ``primeagent`` via a late import (same pattern as
    ``_load_registry_user_aware``): ``wfx`` may run without the backend.
    """
    ...
    async def run_flow(self) -> Data:
        ...
        try:
            from primeagent.agentic.services.user_components_context import current_user_id
        ...
        try:
            ...
            from primeagent.agentic.services.flow_preparation import inject_model_into_flow
...
class GenerateComponent(Component):
    """Generate, validate and register a NEW custom Primeagent component.
    ...
    imports ``primeagent`` (same decoupling as ``RunFlow``): ``wfx`` may run
    without the backend.
    """

    display_name = "Generate Component"
    description = (
        "Create a brand-new custom Primeagent component from a natural-language description. "
    ...
    async def generate_component(self) -> Data:
        ...
        try:
            from primeagent.agentic.services.assistant_service import execute_flow_with_validation
            from primeagent.agentic.services.file_events import reset_file_events
            from primeagent.agentic.services.flow_types import PRIMEAGENT_ASSISTANT_FLOW
        except ImportError:
            msg = "Component generation is not available in this environment."
            return Data(data={"error": msg, "text": msg})

        try:
            from primeagent.agentic.services.agent_run_context import current_agent_run_model
            from primeagent.agentic.services.user_components_context import current_user_id
        ...
        try:
            from primeagent.agentic.services.component_events import emit_component_generation_failed
