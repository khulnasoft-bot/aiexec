...
from wfx.graph.flow_builder.flow import empty_flow
...
def _load_registry_user_aware() -> dict[str, dict]:
    ...
    Tries the primeagent-side overlay (which reads the current_user_id
    ContextVar and walks ``<sandbox>/.components/*.py`` for that user).
    Falls back to the bare base registry when:
        - the primeagent package isn't installed alongside wfx (e.g., the
          MCP server is running standalone),
        - no user is bound to the context.

    Keeps the wfx package free of a hard dependency on the primeagent
