"""wfx pull -- fetch flows from a remote Primeagent instance to local files.

Downloads flows from a live Primeagent instance, normalizes them for version
...
Pull all flows from staging to ``flows/``::
    ...
    wfx pull --env staging

Pull all flows in a named project::

    wfx pull --env staging --project "My RAG Pipeline"

Pull a single flow by UUID::

    wfx pull --env staging --flow-id <uuid>

Pull to a custom directory::

    wfx pull --env production --output-dir ./local-flows/
"""
