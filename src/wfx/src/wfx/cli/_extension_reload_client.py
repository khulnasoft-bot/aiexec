"""HTTP client helper for ``wfx extension reload``.
...
in :mod:`wfx.cli._extension_commands` delegates to :func:`reload_via_http`
...
"""Fallback Primeagent server URL when neither ``--target`` nor ``PRIMEAGENT_HOST`` is set."""
...
def resolve_target(explicit: str | None) -> str:
    """Pick the server URL to call.  Precedence: explicit > $PRIMEAGENT_HOST > default."""
    if explicit:
        return explicit.rstrip("/")
    env_target = os.environ.get("PRIMEAGENT_HOST")
    if env_target:
...
def resolve_api_key(explicit: str | None) -> str | None:
    ...
    if explicit:
        return explicit
    return os.environ.get("PRIMEAGENT_API_KEY")
...
def reload_via_http(
    *,
    ...
    """POST a reload request to the Primeagent extension dev server.
                ...
                "errors": [
                    {
                        ...
                        "message": f"Could not reach Primeagent server at {url}: {exc}",
                        "location": url,
                        "hint": (
                            "Start the dev server (e.g. `wfx run`) and pass --target / "
                            "set PRIMEAGENT_HOST if it is not on http://localhost:7860."
                        ),
                    }
                ],
            },
            ...
            "errors": [
                {
                    ...
                    "location": url,
                    "hint": "Confirm the URL points at a Primeagent server with the v1 API enabled.",
                }
            ],
        }
