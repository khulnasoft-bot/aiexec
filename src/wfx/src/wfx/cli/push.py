"""wfx push -- push normalized flow JSON to a remote Primeagent instance.
...
Push a single flow to staging::
    ...
    wfx push my_flow.json --env staging

Push several flows at once::

    wfx push flows/*.json --env staging

Push all flows in a directory and place them in a named project::

    wfx push --dir ./flows/ --env staging --project "My RAG Pipeline"

Dry-run to see what would happen::

    wfx push ./flows/ --env production --dry-run
"""
...
def _extract_flow_id(flow: dict[str, Any], path: Path) -> UUID:
    ...
    raw_id = flow.get("id")
    if not raw_id:
        console.print(f"[red]Error:[/red] {path} has no 'id' field. Run [bold]wfx export[/bold] first.")
        raise typer.Exit(1)
    try:
    ...
    *,
    ...
    if local_file_content is not None:
        try:
            ...
            from primeagent_sdk.exceptions import PrimeagentNotFoundError
            from primeagent_sdk.serialization import flow_to_json, normalize_flow
        ...
        except PrimeagentNotFoundError:
...
def _find_project_root() -> Path | None:
    """Return the wfx project root (directory containing .wfx/), or None.

    Only the ``.wfx`` marker is used — ``.git`` is intentionally excluded so
    that the containment check doesn't reject paths when running inside a
    larger monorepo or outside any wfx project.
    """
    cwd = Path.cwd()
    for directory in (cwd, *cwd.parents):
        if (directory / ".wfx").is_dir():
            return directory
...
def _collect_flow_files(sources: list[str], dir_path: str | None) -> list[Path]:
    ...
    ``flows/`` — mirroring the behaviour of ``wfx pull``.
    """
    paths: list[Path] = []
    root = _find_project_root()

    # Default to flows/ when nothing is specified, just like wfx pull does.
    ...
    *,
    ...
    if not paths:
        console.print(
            ...
            "Run [bold]wfx pull[/bold] first, or pass explicit file paths."
        )
        raise typer.Exit(1)
