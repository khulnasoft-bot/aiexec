...
def register(app: typer.Typer) -> None:
    ...
    def create_command_wrapper(
        ...
        *,
        ...
        """Scaffold a new Primeagent flow JSON from a built-in template (lazy-loaded)."""
        from pathlib import Path

        from wfx.cli.create import create_command, print_templates
    ...
    def requirements_command_wrapper(
        flow_path: str = typer.Argument(help="Path to the Primeagent flow JSON file"),
        ...
        wfx_package: str = typer.Option(
            "wfx",
            "--wfx-package",
            help="Name of the WFX package (default: wfx)",
        ),
        *,
        no_wfx: bool = typer.Option(
            False,
            "--no-wfx",
            help="Exclude the WFX package from output",
        ),
        ...
        """Generate requirements.txt from a Primeagent flow JSON (lazy-loaded)."""
        import json
        from pathlib import Path

        from wfx.utils.flow_requirements import generate_requirements_txt
    ...
    def validate_command_wrapper(
        flow_paths: list[str] = typer.Argument(
            default=None,
            help="Path(s) to Primeagent flow JSON file(s) or directories to validate. Defaults to flows/.",
        ),
        ...
        """Validate Primeagent flow JSON files without executing them (lazy-loaded)."""
        from wfx.cli.validate import validate_command
