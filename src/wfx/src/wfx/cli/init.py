"""wfx init -- scaffold a new Flow DevOps project.
...
Examples::
    ...
    wfx init my-rag-project
    wfx init .                   # scaffold into the current directory
    wfx init my-project --no-github-actions
"""
...
_ENVIRONMENTS_YAML = """\
# .wfx/environments.yaml
#
# Configure your Primeagent instances here.
...
#
# Quick start:
#   1. Open Primeagent → Settings → API Keys → Create a new key
#   2. export PRIMEAGENT_LOCAL_API_KEY=<your key>
#   3. wfx export --env local --flow-id <uuid> --output-dir flows/
...
_TEST_FLOWS_PY = '''\
"""Integration tests for Primeagent flows.

Run against a local instance (started with ``wfx serve``):

    pytest tests/ --primeagent-url http://localhost:8000

Run against a named environment (staging, production, etc.):

    pytest tests/ --primeagent-env staging -m integration
...
def test_flow_output_quality(flow_runner):
    ...
    result = flow_runner("my-flow-endpoint", "What is Primeagent?")
...
_GITIGNORE = """\
# Primeagent credentials -- never commit API keys
# (primeagent-environments.toml may contain literal keys; .wfx/environments.yaml is safe to commit)
primeagent-environments.toml
"""
...
def init_command(
    ...
    *,
    ...
    if example:
        from wfx.cli.create import create_command as _create
    ...
    # .wfx/environments.yaml
    _write(
        target / ".wfx" / "environments.yaml",
        _ENVIRONMENTS_YAML,
        "edit with your instance URLs + API key env var names (safe to commit)",
        **kw,
    )

    # .gitignore — keep primeagent-environments.toml ignored for backward compat
    gitignore = target / ".gitignore"
    if gitignore.exists():
        existing_content = gitignore.read_text(encoding="utf-8")
        if "primeagent-environments.toml" not in existing_content:
    ...
    console.print("  1. Edit [bold].wfx/environments.yaml[/bold] with your instance URL")
    console.print("  2. [bold]export PRIMEAGENT_LOCAL_API_KEY=<key>[/bold]   (Settings → API Keys)")
    if example:
        console.print("  3. [bold]wfx validate flows/hello-world.json[/bold]  (check the starter flow)")
        console.print("  4. [bold]wfx serve flows/hello-world.json[/bold]     (run it locally)")
        console.print("  5. [bold]wfx push --dir flows/ --env local[/bold]    (deploy to Primeagent)")
    else:
        console.print("  3. [bold]wfx create my-flow --template hello-world[/bold]")
        console.print("  4. [bold]wfx push --dir flows/ --env local[/bold]")
    console.print(f"  {'6' if example else '5'}. [bold]pytest tests/ --primeagent-env local[/bold]")
    console.print()
