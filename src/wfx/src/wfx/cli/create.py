"""wfx create -- scaffold a new flow JSON from a built-in template.
...
Examples::
    ...
    wfx create my-chatbot
    wfx create my-rag --template hello-world
    wfx create my-flow --output-dir ./flows --overwrite
    wfx create --list
"""
...
# Descriptions shown in ``wfx create --list``
...
def _is_wfx_project() -> bool:
    """Return True if cwd (or a parent up to .git) contains .wfx/environments.yaml."""
    cwd = Path.cwd()
    for directory in (cwd, *cwd.parents):
        if (directory / ".wfx" / "environments.yaml").is_file():
            return True
        if (directory / ".wfx" / "environments.yml").is_file():
            return True
...
def create_command(
    ...
    *,
    ...
    """
    if not _is_wfx_project():
        console.print(
            "[yellow]Warning:[/yellow] No .wfx/environments.yaml found in this project. "
            "Run [bold]wfx init[/bold] first to scaffold a project."
        )

    available = list_templates()
    if not available:
        console.print("[red]Error:[/red] No flow templates found in the wfx package.")
        raise typer.Exit(1)
    ...
    console.print(f"  [bold]wfx validate {dest}[/bold]")
    console.print(f"  [bold]wfx serve {dest}[/bold]")
