"""Flow persistence backends for wfx serve.
...
- ``FilesystemFlowStore(directory)``: writes each flow as ``{id}.json``.
  Point at ``/tmp/wfx-flows`` for single-pod sharing across uvicorn workers,
...
class FilesystemFlowStore:
    ...
    Use ``/tmp/wfx-flows`` for single-pod worker sharing or a PVC mount path
