...
from wfx.cli.script_loader import find_graph_variable, load_graph_from_script
from wfx.cli.serve_app import FlowAlreadyRegisteredError, FlowMeta, FlowRegistry, create_multi_serve_app
from wfx.load import load_flow_from_json
from wfx.utils.flow_envelope import merge_flow_envelope, split_flow_envelope
...
def _gate_flow_for_serve(
    ...
    *,
    ...
    graph (the same gate ``wfx run`` uses), then re-attaches the envelope so the result is
    ...
    """
    from wfx.upgrade.cli_gate import UpgradeFlowError, apply_upgrade_gate
...
def serve_command(
    ...
    flow_dir: Path | None = typer.Option(
        ...
        help=(
            ...
            "Use /tmp/wfx-flows for single-pod sharing or a PVC mount for cross-pod. "
    ...
    *,
    ...
    if workers > 1 and flow_dir is not None and script_paths:
        # With --flow-dir, workers skip WFX_SERVE_STARTUP_PATHS and rely on warm_from_store().
        ...
        # file directly from WFX_SERVE_STARTUP_PATHS, which works fine.
    ...
    try:
        ...
        console.print(
            Panel.fit(
                f"[bold green]WFX Server Ready[/bold green]\n\n"
        ...
        try:
            if workers > 1:
                ...
                try:
                    uvicorn.run(
                        "wfx.cli.serve_app:create_serve_app",
                        host=host,
                        port=port,
                        workers=workers,
                        log_level=log_level,
                    ...
                    # any WFX_SERVE_* var the operator intentionally exported before launch.
