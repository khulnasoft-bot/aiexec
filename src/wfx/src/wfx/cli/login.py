...
Validate a named environment::
    ...
    wfx login --env staging

Validate inline credentials::

    wfx login --target https://primeagent.example.com --api-key sk-abc123
...
from wfx.cli.common import load_sdk
...
def _api_key_env_name(env_name: str, environments_file: str | None) -> str | None:
    ...
    try:
        from wfx.config.environments import _find_config_file, _load_config
        ...
        from wfx.config.environments import _load_raw
...
def _probe_connection(client: Any, sdk: Any) -> tuple[bool, str, int]:
    ...
    try:
    ...
    except sdk.PrimeagentAuthError:
        return False, "auth", 0
    except sdk.PrimeagentConnectionError as exc:
        return False, f"connection:{exc}", 0
    except sdk.PrimeagentHTTPError as exc:
        ...
        # local Primeagent version exactly.  Treat this as a successful probe.
...
def login_command(
    *,
    ...
    from wfx.config import ConfigError, resolve_environment
    ...
    if env_cfg.api_key is None:
        ...
        if key_env_name:
            ...
            warning_parts.append("  Add [bold]api_key_env: PRIMEAGENT_<ENV>_API_KEY[/bold] to your config,")
    ...
    if not ok:
        if msg == "auth":
            ...
            if key_env_name:
                err_console.print("  1. Open Primeagent → Settings → API Keys → Create a new key")
        ...
        if msg.startswith("connection:"):
            ...
            err_console.print()
            err_console.print("[bold]How to fix:[/bold]")
            err_console.print("  • Make sure your Primeagent instance is running")
            err_console.print("  • Check the URL in your .wfx/environments.yaml")
            err_console.print(
                "  • If running locally: [bold]primeagent run[/bold] or [bold]wfx serve <flow.json>[/bold]"
            )
            raise typer.Exit(1)
    ...
    elif os.environ.get("PRIMEAGENT_API_KEY") == env_cfg.api_key and env_cfg.api_key:
        key_source = "  [dim]from PRIMEAGENT_API_KEY[/dim]"
    ...
    console.print(
        Panel.fit(
            ...
            title="[bold blue]wfx login[/bold blue]",
            border_style="green",
        )
    )
    ...
    if env_cfg.name in ("__inline__", "__env__") and env_cfg.api_key:
        console.print()
        console.print("[dim]Tip: to avoid passing credentials each time, add to .wfx/environments.yaml:[/dim]")
        ...
        console.print(f"[dim]      api_key_env: PRIMEAGENT_{env_display.upper()}_API_KEY[/dim]")
