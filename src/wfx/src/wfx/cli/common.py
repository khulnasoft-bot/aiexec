...
from wfx.run._defaults import apply_run_defaults, resolve_fallback_to_env_vars
...
def get_api_key() -> str:
    ...
    Used by ``wfx serve`` to set the superuser key on the local server.
    For *remote* commands (push, pull, login, …), the per-environment key
    is resolved via :func:`wfx.config.resolve_environment` and the
    ``api_key_env`` field in ``.wfx/environments.yaml``.
    """
    api_key = os.getenv("PRIMEAGENT_API_KEY") or os.getenv("WFX_API_KEY")
    if not api_key:
...
async def execute_graph_with_capture(graph, input_value: str | None, session_id: str | None = None):
    ...
    Args:
        ...
        session_id: Optional session ID. ``None`` auto-generates one so that
            ...
            shell/env-var typos (see ``wfx.run._defaults.validate_provided_id``).
...
def load_sdk(command_name: str) -> Any:
    """Lazily import ``primeagent_sdk`` to keep CLI startup fast.
    ...
    try:
        ...
        msg = f"primeagent-sdk is required for wfx {command_name}. Install it with: pip install primeagent-sdk"
        raise typer.BadParameter(msg) from exc
    else:
        return primeagent_sdk
