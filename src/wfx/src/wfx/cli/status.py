...
Examples::
    ...
    wfx status                          # scans flows/ in cwd, uses [defaults] env
    wfx status --env staging            # compare against staging
    wfx status --dir ./my-flows/        # specify a custom flows directory
    wfx status --env prod --remote-only # also show server flows not tracked locally
"""
...
def _load_sdk() -> tuple[object, object, object, type]:
    """Return (normalize_flow, flow_to_json, Client, PrimeagentNotFoundError) from primeagent_sdk."""
    sdk = load_sdk("status")
    ...
    return normalize_flow, flow_to_json, sdk.Client, PrimeagentNotFoundError
            ...
            if not raw_id:
                statuses.append(
                    FlowStatus(
                        ...
                        path=path,
                        detail="run wfx export --env <env> first to assign a stable id",
                    )
                )
                continue
