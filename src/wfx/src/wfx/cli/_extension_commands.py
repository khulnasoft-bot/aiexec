...
the existing ``wfx validate`` (which validates flow JSON, not extensions).
...
Commands shipped here:
    ...
    - ``dev``       -- register a local extension and launch Primeagent with
                       it loaded.

Each command is implemented as a thin shell over the helpers in
``wfx.extension.*`` so the same code paths are reachable from tests
...
extension_app = typer.Typer(
    name="extension",
    help="Author and inspect Primeagent Extensions.",
...
def validate_command(
    ...
    *,
    ...
    subprocess with a temporary Primeagent state directory.
    """
    from wfx.extension import format_extension_error, validate_extension
...
def list_command(
    ...
    seed_dir: str | None = typer.Option(
        ...
        "--seed-dir",
        help=(
            "Override $PRIMEAGENT_SEED_DIR for this invocation. "
    ...
    Output (text mode) is one row per Extension::
        ...
        wfx-openai          1.2.0     openai              @official  installed  discovered
        wfx-anthropic       0.4.1     anthropic           @official  seed       discovered
    ...
    :func:`~wfx.extension.format_extension_error`.
    ...
    # ``wfx extension list`` should still exit 0 in those cases so CI scripts
    # like ``wfx extension list && ...`` keep flowing -- the warning stays on
    ...
    # listing can immediately spot a wrong-venv mismatch between ``wfx``
    # and ``primeagent run`` without a separate "did my bundle install?"
    # debug cycle.
...
def reload_command(
    extension_id: str | None = typer.Argument(
        None,
        help=("ID of the extension whose Bundle should be reloaded (e.g. wfx-arxiv). Omit when passing --all."),
    ),
    bundle: str | None = typer.Option(
        ...
        help=(
            ...
            "same source ``wfx extension list`` uses)."
        ),
    ),
    target: str | None = typer.Option(
        None,
        "--target",
        help=("Primeagent server URL (default: $PRIMEAGENT_HOST / http://localhost:7860)."),
    ),
    api_key: str | None = typer.Option(
        None,
        "--api-key",
        help="API key for the Primeagent server (default: $PRIMEAGENT_API_KEY).",
    ),
    output_format: str = typer.Option(
        "text",
        "--format",
    ...
    *,
    ...
    if extension_id is None:
        typer.echo(
            "extension reload requires an extension id (e.g. `wfx extension reload wfx-arxiv`), "
...
def _resolve_bundle_from_discovery(extension_id: str) -> str | None:
    ...
    """
    from wfx.extension import discover_all_extensions
    ...
    if not matches:
        typer.echo(
            ...
            "  - Run `wfx extension list` to see what is installed locally.\n"
...
def _post_reload(
    *,
    ...
    from wfx.cli._extension_reload_client import reload_via_http
...
def _render_reload_text(response, *, extension_id: str, bundle_name: str) -> None:
    ...
    from wfx.extension.errors import ExtensionError, format_extension_error
    ...
    for raw in raw_errors:
        ...
        try:
            err = ExtensionError(
                ...
                hint=str(raw.get("hint") or "Run `wfx extension validate` for details."),
...
def _reload_all(*, target: str | None, api_key: str | None, output_format: str) -> None:
    ...
    if not extensions:
        typer.echo(
            ...
            "  - Run `wfx extension list` to confirm what is installed.",
        )
        return
...
def schema_command(
    ...
    ``schemas.prime.khulnasoft.com/extension/v1.json``; the release pipeline uploads
    this same artifact.
    """
    from wfx.extension.schema import build_schema_json
...
def init_command(
    ...
    *,
    ...
    name: str | None = typer.Option(
        None,
        "--name",
        help=("Human-readable display name shown in Primeagent.  Defaults to the title-cased extension id."),
    ),
) -> None:
    ...
    """
    from wfx.extension import format_extension_error
    ...
    typer.echo(f"  wfx extension validate {target_path}")
    typer.echo(f"  wfx extension dev {target_path}")
...
def _resolve_primeagent_executable() -> str | None:
    """Find the ``primeagent`` binary on PATH, if any.

    Returns ``None`` when primeagent isn't installed; the dev command then
    falls back to ``python -m primeagent.__main__`` so an author with the
    ``wfx`` package alone still gets a usable error message.
    """
    return shutil.which("primeagent")


@extension_app.command(
    name="dev",
    help="Register a local extension and launch Primeagent with it loaded.",
)
def dev_command(
    target: str = typer.Argument(
        ".",
        help=(
            ...
            "[tool.primeagent.extension] in pyproject.toml."
        ),
    ),
    *,
    ...
    skip_launch: bool = typer.Option(
        ...
        help=(
            ...
            "``primeagent run``.  Useful for tests and for embedding in "
            "external dev-server scripts."
        ),
    ),
    extra_args: list[str] | None = typer.Argument(
        None,
        help="Extra arguments forwarded to ``primeagent run`` (after a ``--`` separator).",
    ),
) -> None:
    """Register the extension and launch a Primeagent dev server.
    ...
    Flow:
        ...
        5. ``exec``-style hand-off to ``primeagent run`` (or
           ``python -m primeagent``) with the env var
           ``PRIMEAGENT_LAZY_LOAD_COMPONENTS=false`` so dev-extensions are
           visible in the palette immediately.

    AC #4 ("boots Primeagent with the new Extension visible in the palette
    within 5s") is delivered jointly by this command and the startup
    hook in ``primeagent.main`` that consults the dev registry.
    """
    from wfx.extension import format_extension_error, register_dev_extension, validate_extension
    ...
    typer.echo("  - Click 'Reload' on the bundle header in the Primeagent palette.")

    if skip_launch:
        return

    extras = list(extra_args or [])
    cmd = _build_primeagent_run_argv(extras)
    typer.echo("")
    typer.echo("Launching: " + " ".join(shlex.quote(part) for part in cmd))

    env = _build_dev_launch_env(os.environ)

    # Replace the current process so Ctrl-C in the launched primeagent
    # propagates without an extra pty/job-control hop.  When ``primeagent``
...
def _build_dev_launch_env(base_env: os._Environ[str] | dict[str, str]) -> dict[str, str]:
    """Build the env dict handed to the launched ``primeagent run`` process.
    ...
    Flags set:
        - ``PRIMEAGENT_LAZY_LOAD_COMPONENTS=false`` (always, overriding the
          author's shell): dev components must appear in the palette
          eagerly so the AC's 5-second budget holds.
        - ``PRIMEAGENT_ENABLE_EXTENSION_RELOAD=true`` (setdefault): turns on
    ...
    """
    env = dict(base_env)
    env["PRIMEAGENT_LAZY_LOAD_COMPONENTS"] = "false"
    env.setdefault("PRIMEAGENT_ENABLE_EXTENSION_RELOAD", "true")
    return env


def _build_primeagent_run_argv(extra_args: list[str]) -> list[str]:
    """Build the argv list that exec's ``primeagent run``.

    Falls back to ``python -m primeagent`` when the ``primeagent`` binary is
    not on PATH so the dev loop still works inside an ``wfx``-only
    install (author hits a clear failure mode rather than a confusing
    one).
    """
    binary = _resolve_primeagent_executable()
    if binary is not None:
        return [binary, "run", *extra_args]
    return [sys.executable, "-m", "primeagent", "run", *extra_args]
