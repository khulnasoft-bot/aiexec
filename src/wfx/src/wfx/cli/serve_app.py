...
from wfx.cli.runtime_variables import apply_global_vars_to_graph
from wfx.load import load_flow_from_json
from wfx.log.logger import logger
from wfx.utils.flow_validation import validate_flow_for_current_settings

if TYPE_CHECKING:
    from collections.abc import AsyncGenerator

    from wfx.cli.flow_store import FlowStore
...
class FlowRegistry:
    ...
    by a :class:`~wfx.cli.flow_store.FlowStore`; the default ``NullFlowStore``
    keeps everything in memory only.

    - ``NullFlowStore`` (default): single-worker, no disk I/O.
    - ``FilesystemFlowStore("/tmp/wfx-flows")``: all uvicorn workers in the
      same pod share flows via ``/tmp``.
    - ``FilesystemFlowStore("/mnt/wfx-flows")``: same code, cross-pod if the
      path is a PVC mount.
    ...
    **Per-request stale check (FilesystemFlowStore only)**
    ...
    latency matters, prefer ``/tmp/wfx-flows`` (local tmpfs) for single-pod
...
class UploadFlowRequest(BaseModel):
    ...
    id: str | None = Field(default=None, description="Stable flow ID from Primeagent export (must be a valid UUID)")
...
def create_multi_serve_app(
    *,
    ...
    """Create a FastAPI app exposing WFX flows via a mutable registry.

    Routes dispatch to ``registry`` at request time, so flows added after
    startup (via ``POST /flows/upload/``) are immediately reachable.
    """
    app = FastAPI(
        title=f"WFX Multi-Flow Server ({len(registry)})",
        description=(
            "Hosts WFX graphs under the `/flows/{{id}}` prefix. "
...
def create_serve_app() -> FastAPI:
    ...
    worker calls this factory, which reads ``WFX_SERVE_FLOW_DIR`` and
    ``WFX_SERVE_NO_ENV_FALLBACK`` from the environment, pre-warms its own
    ...
    ``uvicorn.run("wfx.cli.serve_app:create_serve_app", workers=N, ...)``.
    """
    import asyncio
    import os
    from pathlib import Path

    from wfx.cli.flow_store import FilesystemFlowStore, NullFlowStore
    ...
    if startup_paths and not flow_dir:
        ...
        from wfx.cli.commands import build_registry_from_directory, build_registry_from_paths
