"""wfx export -- serialize flows to git-friendly JSON.
...
Remote (--env / --flow-id / --project-id)
    Pull flows directly from a running Primeagent instance using the
    ``primeagent-sdk`` HTTP client, normalize them, and write to disk.

Examples:
--------
Normalize a local file in-place::

    wfx export my_flow.json --in-place

Normalize and write to a new file::

    wfx export my_flow.json -o my_flow.normalized.json

Pull a single flow from staging and write to the current directory::

    wfx export --flow-id <uuid> --env staging

Export an entire project from staging into ./flows/::

    wfx export --project-id <uuid> --env staging --output-dir ./flows/
"""
...
from wfx.cli.common import load_sdk, safe_filename
...
def export_command(
    ...
    *,
    ...
    # ------------------------------------------------------------------
    # Remote mode: pull from a live Primeagent instance
    # ------------------------------------------------------------------
