"""Implementation of `wfx upgrade <flow.json>`."""
...
from wfx.upgrade.checker import CompatibilityReport, build_registry_lookup, check_flow_compatibility
...
def load_registry_from_index() -> dict[str, Any]:
    ...
    if not blob or "entries" not in blob:
        typer.echo(
            ...
            "cannot be checked. Try reinstalling wfx (e.g. `pip install --force-reinstall wfx`).",
            err=True,
        )
        raise typer.Exit(1)
...
def upgrade_command(
    ...
    *,
    ...
    # Exported Primeagent flows may have an outer envelope:
