...
from wfx.testing.result import FlowResult, _build_result, _build_result_from_sdk_response
...
def _import_remote_run_request():
    ...
    try:
        from primeagent_sdk.models import RunRequest  # type: ignore[import-untyped]
    except ImportError as exc:
        msg = "primeagent-sdk is required for remote flow testing. Install: pip install primeagent-sdk"
...
class LocalFlowRunner(_BaseLocalFlowRunner):
    ...
    * A ``dict`` (already-parsed flow JSON).
    ...
    Relative paths are resolved against ``--wfx-flow-dir`` (default: ``cwd``).
    """
...
# ---------------------------------------------------------------------------
# Remote runners (requires primeagent-sdk)
# ---------------------------------------------------------------------------


class RemoteFlowRunner(_BaseRemoteFlowRunner):
    """Sync callable that runs flows against a live Primeagent instance.

    Returned by :func:`flow_runner` when ``--primeagent-env`` or
    ``--primeagent-url`` is passed to pytest.  Call it like a function::
...
class AsyncRemoteFlowRunner(_BaseRemoteFlowRunner):
    """Async callable that runs flows against a live Primeagent instance.

    Returned by :func:`async_flow_runner` when ``--primeagent-env`` or
    ``--primeagent-url`` is passed to pytest.  Use with ``await``::
