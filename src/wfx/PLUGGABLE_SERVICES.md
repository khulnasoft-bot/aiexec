...
WFX also supports **adapter registries** -- collections of swappable implementations that share the same protocol.
...
Each adapter type exposes a typed accessor in `wfx.services.deps`:
...
```python
from wfx.services import register_adapter
...
`wfx.toml` (preferred when both files exist):
...
Entry-point groups follow the naming convention `wfx.<adapter_type>.adapters` (e.g. `wfx.deployment.adapters`).

```toml
[project.entry-points."wfx.deployment.adapters"]
local = "my_package.deployment:LocalAdapter"
remote = "my_package.deployment:RemoteAdapter"
```
...
Entry-point groups and config section paths are derived from the `AdapterType` value automatically (e.g. `wfx.deployment.adapters` and `[deployment.adapters]`).
...
When adapters are part of core WFX, prefer namespacing them under `services/adapters`:
